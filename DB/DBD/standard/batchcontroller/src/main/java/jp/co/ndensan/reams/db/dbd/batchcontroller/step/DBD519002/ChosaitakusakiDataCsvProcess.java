...
/**
 * 認定調査委託先情報CSV_ファイル出力です。
 *
 * @reamsid_L DBD-2100-020 liwul
 */
public class ChosaitakusakiDataCsvProcess extends BatchProcessBase<ChosaitakusakiEntity> {

    private static final RString MYBATIS_SELECT_ID = new RString("jp.co.ndensan.reams.db.dbd.persistence.db.mapper.relate.")
            .concat("renekeidatasakuseifourmaster.IRenekeiDataSakuseiFourMasterMapper.get調査委託先情報ＩＦのcsv情報");
    ...
    private static final RString ファイル名_日本語 = new RString("認定調査委託先情報CSV");
    private static final RString EUCエンティティID = new RString("DBD519021");
    ...
    private CsvWriter<ChosaitakusakiDataReamsCsvProcessEntity> csvReamsWriter;
    private CsvWriter<ChosaitakusakiDataCsvProcessEntity> csvWriter;
    ...
    @Override
    protected void process(ChosaitakusakiEntity entity) {
        シーケンシャル番号++;
        RenekeiDataSakuseiFourMasterCsvManager manager = new RenekeiDataSakuseiFourMasterCsvManager();
        if (para.isReams()) {
            csvReamsWriter.writeLine(manager.toChosaitakusakiDataReamsCsvProcessEntity(entity, シーケンシャル番号, para.getShikibetsucode()));
        } else {
            csvWriter.writeLine(manager.toChosaitakusakiDataCsvProcessEntity(entity, シーケンシャル番号, para.getShikibetsucode()));
        }
    }
