...
//                        new ReportId(processParamter.getCyohyoid()), Long.parseLong(processParamter.getSyutsuryokukomoku().toString()));
    ...
    protected void createWriter() {
        ...
        HanyoListKokiKoreishaManager.createInstance().set改頁Key(outputOrder, pageBreakKeys);
    ...
    protected void process(HanyoRisutoKokiKoreishaEntity entity) {
        ...
        HanyoListKokiKoreishaManager.createInstance().get情報設定(eucCsvEntity, entity, association,
        ...
        int index = 0;
        for (KokiKoreishaCsvEnumEntity e : KokiKoreishaCsvEnumEntity.values()) {
...
//                    Method getMethod;
//                    try {
//                        getMethod = clazz.getDeclaredMethod(KokiKoreishaCsvEnumEntity
...
//            }
        ...
        HanyoListKokiKoreishaManager.createInstance().バッチ出力条件リストの出力(processParamter, 導入団体コード,
    ...
    private void 帳票出力とCSV出力(HanyoRisutoKokiKoreishaEntity entity, boolean flag) {
        if (is帳票出力) {
            if (flag) {
                HanyoListEntity hanyolistentity = new HanyoListEntity();
                HanyoListKokiKoreishaManager.createInstance().get方法(entity, hanyolistentity);
                HanyoListReport report = new HanyoListReport(hanyolistentity, processParamter.getHyoudai(),
                        processParamter.getDetasyubetsumesyo(), 項目見出し, 項目内容, association, outputOrder);
                report.writeBy(reportSourceWriter);
            }
    ...
    private RString get項目内容(Method getMethod, RString 項目内容new, HanyoRisutoKokiKoreishaEucCsvEntity eucCsvEntity, int i) {
        try {
