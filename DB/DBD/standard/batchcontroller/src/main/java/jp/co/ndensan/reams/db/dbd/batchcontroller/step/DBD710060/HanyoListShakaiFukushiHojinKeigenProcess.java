 ...
 */
package jp.co.ndensan.reams.db.dbd.batchcontroller.step.DBD710060;
...
import jp.co.ndensan.reams.db.dbd.service.core.dbd710060.HanyoListShakaiFukushiHojinKeigenManager;
...
public class HanyoListShakaiFukushiHojinKeigenProcess extends BatchProcessBase<ShakaiFukushiHojinKeigenEntity> {
    ...
    protected IBatchReader createReader() {
        ...
        return new BatchDbReader(MYBATIS_SELECT_ID,
                processParamter.toHanyoRisutoShakaiFukushiHoujinKeigenMybatisParameter(psmShikibetsuTaisho, psmAtesaki, 出力順));
    }

    @Override
    protected void createWriter() {
        manager = new FileSpoolManager(UzUDE0835SpoolOutputType.EucOther, EUC_ENTITY_ID, UzUDE0831EucAccesslogFileType.Csv);
        eucFilePath = Path.combinePath(manager.getEucOutputDirectry(), new RString("DBD_ShakaiFukushiHojinKeigen_Temp.csv"));
        csvFilePath1 = Path.combinePath(manager.getEucOutputDirectry(), new RString("HanyoList_ShakaiFukushiHojinKeigen.csv"));
        ...
        HanyoListShakaiFukushiHojinKeigenManager.createInstance().set改頁Key(outputOrder, pageBreakKeys);
    ...
    protected void afterExecute() {
        ...
        HanyoListShakaiFukushiHojinKeigenManager.createInstance().バッチ出力条件リストの出力(processParamter, 導入団体コード,
    ...
    private void 帳票出力とCSV出力(ShakaiFukushiHojinKeigenEntity entity, boolean flag) {
        if (is帳票出力) {
            if (flag) {
                HanyoListEntity hanyolistentity = new HanyoListEntity();
                HanyoListShakaiFukushiHojinKeigenManager.createInstance().get方法(entity, hanyolistentity);
                HanyoListReport report = new HanyoListReport(hanyolistentity, processParamter.getHyoudai(),
                        processParamter.getDetasyubetsumesyo(), 項目見出し, 項目内容, association, outputOrder);
                report.writeBy(reportSourceWriter);
            }
    ...
    private RString get出力順() {
        ...
        RString 出力順 = MyBatisOrderByClauseCreator.create(HanyoListShakaiFukushiHojinKeigenOrderby.class, order);
        return 出力順.concat(",社会福祉法人等利用者負担軽減_履歴番号");
    }
    ...
    private RString get項目内容(Method getMethod, RString 項目内容new, ShakaiFukushiHojinKeigenEucCsvEntity eucCsvEntity, int i) {
        try {
