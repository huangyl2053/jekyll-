...
public class HanyoListShakaiFukushiHojinKeigenProcess extends BatchProcessBase<ShakaiFukushiHojinKeigenEntity> {
        ...
        if (hanyoListShutsuryokuKomoku != null) {
            for (int i = 0; i < hanyoListShutsuryokuKomoku.get汎用リスト出力項目リスト().size(); i++) {
                ...
                if (get項目名称 != null && !get項目名称.isEmpty()) {
                    try {
                        Method getMethod = clazz.getDeclaredMethod(ShakaiFukushiHojinKeigenCsvEnumEntity
    ...
    private RString to帳票物理名(RString 項目ID) {
        ...
        } else if (HanyoListShakaiFukushiHojinKeigenOrderby.証記載保険者番号.get項目ID().equals(項目ID)) {
    ...
    private void get方法(ShakaiFukushiHojinKeigenEntity entity, HanyoListEntity hanyolistentity) {
