 ...
 */
package jp.co.ndensan.reams.db.dbd.batchcontroller.step.hanyorisutojyukyusyadaicho;
...
/**
 * 汎用リスト受給者台帳２_process処理クラスです。
 *
 * @reamsid_L DBD-1810-020 yaodongsheng
 */
public class HanyoRisutoJyukyusyaDaichoProcess extends BatchProcessBase<HanyoRisutoJyukyusyaDaichoEntity> {

    private static final RString EUC_WRITER_DELIMITER = new RString(",");
    private static final RString EUC_WRITER_ENCLOSURE = new RString("\"");
    private static final RString 住所地特例 = new RString("住特");
    private static final EucEntityId EUC_ENTITY_ID = new EucEntityId("DBD701002");
    ...
    private static final RString CYUSYUTSUDATA = new RString("【抽出データ】  ");
    private static final RString KESYUDATACYUTSYUTSU = new RString("【申請取消データ抽出】  する");
    private static final RString KESYUDATACYUTSYUTSUSHINAI = new RString("【申請取消データ抽出】  しない");
    private static final RString SAKUJYODATACYUSYUTSU = new RString("【削除データ抽出】  する");
    private static final RString SAKUJYODATACYUSYUTSUSHINAI = new RString("【削除データ抽出】  しない");
    private static final RString SASHIKUBUN = new RString("【喪失区分】  ");
    private static final RString CYUSYUTSUHOHO = new RString("【抽出方法】  ");
    private static final RString KIKAN = new RString("期間");
    private static final RString CYUSYUTSUKIKAN = new RString("【抽出期間】  ");
    ...
    private EucCsvWriter<HanyoRisutoJyukyusyaDaichoEucCsvEntity> eucCsvWriter;
    ...
    protected IBatchReader createReader() {
        ...
        return new BatchDbReader(MYBATIS_SELECT_ID, processParamter.toHanyoRisutoJyukyusyaDaichoMybatisParameter(psmShikibetsuTaisho, psmAtesaki));
    }

    @Override
    protected void createWriter() {
        manager = new FileSpoolManager(UzUDE0835SpoolOutputType.EucOther, EUC_ENTITY_ID, UzUDE0831EucAccesslogFileType.Csv);
        eucFilePath = Path.combinePath(manager.getEucOutputDirectry(), new RString("HanyoList_JukyushaDaicho2.csv"));
        RString 文字コード = DbBusinessConifg.get(ConfigNameDBU.EUC共通_文字コード, RDate.getNowDate(), SubGyomuCode.DBU介護統計報告);
    ...
    @Override
    protected void process(HanyoRisutoJyukyusyaDaichoEntity entity) {
        HanyoRisutoJyukyusyaDaichoEucCsvEntity eucCsvEntity = new HanyoRisutoJyukyusyaDaichoEucCsvEntity();
    ...
    private HanyoRisutoJyukyusyaDaichoEucCsvEntity setBlank() {
        HanyoRisutoJyukyusyaDaichoEucCsvEntity eucCsvEntity = new HanyoRisutoJyukyusyaDaichoEucCsvEntity();
    ...
    private void setBlank2(HanyoRisutoJyukyusyaDaichoEucCsvEntity eucCsvEntity) {
        ...
        eucCsvEntity.set前回調査委託先コード(RString.EMPTY);
        eucCsvEntity.set前回調査員コード(RString.EMPTY);
        eucCsvEntity.set前回委託先名(RString.EMPTY);
        eucCsvEntity.set前回調査員氏名(RString.EMPTY);
        ...
        eucCsvEntity.set前々回調査委託先コード(RString.EMPTY);
        eucCsvEntity.set前々回調査員コード(RString.EMPTY);
        eucCsvEntity.set前々回委託先名(RString.EMPTY);
        eucCsvEntity.set前々回調査員氏名(RString.EMPTY);
        ...
        eucCsvEntity.set申請区分申請時(RString.EMPTY);
        eucCsvEntity.set申請区分法令(RString.EMPTY);
        ...
        eucCsvEntity.set所得段階(RString.EMPTY);
        ...
        eucCsvEntity.set二次予防適用開始日(RString.EMPTY);
        eucCsvEntity.set二次予防適用終了日(RString.EMPTY);
        eucCsvEntity.set二次予防調査実施日(RString.EMPTY);
        eucCsvEntity.set二次予防決定日(RString.EMPTY);
        ...
        eucCsvEntity.set委託先計画事業者番号(RString.EMPTY);
        eucCsvEntity.set委託先計画事業者名(RString.EMPTY);
        eucCsvEntity.set委託先計画事業者カナ(RString.EMPTY);
        eucCsvEntity.set委託先計画管理者名(RString.EMPTY);
        eucCsvEntity.set委託先計画管理者カナ(RString.EMPTY);
    }

    private void setEucCsvEntity(HanyoRisutoJyukyusyaDaichoEucCsvEntity eucCsvEntity, HanyoRisutoJyukyusyaDaichoEntity entity) {
    ...
    private void setEucCsvEntity2(HanyoRisutoJyukyusyaDaichoEucCsvEntity eucCsvEntity, HanyoRisutoJyukyusyaDaichoEntity entity) {
        ...
        eucCsvEntity.set前回調査委託先コード(entity.get前回申請_認定調査委託先コード());
        eucCsvEntity.set前回調査員コード(entity.get前回申請_認定調査員コード());
        eucCsvEntity.set前回委託先名(entity.get前回調査委託先_事業者名称());
        eucCsvEntity.set前回調査員氏名(entity.get前回調査員_調査員氏名());
        ...
        eucCsvEntity.set前々回調査委託先コード(entity.get前々回申請_認定調査委託先コード());
        eucCsvEntity.set前々回調査員コード(entity.get前々回申請_認定調査員コード());
        eucCsvEntity.set前々回委託先名(entity.get前々回調査委託先_事業者名称());
        eucCsvEntity.set前々回調査員氏名(entity.get前々回調査員_調査員氏名());
        ...
        eucCsvEntity.set申請区分申請時(set申請区分申請時(entity.get今回申請_認定申請区分申請時コード()));
        eucCsvEntity.set申請区分法令(set申請区分法令(entity.get今回申請_認定申請区分法令コード()));
        ...
        eucCsvEntity.set後期被保険者番号(entity.get後期高齢者情報_後期高齢被保険者番号());
        ...
        eucCsvEntity.set国保喪失日(set年月日(entity.get国保資格情報_資格喪失日()));
        eucCsvEntity.set国保登録区分(set登録区分(entity.get国保資格情報_登録区分()));
        eucCsvEntity.set二次予防適用開始日(set年月日(entity.get二次予防事業対象情報_適用開始年月日()));
        eucCsvEntity.set二次予防適用終了日(set年月日(entity.get二次予防事業対象情報_適用終了年月日()));
        eucCsvEntity.set二次予防調査実施日(set年月日(entity.get二次予防事業対象情報_調査実施日()));
        eucCsvEntity.set二次予防決定日(set年月日(entity.get二次予防事業対象情報_決定年月日()));
        ...
        eucCsvEntity.set委託先計画事業者番号(entity.get計画事業者_委託先事業者番号());
        eucCsvEntity.set委託先計画事業者名(entity.get委託先事業者_事業者名称());
        eucCsvEntity.set委託先計画事業者カナ(entity.get委託先事業者_事業者名称カナ());
        eucCsvEntity.set委託先計画管理者名(entity.get委託先事業者代表者_代表者名());
        eucCsvEntity.set委託先計画管理者カナ(entity.get委託先事業者代表者_代表者名カナ());
    ...
    private void バッチ出力条件リストの出力() {
        ...
        RString 日本語ファイル名 = new RString("汎用リスト　受給者台帳２CSV");
        RString 英数字ファイル名 = new RString("HanyoList_JukyushaDaicho2.csv");
        ...
        出力条件.add(processParamter.isShinseikeshidetacyusyutsu() ? KESYUDATACYUTSYUTSU : KESYUDATACYUTSYUTSUSHINAI);
        出力条件.add(processParamter.isSakujyodatacyusyutsu() ? SAKUJYODATACYUSYUTSU : SAKUJYODATACYUSYUTSUSHINAI);
        ...
        builder = new RStringBuilder();
        if (ChushutsuTaisho.抽出年月日.get名称().equals(tasyo)) {
            builder.append(CYUSYUTSUKIKAN).append(processParamter.getCyusyutsunichifrom())
                    .append(KIBU).append(processParamter.getCyusyutsunichito());
        } else if (ChushutsuTaisho.医療機関.get名称().equals(tasyo)) {
            builder.append(CYUSYUTSUHANI).append(processParamter.getIryokikancodefrom()).append(KIBU).append(processParamter.getIryokikancodefto());
        } else if (ChushutsuTaisho.調査委託先.get名称().equals(tasyo)) {
            builder.append(CYUSYUTSUHANI).append(processParamter.getCyosaitakusakicodefrom()).append(KIBU)
                    .append(processParamter.getCyosaitakusakicodefto());
        }
        出力条件.add(builder.toRString());
        ReportOutputJokenhyoItem reportOutputJokenhyoItem = new ReportOutputJokenhyoItem(
                new RString("DBD701002"),
                導入団体コード,
                市町村名,
                ジョブ番号,
                日本語ファイル名,
