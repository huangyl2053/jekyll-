...
//                        new ReportId(processParamter.getCyohyoid()), Long.parseLong(processParamter.getSyutsuryokukomoku().toString()));
    ...
    protected void createWriter() {
        ...
        HanyoListKokuhoManager.createInstance().set改頁Key(outputOrder, pageBreakKeys);
...
//            }
        ...
        HanyoListKokuhoManager.createInstance().バッチ出力条件リストの出力(processParamter, 導入団体コード,
    ...
    private void 帳票出力とCSV出力(HanyoRisutoKokuhoEntity entity, boolean flag) {
        if (is帳票出力) {
            if (flag) {
                HanyoListEntity hanyolistentity = new HanyoListEntity();
                HanyoListKokuhoManager.createInstance().get方法(entity, hanyolistentity);
                HanyoListReport report = new HanyoListReport(hanyolistentity, processParamter.getHyoudai(),
                        processParamter.getDetasyubetsumesyo(), 項目見出し, 項目内容, association, outputOrder);
                report.writeBy(reportSourceWriter);
            }
    ...
    private RString get項目内容(Method getMethod, RString 項目内容new, HanyoRisutoKokuhoEucCsvEntity eucCsvEntity, int i) {
        try {
