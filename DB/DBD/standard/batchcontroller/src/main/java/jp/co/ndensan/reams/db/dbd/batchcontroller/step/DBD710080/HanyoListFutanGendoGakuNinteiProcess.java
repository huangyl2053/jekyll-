...
//                        new ReportId(processParamter.getCyohyoid()), Long.parseLong(processParamter.getSyutsuryokukomoku().toString()));
    ...
    protected void createWriter() {
        ...
        HanyoListFutanGendoGakuNinteiManager.createInstance().set改頁Key(outputOrder, pageBreakKeys);
    ...
    protected void process(FutanGendoGakuNinteiEntity entity) {
        ...
        int index = 0;
        for (FutanGendoGakuNinteiCsvEnumEntity e : FutanGendoGakuNinteiCsvEnumEntity.values()) {
...
//                    Method getMethod;
//                    try {
//                        getMethod = clazz.getDeclaredMethod(FutanGendoGakuNinteiCsvEnumEntity
...
//            }
        ...
        HanyoListFutanGendoGakuNinteiManager.createInstance().バッチ出力条件リストの出力(processParamter, 導入団体コード,
    ...
    private void 帳票出力とCSV出力(FutanGendoGakuNinteiEntity entity, boolean flag) {
        if (is帳票出力) {
            if (flag) {
                HanyoListEntity hanyolistentity = new HanyoListEntity();
                HanyoListFutanGendoGakuNinteiManager.createInstance().get方法(entity, hanyolistentity);
                HanyoListReport report = new HanyoListReport(hanyolistentity, processParamter.getHyoudai(),
                        processParamter.getDetasyubetsumesyo(), 項目見出し, 項目内容, association, outputOrder);
                report.writeBy(reportSourceWriter);
            }
    ...
    private RString get項目内容(Method getMethod, RString 項目内容new, FutanGendoGakuNinteiEucCsvEntity eucCsvEntity, int i) {
        try {
