 ...
 */
package jp.co.ndensan.reams.db.dbd.batchcontroller.step.DBD710080;
...
import jp.co.ndensan.reams.db.dbd.definition.core.hanyolistfutangendogakunintei.FutanGendoGakuNinteiCsvEnumEntity;
...
/**
 *
 * 汎用リスト出力(負担限度額認定)_process処理クラスです。
 *
 * @reamsid_L DBD-3930-030 panxiaobo
 */
public class HanyoListFutanGendoGakuNinteiProcess extends BatchProcessBase<FutanGendoGakuNinteiEntity> {

    private static final RString EUC_WRITER_DELIMITER = new RString(",");
    private static final RString EUC_WRITER_ENCLOSURE = new RString("\"");
    private static final EucEntityId EUC_ENTITY_ID = new EucEntityId("DBD701008");
    ...
    private static final RString 区分_5 = new RString("5");
    ...
    private static final RString MYBATIS_SELECT_ID = new RString(
            "jp.co.ndensan.reams.db.dbd.persistence.db.mapper.relate.hanyolistfutangendogakunintei."
            + "IHanyoListFutanGendoGakuNinteiMapper.get汎用リスト");
    ...
    protected IBatchReader createReader() {
        ...
        return new BatchDbReader(MYBATIS_SELECT_ID,
                processParamter.toFutanGendoGakuNinteiMybatisParameter(psmShikibetsuTaisho, psmAtesaki, 出力順));
    }
...
//                    }
    ...
    private RString get出力順(IOutputOrder order) {
        ...
        RString 出力順 = MyBatisOrderByClauseCreator.create(HanyoListFutanGendoGakuNinteiOrderby.class, order);
        return 出力順.concat(",介護保険負担限度額認定_履歴番号");
    }
    ...
    private void setBlank2(FutanGendoGakuNinteiEucCsvEntity eucCsvEntity) {
        ...
        eucCsvEntity.set特定入所者申請日(RString.EMPTY);
        eucCsvEntity.set特定入所者申請理由コード(RString.EMPTY);
        eucCsvEntity.set特定入所者決定日(RString.EMPTY);
        eucCsvEntity.set特定入所者決定区分(RString.EMPTY);
        eucCsvEntity.set特定入所者開始日(RString.EMPTY);
        eucCsvEntity.set特定入所者終了日(RString.EMPTY);
        eucCsvEntity.set特定入所者旧措置者(RString.EMPTY);
        eucCsvEntity.set特定入所者負担段階(RString.EMPTY);
        eucCsvEntity.set食費(RString.EMPTY);
        eucCsvEntity.set居住費ユニット型個室(RString.EMPTY);
        eucCsvEntity.set居住費ユニット型準個室(RString.EMPTY);
        eucCsvEntity.set居住費従来型個室特養(RString.EMPTY);
        eucCsvEntity.set居住費従来型個室老健(RString.EMPTY);
        eucCsvEntity.set居住費多床室(RString.EMPTY);
        eucCsvEntity.set特例減額措置区分(RString.EMPTY);
        eucCsvEntity.set居室種別(RString.EMPTY);
        eucCsvEntity.set境界層(RString.EMPTY);
        eucCsvEntity.set特定入所者激変緩和(RString.EMPTY);
        eucCsvEntity.set配偶者の有無(RString.EMPTY);
        eucCsvEntity.set配偶者氏名(RString.EMPTY);
        eucCsvEntity.set配偶者氏名カナ(RString.EMPTY);
        eucCsvEntity.set配偶者生年月日(RString.EMPTY);
        eucCsvEntity.set配偶者住所(RString.EMPTY);
        eucCsvEntity.set配偶者連絡先(RString.EMPTY);
        eucCsvEntity.set配偶者住所２(RString.EMPTY);
        eucCsvEntity.set配偶者課税区分(RString.EMPTY);
        eucCsvEntity.set配偶者住民コード(RString.EMPTY);
        eucCsvEntity.set預貯金額(RString.EMPTY);
        eucCsvEntity.set有価証券評価概算額(RString.EMPTY);
        eucCsvEntity.setその他金額(RString.EMPTY);
    ...
    private void setEucCsvEntity2(FutanGendoGakuNinteiEucCsvEntity eucCsvEntity, FutanGendoGakuNinteiEntity entity) {
...
//        eucCsvEntity.setみなし更新認定(setみなし更新認定(entity.get受給者台帳_みなし要介護区分コード()));
        ...
        eucCsvEntity.set特定入所者申請日(set年月日(entity.get介護保険負担限度額認定_申請年月日()));
        eucCsvEntity.set特定入所者申請理由コード(ShinseiRiyuKubun.toValue(entity.get介護保険負担限度額認定_申請事由()).get名称());
        eucCsvEntity.set特定入所者決定日(set年月日(entity.get介護保険負担限度額認定_決定年月日()));
        eucCsvEntity.set特定入所者決定区分(set決定区分(entity.get介護保険負担限度額認定_決定区分()));
        eucCsvEntity.set特定入所者開始日(set年月日(entity.get介護保険負担限度額認定_適用開始年月日()));
        eucCsvEntity.set特定入所者終了日(set年月日(entity.get介護保険負担限度額認定_適用終了年月日()));
        eucCsvEntity.set特定入所者旧措置者(set旧措置者(entity.get介護保険負担限度額認定_旧措置者有無()));
        eucCsvEntity.set特定入所者負担段階(RiyoshaFutanDankai.toValue(entity.get介護保険負担限度額認定_利用者負担段階()).get名称());
        eucCsvEntity.set食費(new RString(String.valueOf(entity.get介護保険負担限度額認定_食費負担限度額())));
        eucCsvEntity.set居住費ユニット型個室(new RString(String.valueOf(entity.get介護保険負担限度額認定_ユニット型個室())));
        eucCsvEntity.set居住費ユニット型準個室(new RString(String.valueOf(entity.get介護保険負担限度額認定_ユニット型準個室())));
        eucCsvEntity.set居住費従来型個室特養(new RString(String.valueOf(entity.get介護保険負担限度額認定_従来型個室_特養等())));
        eucCsvEntity.set居住費従来型個室老健(new RString(String.valueOf(entity.get介護保険負担限度額認定_従来型個室_老健_療養等())));
        eucCsvEntity.set居住費多床室(new RString(String.valueOf(entity.get介護保険負担限度額認定_多床室())));
        eucCsvEntity.set特例減額措置区分(RString.EMPTY);
        eucCsvEntity.set居室種別(set居室(entity.get介護保険負担限度額認定_居室種別()));
        eucCsvEntity.set境界層(entity.is介護保険負担限度額認定_境界層該当者区分() ? new RString("境") : RString.EMPTY);
        eucCsvEntity.set特定入所者激変緩和(entity.is介護保険負担限度額認定_激変緩和措置対象者区分() ? new RString("緩和") : RString.EMPTY);
        eucCsvEntity.set配偶者の有無(entity.is介護保険負担限度額認定_配偶者の有無() ? new RString("有") : new RString("無"));
        eucCsvEntity.set配偶者氏名(entity.get介護保険負担限度額認定_配偶者氏名().value());
        eucCsvEntity.set配偶者氏名カナ(entity.get介護保険負担限度額認定_配偶者氏名カナ().value());
        eucCsvEntity.set配偶者生年月日(set年月日(entity.get介護保険負担限度額認定_配偶者生年月日()));
        eucCsvEntity.set配偶者住所(entity.get介護保険負担限度額認定_配偶者住所().value());
        eucCsvEntity.set配偶者連絡先(entity.get介護保険負担限度額認定_配偶者連絡先());
    ...
    private RString set課税区分(RString 課税区分) {
        RString 課税区分者new;
        if (課税区分.equals(区分_1)) {
            課税区分者new = new RString("課税");
        } else if (課税区分.equals(区分_2)) {
            課税区分者new = new RString("非課税");
        } else {
            return RString.EMPTY;
        }
        return 課税区分者new;
    ...
    private RString set居室(RString 居室種別) {
        RString 居室種別new;
        if (居室種別.equals(区分_1)) {
            居室種別new = new RString("ユ個");
        } else if (居室種別.equals(区分_2)) {
            居室種別new = new RString("ユ準");
        } else if (居室種別.equals(区分_3)) {
            居室種別new = new RString("従個");
        } else if (居室種別.equals(区分_4)) {
            居室種別new = new RString("多床");
        } else if (居室種別.equals(区分_5)) {
            居室種別new = new RString("従老");
        } else {
            return RString.EMPTY;
        }
        return 居室種別new;
    ...
    private void set指定事業者(FutanGendoGakuNinteiEucCsvEntity eucCsvEntity, FutanGendoGakuNinteiEntity entity) {
    ...
    private PersonalData toPersonalData(FutanGendoGakuNinteiEntity entity) {
