...
import jp.co.ndensan.reams.db.dbd.persistence.db.mapper.relate.ninteishinseiinfoif.IModifyNinteiShinseiJohoMapper;
...
public class ModifyNinteiShinseiJohoProcess extends BatchProcessBase<ModifyNinteiShinseiJohoEntity> {
    ...
    private IModifyNinteiShinseiJohoMapper mapper;
    ...
    protected void initialize() {
        mapper = getMapper(IModifyNinteiShinseiJohoMapper.class);
    }
    ...
    protected IBatchReader createReader() {
        if (parameter.get認定申請IF種類().equals(new RString("3")) || parameter.get認定申請IF種類().equals(new RString("5"))) {
            ...
            return new BatchDbReader(MYBATIS_MAPPER_ID4);
        }
    }
    ...
    protected void process(ModifyNinteiShinseiJohoEntity entity) {
        ...
        RString シーケンシャル番号 = new RString(String.format("%04d", i));
        KouikiyoukaigoNinteishinseiJouhouTempEntity tempEntity = creatTempEntity(entity, シーケンシャル番号);
        ...
        List<DbT4150RenrakusakiJohoEntity> 介護連絡先情報List = mapper.select連絡先情報(entity.get申請書管理番号());
        if (!parameter.get認定申請IF種類().equals(new RString("4"))) {
    ...
    private KouikiyoukaigoNinteishinseiJouhouTempEntity creatTempEntity(ModifyNinteiShinseiJohoEntity entity,
            RString シーケンシャル番号) {
    ...
    private void setTempEntity電算標準版(ModifyNinteiShinseiJohoEntity entity,
        ...
        tempEntity.setShichousonCode(parameter.get市町村コード().value());
