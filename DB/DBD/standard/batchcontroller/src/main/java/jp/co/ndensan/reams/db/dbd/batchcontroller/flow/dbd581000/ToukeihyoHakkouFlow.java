 ...
 */
public class ToukeihyoHakkouFlow extends BatchFlowBase<DBD581000Parameter> {

    private static final String 実施状況表の発行 = "jissiJyokyohyoHakkou";
    private static final String 受給者数状況表の発行 = "ukeishaJyokyohyoHakkou";
    private YokaigoJissiJyokyohyoParameter parameter;
    private final RString 要介護認定実施状況表 = new RString("要介護認定実施状況表（その月に申請・認定を受けた受給者の統計）");
    private final RString 要介護認定月別受給者数状況表 = new RString("要介護認定月別受給者認定者数状況表（その月に基準日時点の受給者の統計）");
    private final RString BATCH_ID_DBD581001 = new RString("JissiJyokyohyoFlow");
    private final RString BATCH_ID_DBD582001 = new RString("DBD582001");

    @Override
    protected void defineFlow() {
        if (parameter.get出力帳票().equals(要介護認定実施状況表)) {
            executeStep(実施状況表の発行);
        }
        if (parameter.get出力帳票().equals(要介護認定月別受給者数状況表)) {
