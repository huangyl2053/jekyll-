...
//                        new ReportId(processParamter.getCyohyoid()), Long.parseLong(processParamter.getSyutsuryokukomoku().toString()));
    ...
    protected void createWriter() {
        ...
        HanyoListHomonKaigoRiyoshaFutanGakuGengakuManager.createInstance().set改頁Key(outputOrder, pageBreakKeys);
    ...
    protected void process(HomonKaigoRiyoshaFutanGakuGengakuEntity entity) {
        ...
        HanyoListHomonKaigoRiyoshaFutanGakuGengakuManager.createInstance().get情報設定(eucCsvEntity, entity, association,
        ...
        int index = 0;
        for (HomonKaigoRiyoshaFutanGakuGengakuCsvEnumEntity e : HomonKaigoRiyoshaFutanGakuGengakuCsvEnumEntity.values()) {
...
//                    Method getMethod;
//                    try {
//                        getMethod = clazz.getDeclaredMethod(HomonKaigoRiyoshaFutanGakuGengakuCsvEnumEntity
...
//            }
        ...
        HanyoListHomonKaigoRiyoshaFutanGakuGengakuManager.createInstance().バッチ出力条件リストの出力(processParamter, 導入団体コード,
    ...
    private void 帳票出力とCSV出力(HomonKaigoRiyoshaFutanGakuGengakuEntity entity, boolean flag) {
        if (is帳票出力) {
            if (flag) {
                HanyoListEntity hanyolistentity = new HanyoListEntity();
                HanyoListHomonKaigoRiyoshaFutanGakuGengakuManager.createInstance().get方法(entity, hanyolistentity);
    ...
    private RString get項目内容(Method getMethod, RString 項目内容new, HomonKaigoRiyoshaFutanGakuGengakuEucCsvEntity eucCsvEntity, int i) {
        try {
