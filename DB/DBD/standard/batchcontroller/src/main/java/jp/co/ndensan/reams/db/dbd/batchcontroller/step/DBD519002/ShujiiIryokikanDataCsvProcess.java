...
/**
 * 主治医医療機関情報CSV_ファイル出力です。
 *
 * @reamsid_L DBD-2100-020 liwul
 */
public class ShujiiIryokikanDataCsvProcess extends BatchProcessBase<ShujiiIryokikanEntity> {

    private static final RString MYBATIS_SELECT_ID = new RString("jp.co.ndensan.reams.db.dbd.persistence.db.mapper.relate.")
            .concat("renekeidatasakuseifourmaster.IRenekeiDataSakuseiFourMasterMapper.get主治医医療機関情報ＩＦのcsv情報");
    ...
    private static final RString ファイル名_日本語 = new RString("主治医医療機関情報CSV");
    private static final RString EUCエンティティID = new RString("DBD519023");
    ...
    private CsvWriter<ShujiiIryokikanDataReamsCsvEntity> csvReamsWriter;
    private CsvWriter<ShujiiIryokikanDataCsvEntity> csvWriter;
    ...
    @Override
    protected void process(ShujiiIryokikanEntity entity) {
        シーケンシャル番号++;
        RenekeiDataSakuseiFourMasterCsvManager manager = new RenekeiDataSakuseiFourMasterCsvManager();
        if (para.isReams()) {
            csvReamsWriter.writeLine(manager.toShujiiIryokikanDataReamsCsvEntity(entity, シーケンシャル番号, para.getShikibetsucode()));
        } else {
            csvWriter.writeLine(manager.toShujiiIryokikanDataCsvEntity(entity, シーケンシャル番号, para.getShikibetsucode()));
        }
    }
