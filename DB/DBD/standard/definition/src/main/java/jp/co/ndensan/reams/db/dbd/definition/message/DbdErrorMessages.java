...
public enum DbdErrorMessages implements IMessageGettable {
    ...
    特別地域加算減免_軽減率範囲外(22, "軽減率は?～100の範囲で設定してください。"),
    ...
    被保_受給者登録なし(40, "既に指定の対象年に?は登録されています。"),
    ...
    預貯金チェック(46, "預貯金情報が未チェックです。"),
    基準日被保険者データなし(47, "基準日時点で被保険者ではありません。"),
    基準日受給者_事業対象者データなし(48, "基準日時点で受給者または事業対象者ではありません。"),
    基準日負担割合変更なし(49, "基準日時点で負担割合の変更はありません。"),
    適用期間_重複_空きエラー(50, "適用開始日と適用終了日の関係に不整合があります。適用期間の重複や空きがないことを確認してください。"),
    適用期間_年度エラー(51, "適用開始日と適用終了日の関係に不整合があります。適用期間が年度内であることを確認してください。"),
    該当年度負担割合情報なし(52, "該当年度の負担割合情報がありません。"),
    年次判定処理不可(53, "年次負担割合判定処理は、当初所得引出後に、該当年度分の処理が行えます。<br>対象は平成27年度以降です。"),
    年次判定未処理(54, "年次負担割合判定処理が未処理です。年次負担割合判定処理を行ってください。"),
    負担割合判定_未抽出期間エラー(55, "前回終了年月日と今回開始年月日に抽出されない期間があります。"),
    抽出期間不正１(56, "今回開始日時には前回終了日時と同じか、それより前の日時を設定してください。"),
    抽出期間不正２(57, "今回終了日時には今回開始日時以降の日時を設定してください。");
