 ...
 */
package jp.co.ndensan.reams.db.dbd.definition.mybatisprm.hanyolistriyoshafutangakugengaku;
...
/**
 *
 * 汎用リスト出力(利用者負担額減免)のMyBatis用パラメータクラスです。
 *
 * @reamsid_L DBD-3950-030 panxiaobo
 */
@SuppressWarnings("PMD.UnusedPrivateField")
@Getter
public class RiyoshaFutanGakuGengakuMybatisParameter implements IMyBatisParameter {
    ...
    private final RString kyusochisha;
     ...
     * @param 出力順 出力順
     */
    public RiyoshaFutanGakuGengakuMybatisParameter(
        ...
        if (kyusochisha.equals(new RString("旧措置者"))) {
            is旧措置者 = true;
        } else if (kyusochisha.equals(new RString("旧措置他"))) {
            is旧措置他 = true;
        }
    ...
    private void set市町村コード() {
        if (nullHandan(atenacyusyutsujyoken.getShichoson_Code())
                && new RString("すべて").equals(atenacyusyutsujyoken.getShichoson_Code().getColumnValue())) {
            has市町村コード = true;
