 ...
 */
package jp.co.ndensan.reams.db.dbd.divcontroller.handler.parentdiv.DBD2000021;
...
import jp.co.ndensan.reams.db.dbd.service.core.panelall.HonsanteiIdoKanendo;
import jp.co.ndensan.reams.db.dbd.service.core.riyoshafutanwariaihanteimanager.RiyoshaFutanWariaiHanteiManager;
...
public class PanelAllHandler {
    ...
    private final DBD2000021PanelAllDiv div;
     ...
     */
    public PanelAllHandler(DBD2000021PanelAllDiv div) {
        this.div = div;
    }
    ...
    public RiyoshaFutanWariaiHanteiManagerResult hannTei(HihokenshaNo 被保険者番号, FlexibleYear 年度, FlexibleDate 基準日) {
        ...
        if (NUM_4.equals(判定結果.get判定区分())) {
            throw new ApplicationException(DbdErrorMessages.基準日被保険者データなし.getMessage());
        } else if (NUM_5.equals(判定結果.get判定区分())) {
            throw new ApplicationException(DbdErrorMessages.基準日受給者_事業対象者データなし.getMessage());
        } else if (NUM_3.equals(判定結果.get判定区分())) {
            throw new ApplicationException(DbdErrorMessages.基準日負担割合変更なし.getMessage());
        }
        return 判定結果;
    }
