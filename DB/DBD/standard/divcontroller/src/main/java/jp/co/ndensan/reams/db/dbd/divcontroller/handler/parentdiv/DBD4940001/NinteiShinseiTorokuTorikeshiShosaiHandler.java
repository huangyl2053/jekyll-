...
public class NinteiShinseiTorokuTorikeshiShosaiHandler {
    ...
    public YokaigoNinteiJoho onLoad(RString 被保険者番号) {
        ...
        YokaigoNinteiJoho 認定情報 = get認定情報(被保険者番号);
        if (null == 認定情報) {
            return null;
        }
        setHdnArea(認定情報);
    ...
    public void save(YokaigoNinteiJoho 認定情報) {
        ...
        YokaigoNinteiJohoManager.createInstance().save介護認定申請情報(認定情報);
...
//    }
    private YokaigoNinteiJoho get認定情報(RString 被保険者番号) {
...
//        RString.EMPTY, 被保険者番号);
        ...
        介護認定申請基本情報入力Div.setTxtShinseiJokyo(ShinseiJokyoKubun.toValue(認定情報.get申請状況区分()).get名称());
