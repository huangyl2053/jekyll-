...
public enum SystemKanriNinteiShienTotalSpec implements IPredicate<SystemKanriNinteiShienTotalDiv> {
    ...
    対象更新のデータ変更チェック {
                ...
                public boolean apply(SystemKanriNinteiShienTotalDiv div) {
                    ...
                    return !ＩＦ種類_初期データ.equals(ＩＦ種類_変更データ)
                    || (!div.getSystemKanri().getSystemKanriNinteiShien().getDdlShujiiChosainJohoTorikomi().isDisplayNone()
                    && !医調取込み_初期データ.equals(医調取込み_変更データ))
                    || !認定ＩＦ改定日_初期データ.equals(認定ＩＦ改定日_変更データ)
                    || !類似変換_初期データ.equals(類似変換_変更データ)
                    || !一括認定日区分_初期データ.equals(一括認定日区分_変更データ)
                    | !認定結果ファイル_初期データ.equals(認定結果ファイル_変更データ)
                    ...
                    || !認定結果ファイル_新_初期データ.equals(認定結果ファイル_新_変更データ)
                    ...
                    || (!div.getSystemKanri().getSystemKanriNinteiShien().getTxtShiboJohoFile().isDisplayNone()
                    && !転入_資格喪失情報_初期データ.equals(死亡情報ファイル_変更データ))
                    || (!div.getSystemKanri().getSystemKanriNinteiShien().getTxtTennyuShiboFile().isDisplayNone()
                    && !転入_資格喪失情報_初期データ.equals(転入死亡ファイル_変更データ));
                }
            };
}
