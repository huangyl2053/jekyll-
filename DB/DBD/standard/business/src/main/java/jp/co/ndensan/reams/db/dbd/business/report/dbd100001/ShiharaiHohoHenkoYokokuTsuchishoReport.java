...
public final class ShiharaiHohoHenkoYokokuTsuchishoReport extends Report<ShiharaiHohoHenkoYokokuTsuchishoReportSource> {
    ...
    public static ShiharaiHohoHenkoYokokuTsuchishoReport createReport(IKojin 個人情報, IAtesaki 宛先,
        ...
        return new ShiharaiHohoHenkoYokokuTsuchishoReport(個人情報, 宛先, 帳票制御共通, 地方公共団体, 文書番号,
                通知書定型文リスト, 認証者ソースビルダー, 帳票情報);
    }

    private ShiharaiHohoHenkoYokokuTsuchishoReport(IKojin 個人情報, IAtesaki 宛先, ChohyoSeigyoKyotsu 帳票制御共通,
    ...
    public void writeBy(ReportSourceWriter<ShiharaiHohoHenkoYokokuTsuchishoReportSource> writer) {
        ...
        for (int index = 0; index < 年度1リスト.size() || index < 年度2リスト.size() || index < 年度3リスト.size(); index++) {
            IShiharaiHohoHenkoYokokuTsuchishoEditor bodyEditor = new ShiharaiHohoHenkoYokokuTsuchishoEditor(個人情報, 宛先,
                    帳票制御共通, 地方公共団体, 文書番号, 通知書定型文リスト, 認証者ソースビルダー, 帳票情報, 最新賦課年度,
                    年度1リスト, 年度2リスト, 年度3リスト, index);
