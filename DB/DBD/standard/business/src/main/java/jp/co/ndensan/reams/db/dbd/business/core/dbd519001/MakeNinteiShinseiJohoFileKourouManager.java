...
/**
 * 申請情報IFファイル作成厚労省_business処理クラスです。
 *
 * @reamsid_L DBD-1480-020 liuyl
 */
public class MakeNinteiShinseiJohoFileKourouManager {
     ...
     * @return MakeNinteiShinseiJohoFileCsvKourouEntity
     */
    public MakeNinteiShinseiJohoFileCsvKourouEntity createKourouEntity(KouikiyoukaigoNinteishinseiJouhouTempEntity entity) {
    ...
    public EucFileOutputJokenhyoItem バッチ出力条件リストの出力(MakeNinteiShinseiJohoFileProcessParameter parameter, Association 地方公共団体) {
        ...
        if (parameter.get被保険者番号() != null) {
            出力条件.add(被保険者番号.concat(parameter.get被保険者番号().value()));
        } else {
            出力条件.add(被保険者番号.concat(RString.HALF_SPACE));
        }
