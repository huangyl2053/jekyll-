...
public class RiysFutgGengMenjNinteishoBodyEditor implements IRiysFutgGengMenjNinteishoEditor {
    ...
    private RiysFutgGengMenjNinteishoReportSource bodyEdit(RiysFutgGengMenjNinteishoReportSource source) {
        ...
        source.tekiyoGengo = item.get利用者負担額減額情報().get適用開始年月日().wareki().toDateString().substring(INDEX_1, INDEX_2);
        source.tekiyoYYYY = item.get利用者負担額減額情報().get適用開始年月日().wareki().toDateString().substring(INDEX_2, INDEX_4);
        source.tekiyoMM = item.get利用者負担額減額情報().get適用開始年月日().wareki().toDateString().substring(INDEX_5, INDEX_7);
        source.tekiyoDD = item.get利用者負担額減額情報().get適用開始年月日().wareki().toDateString().substring(INDEX_8, INDEX_10);
        source.yukoGengo = item.get利用者負担額減額情報().get適用終了年月日().wareki().toDateString().substring(INDEX_1, INDEX_2);
        source.yukoYYYY = item.get利用者負担額減額情報().get適用終了年月日().wareki().toDateString().substring(INDEX_2, INDEX_4);
        source.yukoMM = item.get利用者負担額減額情報().get適用終了年月日().wareki().toDateString().substring(INDEX_5, INDEX_7);
        source.yukoDD = item.get利用者負担額減額情報().get適用終了年月日().wareki().toDateString().substring(INDEX_8, INDEX_10);
