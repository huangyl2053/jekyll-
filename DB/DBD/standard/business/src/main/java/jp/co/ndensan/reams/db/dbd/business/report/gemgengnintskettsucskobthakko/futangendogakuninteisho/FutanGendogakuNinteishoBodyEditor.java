...
import jp.co.ndensan.reams.db.dbd.entity.report.futangendogakuninteisho.FutanGendogakuNinteishoReportSource;
...
public class FutanGendogakuNinteishoBodyEditor implements IFutanGendogakuNinteishoEditor {
    ...
    private FutanGendogakuNinteishoReportSource bodyEdit(FutanGendogakuNinteishoReportSource source) {
        ...
        source.tekiyoGengo = item.get負担限度額認定().get適用開始年月日().wareki().toDateString().substring(1, 2);
        source.tekiyoYYYY = item.get負担限度額認定().get適用開始年月日().wareki().toDateString().substring(2, 4);
        source.tekiyoMM = item.get負担限度額認定().get適用開始年月日().wareki().toDateString().substring(5, 7);
        source.tekiyoDD = item.get負担限度額認定().get適用開始年月日().wareki().toDateString().substring(8, 10);
        source.yukoGengo = item.get負担限度額認定().get適用終了年月日().wareki().toDateString().substring(1, 2);
        source.yukoYYYY = item.get負担限度額認定().get適用終了年月日().wareki().toDateString().substring(2, 4);
        source.yukoMM = item.get負担限度額認定().get適用終了年月日().wareki().toDateString().substring(5, 7);
        source.yukoDD = item.get負担限度額認定().get適用終了年月日().wareki().toDateString().substring(8, 10);

        setKyojuhiName(source, item);
        setFutanGaku(source, item);

        source.hokenshaNo1 = item.get負担限度額認定().get被保険者番号().getColumnValue().substring(0, 1);
        source.hokenshaNo2 = item.get負担限度額認定().get被保険者番号().getColumnValue().substring(1, 2);
        source.hokenshaNo3 = item.get負担限度額認定().get被保険者番号().getColumnValue().substring(2, 3);
        source.hokenshaNo4 = item.get負担限度額認定().get被保険者番号().getColumnValue().substring(3, 4);
        source.hokenshaNo5 = item.get負担限度額認定().get被保険者番号().getColumnValue().substring(4, 5);
        source.hokenshaNo6 = item.get負担限度額認定().get被保険者番号().getColumnValue().substring(5, 6);
    ...
    private static NinshoshaSource getCompNinshosha(FutanGendogakuNinteishoItem item) {
