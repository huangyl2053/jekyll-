 ...
 */
package jp.co.ndensan.reams.db.dbd.business.core.dbd710070;
...
/**
 * 汎用リスト出力(特別地域加算減免)_Businessクラスです。
 *
 * @reamsid_L DBD-3910-030 panxiaobo
 */
public class HanyoListTokubetsuChiikiKasanBusiness {
    ...
    public void setEucCsvEntity(Association association, boolean csvhitsukesurasyuhensyu,
            TokubetsuChiikiKasanGemmenEucCsvEntity eucCsvEntity, TokubetsuChiikiKasanGemmenEntity entity, HokenshaList hokenshaList, RString 市町村名) {
        ...
        eucCsvEntity.set資格証記載保険者番号((lasdecCode.isEmpty()) ? RString.EMPTY : hokenshaList.get(lasdecCode).get証記載保険者番号().value());
     ...
     */
    public TokubetsuChiikiKasanGemmenEucCsvEntity setNewBlank() {
    ...
    private void setBlank2(TokubetsuChiikiKasanGemmenEucCsvEntity eucCsvEntity) {
        ...
        eucCsvEntity.set特別地域加算減免申請日(RString.EMPTY);
        eucCsvEntity.set特別地域加算減免決定日(RString.EMPTY);
        eucCsvEntity.set特別地域加算減免承認区分(RString.EMPTY);
        eucCsvEntity.set特別地域加算減免開始日(RString.EMPTY);
        eucCsvEntity.set特別地域加算減免終了日(RString.EMPTY);
        eucCsvEntity.set特別地域加算減免率(RString.EMPTY);
    ...
    private void setEucCsvEntity2(TokubetsuChiikiKasanGemmenEucCsvEntity eucCsvEntity, TokubetsuChiikiKasanGemmenEntity entity) {
...
//        eucCsvEntity.setみなし更新認定(setみなし更新認定(entity.get受給者台帳_みなし要介護区分コード()));
        ...
        eucCsvEntity.set確認番号(entity.get特別地域加算減免_確認番号());
        eucCsvEntity.set特別地域加算減免申請日(set年月日(entity.get特別地域加算減免_申請年月日()));
        eucCsvEntity.set特別地域加算減免決定日(set年月日(entity.get特別地域加算減免_決定年月日()));
        eucCsvEntity.set特別地域加算減免承認区分(entity.get特別地域加算減免_決定区分().equals(区分_0) ? 却下 : 承認);
        eucCsvEntity.set特別地域加算減免開始日(set年月日(entity.get特別地域加算減免_適用開始年月日()));
        eucCsvEntity.set特別地域加算減免終了日(set年月日(entity.get特別地域加算減免_適用終了年月日()));
        eucCsvEntity.set特別地域加算減免率(entity.get特別地域加算減免_減額率());
    ...
    private void set指定事業者(TokubetsuChiikiKasanGemmenEucCsvEntity eucCsvEntity, TokubetsuChiikiKasanGemmenEntity entity) {
