...
@lombok.Setter
@OnNextSchema("rgdb")
public class HikazeiNenkinTaishoshaReibanNashiEUCEntity implements IHikazeiNenkinTaishoshaEUCEntity {
    ...
    @CsvField(order = 55, name = "年度")
    private RString 年度;
    @CsvField(order = 56, name = "処理区分")
    private RString 処理区分;
    @CsvField(order = 57, name = "対象月")
    private RString 対象月;
    @CsvField(order = 58, name = "基礎年金番号")
    private RString 基礎年金番号;
    @CsvField(order = 59, name = "現基礎年金番号")
    private RString 現基礎年金番号;
    @CsvField(order = 60, name = "年金コード")
    private RString 年金コード;
    @CsvField(order = 61, name = "IF市町村コード")
    private RString if市町村コード;
    @CsvField(order = 62, name = "IF年金保険者コード")
    private RString if年金保険者コード;
    @CsvField(order = 63, name = "IF通知内容コード")
    private RString if通知内容コード;
    @CsvField(order = 64, name = "IF作成年月日")
    private RString if作成年月日;
    @CsvField(order = 65, name = "IF性別")
    private RString if性別;
    @CsvField(order = 66, name = "IFカナ氏名")
    private RString ifカナ氏名;
    @CsvField(order = 67, name = "IF漢字氏名")
    private RString if漢字氏名;
    @CsvField(order = 68, name = "IF郵便番号")
    private RString if郵便番号;
    @CsvField(order = 69, name = "IFカナ住所")
    private RString ifカナ住所;
    @CsvField(order = 70, name = "IF漢字住所")
    private RString if漢字住所;
    @CsvField(order = 71, name = "IF対象年")
    private RString if対象年;
    @CsvField(order = 72, name = "IF訂正表示")
    private RString if訂正表示;
    @CsvField(order = 73, name = "IF各種区分")
    private RString if各種区分;
    @CsvField(order = 74, name = "IF処理結果")
    private RString if処理結果;
    @CsvField(order = 75, name = "IF金額１")
    private RString if金額１;
    @CsvField(order = 76, name = "登録区分")
