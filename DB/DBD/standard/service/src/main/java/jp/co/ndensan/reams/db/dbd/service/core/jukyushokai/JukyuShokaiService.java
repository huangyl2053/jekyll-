...
public class JukyuShokaiService {
    ...
    public List<NinteiChosaJokyoDataPass> get履歴情報(HihokenshaNo 被保険者番号, FlexibleDate 認定日) {
        ...
        for (NinteiChosaJokyoDataPassEntity entity : result) {
            ...
            dataPass.set主治医意見書作成依頼年月日(entity.get主治医意見書作成依頼年月日());
            dataPass.set主治医意見書登録予定年月日(entity.get主治医意見書登録予定年月日());
            dataPass.set主治医意見書記入年月日(entity.get主治医意見書記入年月日());
            ...
            dataPass.set二次判定年月日(entity.get二次判定年月日());
            ...
            dataPass.set介護認定審査会資料作成年月日(entity.get介護認定審査会資料作成年月日());
            ...
            dataPass.set延期通知発行年月日(entity.get延期通知発行年月日());
            dataPass.set指定医フラグ(entity.is指定医フラグ());
            dataPass.set申請書管理番号(entity.get申請書管理番号());
            dataPass.set要介護認定一次判定年月日(entity.get要介護認定一次判定年月日());
            ...
            dataPass.set認定審査会予定年月日(entity.get認定審査会予定年月日());
            dataPass.set認定調査予定年月日(entity.get認定調査予定年月日());
            dataPass.set認定調査依頼年月日(entity.get認定調査依頼年月日());
            dataPass.set認定調査員コード(entity.get認定調査員コード());
            dataPass.set認定調査委託先コード(entity.get認定調査委託先コード());
            dataPass.set認定調査実施年月日(entity.get認定調査実施年月日());
