...
public class ShiharaiHohoHenkoYokokuTsuchishoService {
     ...
     */
    public void print(IKojin 個人情報, IAtesaki 宛先, List<ChohyoSeigyoHanyo> 帳票制御汎用リスト, RString 文書番号,
            ChohyoSeigyoKyotsu 帳票制御共通, Association 地方公共団体, FlexibleDate 発行日, List<RString> 通知書定型文リスト,
            RString 帳票分類ID, Ninshosha 認証者, ShiharaiHohoHenko 帳票情報, ReportManager reportManager) {
...
//                report.writeBy(reportSourceWriter);
    ...
    private static List<ShiharaiHohoHenkoYokokuTsuchishoReport> toReport(IKojin 個人情報, IAtesaki 宛先, Ninshosha 認証者,
            List<ChohyoSeigyoHanyo> 帳票制御汎用リスト, ChohyoSeigyoKyotsu 帳票制御共通, Association 地方公共団体, RString 帳票分類ID,
            FlexibleDate 発行日, RString 文書番号, List<RString> 通知書定型文リスト, ShiharaiHohoHenko 帳票情報) {
    ...
    private Iterable<ShiharaiHohoHenkoYokokuTsuchishoReport> toReport(IKojin 個人情報, IAtesaki 宛先, List<ChohyoSeigyoHanyo> 帳票制御汎用リスト, ChohyoSeigyoKyotsu 帳票制御共通, Association 地方公共団体, FlexibleDate 発行日, RString 文書番号, List<RString> 通知書定型文リスト, RString 帳票分類ID, Ninshosha 認証者, ShiharaiHohoHenko 帳票情報) {
