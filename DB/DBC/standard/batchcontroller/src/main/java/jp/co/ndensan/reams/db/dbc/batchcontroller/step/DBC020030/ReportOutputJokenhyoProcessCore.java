...
public class ReportOutputJokenhyoProcessCore {
     ...
     * @return 出力条件
     */
    public List<RString> get出力条件(KogakuKaigoServiceProcessParameter parameter, IOutputOrder order) {
        List<RString> 出力条件 = new ArrayList<>();
        if (受付日.equals(parameter.get抽出モード())) {
            出力条件.add(TITLE_受付日.concat(RString.HALF_SPACE)
                    .concat(toパターン6(parameter.get抽出条件日付From())).concat(RString.FULL_SPACE).concat(波線).concat(RString.FULL_SPACE)
                    .concat(toパターン6(parameter.get抽出条件日付To())));
        } else if (決定日.equals(parameter.get抽出モード())) {
            出力条件.add(TITLE_決定日_期間.concat(RString.HALF_SPACE)
                    .concat(toパターン6(parameter.get抽出条件日付From())).concat(RString.FULL_SPACE).concat(波線).concat(RString.FULL_SPACE)
                    .concat(toパターン6(parameter.get抽出条件日付To())));
