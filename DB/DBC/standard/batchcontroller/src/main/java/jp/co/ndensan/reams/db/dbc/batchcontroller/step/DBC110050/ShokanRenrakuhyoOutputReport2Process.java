 ...
 */
public class ShokanRenrakuhyoOutputReport2Process extends BatchKeyBreakBase<ShokanRenrakuhyoOutputReportEntity> {
    ...
    private ShokanRenrakuhyoSofuIchiranCsvEntity editCsvEntity(ShokanRenrakuhyoOutputReportEntity entity) {
        ...
        if (entity.get償還払支給申請一時TBL().getServiceTeikyoYM() != null) {
            ...
            if (!entity.get償還払支給申請一時TBL().getYokaigoJotaiKubunCode().isEmpty()) {
    ...
    private RString get帳票タイトル() {
        ...
        mybatisParam.set帳票分類ID(ReportIdDBC.DBC200026.getReportId().getColumnValue());
