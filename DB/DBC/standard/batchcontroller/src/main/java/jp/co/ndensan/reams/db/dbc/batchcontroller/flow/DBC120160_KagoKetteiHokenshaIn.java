...
import jp.co.ndensan.reams.db.dbc.batchcontroller.step.DBC120160.KagoKetteiHokenshaDoIchiranhyoSakuseiProcess;
 ...
 */
public class DBC120160_KagoKetteiHokenshaIn extends BatchFlowBase<DBC120160_KagoKetteiHokenshaInParameter> {
    ...
    private static final RString ファイル格納フォルダ名 = new RString("DBC120160");
    ...
    protected void defineFlow() {
        try {
            ...
            交換情報識別番号 = DbBusinessConfig.get(
                    ConfigNameDBC.国保連取込_介護給付費過誤決定通知書情報_交換情報識別番号, now, SubGyomuCode.DBC介護給付);
            executeStep(ファイル取得);
            returnEntity
            ...
            flowEntity = getResult(FlowEntity.class, new RString(CSVファイル取込),
                    SogojigyohiKagoKetteiHokenshaReadCsvFileProcess.PARAMETER_OUT_FLOWENTITY);
     ...
     * CSVファイル取込です。
     *
     * @return SogojigyohiKagoKetteiHokenshaReadCsvFileProcess
     */
    @Step(CSVファイル取込)
    protected IBatchFlowCommand callReadCsvFileProcess() {
        KokuhorenkyotsuReadCsvFileProcessParameter parameter = new KokuhorenkyotsuReadCsvFileProcessParameter();
        ...
        return simpleBatch(SogojigyohiKagoKetteiHokenshaReadCsvFileProcess.class).arguments(parameter).define();
    }
    ...
    protected IBatchFlowCommand callDoIchiranhyoSakuseiProcess() {
        KagoKetteiHokenshaDoIchiranhyoSakuseiProcessParameter parameter
                = new KagoKetteiHokenshaDoIchiranhyoSakuseiProcessParameter();
        parameter.setサブ業務コード(SubGyomuCode.DBC介護給付);
        parameter.setシステム日付(RDateTime.now());
        parameter.set処理年月(getParameter().get処理年月());
        parameter.set出力順ID(getParameter().get出力順ID());
        parameter.set帳票ID(ReportIdDBC.DBC200050.getReportId());
        return loopBatch(KagoKetteiHokenshaDoIchiranhyoSakuseiProcess.class).arguments(parameter).define();
    }
