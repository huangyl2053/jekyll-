...
public class DBC120100_ShokanFushikyuKetteiIn extends BatchFlowBase<DBC120100_ShokanFushikyuKetteiInParameter> {
    ...
    private static final RString ファイル格納フォルダ名 = new RString("DBC120100");
    ...
    protected void defineFlow() {
        try {
            ...
            交換情報識別番号 = DbBusinessConfig.get(
                    ConfigNameDBC.国保連取込_償還払不支給決定者一覧情報_交換情報識別番号,
            ...
            for (int i = 0; i < returnEntity.getFileNameList().size(); i++) {
                ...
                flowEntity = getResult(FlowEntity.class, new RString(CSVファイル取込),
                        ShokanFushikyuKetteiInReadCsvFileProcess.PARAMETER_OUT_FLOWENTITY);
            }
    ...
    protected IBatchFlowCommand callReadCsvFileProcess() {
        ...
        return loopBatch(ShokanFushikyuKetteiInReadCsvFileProcess.class).arguments(parameter).define();
    }
    ...
    protected IBatchFlowCommand callDoMasterTorokuProcess() {
        ...
        return simpleBatch(ShokanFushikyuKetteiDoDBTorokuProcess.class).arguments(parameter).define();
    }
    ...
    protected IBatchFlowCommand callDoIchiranhyoSakuseiProcess() {
        ...
        parameter.set帳票ID(ReportIdDBC.DBC200022.getReportId());
        parameter.setシステム日付(RDate.getNowDateTime());
        return loopBatch(ShokanFushikyuKetteiInProcess.class).arguments(parameter)
                .define();
    }
