...
public class IdouShouKannSembetsuProcess extends BatchProcessBase<SetaiYinEntity> {
    ...
    protected void afterExecute() {
        ...
        if (異動中間高額送付情報 != null && !異動中間高額送付情報.isEmpty()) {
            RString 最新の情報 = 異動中間高額送付情報.get(0).get証記載保険者番号().value().concat(異動中間高額送付情報.get(0).get世帯集約番号().value())
                    .concat(異動中間高額送付情報.get(0).get世帯所得区分コード()).concat(異動中間高額送付情報.get(0).get所得区分コード());
            RString 過去の情報 = 異動中間高額送付情報.get(1).get証記載保険者番号().value().concat(異動中間高額送付情報.get(1).get世帯集約番号().value())
                    .concat(異動中間高額送付情報.get(1).get世帯所得区分コード()).concat(異動中間高額送付情報.get(1).get所得区分コード());
            ...
            RString 最新の証記載保険者番号 = 異動中間高額送付情報.get(0).get世帯集約番号().value()
                    .concat(異動中間高額送付情報.get(0).get世帯所得区分コード()).concat(異動中間高額送付情報.get(0).get所得区分コード());
            RString 過去の証記載保険者番号 = 異動中間高額送付情報.get(1).get世帯集約番号().value()
                    .concat(異動中間高額送付情報.get(1).get世帯所得区分コード()).concat(異動中間高額送付情報.get(1).get所得区分コード());
            if (最新の証記載保険者番号.equals(過去の証記載保険者番号) && 異動中間高額送付情報.get(0).is老齢福祉年金受給有フラグ()
