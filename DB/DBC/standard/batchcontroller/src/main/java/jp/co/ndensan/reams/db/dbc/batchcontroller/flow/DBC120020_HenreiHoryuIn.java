...
import jp.co.ndensan.reams.db.dbc.definition.batchprm.DBC120020.DBC120020_HenreiHoryuInParameter;
...
/**
 * 請求明細・給付管理票返戻（保留）一覧表取込のバッチフロークラスです。
 *
 * @reamsid_L DBC-2830-010 yaodongsheng
 */
public class DBC120020_HenreiHoryuIn extends BatchFlowBase<DBC120020_HenreiHoryuInParameter> {
    ...
    private static final RString ファイル格納フォルダ名 = new RString("DBC120020");
    ...
    protected void defineFlow() {
        try {
            帳票ID = ReportIdDBC.DBC200001.getReportId();
            RDate now = RDate.getNowDate();
            交換情報識別番号 = DbBusinessConfig.get(
                    ConfigNameDBC.国保連取込_請求明細給付管理票返戻保留一覧表情報_交換情報識別番号,
            ...
            for (int i = 0; i < returnEntity.getFileNameList().size(); i++) {
                ...
                flowEntity = getResult(SogojigyohiKohiJukyushaFlowEntity.class, new RString(CSVファイル取込),
                        HenreiHoryuInReadCsvFileProcess.PARAMETER_OUT_FLOWENTITY);
     ...
     * CSVファイル取込です。
     *
     * @return HenreiHoryuInReadCsvFileProcess
     */
    @Step(CSVファイル取込)
