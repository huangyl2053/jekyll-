...
/**
 * 保険者保有給付実績情報作成の処理詳細_DB更新Processです。
 *
 * @reamsid_L DBC-2460-010 gongliang
 */
public class UpdDataRecordTempProcess extends BatchProcessBase<DbWT1131KyufujissekiKihonTempEntity> {
    ...
    private static final RString READ_DATA_ID = new RString("jp.co.ndensan.reams.db.dbc.persistence.db.mapper.relate.hokenshakyufujisseki."
            + "IHokenshaKyufujissekiMapper.get給付実績基本一時");
    ...
    protected void process(DbWT1131KyufujissekiKihonTempEntity entity) {
        ...
        finder.update給付実績特定診療費(mybatisParam);
        finder.update給付実績特定診療費_特別療養費(mybatisParam);
        finder.update給付実績食事費用(mybatisParam);
        finder.update給付実績居宅サービス計画費(mybatisParam);
        finder.update給付実績福祉用具販売費(mybatisParam);
        finder.update給付実績住宅改修費(mybatisParam);
        finder.update給付実績集計(mybatisParam);
        finder.update給付実績特定入所者介護サービス費用(mybatisParam);
        finder.update給付実績社会福祉法人軽減額(mybatisParam);
        finder.update給付実績所定疾患施設療養費等(mybatisParam);
        finder.update給付実績明細_住所地特例(mybatisParam);
    }
