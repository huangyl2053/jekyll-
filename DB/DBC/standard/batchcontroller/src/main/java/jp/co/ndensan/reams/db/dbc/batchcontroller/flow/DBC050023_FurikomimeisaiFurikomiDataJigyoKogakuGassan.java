...
import jp.co.ndensan.reams.db.dbc.batchcontroller.step.DBC050021.FurikomimeisaiDataReportProcess;
...
import jp.co.ndensan.reams.db.dbc.batchcontroller.step.DBC050023.DelJigyoKogakuGassanKetteiProcess;
import jp.co.ndensan.reams.db.dbc.batchcontroller.step.DBC050023.InsJigyoKogakuGassanProcess;
import jp.co.ndensan.reams.db.dbc.batchcontroller.step.DBC050023.JigyoKogakuGassanJukyushaProcess;
import jp.co.ndensan.reams.db.dbc.batchcontroller.step.DBC050023.UpdJigyoKogakuGassanKetteiProcess;
...
/**
 * 振込明細・振込みデータ作成（事業高額合算）のバッチフロークラスです。
 *
 * @reamsid_L DBC-4870-030 gongliang
 */
public class DBC050023_FurikomimeisaiFurikomiDataJigyoKogakuGassan
        extends BatchFlowBase<DBC050023_FurikomimeisaiFurikomiDataJigyoKogakuGassanParameter> {

    private static final String 事業高額合算データ取得 = "insJigyoKogakuGassanProcess";
    ...
    private static final String 依頼済登録_事業高額合算 = "updJigyoKogakuGassanKetteiProcess";
    private static final String 依頼済取消_事業高額合算 = "delJigyoKogakuGassanKetteiProcess";
    private static final String 振込明細一覧表作成_受給取得状況 = "jigyoKogakuGassanJukyushaProcess";
    private static final String 振込明細一覧表作成 = "furikomimeisaiDataReportProcess";
    ...
    protected void initialize() {
        ...
        processParameter.setBatchKunbun(事業高額合算KUNBUN);
        processParameter.set振込業務内区分(FurikomiGyomunaiKubun.事業高額合算);
    ...
    protected void defineFlow() {
        executeStep(事業高額合算データ取得);
        int 登録件数 = getResult(
                Integer.class, new RString(事業高額合算データ取得), InsJigyoKogakuGassanProcess.PARAMETER_OUT_登録件数);
        if (登録件数 != INT_0) {
    ...
    private void splitBatch() {
        ...
        executeStep(依頼済登録_事業高額合算);
        executeStep(依頼済取消_事業高額合算);
    }
     ...
     */
    @Step(事業高額合算データ取得)
    protected IBatchFlowCommand callInsJigyoKogakuGassanProcess() {
        return loopBatch(InsJigyoKogakuGassanProcess.class).arguments(processParameter).define();
    }
     ...
     */
    @Step(依頼済登録_事業高額合算)
    protected IBatchFlowCommand callUpdJigyoKogakuGassanKetteiProcess() {
        return loopBatch(UpdJigyoKogakuGassanKetteiProcess.class).define();
    }
     ...
     */
    @Step(依頼済取消_事業高額合算)
    protected IBatchFlowCommand callDelJigyoKogakuGassanKetteiProcess() {
        return loopBatch(DelJigyoKogakuGassanKetteiProcess.class).define();
    }
    ...
    protected IBatchFlowCommand callJigyoKogakuGassanJukyushaProcess() {
        return loopBatch(JigyoKogakuGassanJukyushaProcess.class).arguments(processParameter).define();
    }
     ...
     */
    @Step(振込明細一覧表作成)
    protected IBatchFlowCommand callFurikomimeisaiDataReportProcess() {
        processParameter.set出力順ID(getParameter().get出力順ID());
        return loopBatch(FurikomimeisaiDataReportProcess.class).arguments(processParameter).define();
    }
