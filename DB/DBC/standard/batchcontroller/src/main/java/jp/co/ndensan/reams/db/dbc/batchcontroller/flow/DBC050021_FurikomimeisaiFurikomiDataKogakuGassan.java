...
import jp.co.ndensan.reams.db.dbc.batchcontroller.step.DBC050021.DelKogakuGassanKetteiProcess;
...
import jp.co.ndensan.reams.db.dbc.batchcontroller.step.DBC050021.InsKogakuGassanProcess;
import jp.co.ndensan.reams.db.dbc.batchcontroller.step.DBC050021.KogakuGassanJukyushaProcess;
import jp.co.ndensan.reams.db.dbc.batchcontroller.step.DBC050021.UpdKogakuGassanKetteiProcess;
...
/**
 * 振込明細・振込みデータ作成（高額合算）のバッチフロークラスです。
 *
 * @reamsid_L DBC-4870-030 gongliang
 */
public class DBC050021_FurikomimeisaiFurikomiDataKogakuGassan extends BatchFlowBase<DBC050021_FurikomimeisaiFurikomiDataKogakuGassanParameter> {

    private static final String 高額合算データ取得 = "insKogakuGassanProcess";
    ...
    private static final String 依頼済登録_高額合算 = "updKogakuGassanKetteiProcess";
    private static final String 依頼済取消_高額合算 = "delKogakuGassanKetteiProcess";
    private static final String 振込明細一覧表作成_受給取得状況 = "kogakuGassanJukyushaProcess";
    ...
    protected void initialize() {
        ...
        processParameter.setBatchKunbun(高額合算KUNBUN);
        processParameter.set振込業務内区分(FurikomiGyomunaiKubun.高額合算);
        processParameter.setシステム日時(YMDHMS.now());
        processParameter.set処理名(ShoriName.給付振込データ作成_高額合算);
    ...
    protected void defineFlow() {
        executeStep(高額合算データ取得);
        int 登録件数 = getResult(
                Integer.class, new RString(高額合算データ取得), InsKogakuGassanProcess.PARAMETER_OUT_登録件数);
        if (登録件数 != INT_0) {
     ...
     */
    @Step(高額合算データ取得)
    protected IBatchFlowCommand callInsKogakuGassanProcess() {
        return loopBatch(InsKogakuGassanProcess.class).arguments(processParameter).define();
    }
     ...
     */
    @Step(依頼済登録_高額合算)
    protected IBatchFlowCommand callUpdKogakuGassanKetteiProcess() {
        return loopBatch(UpdKogakuGassanKetteiProcess.class).define();
    }
     ...
     */
    @Step(依頼済取消_高額合算)
    protected IBatchFlowCommand callDelKogakuGassanKetteiProcess() {
        return loopBatch(DelKogakuGassanKetteiProcess.class).define();
    }
    ...
    protected IBatchFlowCommand callKogakuGassanJukyushaProcess() {
        return loopBatch(KogakuGassanJukyushaProcess.class).arguments(processParameter).define();
    }
