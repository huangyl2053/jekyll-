 ...
 */
public class KyodoShoriShouKannIttokiProcess extends BatchProcessBase<DbT3003KyodoShoriyoJukyushaIdoShokanSofuEntity> {

    private static final RString MYBATIS_SELECT_ID = new RString(
            "jp.co.ndensan.reams.db.dbc.persistence.db.mapper.relate.honnsanteifuka."
            + "IHonnSanteiFukaMapper.select共同処理償還送付一時");
    private static final RString TABLE_共同処理償還送付 = new RString("KyodoShoriShouKannIttokiTemp");
    private HonnsanteiFukaProcessParamter processParameter;
    @BatchWriter
    BatchEntityCreatedTempTableWriter 共同処理償還送付一時;
    ...
    protected void createWriter() {
        共同処理償還送付一時 = new BatchEntityCreatedTempTableWriter(TABLE_共同処理償還送付,
    ...
    protected void process(DbT3003KyodoShoriyoJukyushaIdoShokanSofuEntity entity) {
        共同処理償還送付一時.insert(business.set共同処理償還送付全件(entity));
    }
}
