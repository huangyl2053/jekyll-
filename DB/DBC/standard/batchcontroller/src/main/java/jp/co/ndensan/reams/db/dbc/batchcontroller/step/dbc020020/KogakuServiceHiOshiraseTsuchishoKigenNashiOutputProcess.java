 ...
 */
public class KogakuServiceHiOshiraseTsuchishoKigenNashiOutputProcess extends BatchProcessBase<ShinseiJohoChohyoTempRelateEntity> {
    ...
    @BatchWriter
    private BatchReportWriter<KogakuOshiraseTsuchiTeshutsuKigenNashiSource> 自動償還分BatchReportWriter;
    private ReportSourceWriter<KogakuOshiraseTsuchiTeshutsuKigenNashiSource> 自動償還分ReportSourceWriter;
    ...
    protected void createWriter() {
        通常分BatchReportWriter = BatchReportFactory.createBatchReportWriter(ReportIdDBC.DBC100012_申請分.getReportId().value()).create();
        通常分ReportSourceWriter = new ReportSourceWriter<>(通常分BatchReportWriter);

        自動償還分BatchReportWriter = BatchReportFactory.createBatchReportWriter(ReportIdDBC.DBC100012_償還分.getReportId().value()).create();
        自動償還分ReportSourceWriter = new ReportSourceWriter<>(自動償還分BatchReportWriter);
    }

    @Override
    protected void beforeExecute() {
        認証者情報_通常分 = ReportUtil.get認証者情報(SubGyomuCode.DBC介護給付, ReportIdDBC.DBC100012_申請分.getReportId(), FlexibleDate.getNowDate(),
                NinshoshaDenshikoinshubetsuCode.保険者印, 通常分ReportSourceWriter);
        認証者情報_償還分 = ReportUtil.get認証者情報(SubGyomuCode.DBC介護給付, ReportIdDBC.DBC100012_償還分.getReportId(), FlexibleDate.getNowDate(),
