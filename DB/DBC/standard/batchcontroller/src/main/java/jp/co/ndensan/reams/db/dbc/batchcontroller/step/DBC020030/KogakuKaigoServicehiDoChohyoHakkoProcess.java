...
public class KogakuKaigoServicehiDoChohyoHakkoProcess extends BatchKeyBreakBase<KogakuServiceReportEntity> {
    ...
    List<KogakuShikyuFushikyuKetteiTsuchiHakkoEntity> itemList = new ArrayList<>();
    ...
    protected void createWriter() {
        if (帳票タイプ_1.equals(帳票タイプ)) {
            batchReportWriter1 = BatchReportFactory.
                    createBatchReportWriter(ReportIdDBC.DBC100007_支給.getReportId().getColumnValue()).create();
            ...
            batchReportWriter2
                    = BatchReportFactory.createBatchReportWriter(ReportIdDBC.DBC100008.getReportId().getColumnValue()).create();
            ...
            batchReportWriter3
                    = BatchReportFactory.createBatchReportWriter(ReportIdDBC.DBC100009.getReportId().getColumnValue()).create();
            ...
            batchReportWriter4
                    = BatchReportFactory.createBatchReportWriter(ReportIdDBC.DBC100010.getReportId().getColumnValue()).create();
    ...
    protected void usualProcess(KogakuServiceReportEntity entity) {
        ...
        if (帳票タイプ_1.equals(帳票タイプ)) {
            ...
            KogakuKetteiTsuchiShoShiharaiYoteiBiYijiNaReport report1
                    = new KogakuKetteiTsuchiShoShiharaiYoteiBiYijiNaReport(reportEntity1, 連番, getタイトル(一時Entity), get通知書定型文(),
            ...
            KogakuKetteiTsuchiShoSealerReport report3 = new KogakuKetteiTsuchiShoSealerReport(reportEntity3, parameter.get文書番号(),
                    get通知書定型文(), インフォ, ninshoshaSource3, タイトルlist, 連番);
    ...
    protected void afterExecute() {
        ...
        Decimal 支給額給額合計_支給 = Decimal.ZERO;
        Decimal 本人支給額合計_支給 = Decimal.ZERO;
        int 支給件数 = 0;
        Decimal 支給額給額合計_不支給 = Decimal.ZERO;
        Decimal 本人支給額合計_不支給 = Decimal.ZERO;
        int 不支給件数 = 0;

        if (itemList == null || itemList.isEmpty()) {
            List<KogakuShikyuFushikyuKetteiTsuchiHakkoEntity> emptyList = new ArrayList<>();
            KogakuShikyuFushikyuKetteiTsuchiHakkoEntity emptyEntity = new KogakuShikyuFushikyuKetteiTsuchiHakkoEntity();
            emptyList.add(emptyEntity作成(emptyEntity));

            KogakuShikyuFushikyuKetteiTsuchiHakkoReport 支給report = new KogakuShikyuFushikyuKetteiTsuchiHakkoReport(emptyList);
            支給report.writeBy(reportSourceWriter5);
        } else {
            List<KogakuShikyuFushikyuKetteiTsuchiHakkoEntity> 支給List = new ArrayList<>();
            List<KogakuShikyuFushikyuKetteiTsuchiHakkoEntity> 不支給List = new ArrayList<>();
            for (KogakuShikyuFushikyuKetteiTsuchiHakkoEntity entity : itemList) {
            ...
            KogakuShikyuFushikyuKetteiTsuchiHakkoEntity 支給合計entity = new KogakuShikyuFushikyuKetteiTsuchiHakkoEntity();
            支給List.add(合計レコード作成(支給件数, 支給額給額合計_支給, 本人支給額合計_支給, new RString("支給計"), 支給合計entity));

            KogakuShikyuFushikyuKetteiTsuchiHakkoEntity 不支給合計entity = new KogakuShikyuFushikyuKetteiTsuchiHakkoEntity();
            不支給List.add(合計レコード作成(不支給件数, 支給額給額合計_不支給, 本人支給額合計_不支給, new RString("不支給計"), 不支給合計entity));

            KogakuShikyuFushikyuKetteiTsuchiHakkoReport 支給report = new KogakuShikyuFushikyuKetteiTsuchiHakkoReport(支給List);
            支給report.writeBy(reportSourceWriter5);

            KogakuShikyuFushikyuKetteiTsuchiHakkoReport 不支給report = new KogakuShikyuFushikyuKetteiTsuchiHakkoReport(不支給List);
            不支給report.writeBy(reportSourceWriter5);
        }
    ...
    private void eucFileOutputJohoFactory() {
        ...
        ReportOutputJokenhyoItem reportOutputJokenhyoItem = new ReportOutputJokenhyoItem(
                ReportIdDBC.DBC100007.getReportId().getColumnValue(),
    ...
    private KogakuShikyuFushikyuKetteiTsuchiHakkoEntity 合計レコード作成(int 支給件数, Decimal 支給額給額合計, Decimal 本人支給額合計,
            RString タイトル,
    ...
    private KogakuShikyuFushikyuKetteiTsuchiHakkoEntity emptyEntity作成(KogakuShikyuFushikyuKetteiTsuchiHakkoEntity emptyEntity) {

        set出力順と改頁(emptyEntity);
        emptyEntity.set被保険者氏名(被保険者氏名_出力ない);
    ...
    private KogakuKetteiTsuchiShoShiharaiYoteiBiYijiAriEntity getReportEntity(KetteiTsuchishoInfoTempEntity entity,
        ...
        if (更新しない.equals(parameter.get決定日一括更新区分())) {
            reportEntity.set決定年月日(toRDate(entity.getKetteiYMD()));
        } else if (更新する.equals(parameter.get決定日一括更新区分())) {
            reportEntity.set決定年月日(parameter.get決定日());
        }
        ...
        RString 持ち物内容文言１ = get帳票制御汎用(manager, new RString("持ち物内容文言１"));
        RString 持ち物内容文言２ = get帳票制御汎用(manager, new RString("持ち物内容文言２"));
        RString 持ち物内容文言３ = get帳票制御汎用(manager, new RString("持ち物内容文言３"));
        reportEntity.setお持ちいただくもの１(持ち物内容文言１);
        reportEntity.setお持ちいただくもの２(持ち物内容文言２);
        reportEntity.setお持ちいただくもの３(持ち物内容文言３);
    ...
    private KogakuKetteiTsuchiShoEntity getShoSealer2ReportEntity(KetteiTsuchishoInfoTempEntity entity,
        ...
        if (更新しない.equals(parameter.get決定日一括更新区分())) {
            ...
            reportEntity.set決定年月日(new FlexibleDate(parameter.get決定日().toDateString()));
        }
    ...
    private KogakuShikyuFushikyuKetteiTsuchiHakkoEntity getFushikyuReportEntity(KogakuServiceReportEntity entity,
        ...
        returnEntity.set本人支払額(一時Entity.getRiyoshaFutanGaku());
        returnEntity.set支給額(一時Entity.getKogakuShikyuGaku());
        ...
        returnEntity.set連番(new RString(String.valueOf(連番)));
        return returnEntity;
    }
    ...
    private RString get帳票制御汎用(ChohyoSeigyoHanyoManager 帳票制御汎用Manager, RString 項目名) {
        ...
        ChohyoSeigyoHanyo chohyoSeigyoHanyo = 帳票制御汎用Manager.get帳票制御汎用(SubGyomuCode.DBC介護給付, ReportIdDBC.DBC100007.getReportId(),
