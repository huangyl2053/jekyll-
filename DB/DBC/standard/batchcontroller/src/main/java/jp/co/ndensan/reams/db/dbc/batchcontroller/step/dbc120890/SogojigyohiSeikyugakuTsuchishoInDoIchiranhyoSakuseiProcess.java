...
import jp.co.ndensan.reams.db.dbc.service.core.dbc120890.SogojigyohiSeikyugakuTsuchishoInManager;
import jp.co.ndensan.reams.db.dbc.service.report.sogojigyohiSeikyugakuTsuchisho.SogojigyohiSeikyugakuTsuchishoInPrintService;
 ...
 */
public class SogojigyohiSeikyugakuTsuchishoInDoIchiranhyoSakuseiProcess extends SimpleBatchProcessBase {
    ...
    protected void process() {
        ...
        SogojigyohiSeikyugakuTsuchishoInManager tsuchishoInManager = SogojigyohiSeikyugakuTsuchishoInManager.createInstance();
        List<DbWT1511SeikyugakuTsuchishoTempEntity> list = tsuchishoInManager.get帳票出力対象データ();
        if (null == list || list.isEmpty()) {
            return;
        }
        SogojigyohiSeikyugakuTsuchishoInPrintService printService
                = new SogojigyohiSeikyugakuTsuchishoInPrintService();
        printService.printTaitsu(list, parameter.getシステム日付());
        do帳票のCSVファイル作成(list, parameter.getシステム日付());

    }

    private void do帳票のCSVファイル作成(List<DbWT1511SeikyugakuTsuchishoTempEntity> list, RDateTime 作成日時) {
        ...
        for (int index = 0; index < list.size(); index++) {
            ...
            DbWT1511SeikyugakuTsuchishoTempEntity 請求額通知書一時 = list.get(index);
            if (index == 0) {
            ...
            if (請求額通知書一時.get合計_帳票レコード種別() != RString.EMPTY) {
            ...
            if (請求額通知書一時.get累計_帳票レコード種別() != RString.EMPTY) {
            ...
            if (請求額通知書一時.get手数料_帳票レコード種別() != RString.EMPTY) {
