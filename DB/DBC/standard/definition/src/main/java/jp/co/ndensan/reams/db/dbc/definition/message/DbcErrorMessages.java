...
public enum DbcErrorMessages implements IMessageGettable {
    ...
    確認情報受取年月不正(52, "指定された年月の自己負担額確認情報は取り込まれていないため処理できません。"),
    ...
    国保連送付処理_処理月チェック(125, "国保連送付データが存在しません。スケジュール設定を行ってください。"),
    償還払い費支給申請決定_削除済修正(126, "削除済のため、修正できません。"),
    事業者_サービス種類_組み合わせ不正(128, "サービス年月時点で、対象事業者には指定されたサービスの利用登録がありません。"),
    提供_購入_年月チェック(129, "提供(購入)年月が認定有効期間外です。");
