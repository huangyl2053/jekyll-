...
public class ShokanShukeiListEntity {
     ...
     * @param 状態
     */
    protected ShokanShukeiListEntity(RString 連番, ServiceShuruiCode サービス種類,
     ...
     * @return
     */
    public static ShokanShukeiListEntity createSelectByKeyParam(RString 連番, ServiceShuruiCode サービス種類,
            int サービス実日数, int 計画単位数, int 限度額管理対象単位数, int 限度額管理対象外単位数,
            int 短期入所計画日数, int 短期入所実日数, int 単位数合計, Decimal 単位数単価, Decimal 請求額,
            int 利用者負担, int 出来高医療費単位数合計, Decimal 出来高医療費請求額, Decimal 出来高医療費利用者負担額,
            RString 審査方法区分コード, RString 状態) {
        return new ShokanShukeiListEntity(連番, サービス種類, サービス実日数, 計画単位数, 限度額管理対象単位数,
