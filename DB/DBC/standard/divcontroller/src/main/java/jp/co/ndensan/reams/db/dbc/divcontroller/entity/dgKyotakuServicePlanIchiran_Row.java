package jp.co.ndensan.reams.db.dbc.divcontroller.entity;
/**
 * このコードはツールによって生成されました。
 * このファイルへの変更は、以下の状況下で不正な動作の原因になったり、
 * コードが再生成されるときに損失したりします。
 * Mon Apr 28 17:28:45 JST 2014 
 */



import jp.co.ndensan.reams.uz.uza.ui.binding.DataRow;
import jp.co.ndensan.reams.uz.uza.ui.binding.Button;
import jp.co.ndensan.reams.uz.uza.lang.RString;


/**
 * dgKyotakuServicePlanIchiran_Row のクラスファイル 
 * 
 * @author 自動生成
 */

public class dgKyotakuServicePlanIchiran_Row extends DataRow {

    private Button btnSelect;
    private RString txtTaishoYMInvisible;
    private RString txtTaishoYM;
    private RString txtPlanSakuseiKubun;
    private RString txtZantei;
    private RString txtJigyoshaNo;
    private RString txtJigyoshaName;
    private RString txtStartYMD;
    private RString txtEndYMD;
    private RString txtEnable;

    public dgKyotakuServicePlanIchiran_Row() {
        super();
    }

    public dgKyotakuServicePlanIchiran_Row(Button btnSelect, RString txtTaishoYMInvisible, RString txtTaishoYM, RString txtPlanSakuseiKubun, RString txtZantei, RString txtJigyoshaNo, RString txtJigyoshaName, RString txtStartYMD, RString txtEndYMD, RString txtEnable) {
        super();
        this.setOriginalData("btnSelect", btnSelect);
        this.setOriginalData("txtTaishoYMInvisible", txtTaishoYMInvisible);
        this.setOriginalData("txtTaishoYM", txtTaishoYM);
        this.setOriginalData("txtPlanSakuseiKubun", txtPlanSakuseiKubun);
        this.setOriginalData("txtZantei", txtZantei);
        this.setOriginalData("txtJigyoshaNo", txtJigyoshaNo);
        this.setOriginalData("txtJigyoshaName", txtJigyoshaName);
        this.setOriginalData("txtStartYMD", txtStartYMD);
        this.setOriginalData("txtEndYMD", txtEndYMD);
        this.setOriginalData("txtEnable", txtEnable);
        this.btnSelect = btnSelect;
        this.txtTaishoYMInvisible = txtTaishoYMInvisible;
        this.txtTaishoYM = txtTaishoYM;
        this.txtPlanSakuseiKubun = txtPlanSakuseiKubun;
        this.txtZantei = txtZantei;
        this.txtJigyoshaNo = txtJigyoshaNo;
        this.txtJigyoshaName = txtJigyoshaName;
        this.txtStartYMD = txtStartYMD;
        this.txtEndYMD = txtEndYMD;
        this.txtEnable = txtEnable;
    }

    public Button getBtnSelect() {
        return btnSelect;
    }

    public RString getTxtTaishoYMInvisible() {
        return txtTaishoYMInvisible;
    }

    public RString getTxtTaishoYM() {
        return txtTaishoYM;
    }

    public RString getTxtPlanSakuseiKubun() {
        return txtPlanSakuseiKubun;
    }

    public RString getTxtZantei() {
        return txtZantei;
    }

    public RString getTxtJigyoshaNo() {
        return txtJigyoshaNo;
    }

    public RString getTxtJigyoshaName() {
        return txtJigyoshaName;
    }

    public RString getTxtStartYMD() {
        return txtStartYMD;
    }

    public RString getTxtEndYMD() {
        return txtEndYMD;
    }

    public RString getTxtEnable() {
        return txtEnable;
    }

    public void setBtnSelect(Button btnSelect) {
        this.setOriginalData("btnSelect", btnSelect);
        this.btnSelect = btnSelect;
    }

    public void setTxtTaishoYMInvisible(RString txtTaishoYMInvisible) {
        this.setOriginalData("txtTaishoYMInvisible", txtTaishoYMInvisible);
        this.txtTaishoYMInvisible = txtTaishoYMInvisible;
    }

    public void setTxtTaishoYM(RString txtTaishoYM) {
        this.setOriginalData("txtTaishoYM", txtTaishoYM);
        this.txtTaishoYM = txtTaishoYM;
    }

    public void setTxtPlanSakuseiKubun(RString txtPlanSakuseiKubun) {
        this.setOriginalData("txtPlanSakuseiKubun", txtPlanSakuseiKubun);
        this.txtPlanSakuseiKubun = txtPlanSakuseiKubun;
    }

    public void setTxtZantei(RString txtZantei) {
        this.setOriginalData("txtZantei", txtZantei);
        this.txtZantei = txtZantei;
    }

    public void setTxtJigyoshaNo(RString txtJigyoshaNo) {
        this.setOriginalData("txtJigyoshaNo", txtJigyoshaNo);
        this.txtJigyoshaNo = txtJigyoshaNo;
    }

    public void setTxtJigyoshaName(RString txtJigyoshaName) {
        this.setOriginalData("txtJigyoshaName", txtJigyoshaName);
        this.txtJigyoshaName = txtJigyoshaName;
    }

    public void setTxtStartYMD(RString txtStartYMD) {
        this.setOriginalData("txtStartYMD", txtStartYMD);
        this.txtStartYMD = txtStartYMD;
    }

    public void setTxtEndYMD(RString txtEndYMD) {
        this.setOriginalData("txtEndYMD", txtEndYMD);
        this.txtEndYMD = txtEndYMD;
    }

    public void setTxtEnable(RString txtEnable) {
        this.setOriginalData("txtEnable", txtEnable);
        this.txtEnable = txtEnable;
    }

}