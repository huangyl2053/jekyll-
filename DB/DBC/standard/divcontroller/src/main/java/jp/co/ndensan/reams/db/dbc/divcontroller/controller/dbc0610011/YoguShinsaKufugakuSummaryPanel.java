...
public class YoguShinsaKufugakuSummaryPanel {
    ...
    private void setSummary(YoguShinsaKufugakuSummaryPanelDiv panel) {
        ...
        panel.getKyufugakuSummary().getTblSeikyuSummary().getTxtHiyoTotalNow().setValue(cg.getAsDecimal("費用額後"));
        panel.getKyufugakuSummary().getTblSeikyuSummary().getTxtHokenTaishoHiyoNow().setValue(cg.getAsDecimal("保険対象額後"));
        panel.getKyufugakuSummary().getTblSeikyuSummary().getTxtHokenKyufuAmountNow().setValue(cg.getAsDecimal("給付額後"));
        panel.getKyufugakuSummary().getTblSeikyuSummary().getTxtRiyoshaFutanAmountNow().setValue(cg.getAsDecimal("利用者負担額後"));
    }
}
