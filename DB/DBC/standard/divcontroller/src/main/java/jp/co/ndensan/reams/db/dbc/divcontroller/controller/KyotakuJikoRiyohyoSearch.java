...
/**
 * 居宅サービス計画作成依頼届出情報登録の該当者検索のコントロールです。
 *
 * @author N8187 久保田 英男
 */
public class KyotakuJikoRiyohyoSearch {
    ...
    public ResponseData onLoad(KyotakuJikoRiyohyoSearchDiv panel) {
        ...
        panel.getKyotakuJikoRiyohyoHihokenshaFinder().setIsOpen(true);
        panel.getKyotakuJikoRiyohyoHihokenshaList().setIsOpen(false);
        response.data = panel;
        return response;
    }
...
//        kojin.getTxtJusho().setValue(cg.getAsRString("住所"));
     ...
     * @return ResponseData
     */
    public ResponseData onClickBtnToSearch(KyotakuJikoRiyohyoSearchDiv panel) {
        ResponseData<KyotakuJikoRiyohyoSearchDiv> response = new ResponseData<>();
        setHihokenshaListData(panel);
        panel.getKyotakuJikoRiyohyoHihokenshaFinder().setIsOpen(false);
        panel.getKyotakuJikoRiyohyoHihokenshaList().setIsOpen(true);
        response.data = panel;
        return response;
    }
     ...
     * @return ResponseData
     */
    public ResponseData onClickBtnToResearch(KyotakuJikoRiyohyoSearchDiv panel) {
        ResponseData<KyotakuJikoRiyohyoSearchDiv> response = new ResponseData<>();
        SearchResultOfHihokenshaDiv hihokensha = panel.getKyotakuJikoRiyohyoHihokenshaList();
    ...
    private void setHihokenshaListData(KyotakuJikoRiyohyoSearchDiv panel) {
        SearchResultOfHihokenshaDiv hihokensha = panel.getKyotakuJikoRiyohyoHihokenshaList();
        List<dgSearchResult_Row> dgList = hihokensha.getDgSearchResult().getDataSource();
        dgList.clear();
