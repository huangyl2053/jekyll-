...
import jp.co.ndensan.reams.db.dbc.divcontroller.entity.parentdiv.DBC1270011.DataGrid1_Row;
...
public class KogakuGassanKyufuJissekiHandler {
     ...
     */
    public void onLoad(HihokenshaNo 被保険者番号, ShikibetsuCode 識別コード, List<KogakuGassanKyufuJisseki> 高額合算給付実績情報) {
     ...
     * @param row 選択データ
     */
    public void onclik_selectButton(DataGrid1_Row row) {
        div.getTextBox1().setValue(row.getTxtKokanShikibetsu());
        div.getTextBox6().setValue(row.getTxtShikyuShinseishoSeiriNo());
        div.getTextBox5().setValue(row.getTxtJikoFutanSeiriNo());
        div.getTextBox4().setValue(row.getTxtSakuseiKubun());
        div.getTextBox3().setValue(row.getTxtHokenshaNo());
        if (!RString.isNullOrEmpty(row.getTxtShinseiYMD())) {
            div.getTextBoxDate1().setValue(new RDate(row.getTxtShinseiYMD().toString()));
        }
        if (!RString.isNullOrEmpty(row.getTxtKetteiYMD())) {
            div.getTextBoxDate3().setValue(new RDate(row.getTxtKetteiYMD().toString()));
        }
        if (!RString.isNullOrEmpty(row.getTxtShikyugaku())) {
            div.getTextBoxNum1().setValue(new Decimal(row.getTxtShikyugaku().toString()));
            div.getTextBoxNum3().setValue(new Decimal(row.getTxtShikyugaku().toString()));
        } else {
            div.getTextBoxNum1().setValue(new Decimal(0));
            div.getTextBoxNum3().setValue(new Decimal(0));
        }
        if (!RString.isNullOrEmpty(row.getTxtShoriYM())) {
            div.getTextBoxDate6().setValue(new RDate(row.getTxtShoriYM().toString()));
        }
        if (!RString.isNullOrEmpty(row.getTxtUketoriNengetsu())) {
            div.getTextBoxDate5().setValue(new RDate(row.getTxtUketoriNengetsu().toString()));
        }
        if (!RString.isNullOrEmpty(row.getTxtSofuNengetsu())) {
            div.getTextBoxDate4().setValue(new RDate(row.getTxtSofuNengetsu().toString()));
        }
    }

    private void set一覧(List<KogakuGassanKyufuJisseki> 高額合算給付実績情報) {
