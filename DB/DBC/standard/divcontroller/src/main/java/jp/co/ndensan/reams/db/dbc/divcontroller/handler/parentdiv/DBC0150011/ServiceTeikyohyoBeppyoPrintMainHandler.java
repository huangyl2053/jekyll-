...
public class ServiceTeikyohyoBeppyoPrintMainHandler {
    ...
    public SourceDataCollection to帳票発行処理(Map<RString, Object> map) {
        div.getTxtJikoSakuseiKeikakuYm().setReadOnly(false);
...
//        param.set合計情報(合計情報);
        param.set帳票ヘッダー(TeikyohyoBeppyoManager.createInstance().
                get被保険者情報(被保険者番号, 対象年月, 履歴番号, 自己作成計画年月));
        param.set帳票情報マップ(TeikyohyoBeppyoManager.createInstance().
                get事業者別マップ(被保険者番号, 対象年月, 履歴番号, 自己作成計画年月));
        param.set短期入所情報(TeikyohyoBeppyoManager.createInstance().
                get短期入所利用日数(被保険者番号, 対象年月, 履歴番号, 自己作成計画年月));
        param.set種類別支給限度情報(TeikyohyoBeppyoManager.createInstance().
                get種類別支給限度情報(被保険者番号, 対象年月, 履歴番号, 自己作成計画年月));
        return null;
