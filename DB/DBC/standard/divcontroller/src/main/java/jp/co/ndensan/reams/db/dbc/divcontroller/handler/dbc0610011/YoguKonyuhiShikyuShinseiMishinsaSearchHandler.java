...
public class YoguKonyuhiShikyuShinseiMishinsaSearchHandler {
    ...
    public void 未審査分検索処理() {
        ...
        if (支給申請日From != null && !"".equals(支給申請日From)) {
            申請日From = new FlexibleDate(支給申請日From.toString());
        }
        if (支給申請日To != null && !"".equals(支給申請日To)) {
    ...
    private void setグリッド(List<ShokanShinseiEntity> resultList) {
        ...
        for (ShokanShinseiEntity entity : resultList) {
            ...
            row.getTxtShikyuShinseiDate().setValue(new RDate(entity.get償還払支給申請Entity().getShinseiYMD().toString()));
            row.getTxtTenkyoYM().setValue(new RDate(entity.get償還払請求基本Entity().getServiceTeikyoYM().toString()));
...
//            row.getTxtHihoName().setValue(entity.get氏名().getColumnValue());
    ...
    public void 審査決定処理() {
        ...
        for (int i = 0; i < selectedMishinsaShikyuShinsei.size(); i++) {
            dgYoguKonyuhiShisaMishinsaShikyuShinseiList_Row row = selectedMishinsaShikyuShinsei.get(i);
            Boolean 限度額flag = 限度額チェッ(row);
            Boolean 品目flag = 品目チェック(row);
            if (限度額flag == true && 品目flag == false) {
                row.getTxtShinsaNo().setValue(ShinsaNaiyoKubun.承認する.getコード());
                row.getTxtShinsaResult().setValue(承認する);
            }else if (限度額flag == false || 品目flag == true) {
    ...
    private Boolean 限度額チェッ(dgYoguKonyuhiShisaMishinsaShikyuShinseiList_Row row) {
        ...
        // TODO おそらく置いて変えて、先に落ちるのを注釈して、後で改正します
...
//                サービス提供年月, entityList, 整理番号);
    ...
    public void 決定日入力チェック(RDate 決定日) {
        if (決定日 == null || "".equals(決定日)) {
            throw new ApplicationException(UrErrorMessages.必須.getMessage().replace(new String("決定日")));
        }
    }
