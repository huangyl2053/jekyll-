...
//TODO Key項目のテストメソッドは削除して下さい。
        ...
        public void 戻り値の70歳以上の者に係る所得区分は_設定した値と同じ70歳以上の者に係る所得区分を返す() {
            business = sut.set70歳以上の者に係る所得区分(DbT3070KogakuGassanJikoFutanGakuEntityGenerator.DEFAULT_70歳以上の者に係る所得区分).build();
        ...
        @Test
        public void 戻り値の合計・自己負担額は_設定した値と同じ合計・自己負担額を返す() {
            business = sut.set合計・自己負担額(DbT3070KogakuGassanJikoFutanGakuEntityGenerator.DEFAULT_合計・自己負担額).build();
            assertThat(business.get合計・自己負担額(), is(DbT3070KogakuGassanJikoFutanGakuEntityGenerator.DEFAULT_合計・自己負担額));
        }

        @Test
        public void 戻り値の合計・70-74自己負担額（内訳）は_設定した値と同じ合計・70-74自己負担額（内訳）を返す() {
            business = sut.set合計・70-74自己負担額（内訳）(DbT3070KogakuGassanJikoFutanGakuEntityGenerator.DEFAULT_合計・70-74自己負担額（内訳）).build();
            assertThat(business.get合計・70-74自己負担額（内訳）(), is(DbT3070KogakuGassanJikoFutanGakuEntityGenerator.DEFAULT_合計・70-74自己負担額（内訳）));
        }

        @Test
        public void 戻り値の合計・70未満高額支給額は_設定した値と同じ合計・70未満高額支給額を返す() {
            business = sut.set合計・70未満高額支給額(DbT3070KogakuGassanJikoFutanGakuEntityGenerator.DEFAULT_合計・70未満高額支給額).build();
            assertThat(business.get合計・70未満高額支給額(), is(DbT3070KogakuGassanJikoFutanGakuEntityGenerator.DEFAULT_合計・70未満高額支給額));
        }

        @Test
        public void 戻り値の合計・70-74高額支給額は_設定した値と同じ合計・70-74高額支給額を返す() {
            business = sut.set合計・70-74高額支給額(DbT3070KogakuGassanJikoFutanGakuEntityGenerator.DEFAULT_合計・70-74高額支給額).build();
            assertThat(business.get合計・70-74高額支給額(), is(DbT3070KogakuGassanJikoFutanGakuEntityGenerator.DEFAULT_合計・70-74高額支給額));
        }

        @Test
        public void 戻り値の補正済・合計・自己負担額は_設定した値と同じ補正済・合計・自己負担額を返す() {
            business = sut.set補正済・合計・自己負担額(DbT3070KogakuGassanJikoFutanGakuEntityGenerator.DEFAULT_補正済・合計・自己負担額).build();
            assertThat(business.get補正済・合計・自己負担額(), is(DbT3070KogakuGassanJikoFutanGakuEntityGenerator.DEFAULT_補正済・合計・自己負担額));
        }

        @Test
        public void 戻り値の補正済・合計・70-74自己負担額（内訳）は_設定した値と同じ補正済・合計・70-74自己負担額（内訳）を返す() {
            business = sut.set補正済・合計・70-74自己負担額（内訳）(DbT3070KogakuGassanJikoFutanGakuEntityGenerator.DEFAULT_補正済・合計・70-74自己負担額（内訳）).build();
            assertThat(business.get補正済・合計・70-74自己負担額（内訳）(), is(DbT3070KogakuGassanJikoFutanGakuEntityGenerator.DEFAULT_補正済・合計・70-74自己負担額（内訳）));
        }

        @Test
        public void 戻り値の補正済・合計・70未満高額支給額は_設定した値と同じ補正済・合計・70未満高額支給額を返す() {
            business = sut.set補正済・合計・70未満高額支給額(DbT3070KogakuGassanJikoFutanGakuEntityGenerator.DEFAULT_補正済・合計・70未満高額支給額).build();
            assertThat(business.get補正済・合計・70未満高額支給額(), is(DbT3070KogakuGassanJikoFutanGakuEntityGenerator.DEFAULT_補正済・合計・70未満高額支給額));
        }

        @Test
        public void 戻り値の補正済・合計・70-74高額支給額は_設定した値と同じ補正済・合計・70-74高額支給額を返す() {
            business = sut.set補正済・合計・70-74高額支給額(DbT3070KogakuGassanJikoFutanGakuEntityGenerator.DEFAULT_補正済・合計・70-74高額支給額).build();
            assertThat(business.get補正済・合計・70-74高額支給額(), is(DbT3070KogakuGassanJikoFutanGakuEntityGenerator.DEFAULT_補正済・合計・70-74高額支給額));
        }
        ...
        @Test
        public void 戻り値の後期・国保処理区分は_設定した値と同じ後期・国保処理区分を返す() {
            business = sut.set後期・国保処理区分(DbT3070KogakuGassanJikoFutanGakuEntityGenerator.DEFAULT_後期・国保処理区分).build();
            assertThat(business.get後期・国保処理区分(), is(DbT3070KogakuGassanJikoFutanGakuEntityGenerator.DEFAULT_後期・国保処理区分));
        }
