...
//TODO 主キー名を置換してください
        ...
        public void 主キー名1がnullである場合に_NullPointerExceptionが発生する() {
            sut = new ShokanShokujiHiyo(null, 主キー名2);
        }

        @Test(expected = NullPointerException.class)
        public void 主キー名2がnullである場合に_NullPointerExceptionが発生する() {
            sut = new ShokanShokujiHiyo(主キー名1, null);
        }

        @Test
        public void 指定したキーが保持するDbT3043ShokanShokujiHiyoEntityにセットされている() {
            sut = new ShokanShokujiHiyo(主キー名1, 主キー名2);
    ...
    public static class getterテスト extends DbcTestBase {
        ...
        public void get標準負担額（月額）は_entityが持つ標準負担額（月額）を返す() {
            assertThat(sut.get標準負担額（月額）(), is(ShokanShokujiHiyoEntity.getGetsugakuHyojunFutangaku()));
        }
        ...
        public void get標準負担額（日額）は_entityが持つ標準負担額（日額）を返す() {
            assertThat(sut.get標準負担額（日額）(), is(ShokanShokujiHiyoEntity.getNichigakuHyojunFutangaku()));
        }
        ...
        public void get点数／金額は_entityが持つ点数／金額を返す() {
            assertThat(sut.get点数／金額(), is(ShokanShokujiHiyoEntity.getTensuKingaku()));
        }
