...
//TODO 主キー値を適切な値に置換してください
        主キー名1 = DbT3039ShokanMeisaiEntityGenerator.DEFAULT_主キー名1;
        主キー名2 = DbT3039ShokanMeisaiEntityGenerator.DEFAULT_主キー名2;
    }
    ...
    public static class getterSetterTest extends FdaTestBase {
        ...
        public void setUp() {
            ...
            ShokanMeisaiEntity.setXXX(主キー名1);
            ShokanMeisaiEntity.setXXX(主キー名2);
...
//TODO Key項目のテストメソッドは削除して下さい。
        ...
        public void 戻り値の順次番号は_設定した値と同じ順次番号を返す() {
            business = sut.set順次番号(DbT3039ShokanMeisaiEntityGenerator.DEFAULT_順次番号).build();
            assertThat(business.get順次番号(), is(DbT3039ShokanMeisaiEntityGenerator.DEFAULT_順次番号));
        }

        @Test
        public void 戻り値の履歴番号は_設定した値と同じ履歴番号を返す() {
            business = sut.set履歴番号(DbT3039ShokanMeisaiEntityGenerator.DEFAULT_履歴番号).build();
            assertThat(business.get履歴番号(), is(DbT3039ShokanMeisaiEntityGenerator.DEFAULT_履歴番号));
        }
        ...
        public void 戻り値の日数・回数は_設定した値と同じ日数・回数を返す() {
            business = sut.set日数・回数(DbT3039ShokanMeisaiEntityGenerator.DEFAULT_日数・回数).build();
            assertThat(business.get日数・回数(), is(DbT3039ShokanMeisaiEntityGenerator.DEFAULT_日数・回数));
        }
