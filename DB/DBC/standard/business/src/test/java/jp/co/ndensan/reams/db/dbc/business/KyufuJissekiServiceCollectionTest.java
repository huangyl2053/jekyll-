...
////            assertThat(sut.get給付実績サービス(new ServiceShuruiCode(new Code("11"), new RString("名称"), new RString("略称"))).get利用者負担額合計(), is(new Decimal("10")));
//            assertThat(sut.get単位数合計Byサービス種類(new ServiceShuruiCode(new RString("11"))), is(new Decimal("10")));
//        }
        @Test
        public void サービス種類_11を指定したとき_サービス種類11の_単位数合計を返す() {
            sut = new KyufuJissekiServiceCollection(create給付実績サービス集計List());
            assertThat(sut.get単位数合計Byサービス種類(new ServiceShuruiCode(new RString("11"))), is(new Decimal("0")));
        }
