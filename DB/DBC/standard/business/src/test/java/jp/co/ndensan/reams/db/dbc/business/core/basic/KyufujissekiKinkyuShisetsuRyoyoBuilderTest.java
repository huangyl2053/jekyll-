...
//TODO Key項目のテストメソッドは削除して下さい。
        ...
        public void 戻り値の後・往診日数は_設定した値と同じ後・往診日数を返す() {
            business = sut.set後・往診日数(DbT3019KyufujissekiKinkyuShisetsuRyoyoEntityGenerator.DEFAULT_後・往診日数).build();
            assertThat(business.get後・往診日数(), is(DbT3019KyufujissekiKinkyuShisetsuRyoyoEntityGenerator.DEFAULT_後・往診日数));
        }

        @Test
        public void 戻り値の後・通院日数は_設定した値と同じ後・通院日数を返す() {
            business = sut.set後・通院日数(DbT3019KyufujissekiKinkyuShisetsuRyoyoEntityGenerator.DEFAULT_後・通院日数).build();
            assertThat(business.get後・通院日数(), is(DbT3019KyufujissekiKinkyuShisetsuRyoyoEntityGenerator.DEFAULT_後・通院日数));
        }

        @Test
        public void 戻り値の後・緊急時治療管理単位数は_設定した値と同じ後・緊急時治療管理単位数を返す() {
            business = sut.set後・緊急時治療管理単位数(DbT3019KyufujissekiKinkyuShisetsuRyoyoEntityGenerator.DEFAULT_後・緊急時治療管理単位数).build();
            assertThat(business.get後・緊急時治療管理単位数(), is(DbT3019KyufujissekiKinkyuShisetsuRyoyoEntityGenerator.DEFAULT_後・緊急時治療管理単位数));
        }

        @Test
        public void 戻り値の後・緊急時治療管理日数は_設定した値と同じ後・緊急時治療管理日数を返す() {
            business = sut.set後・緊急時治療管理日数(DbT3019KyufujissekiKinkyuShisetsuRyoyoEntityGenerator.DEFAULT_後・緊急時治療管理日数).build();
            assertThat(business.get後・緊急時治療管理日数(), is(DbT3019KyufujissekiKinkyuShisetsuRyoyoEntityGenerator.DEFAULT_後・緊急時治療管理日数));
        }

        @Test
        public void 戻り値の後・リハビリテーション点数は_設定した値と同じ後・リハビリテーション点数を返す() {
            business = sut.set後・リハビリテーション点数(DbT3019KyufujissekiKinkyuShisetsuRyoyoEntityGenerator.DEFAULT_後・リハビリテーション点数).build();
            assertThat(business.get後・リハビリテーション点数(), is(DbT3019KyufujissekiKinkyuShisetsuRyoyoEntityGenerator.DEFAULT_後・リハビリテーション点数));
        }

        @Test
        public void 戻り値の後・処置点数は_設定した値と同じ後・処置点数を返す() {
            business = sut.set後・処置点数(DbT3019KyufujissekiKinkyuShisetsuRyoyoEntityGenerator.DEFAULT_後・処置点数).build();
            assertThat(business.get後・処置点数(), is(DbT3019KyufujissekiKinkyuShisetsuRyoyoEntityGenerator.DEFAULT_後・処置点数));
        }

        @Test
        public void 戻り値の後・手術点数は_設定した値と同じ後・手術点数を返す() {
            business = sut.set後・手術点数(DbT3019KyufujissekiKinkyuShisetsuRyoyoEntityGenerator.DEFAULT_後・手術点数).build();
            assertThat(business.get後・手術点数(), is(DbT3019KyufujissekiKinkyuShisetsuRyoyoEntityGenerator.DEFAULT_後・手術点数));
        }

        @Test
        public void 戻り値の後・麻酔点数は_設定した値と同じ後・麻酔点数を返す() {
            business = sut.set後・麻酔点数(DbT3019KyufujissekiKinkyuShisetsuRyoyoEntityGenerator.DEFAULT_後・麻酔点数).build();
            assertThat(business.get後・麻酔点数(), is(DbT3019KyufujissekiKinkyuShisetsuRyoyoEntityGenerator.DEFAULT_後・麻酔点数));
        }

        @Test
        public void 戻り値の後・放射線治療点数は_設定した値と同じ後・放射線治療点数を返す() {
            business = sut.set後・放射線治療点数(DbT3019KyufujissekiKinkyuShisetsuRyoyoEntityGenerator.DEFAULT_後・放射線治療点数).build();
            assertThat(business.get後・放射線治療点数(), is(DbT3019KyufujissekiKinkyuShisetsuRyoyoEntityGenerator.DEFAULT_後・放射線治療点数));
        }
