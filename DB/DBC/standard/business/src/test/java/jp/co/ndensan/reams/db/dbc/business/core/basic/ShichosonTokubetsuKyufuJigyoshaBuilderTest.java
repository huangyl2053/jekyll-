...
//TODO Key項目のテストメソッドは削除して下さい。
        ...
        @Test
        public void 戻り値の申請者氏名（漢字）は_設定した値と同じ申請者氏名（漢字）を返す() {
            business = sut.set申請者氏名（漢字）(DbT3065ShichosonTokubetsuKyufuJigyoshaEntityGenerator.DEFAULT_申請者氏名（漢字）).build();
            assertThat(business.get申請者氏名（漢字）(), is(DbT3065ShichosonTokubetsuKyufuJigyoshaEntityGenerator.DEFAULT_申請者氏名（漢字）));
        }

        @Test
        public void 戻り値の申請者氏名（カナ）は_設定した値と同じ申請者氏名（カナ）を返す() {
            business = sut.set申請者氏名（カナ）(DbT3065ShichosonTokubetsuKyufuJigyoshaEntityGenerator.DEFAULT_申請者氏名（カナ）).build();
            assertThat(business.get申請者氏名（カナ）(), is(DbT3065ShichosonTokubetsuKyufuJigyoshaEntityGenerator.DEFAULT_申請者氏名（カナ）));
        }
        ...
        @Test
        public void 戻り値の申請者住所（カナ）は_設定した値と同じ申請者住所（カナ）を返す() {
            business = sut.set申請者住所（カナ）(DbT3065ShichosonTokubetsuKyufuJigyoshaEntityGenerator.DEFAULT_申請者住所（カナ）).build();
            assertThat(business.get申請者住所（カナ）(), is(DbT3065ShichosonTokubetsuKyufuJigyoshaEntityGenerator.DEFAULT_申請者住所（カナ）));
        }
