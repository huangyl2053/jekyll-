...
//TODO Key項目のテストメソッドは削除して下さい。
        ...
        @Test
        public void 戻り値の利用者負担・決定区分は_設定した値と同じ利用者負担・決定区分を返す() {
            business = sut.set利用者負担・決定区分(DbT3097TandokuJoseikinKyufuKetteiEntityGenerator.DEFAULT_利用者負担・決定区分).build();
            assertThat(business.get利用者負担・決定区分(), is(DbT3097TandokuJoseikinKyufuKetteiEntityGenerator.DEFAULT_利用者負担・決定区分));
        }

        @Test
        public void 戻り値の利用者負担・不承認理由は_設定した値と同じ利用者負担・不承認理由を返す() {
            business = sut.set利用者負担・不承認理由(DbT3097TandokuJoseikinKyufuKetteiEntityGenerator.DEFAULT_利用者負担・不承認理由).build();
            assertThat(business.get利用者負担・不承認理由(), is(DbT3097TandokuJoseikinKyufuKetteiEntityGenerator.DEFAULT_利用者負担・不承認理由));
        }
        ...
        @Test
        public void 戻り値の保留区分・滞納は_設定した値と同じ保留区分・滞納を返す() {
            business = sut.set保留区分・滞納(DbT3097TandokuJoseikinKyufuKetteiEntityGenerator.DEFAULT_保留区分・滞納).build();
            assertThat(business.get保留区分・滞納(), is(DbT3097TandokuJoseikinKyufuKetteiEntityGenerator.DEFAULT_保留区分・滞納));
        }

        @Test
        public void 戻り値の保留区分・認定申請中は_設定した値と同じ保留区分・認定申請中を返す() {
            business = sut.set保留区分・認定申請中(DbT3097TandokuJoseikinKyufuKetteiEntityGenerator.DEFAULT_保留区分・認定申請中).build();
            assertThat(business.get保留区分・認定申請中(), is(DbT3097TandokuJoseikinKyufuKetteiEntityGenerator.DEFAULT_保留区分・認定申請中));
        }

        @Test
        public void 戻り値の保留区分・認定有効期限切れは_設定した値と同じ保留区分・認定有効期限切れを返す() {
            business = sut.set保留区分・認定有効期限切れ(DbT3097TandokuJoseikinKyufuKetteiEntityGenerator.DEFAULT_保留区分・認定有効期限切れ).build();
            assertThat(business.get保留区分・認定有効期限切れ(), is(DbT3097TandokuJoseikinKyufuKetteiEntityGenerator.DEFAULT_保留区分・認定有効期限切れ));
        }
