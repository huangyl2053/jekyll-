...
//        }
        ...
        public void 戻り値の口座名義人は_設定した値と同じ口座名義人を返す() {
            business = sut.set口座名義人(DbT3077JuryoininKeiyakuJigyoshaEntityGenerator.DEFAULT_口座名義人カナ).build();
            assertThat(business.get口座名義人(), is(DbT3077JuryoininKeiyakuJigyoshaEntityGenerator.DEFAULT_口座名義人カナ));
        }

        @Test
        public void 戻り値の口座名義人漢字は_設定した値と同じ口座名義人漢字を返す() {
            business = sut.set口座名義人漢字(DbT3077JuryoininKeiyakuJigyoshaEntityGenerator.DEFAULT_口座名義人).build();
            assertThat(business.get口座名義人漢字(), is(DbT3077JuryoininKeiyakuJigyoshaEntityGenerator.DEFAULT_口座名義人));
        }

        @Test
        public void 戻り値の事業者FAX番号は_設定した値と同じ事業者FAX番号を返す() {
            business = sut.set事業者FAX番号(DbT3077JuryoininKeiyakuJigyoshaEntityGenerator.DEFAULT_契約事業者FAX番号).build();
            assertThat(business.get事業者FAX番号(), is(DbT3077JuryoininKeiyakuJigyoshaEntityGenerator.DEFAULT_契約事業者FAX番号));
        }
