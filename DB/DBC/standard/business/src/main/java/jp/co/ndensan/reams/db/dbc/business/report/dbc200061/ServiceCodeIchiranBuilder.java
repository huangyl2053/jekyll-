/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package jp.co.ndensan.reams.db.dbc.business.report.dbc200061;

import jp.co.ndensan.reams.db.dbc.entity.report.dbc200061.ServiceCodeIchiranSource;
import jp.co.ndensan.reams.uz.uza.report.ReportEditorJoiner;

/**
 * 帳票設計_DBCMNJ2006_サービスコード一覧表Builder
 *
 * @reamsid_L DBC-3310-040 jiangxiaolong
 */
public class ServiceCodeIchiranBuilder
        implements IServiceCodeIchiranBuilder {

    private final IServiceCodeIchiranEditor headerEditor;
    private final IServiceCodeIchiranEditor bodyEditor;

    /**
     * コンストラクタです
     *
     * @param headerEditor HeaderEditor
     * @param bodyEditor BodyEditor
     */
    public ServiceCodeIchiranBuilder(
            IServiceCodeIchiranEditor headerEditor,
            IServiceCodeIchiranEditor bodyEditor) {
        this.headerEditor = headerEditor;
        this.bodyEditor = bodyEditor;
    }

    @Override
    public ServiceCodeIchiranSource build() {
        return ReportEditorJoiner.from(new ServiceCodeIchiranSource())
                .join(headerEditor).join(bodyEditor).buildSource();
    }

}
