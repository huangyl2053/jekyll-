...
/**
 * 帳票設計_DBCMN43002_事業高額決定通知書 Editorするクラスです。
 *
 * @reamsid_L DBC-2000-100 lijian
 */
public class JigyoKogakuKetteiTsuchishoYijiNashiEditor implements IJigyoKogakuKetteiTsuchishoYijiNashiEditor {
     ...
     */
    public JigyoKogakuKetteiTsuchishoYijiNashiEditor(
    ...
    @Override
    public JigyoKogakuKetteiTsuchishoYijiNashiSource edit(JigyoKogakuKetteiTsuchishoYijiNashiSource source) {
        ...
        source.shiharaiStartYMD = 年月日編集(帳票情報.get支払期間());
        source.karaFugo = 接続文字;
        source.shiharaiEndYMD = 年月日編集(帳票情報.get支払期間());
        source.shiharaiStartHMS = 年月日編集(帳票情報.get支払期間());
        source.shiharaiEndHMS = 年月日編集(帳票情報.get支払期間());
    ...
    private void set通知文(JigyoKogakuKetteiTsuchishoYijiNashiSource source) {
        ...
        source.tsuchibun2 = get通知文(INDEX_TWO);
        if (定型文文字サイズTWO.equals(帳票制御共通情報.get定型文文字サイズ())) {
            source.tsuchibunLarge = get通知文(INDEX_TWO);
        }
        if (定型文文字サイズTHREE.equals(帳票制御共通情報.get定型文文字サイズ())) {
            source.tsuchibunMix1 = get通知文(INDEX_TWO);
            source.tsuchibunMix2 = get通知文(INDEX_THREE);
        }
        if (定型文文字サイズFOUR.equals(帳票制御共通情報.get定型文文字サイズ())) {
            source.tsuchibunMixTwo1 = get通知文(INDEX_TWO);
            source.tsuchibunMixTwo2 = get通知文(INDEX_THREE);
        }
    ...
    private void setタイトル(JigyoKogakuKetteiTsuchishoYijiNashiSource source) {
    ...
    private void set雛形部品CompNinshosha(JigyoKogakuKetteiTsuchishoYijiNashiSource source) {
        if (認証者ソースデータ != null) {
