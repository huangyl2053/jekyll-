...
public class KogakuJigyoShinseishoHakkoIchiranEditor implements IKogakuJigyoShinseishoHakkoIchiranEditor {
     ...
     * @param 市町村名 RString
     */
    public KogakuJigyoShinseishoHakkoIchiranEditor(IOutputOrder 出力順,
            ShinseiJohoChohyoTempEntity 帳票出力対象データ,
            RDateTime システム日付, RString 連番, RString 市町村名) {
        this.出力順 = 出力順;
    ...
    public KogakuJigyoShinseishoHakkoIchiranSource edit(
        ...
        source.shichosonNo = 帳票出力対象データ.getShoKisaiHokenshaNoChohyo().value();
        ...
        source.listHakkoTaishosha_1 = 連番;
        source.listHakkoTaishosha_2 = 帳票出力対象データ.getHihokenshaNoChohyo().value();
        source.listHakkoTaishosha_3 = 帳票出力対象データ.getServiceTeikyoYMChohyo().wareki().toDateString();
        source.listHakkoTaishosha_4 = 帳票出力対象データ.getShinseishaShimeiChohyo().value();
        source.listHakkoTaishosha_5 = 帳票出力対象データ.getYubinNo().value();
        source.listHakkoTaishosha_6 = 帳票出力対象データ.getJusho().value();
        source.listHakkoTaishosha_7 = 帳票出力対象データ.getGyoseikuCode().value();
        ...
        source.listHakkoTaishosha_10 = 帳票出力対象データ.getYokaigoJotaiKubunCode().value();
        ...
        if (帳票出力対象データ.isHojinKeigenTaishoFlagChohyo()) {
            source.listHakkoTaishosha_15 = new RString("○");
        }
        if (帳票出力対象データ.isJidoShokanTaishoFlagChohyo()) {
            source.listHakkoTaishosha_16 = new RString("○");
        }
        if (帳票出力対象データ.getShikakuSoshitsuJiyuCodeChohyo() != null) {
            source.listHakkoTaishosha_17 = new RString("○");
        }
