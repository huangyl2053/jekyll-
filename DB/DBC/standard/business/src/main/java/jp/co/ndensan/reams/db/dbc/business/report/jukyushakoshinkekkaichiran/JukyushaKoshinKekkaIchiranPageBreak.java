...
public class JukyushaKoshinKekkaIchiranPageBreak extends PageBreaker<JukyushaKoshinKekkaIchiranSource> {
    ...
    public boolean isBreak(ReportLineRecord<JukyushaKoshinKekkaIchiranSource> currentSource,
        ...
        boolean flag = false;
        if (this.breakKeysList.contains(JukyushaKoshinKekkaIchiranOutputOrder.保険者番号.get項目ID())
                && !currentSource.getSource().hokenshaNo.equals(nextSource.getSource().hokenshaNo)) {
            flag = true;
        } else if (this.breakKeysList.contains(JukyushaKoshinKekkaIchiranOutputOrder.被保険者番号.get項目ID())
                && !currentSource.getSource().listList1_4.equals(nextSource.getSource().listList1_4)) {
            flag = true;
        } else if (this.breakKeysList.contains(JukyushaKoshinKekkaIchiranOutputOrder.郵便番号.get項目ID())
                && !currentSource.getSource().yubinNo.equals(nextSource.getSource().yubinNo)) {
            flag = true;
        } else if (this.breakKeysList.contains(JukyushaKoshinKekkaIchiranOutputOrder.町域コード.get項目ID())
                && !currentSource.getSource().choikiCode.equals(nextSource.getSource().choikiCode)) {
            flag = true;
        } else if (this.breakKeysList.contains(JukyushaKoshinKekkaIchiranOutputOrder.行政区コード.get項目ID())
                && !currentSource.getSource().gyoseikuCode.equals(nextSource.getSource().gyoseikuCode)) {
            flag = true;
        } else if (this.breakKeysList.contains(JukyushaKoshinKekkaIchiranOutputOrder.氏名５０音カナ.get項目ID())
                && !currentSource.getSource().shimei50onKana.equals(nextSource.getSource().shimei50onKana)) {
            flag = true;
        } else if (this.breakKeysList.contains(JukyushaKoshinKekkaIchiranOutputOrder.市町村コード.get項目ID())
