...
/**
 * 汎用リスト出力(総合事業費情報)_基本情報_連番なしのCSV用データEntityクラスです。
 *
...
@lombok.Setter
@OnNextSchema("rgdb")
public class HanyoListSogoJigyoHiKiHonReibanNashiEUCEntity implements IHanyoListSogoJigyoHiEUCEntity {
    ...
    private RString 空白;
    @CsvField(order = 27, name = "被保険者番号")
    private RString 被保険者番号;
    @CsvField(order = 28, name = "給付実績情報作成区分")
    private RString 給付実績情報作成区分;
    @CsvField(order = 29, name = "給付実績区分")
    private RString 給付実績区分;
    @CsvField(order = 30, name = "(給付)証記載保険者番号")
    private RString 給付証記載保険者番号;
    @CsvField(order = 31, name = "サービス提供年月")
    private RString サービス提供年月;
    @CsvField(order = 32, name = "給付実績事業者番号")
    private RString 給付実績事業者番号;
    @CsvField(order = 33, name = "給付実績事業者名")
    private RString 給付実績事業者名;
    @CsvField(order = 34, name = "整理番号")
    private RString 整理番号;
    @CsvField(order = 35, name = "給付実績生年月日")
    private RString 給付実績生年月日;
    @CsvField(order = 36, name = "給付実績性別")
    private RString 給付実績性別;
    @CsvField(order = 37, name = "給付実績要介護状態区分")
    private RString 給付実績要介護状態区分;
    @CsvField(order = 38, name = "給付実績認定開始日")
    private RString 給付実績認定開始日;
    @CsvField(order = 39, name = "給付実績認定終了日")
    private RString 給付実績認定終了日;
    @CsvField(order = 40, name = "老人保健市町村番号")
    private RString 老人保健市町村番号;
    @CsvField(order = 41, name = "老人保健受給者番号")
    private RString 老人保健受給者番号;
    @CsvField(order = 42, name = "保険者番号(後期)")
    private RString 保険者番号後期;
    @CsvField(order = 43, name = "被保険者番号(後期)")
    private RString 被保険者番号後期;
    @CsvField(order = 44, name = "保険者番号(国保)")
    private RString 保険者番号国保;
    @CsvField(order = 45, name = "被保険者証番号(国保)")
    private RString 被保険者証番号国保;
    @CsvField(order = 46, name = "個人番号(国保)")
    private RString 個人番号国保;
    @CsvField(order = 47, name = "開始年月日")
    private RString 開始年月日;
    @CsvField(order = 48, name = "中止年月日")
    private RString 中止年月日;
    @CsvField(order = 49, name = "(前)保険サービス単位数")
    private int 前保険サービス単位数;
    @CsvField(order = 50, name = "(前)保険請求額")
    private int 前保険請求額;
    @CsvField(order = 51, name = "(前)保険利用者負担額")
    private int 前保険利用者負担額;
    @CsvField(order = 52, name = "(後)保険サービス単位数")
    private int 後保険サービス単位数;
    @CsvField(order = 53, name = "(後)保険請求額")
    private int 後保険請求額;
    @CsvField(order = 54, name = "(後)保険利用者負担額")
    private int 後保険利用者負担額;
    @CsvField(order = 55, name = "警告区分")
    private RString 警告区分;
    @CsvField(order = 56, name = "審査年月")
    private RString 審査年月;
    @CsvField(order = 57, name = "取込年月")
