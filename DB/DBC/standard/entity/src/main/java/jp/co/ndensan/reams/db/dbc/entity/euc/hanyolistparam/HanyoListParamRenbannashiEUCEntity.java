...
/**
 * 汎用リスト_基準収入額適用情報連番がないCSV用データEntityです。
 *
...
@lombok.Setter
@OnNextSchema("rgdb")
public class HanyoListParamRenbannashiEUCEntity {
    ...
    @CsvField(order = 56, name = "受給要介護度")
    private RString 受給要介護度;
    @CsvField(order = 57, name = "受給認定開始日")
    private RString 受給認定開始日;
    @CsvField(order = 58, name = "受給認定終了日")
    private RString 受給認定終了日;
    @CsvField(order = 59, name = "受給認定日")
    private RString 受給認定日;
    @CsvField(order = 60, name = "受給旧措置")
    private RString 受給旧措置;
    @CsvField(order = 61, name = "受給みなし更新認定")
    private RString 受給みなし更新認定;
    @CsvField(order = 62, name = "受給直近事由")
    private RString 受給直近事由;
    @CsvField(order = 63, name = "世帯コード")
    private RString 基準収入額世帯コード;
    @CsvField(order = 64, name = "年度")
    private RString 年度;
    @CsvField(order = 65, name = "履歴番号")
    private RString 履歴番号;
    @CsvField(order = 66, name = "公的年金の収入")
    private Decimal 公的年金の収入;
    @CsvField(order = 67, name = "給与")
    private Decimal 給与;
    @CsvField(order = 68, name = "その他の収入")
    private Decimal その他の収入;
    @CsvField(order = 69, name = "算定基準額")
    private Decimal 算定基準額;
    @CsvField(order = 70, name = "適用開始年月")
    private RString 適用開始年月;
    @CsvField(order = 71, name = "申請日")
    private RString 申請日;
    @CsvField(order = 72, name = "決定日")
    private RString 決定日;
    @CsvField(order = 73, name = "申請書作成日")
    private RString 申請書作成日;
    @CsvField(order = 74, name = "世帯基準日")
    ...
    @CsvField(order = 80, name = "課税所得")
    private Decimal 課税所得;
    @CsvField(order = 81, name = "課税所得控除後")
    private Decimal 課税所得控除後;
    @CsvField(order = 82, name = "決定通知書発行日")
    private RString 決定通知書発行日;
    @CsvField(order = 83, name = "削除データ")
