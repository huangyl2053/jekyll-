...
/**
 * 汎用リスト出力(総合事業費情報)_基本集計情報_連番ありのCSV用データEntityクラスです。
 *
...
@lombok.Setter
@OnNextSchema("rgdb")
public class HanyoListSogoJigyoHiKiHonShukeiReibanAriEUCEntity implements IHanyoListSogoJigyoHiEUCEntity {
    ...
    private RString 空白;
    @CsvField(order = 28, name = "サービス種類コード")
    private RString サービス種類コード;
    @CsvField(order = 29, name = "サービス種類名称")
    private RString サービス種類名称;
    @CsvField(order = 30, name = "サービス実日数")
    private int サービス実日数;
    @CsvField(order = 31, name = "計画点数")
    private int 計画点数;
    @CsvField(order = 32, name = "限度額管理対象点数")
    private int 限度額管理対象点数;
    @CsvField(order = 33, name = "限度額管理対象外点数")
    private int 限度額管理対象外点数;
    @CsvField(order = 34, name = "請求点数合計")
    private int 請求点数合計;
    @CsvField(order = 35, name = "請求点数単価")
    private int 請求点数単価;
    @CsvField(order = 36, name = "請求額")
    private int 請求額;
    @CsvField(order = 37, name = "利用者負担額")
    private int 利用者負担額;
    @CsvField(order = 38, name = "(後)点数合計")
    private int 後点数合計;
    @CsvField(order = 39, name = "(後)保険請求分請求額")
    private int 後保険請求分請求額;
    @CsvField(order = 40, name = "再審査回数")
    private int 再審査回数;
    @CsvField(order = 41, name = "過誤回数")
    private int 過誤回数;
    @CsvField(order = 42, name = "開始年月日")
    private RString 開始年月日;
    @CsvField(order = 43, name = "中止年月日")
    private RString 中止年月日;
    @CsvField(order = 44, name = "(前)保険サービス単位数")
    private int 前保険サービス単位数;
    @CsvField(order = 45, name = "(前)保険請求額")
    private int 前保険請求額;
    @CsvField(order = 46, name = "(前)保険利用者負担額")
    private int 前保険利用者負担額;
    @CsvField(order = 47, name = "(後)保険サービス単位数")
    private int 後保険サービス単位数;
    @CsvField(order = 48, name = "(後)保険請求額")
    private int 後保険請求額;
    @CsvField(order = 49, name = "(後)保険利用者負担額")
    private int 後保険利用者負担額;
    @CsvField(order = 50, name = "警告区分")
    private RString 警告区分;
    @CsvField(order = 51, name = "審査年月")
    private RString 審査年月;
    @CsvField(order = 52, name = "取込年月")
