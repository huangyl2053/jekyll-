...
/**
 * 汎用リスト出力(総合事業費情報)_基本マネジメント情報_連番ありのCSV用データEntityクラスです。
 *
...
@lombok.Setter
@OnNextSchema("rgdb")
public class HanyoListSogoJigyoHiKiHonManejimentoReibanAriEUCEntity implements IHanyoListSogoJigyoHiEUCEntity {
    ...
    private RString 空白;
    @CsvField(order = 28, name = "指定基準該当事業者区分")
    private RString 指定基準該当事業者区分;
    @CsvField(order = 29, name = "単位数単価")
    private int 単位数単価;
    @CsvField(order = 30, name = "サービスコード")
    private RString サービスコード;
    @CsvField(order = 31, name = "サービス名称")
    private RString サービス名称;
    @CsvField(order = 32, name = "単位数")
    private int 単位数;
    @CsvField(order = 33, name = "回数")
    private int 回数;
    @CsvField(order = 34, name = "サービス単位数")
    private int サービス単位数;
    @CsvField(order = 35, name = "サービス単位数合計")
    private int サービス単位数合計;
    @CsvField(order = 36, name = "請求金額")
    private int 請求金額;
    @CsvField(order = 37, name = "摘要")
    private RString 摘要;
    @CsvField(order = 38, name = "利用者負担額")
    private int 利用者負担額;
    @CsvField(order = 39, name = "(後)単位数")
    private int 後単位数;
    @CsvField(order = 40, name = "(後)回数")
    private int 後回数;
    @CsvField(order = 41, name = "(後)サービス単位数")
    private int 後サービス単位数;
    @CsvField(order = 42, name = "(後)サービス単位数合計")
    private int 後サービス単位数合計;
    @CsvField(order = 43, name = "(後)請求金額")
    private int 後請求金額;
    @CsvField(order = 44, name = "(後)利用者負担額")
    private int 後利用者負担額;
    @CsvField(order = 45, name = "再審査回数")
    private int 再審査回数;
    @CsvField(order = 46, name = "過誤回数")
    private int 過誤回数;
