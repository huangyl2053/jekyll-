...
/**
 * 汎用リスト 再審査結果情報連番があるCSV用データEntityです。
 *
...
@lombok.Setter
@OnNextSchema("rgdb")
public class HanyoListSaishinsaKekkaRenbanAriEUCEntity implements IHanyoListSaishinsaKekkaEUCEntity {
    ...
    @CsvField(order = 55, name = "取扱年月")
    private RString 取扱年月;
    @CsvField(order = 56, name = "保険者区分")
    private RString 保険者区分;
    @CsvField(order = 57, name = "サービス提供年月")
    private RString サービス提供年月;
    @CsvField(order = 58, name = "再審査事業者番号")
    private RString 再審査事業者番号;
    @CsvField(order = 59, name = "再審査事業者名")
    private RString 再審査事業者名;
    @CsvField(order = 60, name = "サービス種類")
    private RString サービス種類;
    @CsvField(order = 61, name = "サービス種類名")
    private RString サービス種類名;
    @CsvField(order = 62, name = "申立事由コード")
    private RString 申立事由コード;
    @CsvField(order = 63, name = "申立事由")
    private RString 申立事由;
    @CsvField(order = 64, name = "再審査結果コード")
    private RString 再審査結果コード;
    @CsvField(order = 65, name = "再審査結果")
    private RString 再審査結果;
    @CsvField(order = 66, name = "当初請求件数")
    private RString 当初請求件数;
    @CsvField(order = 67, name = "原審単位数")
    private RString 原審単位数;
    @CsvField(order = 68, name = "申立単位数")
    private RString 申立単位数;
    @CsvField(order = 69, name = "決定単位数")
    private RString 決定単位数;
    @CsvField(order = 70, name = "調整単位数")
    private RString 調整単位数;
    @CsvField(order = 71, name = "保険者負担額")
    private RString 保険者負担額;
    @CsvField(order = 72, name = "公費受給者番号")
    private RString 公費受給者番号;
    @CsvField(order = 73, name = "（給付）証記載保険者番号")
    private RString 給付証記載保険者番号;
    @CsvField(order = 74, name = "受給申請事由")
    private RString 受給申請事由;
    @CsvField(order = 75, name = "受給申請日")
    private RString 受給申請日;
    @CsvField(order = 76, name = "受給要介護度")
    private RString 受給要介護度;
    @CsvField(order = 77, name = "受給認定開始日")
    private RString 受給認定開始日;
    @CsvField(order = 78, name = "受給認定終了日")
    private RString 受給認定終了日;
    @CsvField(order = 79, name = "受給認定日")
    private RString 受給認定日;
    @CsvField(order = 80, name = "受給旧措置")
    private RString 受給旧措置;
    @CsvField(order = 81, name = "受給みなし更新認定")
    private RString 受給みなし更新認定;
    @CsvField(order = 82, name = "受給直近事由")
