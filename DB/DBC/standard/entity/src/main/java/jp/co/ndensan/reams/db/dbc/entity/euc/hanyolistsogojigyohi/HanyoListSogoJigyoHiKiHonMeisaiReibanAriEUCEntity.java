...
/**
 * 汎用リスト出力(総合事業費情報)_基本明細情報_連番ありのCSV用データEntityクラスです。
 *
...
@lombok.Setter
@OnNextSchema("rgdb")
public class HanyoListSogoJigyoHiKiHonMeisaiReibanAriEUCEntity implements IHanyoListSogoJigyoHiEUCEntity {
    ...
    @CsvField(order = 30, name = "サービス項目コード")
    private RString サービス項目コード;
    @CsvField(order = 31, name = "サービス項目名称")
    private RString サービス項目名称;
    @CsvField(order = 32, name = "点数")
    private int 点数;
    @CsvField(order = 33, name = "日数／回数")
    private int 日数回数;
    @CsvField(order = 34, name = "サービス点数")
    private int サービス点数;
    @CsvField(order = 35, name = "摘要")
    private RString 摘要;
    @CsvField(order = 36, name = "(後)点数")
    private int 後点数;
    @CsvField(order = 37, name = "(後)日数／回数")
    private int 後日数回数;
    @CsvField(order = 38, name = "(後）サービス点数")
    private int 後サービス点数;
    ...
    @CsvField(order = 43, name = "(前)保険サービス単位数")
    private int 前保険サービス単位数;
    @CsvField(order = 44, name = "(前)保険請求額")
    private int 前保険請求額;
    @CsvField(order = 45, name = "(前)保険利用者負担額")
    private int 前保険利用者負担額;
    @CsvField(order = 46, name = "(後)保険サービス単位数")
    private int 後保険サービス単位数;
    @CsvField(order = 47, name = "(後)保険請求額")
    private int 後保険請求額;
    @CsvField(order = 48, name = "(後)保険利用者負担額")
