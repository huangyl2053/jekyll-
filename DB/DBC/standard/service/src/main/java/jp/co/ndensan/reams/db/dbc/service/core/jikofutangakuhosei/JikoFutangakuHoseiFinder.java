...
public class JikoFutangakuHoseiFinder {
    ...
    public SearchResult<KogakuGassanJikoFutanGaku> selectJyutakukaisyuList(HihokenshaNo 被保険者番号) {
        ...
        List<DbT3070KogakuGassanJikoFutanGakuEntity> entityList = mapper.get履歴オフ(parameter);
     ...
     *
     * @param 被保険者番号 被保険者番号
     * @return SearchResult<JigyoKogakuGassanJikoFutanGaku> 高額合算自己負担額一覧
     */
    public SearchResult<JigyoKogakuGassanJikoFutanGaku> selectList(HihokenshaNo 被保険者番号) {
        List<JigyoKogakuGassanJikoFutanGaku> 高額合算自己負担額一覧 = new ArrayList<>();
        IJikoFutangakuHoseiMapper mapper = mapperProvider.create(IJikoFutangakuHoseiMapper.class);
        Map<String, Object> parameter = new HashMap<>();
        parameter.put(名称_被保険者番号.toString(), 被保険者番号);
        List<DbT3170JigyoKogakuGassanJikoFutanGakuEntity> entityList = mapper.get履歴チェックオフ(parameter);
        for (DbT3170JigyoKogakuGassanJikoFutanGakuEntity entity : entityList) {
            entity.initializeMd5();
            高額合算自己負担額一覧.add(new JigyoKogakuGassanJikoFutanGaku(entity));
        }
        return SearchResult.of(高額合算自己負担額一覧, 0, false);
    }
}
