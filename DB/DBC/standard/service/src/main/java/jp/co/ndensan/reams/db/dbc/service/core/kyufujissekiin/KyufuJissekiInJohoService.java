 ...
 */
package jp.co.ndensan.reams.db.dbc.service.core.kyufujissekiin;
 ...
 */
public class KyufuJissekiInJohoService {
    ...
    /**
     * {@link InstanceProvider#create}にて生成した{@link KyufuJissekiInJohoService}のインスタンスを返します。
     *
     * @return KyufuJissekiInJohoService
     */
    public static KyufuJissekiInJohoService createInstance() {

        return InstanceProvider.create(KyufuJissekiInJohoService.class);
    }

    /**
     * CSVファイル1行目データを出力する
     *
     * @param entity KyufujissekiTorikomiIchiranEntity
     * @param parameter KyufuJissekiInDoIchiranhyoSakuseiProcessParameter
     * @param 連番 int
     * @return KyufuJissekiInCsvEntity
     */
    public KyufuJissekiInCsvEntity
            toヘッダのデータ(KyufujissekiTorikomiIchiranEntity entity,
                    KyufuJissekiInDoIchiranhyoSakuseiProcessParameter parameter, int 連番) {
        KyufuJissekiInCsvEntity returnEntity = to明細項目(entity, 連番);
     ...
     * @return KyufuJissekiInCsvEntity
     */
    public KyufuJissekiInCsvEntity to明細項目(KyufujissekiTorikomiIchiranEntity entity, int 連番) {
        KyufuJissekiInCsvEntity resultEntity
                = new KyufuJissekiInCsvEntity();
        resultEntity.set保険者番号(entity.get給付実績_保険者番号().getColumnValue());
        resultEntity.set保険者名(entity.get給付実績_保険者名());
        ...
        if (!RString.isNullOrEmpty(entity.get給付実績_情報作成区分コード())) {
            KyufuSakuseiKubun 給付実績情報作成区分 = KyufuSakuseiKubun.toValue(entity.get給付実績_情報作成区分コード());
            resultEntity.set作成区分(entity.get給付実績_情報作成区分コード());
            resultEntity.set作成区分名(給付実績情報作成区分.get名称());
        }
        resultEntity.set被保険者番号(entity.get被保険者_登録被保険者番号().getColumnValue());
        ...
        resultEntity.set事業者番号(entity.get給付実績_事業所番号().getColumnValue());
        if (!RString.isNullOrEmpty(entity.get給付実績_事業所番号().getColumnValue()) && entity.get給付実績_事業者名称().isEmpty()) {
    ...
    private void 明細設定(KyufuJissekiInCsvEntity resultEntity, KyufujissekiTorikomiIchiranEntity entity) {
     ...
     * @return KyufuJissekiInCsvEntity
     */
    public KyufuJissekiInCsvEntity to集計項目(KyufujissekiTorikomiIchiranEntity entity, int 合計件数) {
        KyufuJissekiInCsvEntity resultEntity
                = new KyufuJissekiInCsvEntity();
        resultEntity.set保険者番号(entity.get給付実績_保険者番号().getColumnValue());
        resultEntity.set保険者名(entity.get給付実績_保険者名());
