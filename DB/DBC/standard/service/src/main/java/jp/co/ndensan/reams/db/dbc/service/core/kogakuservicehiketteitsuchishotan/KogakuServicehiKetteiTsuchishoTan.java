...
public class KogakuServicehiKetteiTsuchishoTan {
    ...
    public JigyouKetteiTutisyoResult editTsuchisho(RString 文書番号,
        ...
        ServiceNoKanribangouRendouEntity 高額介護サービス費支給情報 = mapper.get高額介護サービス費支給情報(param);
        setﾀｲﾄﾙ(決定通知書Entity, 高額介護サービス費支給情報, 様式);
        setﾀｲﾄﾙ_取消し線用(決定通知書Entity, 高額介護サービス費支給情報);
        setﾀｲﾄﾙ_取消し線用_調整用(決定通知書Entity, 高額介護サービス費支給情報);
        ...
        if (高額介護サービス費支給情報 != null) {
            if (定値_0.equals(様式) && 高額介護サービス費支給情報.get受付年月日() != null
                    && !高額介護サービス費支給情報.get受付年月日().isEmpty()) {
                決定通知書Entity.set受付年月日(高額介護サービス費支給情報.get受付年月日().wareki().eraType(EraType.KANJI)
            ...
            決定通知書Entity.set決定年月日(高額介護サービス費支給情報.get決定年月日().wareki().eraType(EraType.KANJI)
                    .firstYear(FirstYear.GAN_NEN).separator(Separator.JAPANESE).fillType(FillType.ZERO).width(Width.FULL).toDateString());
            if (高額介護サービス費支給情報.get本人支払額() != null) {
                決定通知書Entity.set本人支払額(formatDecimal(高額介護サービス費支給情報.get本人支払額()));
            }
            if (ShikyuKubun.支給.getコード().equals(高額介護サービス費支給情報.get支給区分コード())) {
            ...
            if (Decimal.ZERO.compareTo(nullToZero(高額介護サービス費支給情報.get支給金額())) <= 0) {
                決定通知書Entity.set支給金額ﾀｲﾄﾙ(定値_支給金額);
            } else {
                決定通知書Entity.set支給金額ﾀｲﾄﾙ(定値_決定額);
            }
            決定通知書Entity.set支給金額(formatDecimal(高額介護サービス費支給情報.get支給金額()));
            決定通知書Entity.set通知書番号(高額介護サービス費支給情報.get通知書番号());

            set理由(決定通知書Entity, 高額介護サービス費支給情報.get不支給理由());
            set取消(決定通知書Entity, 高額介護サービス費支給情報);
            set窓口払い_口座払い(決定通知書Entity, 高額介護サービス費支給情報, 識別コード, 支払予定日);
            set問い合わせ文言(決定通知書Entity, 様式, 高額介護サービス費支給情報);
        } else {
            決定通知書Entity.set支給(定値_しない);
            決定通知書Entity.set支給金額ﾀｲﾄﾙ(定値_決定額);
            決定通知書Entity.set理由ﾀｲﾄﾙ(定値_不支給の理由);
        ...
        if (高額介護サービス費支給情報 != null) {
            set給付の種類(決定通知書Entity, 高額介護サービス費支給情報.get審査方法区分(), param);
        }
        決定通知書Entity.set連番(RString.EMPTY);
        ChohyoSeigyoHanyo 窓口項目見出し制御 = load帳票制御汎用(帳票分類ID, new RString("窓口項目見出し制御"));
        if (定値_1.equals(窓口項目見出し制御.get設定値())) {
     ...
     * @throws NullPointerException 帳票分類IDがnullの場合
     */
     ...
     */
    public ChohyoSeigyoHanyo load帳票制御汎用(ReportId 帳票分類ID, RString 項目名) {
        requireNonNull(帳票分類ID, UrSystemErrorMessages.値がnull.getReplacedMessage(帳票分類ID.toString()));
    ...
    private void setﾀｲﾄﾙ(JigyouKetteiTutisyoEntity 決定通知書Entity, ServiceNoKanribangouRendouEntity 高額介護サービス費支給情報, RString 様式) {
        ChohyoSeigyoHanyo 取り消し線 = load帳票制御汎用(帳票分類ID, new RString("取り消し線"));
        ChohyoSeigyoHanyo 帳票タイトル_調整用 = load帳票制御汎用(帳票分類ID, new RString("帳票タイトル_調整用"));
        if (取り消し線ない.equals(取り消し線.get設定値()) && 高額介護サービス費支給情報 != null) {
            if (Decimal.ZERO.compareTo(nullToZero(高額介護サービス費支給情報.get支給金額())) <= 0) {
                set帳票タイトル(決定通知書Entity, 様式);
            } else {
                決定通知書Entity.setﾀｲﾄﾙ(帳票タイトル_調整用.get設定値());
            }
        } else {
            決定通知書Entity.setﾀｲﾄﾙ(RString.EMPTY);
        }
    }

    private void set帳票タイトル(JigyouKetteiTutisyoEntity 決定通知書Entity, RString 様式) {
        ChohyoSeigyoHanyo 帳票タイトル_高額自動償還用 = load帳票制御汎用(帳票分類ID, new RString("帳票タイトル_高額自動償還用"));
        ChohyoSeigyoHanyo 帳票タイトル = load帳票制御汎用(帳票分類ID, new RString("帳票タイトル"));
        if (様式.equals(new RString("1"))) {
            決定通知書Entity.setﾀｲﾄﾙ(帳票タイトル_高額自動償還用.get設定値());
        } else {
            決定通知書Entity.setﾀｲﾄﾙ(帳票タイトル.get設定値());
        }
    }

    private void setﾀｲﾄﾙ_取消し線用(JigyouKetteiTutisyoEntity 決定通知書Entity,
            ServiceNoKanribangouRendouEntity 高額介護サービス費支給情報) {
        ChohyoSeigyoHanyo 取り消し線 = load帳票制御汎用(帳票分類ID, new RString("取り消し線"));
        if (取り消し線あり.equals(取り消し線.get設定値()) && 高額介護サービス費支給情報 != null) {
            if (Decimal.ZERO.compareTo(nullToZero(高額介護サービス費支給情報.get支給金額())) <= 0) {
                set帳票タイトル_抹消線あり(決定通知書Entity, 高額介護サービス費支給情報);
            }
        } else {
            決定通知書Entity.setﾀｲﾄﾙ_2_1_取消し線用(RString.EMPTY);
            ...
            決定通知書Entity.setﾀｲﾄﾙ_2_4_取消し線用(RString.EMPTY);
        }
    }

    private void set帳票タイトル_抹消線あり(JigyouKetteiTutisyoEntity 決定通知書Entity,
            ServiceNoKanribangouRendouEntity 高額介護サービス費支給情報) {
        ChohyoSeigyoHanyo 帳票タイトル_抹消線あり１ = load帳票制御汎用(帳票分類ID, new RString("帳票タイトル_抹消線あり１"));
        ChohyoSeigyoHanyo 帳票タイトル_抹消線あり２ = load帳票制御汎用(帳票分類ID, new RString("帳票タイトル_抹消線あり２"));
        ChohyoSeigyoHanyo 帳票タイトル_抹消線あり３ = load帳票制御汎用(帳票分類ID, new RString("帳票タイトル_抹消線あり３"));
        ChohyoSeigyoHanyo 帳票タイトル_抹消線あり４ = load帳票制御汎用(帳票分類ID, new RString("帳票タイトル_抹消線あり４"));
        決定通知書Entity.setﾀｲﾄﾙ_2_1_取消し線用(帳票タイトル_抹消線あり１.get設定値());
        if (高額介護サービス費支給情報.get支給区分コード().equals(new RString("1"))) {
            決定通知書Entity.setﾀｲﾄﾙ_2_2_1_取消し線用(帳票タイトル_抹消線あり２.get設定値());
            決定通知書Entity.setﾀｲﾄﾙ_2_2_2_取消し線用(RString.EMPTY);
            決定通知書Entity.setﾀｲﾄﾙ_2_3_1_取消し線用(RString.EMPTY);
            決定通知書Entity.setﾀｲﾄﾙ_2_3_2_取消し線用(帳票タイトル_抹消線あり３.get設定値());
            決定通知書Entity.setﾀｲﾄﾙ_2_4_取消し線用(帳票タイトル_抹消線あり４.get設定値());
        } else {
            決定通知書Entity.setﾀｲﾄﾙ_2_2_1_取消し線用(RString.EMPTY);
            決定通知書Entity.setﾀｲﾄﾙ_2_2_2_取消し線用(帳票タイトル_抹消線あり２.get設定値());
            決定通知書Entity.setﾀｲﾄﾙ_2_3_1_取消し線用(帳票タイトル_抹消線あり３.get設定値());
            決定通知書Entity.setﾀｲﾄﾙ_2_3_2_取消し線用(RString.EMPTY);
            決定通知書Entity.setﾀｲﾄﾙ_2_4_取消し線用(帳票タイトル_抹消線あり４.get設定値());
        }
    }
    ...
    private void setﾀｲﾄﾙ_取消し線用_調整用(JigyouKetteiTutisyoEntity 決定通知書Entity,
        ...
        if (取り消し線あり.equals(取り消し線.get設定値()) && 高額介護サービス費支給情報 != null) {
            if (Decimal.ZERO.compareTo(nullToZero(高額介護サービス費支給情報.get支給金額())) <= 0) {
                決定通知書Entity.setﾀｲﾄﾙ_3_1_取消し線用_調整用(RString.EMPTY);
                決定通知書Entity.setﾀｲﾄﾙ_3_2_1_取消し線用_調整用(RString.EMPTY);
                決定通知書Entity.setﾀｲﾄﾙ_3_2_2_取消し線用_調整用(RString.EMPTY);
                決定通知書Entity.setﾀｲﾄﾙ_3_3_1_取消し線用_調整用(RString.EMPTY);
                決定通知書Entity.setﾀｲﾄﾙ_3_3_2_取消し線用_調整用(RString.EMPTY);
                決定通知書Entity.setﾀｲﾄﾙ_3_4_取消し線用_調整用(RString.EMPTY);
            } else {
                set帳票タイトル_抹消線あり_調整用(決定通知書Entity, 高額介護サービス費支給情報);
            }
        } else {
            決定通知書Entity.setﾀｲﾄﾙ_3_1_取消し線用_調整用(RString.EMPTY);
            決定通知書Entity.setﾀｲﾄﾙ_3_2_1_取消し線用_調整用(RString.EMPTY);
    ...
    private void set帳票タイトル_抹消線あり_調整用(JigyouKetteiTutisyoEntity 決定通知書Entity,
            ServiceNoKanribangouRendouEntity 高額介護サービス費支給情報) {
        ChohyoSeigyoHanyo 帳票タイトル_抹消線あり１ = load帳票制御汎用(帳票分類ID, new RString("帳票タイトル_抹消線あり１"));
        ChohyoSeigyoHanyo 帳票タイトル_抹消線あり２ = load帳票制御汎用(帳票分類ID, new RString("帳票タイトル_抹消線あり２"));
        ChohyoSeigyoHanyo 帳票タイトル_抹消線あり３ = load帳票制御汎用(帳票分類ID, new RString("帳票タイトル_抹消線あり３"));
        ChohyoSeigyoHanyo 帳票タイトル_抹消線あり４ = load帳票制御汎用(帳票分類ID, new RString("帳票タイトル_抹消線あり４"));
        決定通知書Entity.setﾀｲﾄﾙ_3_1_取消し線用_調整用(帳票タイトル_抹消線あり１.get設定値());
        if (高額介護サービス費支給情報.get支給区分コード().equals(new RString("1"))) {
            決定通知書Entity.setﾀｲﾄﾙ_3_2_1_取消し線用_調整用(帳票タイトル_抹消線あり２.get設定値());
            決定通知書Entity.setﾀｲﾄﾙ_3_2_2_取消し線用_調整用(RString.EMPTY);
            決定通知書Entity.setﾀｲﾄﾙ_3_3_1_取消し線用_調整用(RString.EMPTY);
            決定通知書Entity.setﾀｲﾄﾙ_3_3_2_取消し線用_調整用(帳票タイトル_抹消線あり３.get設定値());
            決定通知書Entity.setﾀｲﾄﾙ_3_4_取消し線用_調整用(帳票タイトル_抹消線あり４.get設定値());
        } else {
            決定通知書Entity.setﾀｲﾄﾙ_3_2_1_取消し線用_調整用(RString.EMPTY);
            決定通知書Entity.setﾀｲﾄﾙ_3_2_2_取消し線用_調整用(帳票タイトル_抹消線あり２.get設定値());
            決定通知書Entity.setﾀｲﾄﾙ_3_3_1_取消し線用_調整用(帳票タイトル_抹消線あり３.get設定値());
            決定通知書Entity.setﾀｲﾄﾙ_3_3_2_取消し線用_調整用(RString.EMPTY);
            決定通知書Entity.setﾀｲﾄﾙ_3_4_取消し線用_調整用(帳票タイトル_抹消線あり４.get設定値());
        }
    }
    ...
    private void set給付の種類(JigyouKetteiTutisyoEntity 決定通知書Entity,
        ...
        IServiceNoKanribangouRendou2Mapper mapper = mapperProvider.create(IServiceNoKanribangouRendou2Mapper.class);
        List<DbT3054KogakuKyufuTaishoshaMeisaiEntity> 対象者明細 = mapper.get対象者明細情報(param);
        ...
        for (DbT3054KogakuKyufuTaishoshaMeisaiEntity entity : 対象者明細) {
            if (isFirst) {
    ...
    private void set取消(JigyouKetteiTutisyoEntity 決定通知書Entity, ServiceNoKanribangouRendouEntity 高額介護サービス費支給情報) {
        if (Decimal.ZERO.compareTo(nullToZero(高額介護サービス費支給情報.get支給金額())) <= 0
                && ShikyuKubun.支給.getコード().equals(高額介護サービス費支給情報.get支給区分コード())
                && ShiharaiHohoKubun.窓口払.getコード().equals(高額介護サービス費支給情報.get支払方法区分コード())) {
            決定通知書Entity.set取消１(RString.EMPTY);
            決定通知書Entity.set取消２(定値_アスタリスク１４桁);
        } else if (Decimal.ZERO.compareTo(nullToZero(高額介護サービス費支給情報.get支給金額())) <= 0
                && ShikyuKubun.支給.getコード().equals(高額介護サービス費支給情報.get支給区分コード())
                && ShiharaiHohoKubun.口座払.getコード().equals(高額介護サービス費支給情報.get支払方法区分コード())) {
            決定通知書Entity.set取消１(定値_アスタリスク１４桁);
            決定通知書Entity.set取消２(RString.EMPTY);
        } else if (Decimal.ZERO.compareTo(nullToZero(高額介護サービス費支給情報.get支給金額())) <= 0
                && ShikyuKubun.不支給.getコード().equals(高額介護サービス費支給情報.get支給区分コード())) {
            決定通知書Entity.set取消１(定値_アスタリスク１４桁);
            決定通知書Entity.set取消２(定値_アスタリスク１４桁);
        } else if (Decimal.ZERO.compareTo(nullToZero(高額介護サービス費支給情報.get支給金額())) > 0) {
            決定通知書Entity.set取消１(定値_アスタリスク１４桁);
    ...
    private void set窓口払い_口座払い(JigyouKetteiTutisyoEntity 決定通知書Entity,
            ServiceNoKanribangouRendouEntity 高額介護サービス費支給情報, ShikibetsuCode 識別コード, FlexibleDate 支払予定日) {
        if (Decimal.ZERO.compareTo(nullToZero(高額介護サービス費支給情報.get支給金額())) <= 0
                && ShikyuKubun.支給.getコード().equals(高額介護サービス費支給情報.get支給区分コード())
                && ShiharaiHohoKubun.窓口払.getコード().equals(高額介護サービス費支給情報.get支払方法区分コード())) {
            set窓口払い(決定通知書Entity, 高額介護サービス費支給情報);
        } else if (Decimal.ZERO.compareTo(nullToZero(高額介護サービス費支給情報.get支給金額())) <= 0
                && ShikyuKubun.支給.getコード().equals(高額介護サービス費支給情報.get支給区分コード())
                && ShiharaiHohoKubun.口座払.getコード().equals(高額介護サービス費支給情報.get支払方法区分コード())) {
            set口座払い(決定通知書Entity, 識別コード, 支払予定日, 高額介護サービス費支給情報);
        } else {
    ...
    private void set窓口払い(JigyouKetteiTutisyoEntity 決定通知書Entity,
        ...
        決定通知書Entity.set支払場所(高額介護サービス費支給情報.get支払場所());
        ...
        決定通知書Entity.set支払期間開始時間(set時間(高額介護サービス費支給情報.get支払窓口開始時間()));
        決定通知書Entity.set支払期間_２(定値_曲線);
        決定通知書Entity.set支払期間終了時間(set時間(高額介護サービス費支給情報.get支払窓口終了時間()));
        決定通知書Entity.set金融機関名(RString.EMPTY);
        決定通知書Entity.set金融機関支店名(RString.EMPTY);
        決定通知書Entity.set口座種別(RString.EMPTY);
        決定通知書Entity.set口座番号(RString.EMPTY);
    ...
    private void set口座払い(JigyouKetteiTutisyoEntity 決定通知書Entity, ShikibetsuCode 識別コード, FlexibleDate 支払予定日,
            ServiceNoKanribangouRendouEntity 高額介護サービス費支給情報) {
        ChohyoSeigyoHanyo 口座マスク有無 = load帳票制御汎用(帳票分類ID, new RString("口座マスク有無"));
        RString マスクフラグ = 口座マスク有無.get設定値();

        IKoza 口座 = null;
        IKozaManager iKozaManager = KozaService.createKozaManager();
        IKozaSearchKey searchKey = new KozaSearchKeyBuilder()
                .set口座ID(高額介護サービス費支給情報.get口座ID())
                .set識別コード(識別コード)
    ...
    private void set問い合わせ文言(JigyouKetteiTutisyoEntity 決定通知書Entity, RString 様式,
            ServiceNoKanribangouRendouEntity 高額介護サービス費支給情報) {
        if (定値_0.equals(様式)) {
            set初回申請用(決定通知書Entity, 高額介護サービス費支給情報);
        } else if (定値_1.equals(様式)) {
            set自動償還用(決定通知書Entity);
        }
    }

    private void set初回申請用(JigyouKetteiTutisyoEntity 決定通知書Entity,
            ServiceNoKanribangouRendouEntity 高額介護サービス費支給情報) {
        if (Decimal.ZERO.compareTo(nullToZero(高額介護サービス費支給情報.get支給金額())) <= 0
                || ShikyuKubun.不支給.getコード().equals(高額介護サービス費支給情報.get支給区分コード())) {
            Map<Integer, RString> レイヤ = ReportUtil
                    .get通知文(SubGyomuCode.DBC介護給付, 帳票分類ID, KamokuCode.EMPTY, NUM_1);
            ...
            決定通知書Entity.set文言1(レイヤ.get(NUM_1));
    ...
    private void set自動償還用(JigyouKetteiTutisyoEntity 決定通知書Entity) {
        Map<Integer, RString> レイヤ = ReportUtil
    ...
    private RString formatDate(FlexibleDate date) {
        return DateConverter.getWarekiYMDOfWeek(new RDate(date.toString()));
    }
