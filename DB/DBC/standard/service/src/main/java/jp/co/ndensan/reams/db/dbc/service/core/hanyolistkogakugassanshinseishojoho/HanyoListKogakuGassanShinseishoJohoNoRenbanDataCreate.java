 ...
 */
public class HanyoListKogakuGassanShinseishoJohoNoRenbanDataCreate {
     ...
     */
    public HanyoListKogakuGassanShinseishoJohoNoRenbanDataCreate(FlexibleDate システム日付) {
        this.システム日付 = システム日付;
    }
    ...
    private void set宛名本人(HanyoListKogakuGassanShinseishoJohoEntity entity,
        ...
        if (entity.get宛名Entity() != null) {
            IKojin 宛名 = ShikibetsuTaishoFactory.createKojin(entity.get宛名Entity());
            GyoseikuCode 行政区コード = 宛名.get行政区画() != null && 宛名.get行政区画()
                    .getGyoseiku() != null ? 宛名.get行政区画().getGyoseiku().getコード() : null;
    ...
    private void set事由(HanyoListKogakuGassanShinseishoJohoNoRenbanCSVEntity csvEntity,
    ...
    private void set宛先(HanyoListKogakuGassanShinseishoJohoEntity entity,
        ...
        if (entity.get宛先Entity() != null) {
            ...
            ZenkokuJushoCode 送付先住所コード = 宛先.get宛先住所() != null
                    ? 宛先.get宛先住所().get全国住所コード() : null;
            YubinNo 送付先郵便番号 = 宛先.get宛先住所() != null ? 宛先.get宛先住所().get郵便番号() : null;
            GyoseikuCode 送付先行政区コード = 宛先.get宛先行政区() != null
                    ? 宛先.get宛先行政区().getコード() : null;
    ...
    private void set送付先住所番地方書(HanyoListKogakuGassanShinseishoJohoEntity entity,
        ...
        if (entity.get宛先Entity() != null) {
            ...
            if (送付先住所 != null && 送付先住所.get番地() != null
                    && 送付先住所.get番地().getBanchi() != null && 送付先住所.get方書() != null) {
    ...
    private void set被保険者台帳管理(HanyoListKogakuGassanShinseishoJohoEntity entity,
        ...
        if (!RString.isNullOrEmpty(entity.get受給者台帳_みなし要介護区分コード())
                && null != MinashiCode.toValue(entity.get受給者台帳_みなし要介護区分コード())
        ...
        if (entity.get受給者台帳_直近異動事由コード() != null) {
            csvEntity.set受給直近事由(get受給直近事由(entity.get受給者台帳_直近異動事由コード()));
        }
    }
