...
//    }
    ...
    public Decimal getShikyuGendoGaku(HihokenshaNo 被保険者番号, FlexibleYearMonth サービス提供年月) {
        ...
        if (shuruiCode == null) {
            ...
            DbT7115UwanoseShokanShuruiShikyuGendoGakuEntity entity = shikyuGendoGakuDac.get上乗せ償還払い給付種類支給限度額(shuruiCode, サービス提供年月);
            if (entity == null) {
                DbT7112ShokanShuruiShikyuGendoGakuEntity gakuentity = shuruiShikyuGendoGakuDac.get償還払い給付種類支給限度額(shuruiCode, サービス提供年月);
                amount = gakuentity.getShikyuGendoTaniSu();
                if (amount == null) {
                    return new Decimal(0);
                } else {
                    return amount;
                ...
                amount = entity.getShikyuGendoTaniSu();
                return amount;
            }
        }
    }
    ...
    public ShokanShinsei getNewJutakuKaishuHi(HihokenshaNo 被保険者番号) {
        ...
        IJutakuKaishuJizenShinseiMapperParameter parameter = IJutakuKaishuJizenShinseiMapperParameter.createParameterForJutakuKaishuJizenShinsei(被保険者番号, RString.EMPTY, RString.EMPTY, FlexibleYearMonth.MAX, FlexibleYearMonth.MAX, ServiceShuruiCode.EMPTY);
    ...
    public ShokanShinsei getJutakuKaishuHi(HihokenshaNo 被保険者番号, FlexibleYearMonth サービス提供年月, RString 住宅改修住宅住所) {
        IJutakuKaishuJizenShinseiMapper mapper = mapperProvider.create(IJutakuKaishuJizenShinseiMapper.class);
        List<DbT3034ShokanShinseiEntity> list;
        IJutakuKaishuJizenShinseiMapperParameter parameter = IJutakuKaishuJizenShinseiMapperParameter.createParameterForJutakuKaishuJizenShinsei(被保険者番号, RString.EMPTY, 住宅改修住宅住所, サービス提供年月, FlexibleYearMonth.EMPTY, ServiceShuruiCode.EMPTY);
        List<DbT3034ShokanShinseiEntity> entityList = mapper.get支払結果計算の開始サービス提供年月限度額リセットがある(parameter);
        if (entityList.isEmpty()) {
            entityList = mapper.get支払結果計算の開始サービス提供年月限度額リセットなし(parameter);
            if (entityList.isEmpty()) {
                return null;
            } else {
                parameter = IJutakuKaishuJizenShinseiMapperParameter.createParameterForJutakuKaishuJizenShinsei(被保険者番号, 住宅改修住宅住所, 住宅改修住宅住所, entityList.get(0).getServiceTeikyoYM(), FlexibleYearMonth.MAX, ServiceShuruiCode.EMPTY);
    ...
    public RString getYoshikiNo(HihokenshaNo 被保険者番号, FlexibleYearMonth サービス提供年月, RString 整理番号) {
        RString yoshiNo = JutakuKaishuDac.get様式番号(被保険者番号, サービス提供年月, 整理番号).getYoshikiNo();
        if (yoshiNo == null) {
            return null;
        } else {
            return yoshiNo;
    ...
    public ShokanShinsei getOldJutakuKaishuHi(HihokenshaNo 被保険者番号, FlexibleYearMonth サービス提供年月, RString 整理番号) {
        DbT3049ShokanJutakuKaishuEntity entity = JutakuKaishuDac.get今回住宅改修の住宅住所(被保険者番号, サービス提供年月, 整理番号);
        if (entity == null) {
            return getNewJutakuKaishuHi(被保険者番号);
        } else {
            return getJutakuKaishuHi(被保険者番号, サービス提供年月, entity.getJutakuKaishuJutakuAddress());
        }

    }

    public RString getShoKisaiHokenshaNo(HihokenshaNo 被保険者番号, FlexibleYearMonth サービス提供年月) {
        daichoDac.selectByHihokenshaNo(被保険者番号);
...
//            dac.selectByKey(HihokenshaNo.EMPTY, FlexibleYearMonth.MAX, mode, Decimal.ZERO);
            DbT3035ShokanJutakuKaishuJizenShinseiEntity entity = new DbT3035ShokanJutakuKaishuJizenShinseiEntity();
            entity.setHiHokenshaNo(parameter.get被保険者番号());
            entity.setServiceTeikyoYM(parameter.getサービス提供年月());
            entity.setSeiriNo(parameter.get整理番号());
