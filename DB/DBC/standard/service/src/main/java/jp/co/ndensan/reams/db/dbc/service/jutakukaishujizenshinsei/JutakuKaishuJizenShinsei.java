...
//    }
    ...
    public YokaigoNinteiJyoho getYokaigoNinteiJyoho(HihokenshaNo 被保険者番号, FlexibleYearMonth サービス提供年月) {
        DbT4001JukyushaDaichoEntity entity = 受給者台帳Dac.getYokaigoNinteiJyoho(被保険者番号, new FlexibleDate(サービス提供年月.toDateString()));
        if (entity == null) {
            return null;
        } else {
...
//        }
    ...
    public Decimal getShikyuGendoGaku(HihokenshaNo 被保険者番号, FlexibleYearMonth サービス提供年月) {
        ...
        if (null == サービス種類コード) {
            ...
            IJutakuKaishuJizenShinseiMapper mapper = mapperProvider.create(IJutakuKaishuJizenShinseiMapper.class);
            IJutakuKaishuJizenShinseiMapperParameter parameter = IJutakuKaishuJizenShinseiMapperParameter.createParameterForJutakuKaishuJizenShinsei(
                    被保険者番号, RString.EMPTY, RString.EMPTY, サービス提供年月, サービス種類コード);
    ...
    public ShiharaiKekkaEntity getNewJutakuKaishuHi(HihokenshaNo 被保険者番号) {
        IJutakuKaishuJizenShinseiMapper mapper = mapperProvider.create(IJutakuKaishuJizenShinseiMapper.class);
    ...
    public ShiharaiKekkaEntity getJutakuKaishuHi(HihokenshaNo 被保険者番号, FlexibleYearMonth サービス提供年月, RString 住宅改修住宅住所) {
        IJutakuKaishuJizenShinseiMapper mapper = mapperProvider.create(IJutakuKaishuJizenShinseiMapper.class);
        List<ShiharaiKekkaEntity> list;
...
//
    ...
    public ShiharaiKekkaEntity getOldJutakuKaishuHi(HihokenshaNo 被保険者番号, FlexibleYearMonth サービス提供年月, RString 整理番号) {
        List<DbT3049ShokanJutakuKaishuEntity> list = JutakuKaishuDac.get今回住宅改修の住宅住所(被保険者番号, サービス提供年月, 整理番号);
        if (list.isEmpty()) {
            return getNewJutakuKaishuHi(被保険者番号);
        } else {
            RString 住宅改修住宅住所 = list.get(0).getJutakuKaishuJutakuAddress();
            return getJutakuKaishuHi(被保険者番号, サービス提供年月, 住宅改修住宅住所);
        }
