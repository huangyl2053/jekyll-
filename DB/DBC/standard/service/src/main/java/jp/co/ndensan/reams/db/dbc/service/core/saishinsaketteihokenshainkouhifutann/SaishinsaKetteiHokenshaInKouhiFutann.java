 ...
 */
package jp.co.ndensan.reams.db.dbc.service.core.saishinsaketteihokenshainkouhifutann;
...
import jp.co.ndensan.reams.db.dbc.entity.csv.saishinsaketteihokenshainkouhifutann.SaishinsaKetteiHokenshaInKouhiFutannDataEntity;
import jp.co.ndensan.reams.db.dbc.entity.csv.saishinsaketteihokenshainkouhifutann.SaishinsaKetteiHokenshaInKouhiFutannEntity;
import jp.co.ndensan.reams.db.dbc.entity.csv.saishinsaketteihokenshainkouhifutann.SaishinsaKetteiHokenshaInKouhiFutannGokeiEntity;
import jp.co.ndensan.reams.db.dbc.entity.csv.saishinsaketteihokenshainkouhifutann.SaishinsaKetteiHokenshaInKouhiFutannHeadEntity;
import jp.co.ndensan.reams.db.dbc.entity.csv.saishinsaketteihokenshainkouhifutann.SaishinsaKetteiHokenshaInKouhiFutannMeisaiEntity;
import jp.co.ndensan.reams.db.dbc.persistence.db.mapper.relate.saishinsaketteihokenshainkouhifutann.ISaishinsaKetteiHokenshaInKouhiFutannMapper;
...
/**
 * 再審査決定通知書情報取込共通処理（公費負担者分・ファイル読込）のCSVファイル読取 。
 *
 * @reamsid_L DBC-2820-011 surun
 */
public class SaishinsaKetteiHokenshaInKouhiFutann {

    private final MapperProvider mapperProvider;
    private SaishinsaKetteiHokenshaInKouhiFutannEntity entity;
    private KagoKetteiHokenshaInControlCsvEntity controlCsvEntity;
    private SaishinsaKetteiHokenshaInKouhiFutannDataEntity dataEntity;
    private SaishinsaKetteiHokenshaInKouhiFutannHeadEntity csvHeadEntity;
    private SaishinsaKetteiHokenshaInKouhiFutannMeisaiEntity csvMeisaiEntity;
    private SaishinsaKetteiHokenshaInKouhiFutannGokeiEntity csvGokeiEntity;
    private FlowEntity flowEntity;
    private List<SaishinsaKetteiHokenshaInKouhiFutannDataEntity> dataList;
    private List<SaishinsaKetteiHokenshaInKouhiFutannMeisaiEntity> meisaiList;
     ...
     * コンストラクタです。
     */
    public SaishinsaKetteiHokenshaInKouhiFutann() {
     ...
     */
    SaishinsaKetteiHokenshaInKouhiFutann(MapperProvider mapperProvider) {
        this.mapperProvider = mapperProvider;
    }

    /**
     * {@link InstanceProvider#create}にて生成した{@link SaishinsaKetteiHokenshaInKouhiFutann}のインスタンスを返します。
     *
     * @return {@link InstanceProvider#create}にて生成した{@link SaishinsaKetteiHokenshaInKouhiFutann}のインスタンス
     */
    public static SaishinsaKetteiHokenshaInKouhiFutann createInstance() {
        return InstanceProvider.create(SaishinsaKetteiHokenshaInKouhiFutann.class);
    }
    ...
    public FlowEntity 一時TBL作成と読込と登録(FlexibleYearMonth 処理年月, RString 保存先フォルダ, List<RString> エントリ情報List) {
        ...
        ISaishinsaKetteiHokenshaInKouhiFutannMapper mapper = this.mapperProvider.create(ISaishinsaKetteiHokenshaInKouhiFutannMapper.class);
    ...
    private FlowEntity 一時TBLに読込と登録(FlexibleYearMonth 処理年月, RString 保存先フォルダ, List<RString> エントリ情報List) {
        List<SaishinsaKetteiHokenshaInKouhiFutannEntity> csvlist = csvファイル読込(保存先フォルダ, エントリ情報List);
    ...
    @Transaction
    private FlowEntity 取込件数確認(List<SaishinsaKetteiHokenshaInKouhiFutannEntity> csvlist) {
        ISaishinsaKetteiHokenshaInKouhiFutannMapper mapper = this.mapperProvider.create(ISaishinsaKetteiHokenshaInKouhiFutannMapper.class);
    ...
    private FlowEntity バッチフロ(List<SaishinsaKetteiHokenshaInKouhiFutannEntity> csvlist) {
    ...
    @Transaction
    private void 再審査決定集計一時TBLに登録(FlexibleYearMonth 処理年月, List<SaishinsaKetteiHokenshaInKouhiFutannEntity> csvlist) {
        ISaishinsaKetteiHokenshaInKouhiFutannMapper mapper = this.mapperProvider.create(ISaishinsaKetteiHokenshaInKouhiFutannMapper.class);
        int 連番 = INDEX_0;
        for (int i = INDEX_0; i < csvlist.size(); i++) {
            SaishinsaKetteiHokenshaInKouhiFutannEntity csvEntity = csvlist.get(i);
            List<SaishinsaKetteiHokenshaInKouhiFutannDataEntity> listDataEntity = csvEntity.getListDataEntity();
    ...
    private void set件数と単位数と費用額(DbWT3063SaishinsaKetteiShukeiTempEntity shukeiTempentity, SaishinsaKetteiHokenshaInKouhiFutannDataEntity dataEntity) {
        ...
        if (dataEntity.getCsvGokeiEntity().get介護給付費請求公費負担額() != null
                && !dataEntity.getCsvGokeiEntity().get介護給付費請求公費負担額().isEmpty()) {
            shukeiTempentity.set介護給付費請求保険者負担額(new Decimal(dataEntity.getCsvGokeiEntity()
                    .get介護給付費請求公費負担額().toString()));
        } else {
        ...
        if (dataEntity.getCsvGokeiEntity().get介護給付費総合事業費決定公費負担額() != null
                && !dataEntity.getCsvGokeiEntity().get介護給付費総合事業費決定公費負担額().isEmpty()) {
            shukeiTempentity.set介護給付費決定保険者負担額(new Decimal(dataEntity.getCsvGokeiEntity()
                    .get介護給付費総合事業費決定公費負担額().toString()));
        }
        ...
        if (dataEntity.getCsvGokeiEntity().get介護給付費総合事業費調整公費負担額() != null
                && !dataEntity.getCsvGokeiEntity().get介護給付費総合事業費調整公費負担額().isEmpty()) {
            shukeiTempentity.set介護給付費調整保険者負担額(new Decimal(dataEntity.getCsvGokeiEntity()
                    .get介護給付費総合事業費調整公費負担額().toString()));
        }
    }

    @Transaction
    private void 再審査決定明細一時TBLに登録(FlexibleYearMonth 処理年月, List<SaishinsaKetteiHokenshaInKouhiFutannEntity> csvlist) {
        ISaishinsaKetteiHokenshaInKouhiFutannMapper mapper = this.mapperProvider.create(ISaishinsaKetteiHokenshaInKouhiFutannMapper.class);
        int 連番 = INDEX_0;
        int 履歴番号 = INDEX_0;
        for (int i = INDEX_0; i < csvlist.size(); i++) {
            ...
            for (int k = INDEX_0; k < listDataEntity.size(); k++) {
                履歴番号 = 履歴番号 + 1;
                List<SaishinsaKetteiHokenshaInKouhiFutannMeisaiEntity> listMeisaiCsvEntity = listDataEntity.get(k).getListCsvMeisaiEntity();
                for (int j = INDEX_0; j < listMeisaiCsvEntity.size(); j++) {
                    ...
                    if (listMeisaiCsvEntity.get(j).get証記載保険者番号() != null && !listMeisaiCsvEntity.get(j).get証記載保険者番号().isEmpty()) {
                    ...
                    if (listMeisaiCsvEntity.get(j).get公費負担額() != null
                            && !listMeisaiCsvEntity.get(j).get公費負担額().isEmpty()) {
    ...
    private void setサービスと申立(DbWT3064SaishinsaKetteiMeisaiTempEntity meisaiTempentity, SaishinsaKetteiHokenshaInKouhiFutannMeisaiEntity meisaiEntity) {
    ...
    @Transaction
    private void 被保険者一時TBLに登録(int 連番, SaishinsaKetteiHokenshaInKouhiFutannMeisaiEntity meisaiCsvEntity) {
        ISaishinsaKetteiHokenshaInKouhiFutannMapper mapper = this.mapperProvider.create(ISaishinsaKetteiHokenshaInKouhiFutannMapper.class);
     ...
     * @param 保存先フォルダ RString
     * @param エントリ情報List List<RString>
     * @return List<SaishinsaKetteiHokenshaInKouhiFutannEntity>
     */
    private List<SaishinsaKetteiHokenshaInKouhiFutannEntity> csvファイル読込(RString 保存先フォルダ, List<RString> エントリ情報List) {
        List<SaishinsaKetteiHokenshaInKouhiFutannEntity> list = new ArrayList<>();
        ...
        for (int j = INDEX_0; j < csvFullPathList.size(); j++) {
            entity = new SaishinsaKetteiHokenshaInKouhiFutannEntity();
            controlCsvEntity = new KagoKetteiHokenshaInControlCsvEntity();
            dataEntity = new SaishinsaKetteiHokenshaInKouhiFutannDataEntity();
            csvHeadEntity = new SaishinsaKetteiHokenshaInKouhiFutannHeadEntity();
            csvMeisaiEntity = new SaishinsaKetteiHokenshaInKouhiFutannMeisaiEntity();
            csvGokeiEntity = new SaishinsaKetteiHokenshaInKouhiFutannGokeiEntity();
                ...
                while (true) {
                    ...
                    if (data != null && !data.isEmpty()) {
                        if (レコード種別.equals(data.get(INDEX_0))) {
                            ...
                            csvHeadEntity = ListToObjectMappingHelper.toObject(SaishinsaKetteiHokenshaInKouhiFutannHeadEntity.class, data);
                        } else if (帳票レコード種別_D1.equals(data.get(INDEX_3))) {
                            csvMeisaiEntity = ListToObjectMappingHelper.toObject(SaishinsaKetteiHokenshaInKouhiFutannMeisaiEntity.class, data);
                            meisaiList.add(csvMeisaiEntity);
                        } else if (帳票レコード種別_T1.equals(data.get(INDEX_3))) {
                            csvGokeiEntity = ListToObjectMappingHelper.toObject(SaishinsaKetteiHokenshaInKouhiFutannGokeiEntity.class, data);
