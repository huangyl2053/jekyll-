...
import jp.co.ndensan.reams.db.dbc.entity.db.relate.jukyushaidorenrakuhyotoroku.JukyushaIdoRenrakuhyoCsvEntity;
...
/**
 * 帳票設計_DBCZV00001_受給者異動連絡票（CSV） Editor
 *
 * @reamsid_L DBC-2100-050 jiangzongyue
 */
public class JukyushaIdoRenrakuhyoCsvEditor {
     ...
     */
    public JukyushaIdoRenrakuhyoCsvEntity editor(JukyushaIdoRenrakuhyoTorokuEntity entity) {
    ...
    private void editorファイルの第一レコード(JukyushaIdoRenrakuhyoCsvEntity csvEntity) {
        csvEntity.setレコード種別1(ONE);
        csvEntity.setレコード番号連1(ONE);
        csvEntity.setボリュム通番(ZERO);
        csvEntity.setレコード件数(ONE);
        csvEntity.setデータ種別(データ種別);
        csvEntity.set福祉事務所特定番号(ZERO);
        KoikiShichosonJohoFinder finder = KoikiShichosonJohoFinder.createInstance();
        SearchResult<KoikiZenShichosonJoho> list = finder.koseiShichosonJoho();
        RString 証記載保険者番号 = list.records().get(0).get証記載保険者番号().getColumnValue();
        csvEntity.set保険者番号(証記載保険者番号);
        csvEntity.set事業所番号(ZERO);
        csvEntity.set都道府県番号(ZERO);
        RString 媒体区分 = DbBusinessConfig.get(ConfigNameDBC.国保連送付媒体_受給異動Ｆ_媒体区分,
    ...
    private void editorファイルの第二レコード(JukyushaIdoRenrakuhyoCsvEntity csvEntity, JukyushaIdoRenrakuhyoTorokuEntity entity) {
        csvEntity.setレコード種別2(TWO);
        csvEntity.setレコード番号連2(TWO);
        csvEntity.set交換情報識別番号(交換情報識別番号);
        csvEntity.set異動年月日(formatYMD(entity.get異動年月日()));
        csvEntity.set異動区分コード(entity.get異動区分());
