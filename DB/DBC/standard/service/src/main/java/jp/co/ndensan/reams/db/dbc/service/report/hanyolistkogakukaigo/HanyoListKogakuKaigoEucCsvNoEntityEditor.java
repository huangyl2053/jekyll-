...
public class HanyoListKogakuKaigoEucCsvNoEntityEditor {
     ...
     */
    public HanyoListKogakuKaigoEucCsvNoEntityEditor(FlexibleDate システム日付) {
        this.システム日付 = システム日付;
    }
    ...
    public HanyouRisutoSyuturyokuEucCsvNoEntity edit(HanyouRisutoSyuturyokuEntity entity,
        ...
        if (資格取得事由コード != null && !資格取得事由コード.isEmpty()) {
            RString 略称 = CodeMaster.getCodeRyakusho(SubGyomuCode.DBA介護資格,
                    DBACodeShubetsu.介護資格取得事由_被保険者.getCodeShubetsu(), new Code(資格取得事由コード),
        ...
        if (喪失事由コード != null && !喪失事由コード.isEmpty()) {
            RString 略称 = CodeMaster.getCodeRyakusho(SubGyomuCode.DBA介護資格,
                    DBACodeShubetsu.介護資格喪失事由_被保険者.getCodeShubetsu(), new Code(喪失事由コード),
                    FlexibleDate.getNowDate());
    ...
    private void set入所施設種類(HanyouRisutoSyuturyokuEntity entity,
        ...
        if (入所施設種類_11.equals(entity.get入所施設種類())) {
            ...
            csvEntity.set指定事業者郵便番号(entity.get郵便番号_05().isEmpty()
                    ? RString.EMPTY
                    : entity.get郵便番号_05().getEditedYubinNo());
    ...
    private void set介護保険施設(HanyouRisutoSyuturyokuEntity entity,
        ...
        csvEntity.set指定事業者郵便番号(entity.get郵便番号_60().isEmpty()
                ? RString.EMPTY
                : entity.get郵便番号_60().getEditedYubinNo());
    ...
    private RString get高額給付根拠(RString 高額給付根拠) {
        ...
        if (高額給付根拠 == null || 高額給付根拠.trim().isEmpty()) {
            ...
            if (RST_生.equals(本人の所得区分コード)) {
                return RST_第１;
            } else if (RST_市.equals(世帯の所得区分コード)) {
