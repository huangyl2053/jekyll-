 ...
 */
package jp.co.ndensan.reams.db.dba.divcontroller.handler.parentdiv.DBA7020011;
...
/**
 * 他特例施設退所通知書発行のバリデーションチェッククラスです。
 *
 * @reamsid_L DBA-0380-020 houtianpeng
 */
public class TatokureiTaishoTsuchishoHakkoHandler {
     ...
     * コンストラクタです。
     *
     * @param div 他特例施設退所通知書発行Div
     */
    public TatokureiTaishoTsuchishoHakkoHandler(TatokureiTaishoTsuchishoHakkoDiv div) {
        this.div = div;
    }
    ...
    public void 適用情報の編集() {
        ...
        if (row.getShisetuTaishoTuchiHakkoDate().getValue().isEmpty()) {
            div.getTajutokuTekiyoJohoIchiran().getReportPublish().getHenshuNaiyo().
            ...
            div.getTajutokuTekiyoJohoIchiran().getReportPublish().getHenshuNaiyo().
                    getCcdPrintContentsSetting().initialize(true, new RDate(row.
                                    getShisetuTaishoTuchiHakkoDate().getValue().toString()), true, false, RDate.MAX, false);
        }
     ...
     */
    public TashichosonJushochiTokurei 他特例施設退所通知書の編集(Models<TashichosonJushochiTokureiIdentifier, TashichosonJushochiTokurei> 他市町村住所地特例,
        ...
        return tashichosonJushochiTokurei.createBuilderForEdit().set施設退所通知発行年月日(new FlexibleDate(div.
