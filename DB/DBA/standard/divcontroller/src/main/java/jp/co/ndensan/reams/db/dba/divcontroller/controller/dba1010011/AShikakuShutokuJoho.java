...
public class AShikakuShutokuJoho {
    ...
    public ResponseData<ShikakuShutokuJohoDiv> initialize(ShikakuShutokuJohoDiv shikakuShutokuDiv, KihonJohoDiv kihonDiv) {
        ...
        //
        //2, 医療保険加入、生活保護、老齢福祉年金、施設入退所の情報を設定する。
        //   共有子Div側に用意されているloadメソッド等を利用して、それぞれ履歴を設定する。
        //
        //3, 保険者情報表示制御、幅調整のモード設定を行う。
        //   保険者の情報を取得し、その情報を元に以下のようにグリッドの表示を変更する。
        //3-1, 介護共通 業務Configの保険者情報が「単一保険者」で被保険者台帳の証記載保険者番号より取得した保険者が「合併なし」の保険者である場合、「所在保険者」「措置元保険者」「旧保険者」列を非表示にする。
        //      (HokenshaJohoDisplayMode=TanitsuGappeiNashi)
        //      (DataGridWidth=Size5)"
        //3-2, 介護共通 業務Configの保険者情報が「単一保険者」で被保険者台帳の証記載保険者番号より取得した保険者が「合併あり」の保険者である場合、「所在保険者」「措置元保険者」列を非表示、「旧保険者」列を表示する。
        //      (HokenshaJohoDisplayMode=TanitsuGappeiAri)
        //      (DataGridWidth=Size3)"
        //3-3, 介護共通 業務Configの保険者情報が「広域保険者」で被保険者台帳の証記載保険者番号より取得した保険者が「合併なし」の保険者である場合、「所在保険者」「措置元保険者」列を表示、「旧保険者」列を非表示にする。
        //      (HokenshaJohoDisplayMode=KoikiGappeiNashi)
        //      (DataGridWidth=Size1)"
        //3-4, 介護共通 業務Configの保険者情報が「広域保険者」で被保険者台帳の証記載保険者番号より取得した保険者が「合併あり」の保険者である場合、「所在保険者」「措置元保険者」「旧保険者」列を表示する。
        //      (HokenshaJohoDisplayMode=KoikiGappeiAri)
        //      (DataGridWidth=Size1)"
        //4, 被保険者台帳管理の最新レコードの状態が「資格未取得」であることをチェックする。(レコードが存在しない、または資格喪失年月日が空白でない)
        //4-1, 被保険者台帳管理が資格未取得の状態でない場合は、資格得喪履歴パネルをReadOnlyに設定し、入力不可の状態にする。"
        //
        //5, 取得事由ドロップダウンリストに、コード種別:0117(介護資格取得事由（被保険者）)のデータを設定する。
        //
