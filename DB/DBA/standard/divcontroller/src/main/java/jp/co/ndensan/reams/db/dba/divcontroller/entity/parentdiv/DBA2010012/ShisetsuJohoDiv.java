package jp.co.ndensan.reams.db.dba.divcontroller.entity.parentdiv.DBA2010012;
/*
 * このコードはツールによって生成されました。
 * このファイルへの変更は、再生成時には損失するため
 * 不正な動作の原因になります。
 */

import com.fasterxml.jackson.annotation.JsonProperty;
import jp.co.ndensan.reams.uz.uza.ui.binding.Panel;
import jp.co.ndensan.reams.uz.uza.ui.binding.RadioButton;
import jp.co.ndensan.reams.uz.uza.ui.binding.TextBox;
import jp.co.ndensan.reams.uz.uza.ui.binding.TextBoxCode;
import jp.co.ndensan.reams.uz.uza.ui.binding.TextBoxFlexibleDate;
import jp.co.ndensan.reams.uz.uza.ui.binding.TextBoxMultiLine;
import jp.co.ndensan.reams.uz.uza.ui.binding.TextBoxYubinNo;

/**
 * ShisetsuJoho のクラスファイル
 *
 * @reamsid_L DBA-0340-040 wanghui
 */
public class ShisetsuJohoDiv extends Panel {

    // <editor-fold defaultstate="collapsed" desc="Created By UIDesigner ver：UZ-deploy-2016-01-15_09-59-03">
    /*
     * [ private の作成 ]
     * クライアント側から取得した情報を元にを検索を行い
     * コントロール名とフィールド名を取得する
     * private + コントロール名 + フィールド名 の文字列を作成
     */
    @JsonProperty("txtShisetsuYukoKaishiYMD")
    private TextBoxFlexibleDate txtShisetsuYukoKaishiYMD;
    @JsonProperty("txtShisetsuYukoShuryoYMD")
    private TextBoxFlexibleDate txtShisetsuYukoShuryoYMD;
    @JsonProperty("radShisetsuKannaiKangaiKubun")
    private RadioButton radShisetsuKannaiKangaiKubun;
    @JsonProperty("txtShisetsuJigyoshaNo")
    private TextBoxCode txtShisetsuJigyoshaNo;
    @JsonProperty("txtShisetsuJigyoshaName")
    private TextBox txtShisetsuJigyoshaName;
    @JsonProperty("txtShisetsuJigyoshaNameKana")
    private TextBox txtShisetsuJigyoshaNameKana;
    @JsonProperty("txtShisetsuRyakusho")
    private TextBox txtShisetsuRyakusho;
    @JsonProperty("txtShisetsuRyakushoKana")
    private TextBox txtShisetsuRyakushoKana;
    @JsonProperty("txtShisetsuJigyoKaishiYMD")
    private TextBoxFlexibleDate txtShisetsuJigyoKaishiYMD;
    @JsonProperty("txtShisetsuJigyoKyushuYMD")
    private TextBoxFlexibleDate txtShisetsuJigyoKyushuYMD;
    @JsonProperty("txtShisetsuJigyoSaikaiYMD")
    private TextBoxFlexibleDate txtShisetsuJigyoSaikaiYMD;
    @JsonProperty("txtShisetsuJigyoHaishiYMD")
    private TextBoxFlexibleDate txtShisetsuJigyoHaishiYMD;
    @JsonProperty("txtShisetsuYubinNo")
    private TextBoxYubinNo txtShisetsuYubinNo;
    @JsonProperty("txtShisetsuTelNo")
    private TextBox txtShisetsuTelNo;
    @JsonProperty("txtShisetsuFaxNo")
    private TextBox txtShisetsuFaxNo;
    @JsonProperty("txtShisetsuJusho")
    private TextBox txtShisetsuJusho;
    @JsonProperty("txtShisetsuJushoKana")
    private TextBox txtShisetsuJushoKana;
    @JsonProperty("txtShisetsuDaihyoshaName")
    private TextBox txtShisetsuDaihyoshaName;
    @JsonProperty("txtShisetsuDaihyoshaNameKana")
    private TextBox txtShisetsuDaihyoshaNameKana;
    @JsonProperty("txtShisetsuYakushoku")
    private TextBox txtShisetsuYakushoku;
    @JsonProperty("txtShisetsuBiko")
    private TextBoxMultiLine txtShisetsuBiko;

    /*
     * [ GetterとSetterの作成 ]
     * クライアント側から取得した情報を元に検索を行い
     * コントロール名とフィールド名を取得する
     * フィールド名のGetterとSetter を作成
     */
    /*
     * gettxtShisetsuYukoKaishiYMD
     * @return txtShisetsuYukoKaishiYMD
     */
    @JsonProperty("txtShisetsuYukoKaishiYMD")
    public TextBoxFlexibleDate getTxtShisetsuYukoKaishiYMD() {
        return txtShisetsuYukoKaishiYMD;
    }

    /*
     * settxtShisetsuYukoKaishiYMD
     * @param txtShisetsuYukoKaishiYMD txtShisetsuYukoKaishiYMD
     */
    @JsonProperty("txtShisetsuYukoKaishiYMD")
    public void setTxtShisetsuYukoKaishiYMD(TextBoxFlexibleDate txtShisetsuYukoKaishiYMD) {
        this.txtShisetsuYukoKaishiYMD = txtShisetsuYukoKaishiYMD;
    }

    /*
     * gettxtShisetsuYukoShuryoYMD
     * @return txtShisetsuYukoShuryoYMD
     */
    @JsonProperty("txtShisetsuYukoShuryoYMD")
    public TextBoxFlexibleDate getTxtShisetsuYukoShuryoYMD() {
        return txtShisetsuYukoShuryoYMD;
    }

    /*
     * settxtShisetsuYukoShuryoYMD
     * @param txtShisetsuYukoShuryoYMD txtShisetsuYukoShuryoYMD
     */
    @JsonProperty("txtShisetsuYukoShuryoYMD")
    public void setTxtShisetsuYukoShuryoYMD(TextBoxFlexibleDate txtShisetsuYukoShuryoYMD) {
        this.txtShisetsuYukoShuryoYMD = txtShisetsuYukoShuryoYMD;
    }

    /*
     * getradShisetsuKannaiKangaiKubun
     * @return radShisetsuKannaiKangaiKubun
     */
    @JsonProperty("radShisetsuKannaiKangaiKubun")
    public RadioButton getRadShisetsuKannaiKangaiKubun() {
        return radShisetsuKannaiKangaiKubun;
    }

    /*
     * setradShisetsuKannaiKangaiKubun
     * @param radShisetsuKannaiKangaiKubun radShisetsuKannaiKangaiKubun
     */
    @JsonProperty("radShisetsuKannaiKangaiKubun")
    public void setRadShisetsuKannaiKangaiKubun(RadioButton radShisetsuKannaiKangaiKubun) {
        this.radShisetsuKannaiKangaiKubun = radShisetsuKannaiKangaiKubun;
    }

    /*
     * gettxtShisetsuJigyoshaNo
     * @return txtShisetsuJigyoshaNo
     */
    @JsonProperty("txtShisetsuJigyoshaNo")
    public TextBoxCode getTxtShisetsuJigyoshaNo() {
        return txtShisetsuJigyoshaNo;
    }

    /*
     * settxtShisetsuJigyoshaNo
     * @param txtShisetsuJigyoshaNo txtShisetsuJigyoshaNo
     */
    @JsonProperty("txtShisetsuJigyoshaNo")
    public void setTxtShisetsuJigyoshaNo(TextBoxCode txtShisetsuJigyoshaNo) {
        this.txtShisetsuJigyoshaNo = txtShisetsuJigyoshaNo;
    }

    /*
     * gettxtShisetsuJigyoshaName
     * @return txtShisetsuJigyoshaName
     */
    @JsonProperty("txtShisetsuJigyoshaName")
    public TextBox getTxtShisetsuJigyoshaName() {
        return txtShisetsuJigyoshaName;
    }

    /*
     * settxtShisetsuJigyoshaName
     * @param txtShisetsuJigyoshaName txtShisetsuJigyoshaName
     */
    @JsonProperty("txtShisetsuJigyoshaName")
    public void setTxtShisetsuJigyoshaName(TextBox txtShisetsuJigyoshaName) {
        this.txtShisetsuJigyoshaName = txtShisetsuJigyoshaName;
    }

    /*
     * gettxtShisetsuJigyoshaNameKana
     * @return txtShisetsuJigyoshaNameKana
     */
    @JsonProperty("txtShisetsuJigyoshaNameKana")
    public TextBox getTxtShisetsuJigyoshaNameKana() {
        return txtShisetsuJigyoshaNameKana;
    }

    /*
     * settxtShisetsuJigyoshaNameKana
     * @param txtShisetsuJigyoshaNameKana txtShisetsuJigyoshaNameKana
     */
    @JsonProperty("txtShisetsuJigyoshaNameKana")
    public void setTxtShisetsuJigyoshaNameKana(TextBox txtShisetsuJigyoshaNameKana) {
        this.txtShisetsuJigyoshaNameKana = txtShisetsuJigyoshaNameKana;
    }

    /*
     * gettxtShisetsuRyakusho
     * @return txtShisetsuRyakusho
     */
    @JsonProperty("txtShisetsuRyakusho")
    public TextBox getTxtShisetsuRyakusho() {
        return txtShisetsuRyakusho;
    }

    /*
     * settxtShisetsuRyakusho
     * @param txtShisetsuRyakusho txtShisetsuRyakusho
     */
    @JsonProperty("txtShisetsuRyakusho")
    public void setTxtShisetsuRyakusho(TextBox txtShisetsuRyakusho) {
        this.txtShisetsuRyakusho = txtShisetsuRyakusho;
    }

    /*
     * gettxtShisetsuRyakushoKana
     * @return txtShisetsuRyakushoKana
     */
    @JsonProperty("txtShisetsuRyakushoKana")
    public TextBox getTxtShisetsuRyakushoKana() {
        return txtShisetsuRyakushoKana;
    }

    /*
     * settxtShisetsuRyakushoKana
     * @param txtShisetsuRyakushoKana txtShisetsuRyakushoKana
     */
    @JsonProperty("txtShisetsuRyakushoKana")
    public void setTxtShisetsuRyakushoKana(TextBox txtShisetsuRyakushoKana) {
        this.txtShisetsuRyakushoKana = txtShisetsuRyakushoKana;
    }

    /*
     * gettxtShisetsuJigyoKaishiYMD
     * @return txtShisetsuJigyoKaishiYMD
     */
    @JsonProperty("txtShisetsuJigyoKaishiYMD")
    public TextBoxFlexibleDate getTxtShisetsuJigyoKaishiYMD() {
        return txtShisetsuJigyoKaishiYMD;
    }

    /*
     * settxtShisetsuJigyoKaishiYMD
     * @param txtShisetsuJigyoKaishiYMD txtShisetsuJigyoKaishiYMD
     */
    @JsonProperty("txtShisetsuJigyoKaishiYMD")
    public void setTxtShisetsuJigyoKaishiYMD(TextBoxFlexibleDate txtShisetsuJigyoKaishiYMD) {
        this.txtShisetsuJigyoKaishiYMD = txtShisetsuJigyoKaishiYMD;
    }

    /*
     * gettxtShisetsuJigyoKyushuYMD
     * @return txtShisetsuJigyoKyushuYMD
     */
    @JsonProperty("txtShisetsuJigyoKyushuYMD")
    public TextBoxFlexibleDate getTxtShisetsuJigyoKyushuYMD() {
        return txtShisetsuJigyoKyushuYMD;
    }

    /*
     * settxtShisetsuJigyoKyushuYMD
     * @param txtShisetsuJigyoKyushuYMD txtShisetsuJigyoKyushuYMD
     */
    @JsonProperty("txtShisetsuJigyoKyushuYMD")
    public void setTxtShisetsuJigyoKyushuYMD(TextBoxFlexibleDate txtShisetsuJigyoKyushuYMD) {
        this.txtShisetsuJigyoKyushuYMD = txtShisetsuJigyoKyushuYMD;
    }

    /*
     * gettxtShisetsuJigyoSaikaiYMD
     * @return txtShisetsuJigyoSaikaiYMD
     */
    @JsonProperty("txtShisetsuJigyoSaikaiYMD")
    public TextBoxFlexibleDate getTxtShisetsuJigyoSaikaiYMD() {
        return txtShisetsuJigyoSaikaiYMD;
    }

    /*
     * settxtShisetsuJigyoSaikaiYMD
     * @param txtShisetsuJigyoSaikaiYMD txtShisetsuJigyoSaikaiYMD
     */
    @JsonProperty("txtShisetsuJigyoSaikaiYMD")
    public void setTxtShisetsuJigyoSaikaiYMD(TextBoxFlexibleDate txtShisetsuJigyoSaikaiYMD) {
        this.txtShisetsuJigyoSaikaiYMD = txtShisetsuJigyoSaikaiYMD;
    }

    /*
     * gettxtShisetsuJigyoHaishiYMD
     * @return txtShisetsuJigyoHaishiYMD
     */
    @JsonProperty("txtShisetsuJigyoHaishiYMD")
    public TextBoxFlexibleDate getTxtShisetsuJigyoHaishiYMD() {
        return txtShisetsuJigyoHaishiYMD;
    }

    /*
     * settxtShisetsuJigyoHaishiYMD
     * @param txtShisetsuJigyoHaishiYMD txtShisetsuJigyoHaishiYMD
     */
    @JsonProperty("txtShisetsuJigyoHaishiYMD")
    public void setTxtShisetsuJigyoHaishiYMD(TextBoxFlexibleDate txtShisetsuJigyoHaishiYMD) {
        this.txtShisetsuJigyoHaishiYMD = txtShisetsuJigyoHaishiYMD;
    }

    /*
     * gettxtShisetsuYubinNo
     * @return txtShisetsuYubinNo
     */
    @JsonProperty("txtShisetsuYubinNo")
    public TextBoxYubinNo getTxtShisetsuYubinNo() {
        return txtShisetsuYubinNo;
    }

    /*
     * settxtShisetsuYubinNo
     * @param txtShisetsuYubinNo txtShisetsuYubinNo
     */
    @JsonProperty("txtShisetsuYubinNo")
    public void setTxtShisetsuYubinNo(TextBoxYubinNo txtShisetsuYubinNo) {
        this.txtShisetsuYubinNo = txtShisetsuYubinNo;
    }

    /*
     * gettxtShisetsuTelNo
     * @return txtShisetsuTelNo
     */
    @JsonProperty("txtShisetsuTelNo")
    public TextBox getTxtShisetsuTelNo() {
        return txtShisetsuTelNo;
    }

    /*
     * settxtShisetsuTelNo
     * @param txtShisetsuTelNo txtShisetsuTelNo
     */
    @JsonProperty("txtShisetsuTelNo")
    public void setTxtShisetsuTelNo(TextBox txtShisetsuTelNo) {
        this.txtShisetsuTelNo = txtShisetsuTelNo;
    }

    /*
     * gettxtShisetsuFaxNo
     * @return txtShisetsuFaxNo
     */
    @JsonProperty("txtShisetsuFaxNo")
    public TextBox getTxtShisetsuFaxNo() {
        return txtShisetsuFaxNo;
    }

    /*
     * settxtShisetsuFaxNo
     * @param txtShisetsuFaxNo txtShisetsuFaxNo
     */
    @JsonProperty("txtShisetsuFaxNo")
    public void setTxtShisetsuFaxNo(TextBox txtShisetsuFaxNo) {
        this.txtShisetsuFaxNo = txtShisetsuFaxNo;
    }

    /*
     * gettxtShisetsuJusho
     * @return txtShisetsuJusho
     */
    @JsonProperty("txtShisetsuJusho")
    public TextBox getTxtShisetsuJusho() {
        return txtShisetsuJusho;
    }

    /*
     * settxtShisetsuJusho
     * @param txtShisetsuJusho txtShisetsuJusho
     */
    @JsonProperty("txtShisetsuJusho")
    public void setTxtShisetsuJusho(TextBox txtShisetsuJusho) {
        this.txtShisetsuJusho = txtShisetsuJusho;
    }

    /*
     * gettxtShisetsuJushoKana
     * @return txtShisetsuJushoKana
     */
    @JsonProperty("txtShisetsuJushoKana")
    public TextBox getTxtShisetsuJushoKana() {
        return txtShisetsuJushoKana;
    }

    /*
     * settxtShisetsuJushoKana
     * @param txtShisetsuJushoKana txtShisetsuJushoKana
     */
    @JsonProperty("txtShisetsuJushoKana")
    public void setTxtShisetsuJushoKana(TextBox txtShisetsuJushoKana) {
        this.txtShisetsuJushoKana = txtShisetsuJushoKana;
    }

    /*
     * gettxtShisetsuDaihyoshaName
     * @return txtShisetsuDaihyoshaName
     */
    @JsonProperty("txtShisetsuDaihyoshaName")
    public TextBox getTxtShisetsuDaihyoshaName() {
        return txtShisetsuDaihyoshaName;
    }

    /*
     * settxtShisetsuDaihyoshaName
     * @param txtShisetsuDaihyoshaName txtShisetsuDaihyoshaName
     */
    @JsonProperty("txtShisetsuDaihyoshaName")
    public void setTxtShisetsuDaihyoshaName(TextBox txtShisetsuDaihyoshaName) {
        this.txtShisetsuDaihyoshaName = txtShisetsuDaihyoshaName;
    }

    /*
     * gettxtShisetsuDaihyoshaNameKana
     * @return txtShisetsuDaihyoshaNameKana
     */
    @JsonProperty("txtShisetsuDaihyoshaNameKana")
    public TextBox getTxtShisetsuDaihyoshaNameKana() {
        return txtShisetsuDaihyoshaNameKana;
    }

    /*
     * settxtShisetsuDaihyoshaNameKana
     * @param txtShisetsuDaihyoshaNameKana txtShisetsuDaihyoshaNameKana
     */
    @JsonProperty("txtShisetsuDaihyoshaNameKana")
    public void setTxtShisetsuDaihyoshaNameKana(TextBox txtShisetsuDaihyoshaNameKana) {
        this.txtShisetsuDaihyoshaNameKana = txtShisetsuDaihyoshaNameKana;
    }

    /*
     * gettxtShisetsuYakushoku
     * @return txtShisetsuYakushoku
     */
    @JsonProperty("txtShisetsuYakushoku")
    public TextBox getTxtShisetsuYakushoku() {
        return txtShisetsuYakushoku;
    }

    /*
     * settxtShisetsuYakushoku
     * @param txtShisetsuYakushoku txtShisetsuYakushoku
     */
    @JsonProperty("txtShisetsuYakushoku")
    public void setTxtShisetsuYakushoku(TextBox txtShisetsuYakushoku) {
        this.txtShisetsuYakushoku = txtShisetsuYakushoku;
    }

    /*
     * gettxtShisetsuBiko
     * @return txtShisetsuBiko
     */
    @JsonProperty("txtShisetsuBiko")
    public TextBoxMultiLine getTxtShisetsuBiko() {
        return txtShisetsuBiko;
    }

    /*
     * settxtShisetsuBiko
     * @param txtShisetsuBiko txtShisetsuBiko
     */
    @JsonProperty("txtShisetsuBiko")
    public void setTxtShisetsuBiko(TextBoxMultiLine txtShisetsuBiko) {
        this.txtShisetsuBiko = txtShisetsuBiko;
    }

    // </editor-fold>
}
