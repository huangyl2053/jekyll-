...
import jp.co.ndensan.reams.db.dba.divcontroller.entity.parentdiv.DBA4010011.HihokenshaShokaiPanelSelectDiv;
...
public class HihokenshaShokaiPanelSelect {
     ...
     * 選択した個人の情報を元に、照会画面で参照できる情報を設定・表示します。もし表示できる情報が存在しない場合、 対象の情報を参照できないように状態遷移ボタンを非活性にします。
     *
...
//        dgSeikatsuHogoRireki_Row row = new dgSeikatsuHogoRireki_Row(RString.EMPTY, new TextBoxFlexibleDate(),
...
//        row.setTaishoJoho(generator.getAsRString("対象情報"));
    ...
    /**
     * 被保険者の各情報を表示するパネルにデータが入っているかどうかをチェックして、 参照できるデータが無いパネルを見れないよう、遷移ボタンを非活性にします。
     *
     * @param selectDiv 照会情報選択Div
     */
