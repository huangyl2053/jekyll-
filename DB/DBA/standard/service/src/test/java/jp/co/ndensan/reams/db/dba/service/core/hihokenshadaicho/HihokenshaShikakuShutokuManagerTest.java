...
public class HihokenshaShikakuShutokuManagerTest extends DbaTestDacBase {
    ...
    public static class getHihokenshaShutokuJyohoのテスト extends DbaTestDacBase {
        ...
        public void テーブルにレコードが存在しない場合_被保険者台帳管理情報取得処理は_NULLを返すこと() {
            HihokenshaShikakuShutokuMapperParameter parameter = HihokenshaShikakuShutokuMapperParameter.createHokenjaNoParam(HihokenshaNo.EMPTY, ShikibetsuCode.EMPTY);
            List<HihokenshaShutokuJyoho> entitylist = manager.getHihokenshaShutokuJyoho(parameter);
            assertThat(entitylist.size(), is(0));
        }

        @Test
        public void テーブルにレコードが存在する場合_被保険者台帳管理情報取得処理は_1件を返すこと() {
            insert_DbT1001HihokenshaDaicho(被保険者番号1, 異動日1, 枝番1, 資格取得年月日, 資格喪失年月日);
            HihokenshaShikakuShutokuMapperParameter parameter = HihokenshaShikakuShutokuMapperParameter.createHokenjaNoParam(被保険者番号1, ShikibetsuCode.EMPTY);
            List<HihokenshaShutokuJyoho> entitylist = manager.getHihokenshaShutokuJyoho(parameter);
            assertThat(entitylist.size(), is(1));
        }

        @Test
        public void テーブルにレコードが存在する場合_被保険者台帳管理情報取得処理は_2件を返すこと() {
            insert_DbT1001HihokenshaDaicho(被保険者番号1, 異動日1, 枝番1, 資格取得年月日, 資格喪失年月日);
...
//            assertThat(manager.saveHihokenshaShutoku(entity, dateOfBirth), is(notNullValue()));
//        }
//    }
...
//            HihokenshaShikakuShutokuMapperParameter parameter = HihokenshaShikakuShutokuMapperParameter.createHokenjaNoParam(被保険者番号1, 異動日1);
...
//    }
    ...
    public static final IDateOfBirth dateOfBirth = new IDateOfBirth() {
        ...
        public FlexibleDate toFlexibleDate() {
            return new FlexibleDate("19451211");
        }
