 ...
 */
package jp.co.ndensan.reams.db.dba.service.core.itakuservicekeikalusakuseiiraitodokedesho;

import java.util.ArrayList;
import java.util.List;
import jp.co.ndensan.reams.db.dba.business.core.tokuteifutangendogakushinseisho.HihokenshaKihonBusiness;
import jp.co.ndensan.reams.db.dba.business.report.itakuservicekeikalusakuseiiraitodokedesho.ItakuServiceKeikaluSakuseiIraiTodokedeshoItem;
import jp.co.ndensan.reams.db.dba.business.report.itakuservicekeikalusakuseiiraitodokedesho.ItakuServiceKeikaluSakuseiIraiTodokedeshoProerty;
import jp.co.ndensan.reams.db.dba.business.report.itakuservicekeikalusakuseiiraitodokedesho.ItakuServiceKeikaluSakuseiIraiTodokedeshoReport;
...
public class KyotakuServiceKeikakuSakuseiIraiTodokedesho {
    ...
    /**
     * 居宅（介護予防）サービス計画作成依頼（変更）届出書Printします。
     *
     * @param 識別コード 識別コード
     * @param 被保険者番号 被保険者番号
     * @return 居宅（介護予防）サービス計画作成依頼（変更）届出書_帳票作成
     */
    public SourceDataCollection createKyotakuServiceKeikakuSakuseiIraiTodokedesho(
            ShikibetsuCode 識別コード, HihokenshaNo 被保険者番号) {
        ItakuServiceKeikaluSakuseiIraiTodokedeshoProerty proerty = new ItakuServiceKeikaluSakuseiIraiTodokedeshoProerty();
        try (ReportManager reportManager = new ReportManager()) {
            try (ReportAssembler<ItakuServiceKeikaluSakuseiIraiTodokedeshoReportSource> assembler = createAssembler(proerty, reportManager)) {
                ...
                for (ItakuServiceKeikaluSakuseiIraiTodokedeshoReport report : toReports(get被保険者基本情報(識別コード, 被保険者番号),
                        ninshoshaSourceBuilder.buildSource().ninshoshaYakushokuMei)) {
                    ReportSourceWriter<ItakuServiceKeikaluSakuseiIraiTodokedeshoReportSource> reportSourceWriter
    ...
    private static List<ItakuServiceKeikaluSakuseiIraiTodokedeshoReport> toReports(
            HihokenshaKihonBusiness entity, RString ninshoshaYakushokuMei) {
        List<ItakuServiceKeikaluSakuseiIraiTodokedeshoReport> list = new ArrayList<>();
        ...
        ItakuServiceKeikaluSakuseiIraiTodokedeshoItem item
                = new ItakuServiceKeikaluSakuseiIraiTodokedeshoItem(
        ...
        list.add(ItakuServiceKeikaluSakuseiIraiTodokedeshoReport.createReport(item));
        return list;
    }
    ...
    private static RString get帳票文言(int 項目番号) {
        ...
        TsuchishoTeikeibunInfo tsuchishoTeikeibunInfo = tsuchisho.get通知書定形文検索(
                ...
                new ReportId("DBC800004_ItakuServiceKeikaluSakuseiIraiTodokedesho"),
                KamokuCode.EMPTY,
                1,
                項目番号,
    ...
    private static RString set生年月日(FlexibleDate 生年月日, RString 生年月日不詳区分) {
        RString 外国人表示制御_生年月日表示方法 = DbBusinessConifg.get(ConfigNameDBU.外国人表示制御_生年月日表示方法, RDate.getNowDate(),
