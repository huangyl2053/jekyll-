...
public class TashichosonJushochitokureishaRenrakuhyoFinder {
    ...
    public TatokuKanrenChohyoSofusaki getDefaultValue(RString 市町村コード, ReportId 遷移元メニューID) {
        ...
        entity.set郵便番号(finder.get市町村宛先(市町村コード == null ? LasdecCode.EMPTY : new LasdecCode(市町村コード),
                SofusakiGroup.住基関連.getCode()).get郵便番号());
        entity.set住所(finder.get市町村宛先(市町村コード == null ? LasdecCode.EMPTY : new LasdecCode(市町村コード),
                SofusakiGroup.住基関連.getCode()).get住所());
        entity.set役所名(finder.get市町村宛先(市町村コード == null ? LasdecCode.EMPTY : new LasdecCode(市町村コード),
