...
import jp.co.ndensan.reams.db.dbx.definition.core.codeshubetsu.code.TanisuSanteiTani;
...
/**
 * 単位数算定単位を表すドメインのテストクラスです。
 *
 * @author LDNS 梁百川
 */
@RunWith(Enclosed.class)
public class TanisuSanteiTaniTest extends DbxTestBase {

    private static TanisuSanteiTani sut;

    @BeforeClass
    public static void setUp() {
        sut = new TanisuSanteiTani(new RString("10"));
    }
    ...
    public static class equalsテスト extends DbxTestBase {
        ...
        @Test
        public void LDNS_Objectがnull以外かつTanisuSanteiTaniではないの場合_equalsは_falseを戻す() {
        ...
        public void LDNS_Objectのcodeとコンストラクタのcodeが不一致の場合_equalsは_falseを戻す() {
            TanisuSanteiTani sut2 = new TanisuSanteiTani(new RString("90"));
            assertFalse(sut.equals(sut2));
        }

        @Test
        public void LDNS_Object値とコンストラクタ値が一致の場合_equalsは_trueを戻す() {
            assertTrue(sut.equals(new TanisuSanteiTani(new RString("10"))));
        }
    ...
    public static class toStringテスト extends DbxTestBase {
        ...
        public void LDNS_codeがnullの場合_toStringは_emptyを戻す() {
            TanisuSanteiTani sut = new TanisuSanteiTani((Code) null);
            assertThat(sut.toString(), is(RString.EMPTY.toString()));
        }

        @Test
        public void LDNS_codeが1234567の場合_toStringは_1234567を戻す() {
            TanisuSanteiTani sut = new TanisuSanteiTani(new Code("1234567"));
    ...
    public static class hashCodeテスト extends DbxTestBase {
        ...
        public void LDNS_設定した対象とコンストラクタ同じの場合_hashCodeは_同じhashCodeを戻す() {
            TanisuSanteiTani sut2 = new TanisuSanteiTani(new RString("10"));
    ...
    public static class getMeishoテスト extends DbxTestBase {
        ...
        @Test
        public void LDNS_codeがnull以外の場合_getMeishoは_単位数算定単位を戻す() {
            TanisuSanteiTani sut = new TanisuSanteiTani(new Code("1234567"));
            assertThat(sut.getMeisho(), is(new RString("単位数算定単位")));
        }
    }
    ...
    public static class getMeishoパラメタあるテスト extends DbxTestBase {
        ...
        @Test
        public void LDNS_入力のパラメタがnull以外の場合_getMeishoは_単位数算定単位を戻す() {
            TanisuSanteiTani sut = new TanisuSanteiTani(new Code("1234567"));
            assertThat(sut.getMeisho(new FlexibleDate("20150303")), is(new RString("単位数算定単位")));
        }
    }
    ...
    public static class getRyakushoテスト extends DbxTestBase {
        ...
        @Test
        public void LDNS_codeがnull以外の場合_getRyakushoは_単位数算定単位を戻す() {
            TanisuSanteiTani sut = new TanisuSanteiTani(new Code("1234567"));
            assertThat(sut.getRyakusho(), is(new RString("単位数算定単位")));
        }
    }
    ...
    public static class getRyakushoパラメタあるテスト extends DbxTestBase {
        ...
        @Test
        public void LDNS_入力のパラメタがnull以外の場合_getRyakushoは_単位数算定単位を戻す() {
            TanisuSanteiTani sut = new TanisuSanteiTani(new Code("1234567"));
            assertThat(sut.getRyakusho(new FlexibleDate("20150303")), is(new RString("単位数算定単位")));
        }
    }
}
