...
public class ShichosonSecurityJohoFinder {
    ...
    /**
     * 業務分類で{@link ShichosonSecurityJoho}を取得します。
     *
    ...
    public ShichosonSecurityJoho getShichosonSecurityJoho(GyomuBunrui gyomuBunrui) {
        ...
        if (list.isEmpty()) {
            return ShichosonSecurityJoho.EMPTY;
        }
        KaigoDonyuKeitai 介護導入形態 = list.get(0);
        return workFlowStep(dispatchFlowByKaigoDonyuKeitai(介護導入形態), 介護導入形態);
    }

    private int dispatchFlowByKaigoDonyuKeitai(KaigoDonyuKeitai 導入形態) {
        RString 業務分類 = 導入形態.get業務分類().code();
        RString 導入形態コード = 導入形態.get導入形態コード().getCode();
    ...
    private ShichosonSecurityJoho workFlowStep(int workFlow, KaigoDonyuKeitai 導入形態) {
        switch (workFlow) {
