...
public class JukyuShikakuShomeishoFinder {
    ...
    public JukyuShikakuShomeishoDataEntity setJukyuShikakuShomeisho(JukyuShikakuShomeishoKaiKoEntity inEntity) {
        ...
        outEntity.set被保険者異動予定日(inEntity.get被保険者異動予定日());
    ...
    private void edit被保険者(JukyuShikakuShomeishoDataEntity outEntity, JukyuShikakuShomeishoKaiKoEntity inEntity) {
        ...
        RString 明治 = new RString("* *");
        RString 大正 = new RString("* *");
        RString 昭和 = new RString("* *");

        ShikibetsuTaishoSearchKeyBuilder key = new ShikibetsuTaishoSearchKeyBuilder(ShikibetsuTaishoGyomuHanteiKeyFactory.createInstance(GyomuCode.DB介護保険, KensakuYusenKubun.住登外優先));
        ...
        if (生年月日 != null) {
            ...
            RString 生年月日西暦 = 生年月日.seireki().separator(Separator.JAPANESE)
                    .fillType(FillType.BLANK).toDateString();

            if (new RString("明治").equals(年号)) {
                明治 = new RString("明治");
            } else if (new RString("大正").equals(年号)) {
                大正 = new RString("大正");
            } else if (new RString("昭和").equals(年号)) {
                昭和 = new RString("昭和");
            }

            if (住民種別コード1.equals(宛名識別対象PSM.getJuminShubetsuCode())
                    || 住民種別コード3.equals(宛名識別対象PSM.getJuminShubetsuCode())) {
                outEntity.set被保険者生年月日(明治.concat(大正).concat(昭和).concat(日付));
            }
            ...
            if (住民種別コード2.equals(宛名識別対象PSM.getJuminShubetsuCode())
                ...
                if (GaikokujinSeinengappiHyojihoho.西暦表示.getコード()
                    ...
                    outEntity.set被保険者生年月日(明治.concat(大正).concat(昭和).concat(生年月日西暦));
        ...
        if (!inEntity.is住所を印字()) {
            ...
            if (new RString("1").equals(宛名識別対象PSM.getJuminJotaiCode())
                ...
                if (宛名識別対象_方書 != null) {
                    方書 = new RString(宛名識別対象_方書.toString());
                }
                転出前の住所部分 = 住所.concat(番地).concat("　").concat(方書);
            }
            ...
            if (new RString("3").equals(宛名識別対象PSM.getJuminJotaiCode())) {
                ...
                if (宛名識別対象_住登内方書 != null) {
                    住登内方書 = new RString(宛名識別対象_住登内方書.toString());
                }
                転出前の住所部分 = 住登内住所.concat(住登内番地).concat("　").concat(住登内方書);

                if ((null == 宛名識別対象PSM.getTenshutsuKakuteiJusho() || AtenaJusho.EMPTY.equals(宛名識別対象PSM.getTenshutsuKakuteiJusho()))
                        && (null == 宛名識別対象PSM.getTenshutsuKakuteiBanchi() || AtenaBanchi.EMPTY.equals(宛名識別対象PSM.getTenshutsuKakuteiBanchi()))) {

                    if ((null == 宛名識別対象PSM.getTenshutsuYoteiJusho() || AtenaJusho.EMPTY.equals(宛名識別対象PSM.getTenshutsuYoteiJusho()))
                            && (null == 宛名識別対象PSM.getTenshutsuYoteiBanchi() || AtenaBanchi.EMPTY.equals(宛名識別対象PSM.getTenshutsuYoteiBanchi()))) {
                        ...
                        if (宛名識別対象_転出予定方書 != null) {
                            転出予定方書 = new RString(宛名識別対象_転出予定方書.toString());
                        }
                        転出後の住所部分 = 転出予定住所.concat(転出予定番地).concat("　").concat(転出予定方書);
                    }
                } else {
                    ...
                    if (宛名識別対象_転出確定方書 != null) {
                        転出確定方書 = new RString(宛名識別対象_転出確定方書.toString());
                    }
                    転出後の住所部分 = 転出確定住所.concat(転出確定番地).concat("　").concat(転出確定方書);
                }
            }
            outEntity.set被保険者住所_転出先予定(転出前の住所部分.concat(転出後の住所部分));
        } else {
    ...
    private void edit保険者番号(JukyuShikakuShomeishoDataEntity outEntity, HihokenshaNo 被保険者番号) {
        ...
        DbT1001HihokenshaDaichoEntity dbT1001HihokenshaDaichoEntity = mapper.get被保険者台帳管理情報(被保険者番号);
        ...
        if (new Code("112").equals(導入形態コード) || new Code("120").equals(導入形態コード)) {
            ...
            LasdecCode 市町村コード = dbT1001HihokenshaDaichoEntity.getKoikinaiTokureiSochimotoShichosonCode();
            if (市町村コード != null) {
                SearchResult<ShichosonCodeYoriShichoson> shichosonCodeYoriShichosonSearchResult
                        = koikiShichosonJohoFinder.shichosonCodeYoriShichosonJoho(市町村コード);
                outEntity.set保険者番号(new RString(shichosonCodeYoriShichosonSearchResult.records().get(0).get証記載保険者番号().toString()));
            } else {
                SearchResult<ShichosonCodeYoriShichoson> shichosonCodeYoriShichosonSearchResult
                        = koikiShichosonJohoFinder.shichosonCodeYoriShichosonJoho(dbT1001HihokenshaDaichoEntity.getShichosonCode());
                outEntity.set保険者番号(new RString(shichosonCodeYoriShichosonSearchResult.records().get(0).get証記載保険者番号().toString()));
            }
        }
        if (outEntity.get保険者番号().isEmpty()) {
            // TODO QA #75375 メッセージがない
            throw new ApplicationException(DbzErrorMessages.出力項目を指定.getMessage());
        }
    }
    ...
    private void get申請状況情報(JukyuShikakuShomeishoDataEntity outEntity, JukyuShikakuShomeishoKaiKoEntity inEntity) {
        ...
        if (new RString("0").equals(dbT4001JukyushaDaichoEntity.getShinseiJokyoKubun())) {
            ...
            List<DbT5121ShinseiRirekiJohoEntity> dbT5121ShinseiRirekiJohoEntityList = dbT5121Dac.select前回申請管理番号(dbT4001JukyushaDaichoEntity.getShinseishoKanriNo());
            ...
            if (inEntity.get申請日().isEmpty()) {
                ...
                outEntity.set申請年月日(new RString(""));
