...
public class KaigoJyuminhyouKoikiunyoCSVDataSakuseiFinder {
    ...
    private void コード変換区分の判定12桁2(RString codeHenkanKubun,
        ...
        if (区分_1.equals(codeHenkanKubun)) {
            ...
            if ((!RString.isNullOrEmpty(entity.get受給者挿入日時())
                    && !RString.isNullOrEmpty(entity.get挿入日時()))
                    && new FlexibleDate(entity.get受給者挿入日時().toString()).isBeforeOrEquals(new FlexibleDate(entity.get挿入日時().toString()))) {
                junientity.set作成日時(entity.get挿入日時());
            }
            if ((!RString.isNullOrEmpty(entity.get更新日時())
                    && !RString.isNullOrEmpty(entity.get受給者挿入日時()))
                    && new FlexibleDate(entity.get更新日時().toString()).isBefore(new FlexibleDate(entity.get受給者挿入日時().toString()))) {
                junientity.set作成日時(entity.get受給者挿入日時());
            }
            if ((!RString.isNullOrEmpty(entity.get更新日時())
                    && !RString.isNullOrEmpty(entity.get受給者挿入日時()))
                    && new FlexibleDate(entity.get受給者挿入日時().toString()).isBeforeOrEquals(new FlexibleDate(entity.get更新日時().toString()))) {
    ...
    private void コード変換区分の判定15桁2(RString codeHenkanKubun,
        ...
        if (区分_1.equals(codeHenkanKubun)) {
            ...
            if ((!RString.isNullOrEmpty(entity.get受給者挿入日時())
                    && !RString.isNullOrEmpty(entity.get挿入日時()))
                    && new FlexibleDate(entity.get受給者挿入日時()).isBeforeOrEquals(new FlexibleDate(entity.get挿入日時()))) {
                junientity.set作成日時(entity.get挿入日時());
            }
            if ((!RString.isNullOrEmpty(entity.get更新日時())
                    && !RString.isNullOrEmpty(entity.get受給者挿入日時()))
                    && new FlexibleDate(entity.get更新日時()).isBefore(new FlexibleDate(entity.get更新日時()))) {
                junientity.set作成日時(entity.get受給者挿入日時());
            }
            if ((!RString.isNullOrEmpty(entity.get更新日時())
                    && !RString.isNullOrEmpty(entity.get受給者挿入日時()))
                    && !new FlexibleDate(entity.get受給者挿入日時()).isBeforeOrEquals(new FlexibleDate(entity.get更新日時()))) {
                junientity.set更新日時(entity.get更新日時());
            }
            if ((!RString.isNullOrEmpty(entity.get更新日時())
                    && !RString.isNullOrEmpty(entity.get受給者挿入日時()))
                    && new FlexibleDate(entity.get更新日時()).isBefore(new FlexibleDate(entity.get受給者挿入日時()))) {
                junientity.set更新日時(entity.get受給者更新日時());
            }
        }
    }
