...
import jp.co.ndensan.reams.db.dbu.business.report.jigyohokokugeppoyoshiki2120161.JigyohokokuGeppoYoshiki2120161Report;
...
import jp.co.ndensan.reams.db.dbu.entity.report.jigyohokokugeppoyoshiki2120161.JigyohokokuGeppoYoshiki2120161ReportSource.ReportSourceFields;
...
/**
 * 事業報告月報様式_03帳票出力のバッチ処理クラスです。
 *
 * @reamsid_L DBU-5570-030 xuyannan
 */
public class JigyoHokokuShiryouReportDBU300021Process extends BatchProcessBase<JigyoHokokuShiryouRelateEntity> {
    ...
    @BatchWriter
    private BatchReportWriter<JigyohokokuGeppoYoshiki2120161ReportSource> 様式_03batchWrite;
    private ReportSourceWriter<JigyohokokuGeppoYoshiki2120161ReportSource> 様式_03reportSourceWriter;
    ...
    protected void createWriter() {
        様式_03batchWrite = BatchReportFactory.createBatchReportWriter(ReportIdDBU.DBU300021.getReportId().value())
                .addBreak(new BreakerCatalog<JigyohokokuGeppoYoshiki2120161ReportSource>().simplePageBreaker(page_break_keys)).create();
        様式_03reportSourceWriter = new ReportSourceWriter(様式_03batchWrite);
    }
    ...
    private void print帳票(RString 市町村コード) {
        for (JigyoHokokuShiryouRelateEntity entity : list) {
            if (市町村コード.equals(entity.getShichosonCode())
                    && (ShukeiNoyoshiki2._1_介護給付_予防給付2二割負担被保険者分_再掲_件数.getコード().equals(entity.getSyukeiNo())
                    || ShukeiNoyoshiki2._1_介護給付_予防給付2二割負担被保険者分_再掲_費用額.getコード().equals(entity.getSyukeiNo())
                    || ShukeiNoyoshiki2._1_介護給付_予防給付2二割負担被保険者分_再掲_給付費.getコード().equals(entity.getSyukeiNo()))) {
                business.set保険者コード(市町村コード);
                JigyohokokuGeppoYoshiki2120161Report 様式_03report = new JigyohokokuGeppoYoshiki2120161Report(business);
                様式_03report.writeBy(様式_03reportSourceWriter);
            }
        }
    }
}
