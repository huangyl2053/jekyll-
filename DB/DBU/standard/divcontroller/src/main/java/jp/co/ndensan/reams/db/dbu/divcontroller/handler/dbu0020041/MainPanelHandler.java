...
public final class MainPanelHandler {
     ...
     * @return 更新前データリスト
     */
    public List<JigyoHokokuTokeiData> get更新前データリスト(RString 引き継ぎデータEntity) {
     ...
     */
    public List<JigyoHokokuTokeiData> get修正データ(RString 様式種類) {
        List<JigyoHokokuTokeiData> 更新前データリスト = get更新前データリスト(様式種類);
        ...
        for (int i = 0; i < 更新前データリスト.size(); i++) {
            if (!更新前データリスト.get(i).get集計結果値().equals(画面データリスト.get(i).get集計結果値())) {
    ...
    private JigyoHokokuTokeiData get画面データ(
        ...
        JigyoHokokuTokeiData 画面データ = new JigyoHokokuTokeiData(更新前データ.get報告年(),
                ...
                縦番号,
                横番号);
        画面データ.toEntity().setShukeiKekkaAtai(件数);
        return 画面データ;
    }
    ...
    private void set第三段階認定件数当該月末現在リスト(List<Decimal> 認定件数当該月末現在リスト) {
        認定件数当該月末現在リスト.add(div.getShokuhikyojunofutannintei().getTablePanel4().
     ...
     */
    public boolean is第4段階データ不整合(Decimal 第1号被保険者, Decimal 第2号被保険者, Decimal 合計) {
