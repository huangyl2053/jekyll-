...
public class NinteichosaResultMock {
    ...
    public static NinteichosaResultOfGaikyo getSpiedNinteichosaResultGaikyoInstance() {
        ...
        result.set調査項目(create調査結果(調査票, サービス区分コード, ServiceKubun.介護.getValue()));
    ...
    public static NinteichosaResultOfKihon getSpiedNinteichosaResultKihonInstance() {
        ...
        result.set調査項目(create調査結果(調査票, 麻痺等の有無_左上肢, NaiAru.ない.getValue()));
        result.set調査項目(create調査結果(調査票, 麻痺等の有無_右上肢, NaiAru.ある.getValue()));
        result.set調査項目(create調査結果(調査票, 麻痺等の有無_左下肢, NaiAru.ない.getValue()));
        result.set調査項目(create調査結果(調査票, 麻痺等の有無_右下肢, NaiAru.ある.getValue()));
        result.set調査項目(create調査結果(調査票, 麻痺等の有無_その他, NaiAru.ない.getValue()));
        result.set調査項目(create調査結果(調査票, 関節の動く範囲の制限_肩関節, NaiAru.ない.getValue()));
        result.set調査項目(create調査結果(調査票, 関節の動く範囲の制限_股関節, NaiAru.ある.getValue()));
        result.set調査項目(create調査結果(調査票, 関節の動く範囲の制限_膝関節, NaiAru.ない.getValue()));
        result.set調査項目(create調査結果(調査票, 関節の動く範囲の制限_その他, NaiAru.ある.getValue()));
        result.set調査項目(create調査結果(調査票, 寝返り, DekiruDekinai3.できる.getValue()));
        result.set調査項目(create調査結果(調査票, 起き上がり, DekiruDekinai3.できない.getValue()));
        result.set調査項目(create調査結果(調査票, 座位保持, DekiruDekinai6.できる.getValue()));
        result.set調査項目(create調査結果(調査票, 両足での立位, DekiruDekinai5.できない.getValue()));
        result.set調査項目(create調査結果(調査票, 歩行, DekiruDekinai3.できる.getValue()));
        result.set調査項目(create調査結果(調査票, 立ち上がり, DekiruDekinai3.できない.getValue()));
        result.set調査項目(create調査結果(調査票, 片足での立位, DekiruDekinai5.できる.getValue()));
        result.set調査項目(create調査結果(調査票, 洗身, Kaijo2.自立.getValue()));
        result.set調査項目(create調査結果(調査票, つめ切り, Kaijo.全介助.getValue()));
        result.set調査項目(create調査結果(調査票, 視力, Shiryoku.普通.getValue()));
        result.set調査項目(create調査結果(調査票, 聴力, Choryoku.判断不能.getValue()));
        result.set調査項目(create調査結果(調査票, 移乗, Kaijo3.自立.getValue()));
        result.set調査項目(create調査結果(調査票, 移動, Kaijo3.全介助.getValue()));
        result.set調査項目(create調査結果(調査票, 嚥下, DekiruDekinai4.できる.getValue()));
        result.set調査項目(create調査結果(調査票, 食事摂取, Kaijo3.自立.getValue()));
        result.set調査項目(create調査結果(調査票, 排尿, Kaijo3.全介助.getValue()));
        result.set調査項目(create調査結果(調査票, 排便, Kaijo3.自立.getValue()));
        result.set調査項目(create調査結果(調査票, 口腔清潔, Kaijo.全介助.getValue()));
        result.set調査項目(create調査結果(調査票, 洗顔, Kaijo.自立.getValue()));
        result.set調査項目(create調査結果(調査票, 整髪, Kaijo.全介助.getValue()));
        result.set調査項目(create調査結果(調査票, 上衣の着脱, Kaijo3.自立.getValue()));
        result.set調査項目(create調査結果(調査票, ズボン等の着脱, Kaijo3.全介助.getValue()));
        result.set調査項目(create調査結果(調査票, 外出頻度, Gaishutsu.月１回以上.getValue()));
        result.set調査項目(create調査結果(調査票, 意思の伝達, DekiruDekinai2.できる.getValue()));
        result.set調査項目(create調査結果(調査票, 毎日の日課を理解, DekiruDekinai.できない.getValue()));
        result.set調査項目(create調査結果(調査票, 生年月日をいう, DekiruDekinai.できる.getValue()));
        result.set調査項目(create調査結果(調査票, 短期記憶, DekiruDekinai.できない.getValue()));
        result.set調査項目(create調査結果(調査票, 自分の名前をいう, DekiruDekinai.できる.getValue()));
        result.set調査項目(create調査結果(調査票, 今の季節を理解, DekiruDekinai.できない.getValue()));
        result.set調査項目(create調査結果(調査票, 場所の理解, DekiruDekinai.できる.getValue()));
        result.set調査項目(create調査結果(調査票, 常時の徘徊, NaiAru2.ない.getValue()));
        result.set調査項目(create調査結果(調査票, 外出して戻れない, NaiAru2.ある.getValue()));
        result.set調査項目(create調査結果(調査票, 被害的, NaiAru2.ない.getValue()));
        result.set調査項目(create調査結果(調査票, 作話, NaiAru2.ある.getValue()));
        result.set調査項目(create調査結果(調査票, 感情が不安定, NaiAru2.ない.getValue()));
        result.set調査項目(create調査結果(調査票, 昼夜逆転, NaiAru2.ある.getValue()));
        result.set調査項目(create調査結果(調査票, 同じ話をする, NaiAru2.ない.getValue()));
        result.set調査項目(create調査結果(調査票, 大声を出す, NaiAru2.ある.getValue()));
        result.set調査項目(create調査結果(調査票, 介護に抵抗, NaiAru2.ない.getValue()));
        result.set調査項目(create調査結果(調査票, 落ち着きなし, NaiAru2.ある.getValue()));
        result.set調査項目(create調査結果(調査票, 一人で出たがる, NaiAru2.ない.getValue()));
        result.set調査項目(create調査結果(調査票, 収集癖, NaiAru2.ある.getValue()));
        result.set調査項目(create調査結果(調査票, 物や衣類を壊す, NaiAru2.ない.getValue()));
        result.set調査項目(create調査結果(調査票, ひどい物忘れ, NaiAru2.ある.getValue()));
        result.set調査項目(create調査結果(調査票, 独り言_独り笑, NaiAru2.ない.getValue()));
        result.set調査項目(create調査結果(調査票, 自分勝手に行動する, NaiAru2.ある.getValue()));
        result.set調査項目(create調査結果(調査票, 話がまとまらない, NaiAru2.ない.getValue()));
        result.set調査項目(create調査結果(調査票, 薬の内服, Kaijo.自立.getValue()));
        result.set調査項目(create調査結果(調査票, 金銭の管理, Kaijo.全介助.getValue()));
        result.set調査項目(create調査結果(調査票, 日常の意思決定, DekiruDekinai7.できる.getValue()));
        result.set調査項目(create調査結果(調査票, 集団への不適応, NaiAru2.ない.getValue()));
        result.set調査項目(create調査結果(調査票, 買い物, Kaijo3.自立.getValue()));
        result.set調査項目(create調査結果(調査票, 簡単な調理, Kaijo3.全介助.getValue()));
        result.set調査項目(create調査結果(調査票, 点滴の管理, NaiAru.ない.getValue()));
        result.set調査項目(create調査結果(調査票, 中心静脈栄養, NaiAru.ある.getValue()));
        result.set調査項目(create調査結果(調査票, 透析, NaiAru.ない.getValue()));
        result.set調査項目(create調査結果(調査票, ストーマの処置, NaiAru.ある.getValue()));
        result.set調査項目(create調査結果(調査票, 酸素療法, NaiAru.ない.getValue()));
        result.set調査項目(create調査結果(調査票, レスピレーター, NaiAru.ある.getValue()));
        result.set調査項目(create調査結果(調査票, 気管切開, NaiAru.ない.getValue()));
        result.set調査項目(create調査結果(調査票, 疼痛の看護, NaiAru.ある.getValue()));
        result.set調査項目(create調査結果(調査票, 経管栄養, NaiAru.ない.getValue()));
        result.set調査項目(create調査結果(調査票, モニター測定, NaiAru.ある.getValue()));
        result.set調査項目(create調査結果(調査票, じょくそうの処置, NaiAru.ない.getValue()));
        result.set調査項目(create調査結果(調査票, カテーテル, NaiAru.ある.getValue()));
        result.set調査項目(create調査結果(調査票, 障害高齢者の日常生活自立度, ShogaiJiritsu.Ａ１.getValue()));
        result.set調査項目(create調査結果(調査票, 認知症高齢者の日常生活自立度, NinchishoJiritsu.Ⅱａ.getValue()));
