...
public class KaigoIryoKikanFinderTest extends TestBase {
    ...
    private static IKaigoIryoKikanDac createDac() {
        ...
        when(dac.select(any(LasdecCode.class), any(KaigoIryoKikanCode.class))).thenReturn(entity);
        when(dac.select(any(LasdecCode.class), any(KaigoIryoKikanCode.class), any(IryoKikanJokyo.class))).thenReturn(entity);

        List<KaigoIryoKikanEntity> 介護医療機関リスト_2件 = create介護医療機関EntityList(2);
        when(dac.select(any(LasdecCode.class))).thenReturn(介護医療機関リスト_2件);

        List<KaigoIryoKikanEntity> 介護医療機関リスト_3件 = create介護医療機関EntityList(3);
        when(dac.select(any(LasdecCode.class), any(IryoKikanJokyo.class))).thenReturn(介護医療機関リスト_3件);
        return dac;
    }
