...
import jp.co.ndensan.reams.db.dbe.business.mapper.ShinsakaiMapper;
...
public class ShinsakaiManager {
    ...
    ShinsakaiManager(ShinsakaiWariateIinJohoDac 割当委員情報Dac, ShinsakaiDetailFinder 審査会情報Finder,
            ShinsakaiIinManager 審査会委員Manager, ShinsakaiAndShinskaiWariateIinDac 審査会割当委員relateDac) {
    ...
    /**
     * 審査会開催番号と審査会開催年月日を指定し、審査会を取得します。
     *
     * @param 審査会開催番号 審査会開催番号
     * @param 審査会開催年月日 審査会開催年月日
     * @return 審査会
     */
    public Shinsakai get審査会(ShinsakaiKaisaiNo 審査会開催番号, FlexibleDate 審査会開催年月日) {
        ShinsakaiDetail 審査会情報 = 審査会情報Finder.get審査会情報(審査会開催番号, 審査会開催年月日);
        return create審査会(審査会情報);
    }

    /**
     * 審査会開催年月日を指定して、その日に開催される審査会のListを取得します。
     *
     * @param 審査会開催年月日 審査会開催年月日
     * @return 審査会List
     */
    public ShinsakaiList get開催審査会List(FlexibleDate 審査会開催年月日) {
        List<Shinsakai> 審査会List = new ArrayList<>();

        List<ShinsakaiDetail> 審査会情報List = 審査会情報Finder.get開催審査会情報(審査会開催年月日);
    ...
    /**
     * 基準年月日を指定して、その日付を含めて、それ以降に開催される審査会をListで取得します。
     *
     * @param 基準年月日 基準年月日
     * @return 審査会List
     */
    ...
    private Shinsakai create審査会(ShinsakaiDetail 審査会情報) {
        ...
        ShinsakaiIinList 委員List = 審査会委員Manager.get審査会委員List(get委員コードList(割当委員EntityList));
    ...
    public boolean save(Shinsakai 審査会) {
        ...
        List<DbT5106ShinsakaiWariateIinJohoEntity> 割当委員情報EntityList =
                ShinsakaiWariateIinMapper.to審査会割当委員EntityList(審査会.get審査会割当委員List());
