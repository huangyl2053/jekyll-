...
public class ShujiiIkenshoIraiJohoBuilder {
     ...
     * @param shujiiIkenshoJoho {@link Models}
     *
     */
    ...
    /**
     * 要介護認定主治医意見書情報のキー情報について判定します。<br>
     * 主治医意見書作成依頼情報に関連できる要介護認定主治医意見書情報である場合、下記の処理に遷移します。<br>
     * キーが一致する場合は要介護認定主治医意見書情報リストに要介護認定主治医意見書情報{@link ShujiiIkenshoJoho}をセットします。<br>
     ...
     */
    public ShujiiIkenshoIraiJohoBuilder setShujiiIkenshoJoho(ShujiiIkenshoJoho 要介護認定主治医意見書情報) {
        if (hasSameIdentifier(要介護認定主治医意見書情報.identifier())) {
            shujiiIkenshoJoho.add(要介護認定主治医意見書情報);
            return this;
        }
        throw new IllegalArgumentException(UrErrorMessages.不正.toString());
    }

    private boolean hasSameIdentifier(ShujiiIkenshoJohoIdentifier 要介護認定主治医意見書情報識別子) {
        return (id.get申請書管理番号().equals(要介護認定主治医意見書情報識別子.get申請書管理番号())
                && id.get主治医意見書作成依頼履歴番号() == 要介護認定主治医意見書情報識別子.get主治医意見書作成依頼履歴番号());
    }
    ...
    public ShujiiIkenshoIraiJoho build() {
        return new ShujiiIkenshoIraiJoho(entity, id, shujiiIkenshoJoho);
    }
}
