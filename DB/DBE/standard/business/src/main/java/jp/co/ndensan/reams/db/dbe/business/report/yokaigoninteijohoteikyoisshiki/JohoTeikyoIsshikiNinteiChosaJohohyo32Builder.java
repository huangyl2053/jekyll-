...
/**
 * 要介護認定情報提供一式認定調査票情報32Builderクラスです。
 *
 * @author N3212 竹内 和紀
 */
public class JohoTeikyoIsshikiNinteiChosaJohohyo32Builder implements IJohoTeikyoIsshikiBuilder {
     ...
     * @param index Index
     */
    public JohoTeikyoIsshikiNinteiChosaJohohyo32Builder(NinteiChosaJohohyoEntity entity, int index) {
        this.entity = entity;
        this.index = index;
    }
    ...
    public YokaigoNinteiJohoTeikyoIsshikiReportSource build() {
        ...
        if (index < entity.get身の回りリスト_32().size()) {
            source.listGogun_1 = entity.get身の回りリスト_32().get(index);
        }
        if (index < entity.get麻痺拘縮1_リスト_32().size()) {
            source.listIchigun1_1 = entity.get麻痺拘縮1_リスト_32().get(index);
        }
        if (index < entity.get麻痺拘縮2_リスト_32().size()) {
            source.listIchigun2_1 = entity.get麻痺拘縮2_リスト_32().get(index);
        }
        if (index < entity.get特別な医療リスト_32().size()) {
            source.listIryo_1 = entity.get特別な医療リスト_32().get(index);
        }
        if (index < entity.get活動参加リスト_32().size()) {
            source.listJugun_1 = entity.get活動参加リスト_32().get(index);
        }
        if (index < entity.get移動リスト_32().size()) {
            source.listNigun_1 = entity.get移動リスト_32().get(index);
        }
        if (index < entity.get意思疎通リスト_32().size()) {
            source.listRokugun_1 = entity.get意思疎通リスト_32().get(index);
        }
        if (index < entity.get問題行動リスト_32().size()) {
            source.listRokugun1_1 = entity.get問題行動リスト_32().get(index);
        }
        if (index < entity.getサービス区分リスト_32().size()) {
            source.listSabisujokyo_1 = entity.getサービス区分リスト_32().get(index);
        }
        if (index < entity.get複雑動作リスト_32().size()) {
            source.listSangun_1 = entity.get複雑動作リスト_32().get(index);
        }
        if (index < entity.get特別介護リスト_32().size()) {
            source.listYongun_1 = entity.get特別介護リスト_32().get(index);
        }
        source.layout = JohoTeikyoIsshiki._06ANinteiChosaJohohyo.getFormGroupIndex();
        return source;
    }
