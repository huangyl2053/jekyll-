...
//    private final Models<NinteiKekkaJohoIdentifier, NinteiKekkaJoho> ninteiKekkaJoho;
    ...
    NinteiShinseiJohoBuilder(
        ...
        this.id = id;
        this.ichiGojiHanteiKekkaJoho = ichiGojiHanteiKekkaJoho.clone();
        this.ichijiHanteiKekkaJoho = ichijiHanteiKekkaJoho.clone();
...
//        this.ninteiKekkaJoho = ninteiKekkaJoho.clone();
        ...
        this.shinseitodokedeJoho = shinseitodokedeJoho.clone();
        this.tennyuShibo = tennyuShibo.clone();
        this.tsuchishoHakkoJoho = tsuchishoHakkoJoho.clone();
    }
     ...
     */
    public NinteiShinseiJohoBuilder set申請書管理番号(ShinseishoKanriNo 申請書管理番号) {
    ...
    /**
     * 要介護認定1.5次判定結果情報のキー情報について判定します。<br>
     * 精神手帳情報に関連できる要介護認定1.5次判定結果情報である場合、下記の処理に遷移します。<br>
    ...
    public NinteiShinseiJohoBuilder setIchiGojiHanteiKekkaJoho(IchiGojiHanteiKekkaJoho 要介護認定1_5次判定結果情報) {
        if (hasSameIdentifier(要介護認定1_5次判定結果情報.identifier())) {
            ichiGojiHanteiKekkaJoho.add(要介護認定1_5次判定結果情報);
            return this;
        }
        throw new IllegalArgumentException(UrErrorMessages.不正.toString());
    }

    private boolean hasSameIdentifier(IchiGojiHanteiKekkaJohoIdentifier 要介護認定1_5次判定結果情報識別子) {
    ...
    private boolean hasSameIdentifier(IchijiHanteiKekkaJohoIdentifier 要介護認定一次判定結果情報識別子) {
    ...
    private boolean hasSameIdentifier(ImageIdentifier イメージ情報識別子) {
    ...
    private boolean hasSameIdentifier(NinteiKanryoJohoIdentifier 要介護認定完了情報識別子) {
    ...
    private boolean hasSameIdentifier(NinteiKeikakuJohoIdentifier 要介護認定計画情報識別子) {
...
//    }
    ...
    private boolean hasSameIdentifier(RenrakusakiJohoIdentifier 介護連絡先情報識別子) {
    ...
    private boolean hasSameIdentifier(ShinsakaiIinJogaiJohoIdentifier 審査会委員除外情報識別子) {
    ...
    private boolean hasSameIdentifier(ShinseiRirekiJohoIdentifier 申請履歴情報識別子) {
    ...
    private boolean hasSameIdentifier(ShinseitodokedeJohoIdentifier 申請届出情報識別子) {
    ...
    private boolean hasSameIdentifier(TennyuShiboIdentifier 転入_死亡情報識別子) {
    ...
    private boolean hasSameIdentifier(TsuchishoHakkoJohoIdentifier 通知書発行情報識別子) {
        return (id.get申請書管理番号().equals(通知書発行情報識別子.get申請書管理番号()));
    }
