...
/**
 * 申請者意見書情報06ACSVエンティティ編集クラスです。
 *
 * @author N3212 竹内 和紀
 */
public class IkenshoJoho06AEucEntityEditor {

    private static final RString 改行 = new RString("<br/>");
    private static final RString 下矢印 = new RString("↓");

    private IkenshoJoho06AEucEntityEditor() {
    }

    /**
     * DBE010003_IkenshoJoho06AEucEntityを編集して返します。
     *
     ...
     * @return DBE010003_IkenshoJoho06AEucEntity
     */
    public static DBE010003_IkenshoJoho06AEucEntity edit(IkenshoJohoEntity entity,
            List<DbT5303ShujiiIkenshoKinyuItemEntity> 記入項目List,
            List<DbT5304ShujiiIkenshoIkenItemEntity> 意見項目List) {
        DBE010003_IkenshoJoho06AEucEntity eucEntity = new DBE010003_IkenshoJoho06AEucEntity();
        ...
        eucEntity.set訪問歯科診療(get意見項目01名称(get意見項目(意見項目List, Renban._90.getValue())));
        eucEntity.set訪問歯科診療_必要性(get意見項目01名称(get意見項目(意見項目List, Renban._91.getValue())));
        eucEntity.set訪問薬剤管理指導(get意見項目01名称(get意見項目(意見項目List, Renban._92.getValue())));
        eucEntity.set訪問薬剤管理指導_必要性(get意見項目01名称(get意見項目(意見項目List, Renban._93.getValue())));
        eucEntity.set訪問リハビリテーション(get意見項目01名称(get意見項目(意見項目List, Renban._94.getValue())));
        eucEntity.set訪問リハビリテーション_必要性(get意見項目01名称(get意見項目(意見項目List, Renban._95.getValue())));
        eucEntity.set短期入所療養介護(get意見項目01名称(get意見項目(意見項目List, Renban._96.getValue())));
        eucEntity.set短期入所療養介護_必要性(get意見項目01名称(get意見項目(意見項目List, Renban._97.getValue())));
        eucEntity.set訪問歯科衛生指導(get意見項目01名称(get意見項目(意見項目List, Renban._98.getValue())));
        eucEntity.set訪問歯科衛生指導_必要性(get意見項目01名称(get意見項目(意見項目List, Renban._99.getValue())));
        eucEntity.set訪問栄養食事指導(get意見項目01名称(get意見項目(意見項目List, Renban._100.getValue())));
        eucEntity.set訪問栄養食事指導_必要性(get意見項目01名称(get意見項目(意見項目List, Renban._101.getValue())));
        eucEntity.set通所リハビリテーション(get意見項目01名称(get意見項目(意見項目List, Renban._102.getValue())));
        eucEntity.set通所リハビリテーション_必要性(get意見項目01名称(get意見項目(意見項目List, Renban._103.getValue())));
        eucEntity.setその他の医療系のサービス(get意見項目01名称(get意見項目(意見項目List, Renban._104.getValue())));
        eucEntity.setその他の医療系のサービス_詳細(get記入項目(記入項目List, Renban._16.getValue()));
        eucEntity.setその他の医療系のサービス_必要性(get意見項目01名称(get意見項目(意見項目List, Renban._105.getValue())));
        eucEntity.set介護サービスの留意事項_血圧(get意見項目17名称(get意見項目(意見項目List, Renban._106.getValue())));
        eucEntity.set介護サービスの留意事項_血圧_詳細(get記入項目(記入項目List, Renban._17.getValue()));
        eucEntity.set介護サービスの留意事項_移動(get意見項目17名称(get意見項目(意見項目List, Renban._107.getValue())));
        eucEntity.set介護サービスの留意事項_移動_詳細(get記入項目(記入項目List, Renban._18.getValue()));
        eucEntity.set介護サービスの留意事項_摂食(get意見項目17名称(get意見項目(意見項目List, Renban._108.getValue())));
        eucEntity.set介護サービスの留意事項_摂食_詳細(get記入項目(記入項目List, Renban._19.getValue()));
        eucEntity.set介護サービスの留意事項_運動(get意見項目17名称(get意見項目(意見項目List, Renban._109.getValue())));
        eucEntity.set介護サービスの留意事項_運動_詳細(get記入項目(記入項目List, Renban._20.getValue()));
        eucEntity.set介護サービスの留意事項_嚥下(get意見項目17名称(get意見項目(意見項目List, Renban._110.getValue())));
        ...
        eucEntity.set感染症の有無(get意見項目18名称(get意見項目(意見項目List, Renban._111.getValue())));
    ...
    private static RString get意見項目09名称(RString code) {
        return (code != null && !code.trim().isEmpty()) ? IkenKomoku09.toValue(code).get名称() : RString.EMPTY;
    }
    ...
    private static RString get意見項目11名称(RString code) {
        return (code != null && !code.trim().isEmpty()) ? IkenKomoku11.toValue(code).get名称() : RString.EMPTY;
    }

    private static RString get意見項目12名称(RString code) {
        return (code != null && !code.trim().isEmpty()) ? IkenKomoku12.toValue(code).get名称() : RString.EMPTY;
    }

    private static RString get意見項目13名称(RString code) {
        return (code != null && !code.trim().isEmpty()) ? IkenKomoku13.toValue(code).get名称() : RString.EMPTY;
    }
    ...
    private static RString get意見項目15名称(RString code) {
        return (code != null && !code.trim().isEmpty()) ? IkenKomoku15.toValue(code).get名称() : RString.EMPTY;
    }

    private static RString get意見項目16名称(RString code) {
        return (code != null && !code.trim().isEmpty()) ? IkenKomoku16.toValue(code).get名称() : RString.EMPTY;
    }
