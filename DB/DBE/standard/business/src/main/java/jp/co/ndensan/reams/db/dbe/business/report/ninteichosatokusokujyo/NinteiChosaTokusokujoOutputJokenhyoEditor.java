...
public class NinteiChosaTokusokujoOutputJokenhyoEditor {
    ...
    private final NinteiChosaTokusokujoProcessParameter param;

    /**
     * コンストラクタです。
     *
     * @param param {@link NinteiChosaTokusokujoProcessParameter}
     */
    public NinteiChosaTokusokujoOutputJokenhyoEditor(NinteiChosaTokusokujoProcessParameter param) {
        this.param = param;
    }
    ...
    public List<RString> edit() {
        ...
        出力条件List.add(new RString("【認定調査委託先コード】").concat(
                param.getTemp_認定調査委託先コード() == null ? RString.EMPTY : param.getTemp_認定調査委託先コード()));
        出力条件List.add(new RString("【認定調査員コード】").concat(
                param.getTemp_認定調査員コード() == null ? RString.EMPTY : param.getTemp_認定調査員コード()));
    ...
    private RString edit督促方法(RString 督促方法コード) {
        switch (督促方法コード.toString()) {
            case 督促方法コード_督促状郵送:
    ...
    private RString edit調査期限(Decimal 期限日数) {
        return new RString("調査期限から").concat(期限日数.toString()).concat("日以上過ぎた申請");
    }
