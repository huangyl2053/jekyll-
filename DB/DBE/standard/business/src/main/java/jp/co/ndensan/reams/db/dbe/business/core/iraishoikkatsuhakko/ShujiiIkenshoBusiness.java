 ...
 * @author n3509
 */
public class ShujiiIkenshoBusiness {
     ...
     * @param processParamter DBより取得した{@link ShujiiIkenshoProcessParamter}
     */
    public ShujiiIkenshoBusiness(ShujiiIkenshoTeishutsuIraishoHakkoRelateEntity entity,
            ShujiiIkenshoProcessParamter processParamter) {
        this.entity = entity;
    ...
    public ShujiiIkenshoTeishutsuIraishoItem setDBE236001Item(NinshoshaSource ninshoshaSource,
        ...
        item.setYubinNo1(getYubinNo(entity.get医療機関郵便番号()));
        item.setJushoText(entity.get医療機関住所());
        item.setKikanNameText(entity.get医療機関名称());
        item.setShimeiText(entity.get主治医氏名());
        RString key = DbBusinessConfig.get(ConfigNameDBE.主治医意見書作成依頼書_宛先敬称, 基準日, SubGyomuCode.DBE認定支援, processParamter.getShichosonCode());
    ...
    public ShujiiIkenshoSeikyuIchiranEntity setDBE013006Entity(List<ShujiiIkenshoHoshuTankaEntity> 意見書作成料リスト) {
        ...
        帳票Entity.set識別コード(new ShikibetsuCode(processParamter.getShoKisaiHokenshaNo().substring(0, 5).concat(entity.get被保険者番号())));
        帳票Entity.set拡張情報(new ExpandedInformation(new Code("0001"), new RString("申請書管理番号"), entity.get申請書管理番号()));
        return 帳票Entity;
    }
    ...
    private List<RString> get被保険者番号(RString 被保険者番号) {
        ...
        for (int i = 0; i < 被保険者番号.length(); i++) {
            if (i == 被保険者番号.length() - 1) {
                被保険者番号List.add(被保険者番号.substring(i));
            } else {
                被保険者番号List.add(被保険者番号.substring(i, i + 1));
            }
        }
        return 被保険者番号List;
    }
    ...
    public ReportOutputJokenhyoItem バッチ出力条件リストの出力(RString 市町村名,
        ...
        builder.append(get出力可否(processParamter.isIkenshoSakuseiIraiIchiran()));
        出力条件.add(builder.toRString());
        builder = new RStringBuilder();
        builder.append(IKENSHOSAKUSEISEIKYUU);
        builder.append(get出力可否(processParamter.isIkenshoSakuseiSeikyuIchiran()));
        出力条件.add(builder.toRString());
        builder = new RStringBuilder();
        ...
        builder.append(get出力可否(processParamter.isIkenshoKinyu()));
        出力条件.add(builder.toRString());
        builder = new RStringBuilder();
        builder.append(IKENSHOSAKUSEISEIKYUUSHO);
        builder.append(get出力可否(processParamter.isIkenshoSakuseiSeikyusho()));
        出力条件.add(builder.toRString());
    ...
    private RString getYubinNo(RString yubinNo) {
