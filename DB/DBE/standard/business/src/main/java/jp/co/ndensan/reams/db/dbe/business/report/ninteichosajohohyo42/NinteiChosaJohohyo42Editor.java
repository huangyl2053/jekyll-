 ...
 */
public class NinteiChosaJohohyo42Editor implements INinteiChosaJohohyo42Editor {
     ...
     * @param index Index
     */
    protected NinteiChosaJohohyo42Editor(NinteiChosaJohohyoEntity entity, int index) {
        this.entity = entity;
        this.index = index;
    }
     ...
     */
    @Override
    public NinteiChosaJohohyo42ReportSource edit(NinteiChosaJohohyo42ReportSource source) {
        editSource_42(source);
        return source;
    }

    private NinteiChosaJohohyo42ReportSource editSource_42(NinteiChosaJohohyo42ReportSource source) {
        ...
        if (index < entity.get身の回りリスト_42().size()) {
            source.listGogun_1 = entity.get身の回りリスト_42().get(index);
        }
        if (index < entity.get麻痺拘縮1_リスト_42().size()) {
            source.listIchigun1_1 = entity.get麻痺拘縮1_リスト_42().get(index);
        }
        if (index < entity.get麻痺拘縮2_リスト_42().size()) {
            source.listIchigun2_1 = entity.get麻痺拘縮2_リスト_42().get(index);
        }
        if (index < entity.get特別な医療リスト_42().size()) {
            source.listIryo_1 = entity.get特別な医療リスト_42().get(index);
        }
        if (index < entity.get移動リスト_42().size()) {
            source.listNigun_1 = entity.get移動リスト_42().get(index);
        }
        if (index < entity.get意思疎通リスト_42().size()) {
            source.listRokugun_1 = entity.get意思疎通リスト_42().get(index);
        }
        if (index < entity.get問題行動リスト_42().size()) {
            source.listRokugun1_1 = entity.get問題行動リスト_42().get(index);
        }
        if (index < entity.getサービス区分リスト_42().size()) {
            source.listSabisujokyo_1 = entity.getサービス区分リスト_42().get(index);
        }
        if (index < entity.get複雑動作リスト_42().size()) {
            source.listSangun_1 = entity.get複雑動作リスト_42().get(index);
        }
        if (index < entity.get特別介護リスト_42().size()) {
            source.listYongun_1 = entity.get特別介護リスト_42().get(index);
        }
        return source;
    }
}
