...
public class ChosahoshuseikyuEdit {
     ...
     * @param 消費税率 消費税率
     * @param 通知文 通知文
     * @param shuseikyu 認定調査報酬請求書
     * @param flag 状態
     * @return Chosahoshuseikyu
     */
    public Chosahoshuseikyu getChosahoshuseikyu(HoshuShiharaiJunbiRelateEntity entity, RString ninteichosaItakusakiCode,
            RString 消費税率, Map<Integer, RString> 通知文, Chosahoshuseikyu shuseikyu, boolean flag) {
        ...
        if (!ninteichosaItakusakiCode.equals(entity.getNinteichosaItakusakiCode())) {
            ...
            shuseikyu = get更新施設件数(entity, shuseikyu, 単価);
        }
        if (flag) {
            shuseikyu = getChosahoshusei(entity, shuseikyu, 単価, ninteichosaItakusakiCode);
        }
        ...
        Decimal 小計 = Decimal.ZERO;
        小計 = 小計.add(新規在宅計).add(新規施設計).add(更新在宅計).add(更新施設計);
    ...
    private Chosahoshuseikyu get更新施設件数(HoshuShiharaiJunbiRelateEntity entity, Chosahoshuseikyu shuseikyu, Decimal 単価) {
        if (entity.getChosaKubun() != null && entity.getHomonShubetsu() != null
                && ChosaKubun.再調査.getコード().equals(entity.getChosaKubun().value())
                && HomonShubetsu.施設.getコード().equals(entity.getHomonShubetsu().value())) {
    ...
    private Chosahoshuseikyu getChosahoshusei(HoshuShiharaiJunbiRelateEntity entity, Chosahoshuseikyu shuseikyu, Decimal 単価,
        ...
        if (ninteichosaItakusakiCode.equals(entity.getNinteichosaItakusakiCode())) {
            if (entity.getChosaKubun() != null && entity.getHomonShubetsu() != null
                    && ChosaKubun.新規調査.getコード().equals(entity.getChosaKubun().value())
                    && HomonShubetsu.在宅.getコード().equals(entity.getHomonShubetsu().value())) {
                shuseikyu.set新規在宅件数(intToRString(rstringToInt(shuseikyu.get新規在宅件数()) + 1));
                shuseikyu.set新規在宅単価(decimalToRString(単価));
            }
            if (entity.getChosaKubun() != null && entity.getHomonShubetsu() != null
                    && ChosaKubun.新規調査.getコード().equals(entity.getChosaKubun().value())
                    && HomonShubetsu.施設.getコード().equals(entity.getHomonShubetsu().value())) {
                shuseikyu.set新規施設件数(intToRString(rstringToInt(shuseikyu.get新規施設件数()) + 1));
                shuseikyu.set新規施設単価(decimalToRString(単価));
            }
            if (entity.getChosaKubun() != null && entity.getHomonShubetsu() != null
                    && ChosaKubun.再調査.getコード().equals(entity.getChosaKubun().value())
                    && HomonShubetsu.在宅.getコード().equals(entity.getHomonShubetsu().value())) {
                shuseikyu.set更新在宅件数(intToRString(rstringToInt(shuseikyu.get更新在宅件数()) + 1));
                shuseikyu.set更新在宅単価(decimalToRString(単価));
            }
            if (entity.getChosaKubun() != null && entity.getHomonShubetsu() != null
                    && ChosaKubun.再調査.getコード().equals(entity.getChosaKubun().value())
                    && HomonShubetsu.施設.getコード().equals(entity.getHomonShubetsu().value())) {
                shuseikyu.set更新施設件数(intToRString(rstringToInt(shuseikyu.get更新施設件数()) + 1));
                shuseikyu.set更新施設単価(decimalToRString(単価));
            }
        }
        return shuseikyu;
