...
import jp.co.ndensan.reams.db.dbe.entity.db.basic.DbT5016IchijiHanteiKekkaJohoEntity;
...
public final class IchijiHanteiResultDetailMapper {
    ...
    public static DbT5016IchijiHanteiKekkaJohoEntity to一次判定結果Entity(IchijiHanteiResultDetail 一次判定結果) {
        ...
        entity.setIchijiHanteiKekkaCode(一次判定結果.get一次判定結果().get一次判定結果コード().asCode());
        entity.setIchijiHanteiKekkaNinchishoKasanCode(一次判定結果.get認知症加算一次判定結果().get一次判定結果コード().asCode());
        ...
        entity.setJotaiAnteiseiCode(一次判定結果.get認定状態安定性().get介護認定状態安定性コード().asCode());
        entity.setNinchishoJiritsudoIIijoNoGaizensei(一次判定結果.get認知症自立度2以上蓋然性());
        entity.setSuiteiKyufuKubunCode(一次判定結果.get推定給付区分().get推定給付区分コード().asCode());
        entity.setNinchishoKoreishaShihyoCode(一次判定結果.get運動能力未低下認知症高齢者指標().get能力未低下認知症高齢者指標コード().asCode());
        ...
        entity.setGaizenseiHyokaCode(一次判定結果.get認知症高齢者日常生活自立度蓋然性評価().get認知症高齢者自立度蓋然性評価コード().asCode());
