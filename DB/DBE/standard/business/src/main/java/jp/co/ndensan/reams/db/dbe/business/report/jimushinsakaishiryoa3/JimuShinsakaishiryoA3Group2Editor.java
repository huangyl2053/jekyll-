...
public class JimuShinsakaishiryoA3Group2Editor implements IJimuShinsakaishiryoA3Editor {
     ...
     * インスタンスを生成します。
     *
     * @param item {@link IchijihanteikekkahyoA3Business item}
     * @param 特記事項List List<RString>
     * @param index int
     * @param 審査番号 int
     */
    protected JimuShinsakaishiryoA3Group2Editor(IchijihanteikekkahyoA3Business item, List<RString> 特記事項List, int index, int 審査番号) {
        this.item = item;
...
//        }
    ...
    private void setLayout1(JimuShinsakaishiryoA3ReportSource source) {
        if (特記事項テキスト_イメージ区分1.equals(item.get特記事項テキスト_イメージ区分())
                && 特記パターン1.equals(item.get特記パターン())) {
            source.layout = JimuShinsakaiShiryoA3Layouts.事務局用一次判定結果票_予防給付_テキスト.index();
        } else if (特記事項テキスト_イメージ区分1.equals(item.get特記事項テキスト_イメージ区分())
                && 特記パターン2.equals(item.get特記パターン())) {
            source.layout = JimuShinsakaiShiryoA3Layouts.事務局用一次判定結果票_予防給付_短冊テキスト.index();
        } else if (特記事項テキスト_イメージ区分2.equals(item.get特記事項テキスト_イメージ区分())
                && 特記パターン1.equals(item.get特記パターン())) {
            source.layout = JimuShinsakaiShiryoA3Layouts.事務局用一次判定結果票_予防給付_全イメージ.index();
        } else if (特記事項テキスト_イメージ区分2.equals(item.get特記事項テキスト_イメージ区分())
                && 特記パターン2.equals(item.get特記パターン())) {
            source.layout = JimuShinsakaiShiryoA3Layouts.事務局用一次判定結果票_予防給付_短冊イメージ.index();
        }
    }

    private void setLayout2(JimuShinsakaishiryoA3ReportSource source) {
        if (特記事項テキスト_イメージ区分1.equals(item.get特記事項テキスト_イメージ区分())
                && 特記パターン1.equals(item.get特記パターン())) {
            source.layout = JimuShinsakaiShiryoA3Layouts.事務局用一次判定結果票_介護給付_テキスト.index();
        } else if (特記事項テキスト_イメージ区分1.equals(item.get特記事項テキスト_イメージ区分())
                && 特記パターン2.equals(item.get特記パターン())) {
            source.layout = JimuShinsakaiShiryoA3Layouts.事務局用一次判定結果票_介護給付_短冊テキスト.index();
        } else if (特記事項テキスト_イメージ区分2.equals(item.get特記事項テキスト_イメージ区分())
                && 特記パターン1.equals(item.get特記パターン())) {
            source.layout = JimuShinsakaiShiryoA3Layouts.事務局用一次判定結果票_介護給付_全イメージ.index();
        } else if (特記事項テキスト_イメージ区分2.equals(item.get特記事項テキスト_イメージ区分())
                && 特記パターン2.equals(item.get特記パターン())) {
            source.layout = JimuShinsakaiShiryoA3Layouts.事務局用一次判定結果票_介護給付_短冊イメージ.index();
        }
    }

    private void setLayout3(JimuShinsakaishiryoA3ReportSource source) {
        if (特記事項テキスト_イメージ区分1.equals(item.get特記事項テキスト_イメージ区分())
                && 特記パターン1.equals(item.get特記パターン())) {
            source.layout = JimuShinsakaiShiryoA3Layouts.事務局用一次判定結果票_テキスト.index();
        } else if (特記事項テキスト_イメージ区分1.equals(item.get特記事項テキスト_イメージ区分())
                && 特記パターン2.equals(item.get特記パターン())) {
            source.layout = JimuShinsakaiShiryoA3Layouts.事務局用一次判定結果票_短冊テキスト.index();
        } else if (特記事項テキスト_イメージ区分2.equals(item.get特記事項テキスト_イメージ区分())
                && 特記パターン1.equals(item.get特記パターン())) {
            source.layout = JimuShinsakaiShiryoA3Layouts.事務局用一次判定結果票_全イメージ.index();
        } else if (特記事項テキスト_イメージ区分2.equals(item.get特記事項テキスト_イメージ区分())
                && 特記パターン2.equals(item.get特記パターン())) {
            source.layout = JimuShinsakaiShiryoA3Layouts.事務局用一次判定結果票_短冊イメージ.index();
        }
    }

    private JimuShinsakaishiryoA3ReportSource editSource1(JimuShinsakaishiryoA3ReportSource source) {
    ...
    private JimuShinsakaishiryoA3ReportSource editSource2(JimuShinsakaishiryoA3ReportSource source) {
