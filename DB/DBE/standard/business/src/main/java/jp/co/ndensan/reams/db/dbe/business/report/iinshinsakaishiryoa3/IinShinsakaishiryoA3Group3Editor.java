...
public class IinShinsakaishiryoA3Group3Editor implements IIinShinsakaishiryoA3Editor {
    ...
    private IinShinsakaishiryoA3ReportSource editSource(IinShinsakaishiryoA3ReportSource source) {
        ...
        if (特記事項テキスト_イメージ区分1.equals(item.get特記事項テキスト_イメージ区分())
            ...
            source.layout = IinShinsakaiShiryoA3Layouts.委員用特記事項_テキスト.index();
        } else if (特記事項テキスト_イメージ区分1.equals(item.get特記事項テキスト_イメージ区分())
                && 特記パターン2.equals(item.get特記パターン())) {
            source.layout = IinShinsakaiShiryoA3Layouts.委員用特記事項_短冊テキスト.index();
        } else if (特記事項テキスト_イメージ区分2.equals(item.get特記事項テキスト_イメージ区分())
                && 特記パターン1.equals(item.get特記パターン())) {
            source.layout = IinShinsakaiShiryoA3Layouts.委員用特記事項_全イメージ.index();
        } else if (特記事項テキスト_イメージ区分2.equals(item.get特記事項テキスト_イメージ区分())
                && 特記パターン2.equals(item.get特記パターン())) {
            source.layout = IinShinsakaiShiryoA3Layouts.委員用特記事項_短冊イメージ.index();
        }
        return source;
    }
...
//    }
    ...
    private IinShinsakaishiryoA3ReportSource set特記事項連番_名称(IinShinsakaishiryoA3ReportSource source, TokkijikoTextImageKubun 特記事項テキストイメージ区分) {
        setTokkijikoNameOddPage(source, 特記事項テキストイメージ区分);
        setTokkijikoNameEvenPage(source, 特記事項テキストイメージ区分);
        return source;
    }

    private void setTokkijikoNameOddPage(IinShinsakaishiryoA3ReportSource source, TokkijikoTextImageKubun 特記事項テキストイメージ区分) {
        if ((PAGE2連番名称COUNT * (page - 2) + PAGE1連番名称COUNT + index) < 特記事項List.size()) {
    ...
    private void setTokkijikoNameEvenPage(IinShinsakaishiryoA3ReportSource source, TokkijikoTextImageKubun 特記事項テキストイメージ区分) {
        if ((PAGE2連番名称COUNT * (page - 2) + PAGE2連番名称COUNT + PAGE1連番名称COUNT + index) < 特記事項List.size()) {
            if (TokkijikoTextImageKubun.テキスト.equals(特記事項テキストイメージ区分)) {
