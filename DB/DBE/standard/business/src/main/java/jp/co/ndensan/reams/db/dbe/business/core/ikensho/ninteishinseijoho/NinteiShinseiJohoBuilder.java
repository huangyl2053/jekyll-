...
public class NinteiShinseiJohoBuilder {
     ...
     * 要介護認定申請情報に関連できる原因疾患を管理である場合、下記の処理に遷移します。<br>
     * キーが一致する場合は精原因疾患を管理リストに原因疾患を管理{@link GeninShikkan}をセットします。<br>
     * キーが一致しない場合、新たに追加します。<br>
     *
     * @param 原因疾患を管理 {@link GeninShikkan}
     * @return {@link NinteiShinseiJohoBuilder}
     * @throws IllegalStateException キーが一致しない場合
     */
    public NinteiShinseiJohoBuilder setGeninShikkan(GeninShikkan 原因疾患を管理) {
        if (hasSameIdentifier(原因疾患を管理.identifier())) {
            geninShikkan.add(原因疾患を管理);
            return this;
        }
        throw new IllegalArgumentException(UrErrorMessages.不正.toString());
    }

    private boolean hasSameIdentifier(GeninShikkanIdentifier 原因疾患を管理識別子) {
        return (id.get申請書管理番号().equals(原因疾患を管理識別子.get申請書管理番号()));
    }
    ...
    public NinteiShinseiJoho build() {
        return new NinteiShinseiJoho(entity, id, geninShikkan, shujiiIkenshoIraiJoho);
    }
