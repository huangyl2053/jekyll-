/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package jp.co.ndensan.reams.db.dbe.business.report.iinYobihanteiKinyuhyo;

import jp.co.ndensan.reams.db.dbe.entity.report.iinYobihanteiKinyuhyoReportSource.IinYobihanteiKinyuhyoReportSource;

/**
 * 委員用予備判定記入表のEditorです。
 *
 * @reamsid_L DBE-0150-150 wangrenze
 */
public class IinYobihanteiKinyuhyoEditor implements IIinYobihanteiKinyuhyoEditor {

    private final IinYobihanteiKinyuhyoItem item;

    /**
     * コンストラクタです。
     *
     * @param item {@link IinYobihanteiKinyuhyoItem}
     */
    protected IinYobihanteiKinyuhyoEditor(IinYobihanteiKinyuhyoItem item) {
        this.item = item;
    }

    /**
     * 委員用予備判定記入表を編集します。
     *
     * @param source 帳票ソース
     * @return 帳票ソース
     */
    @Override
    public IinYobihanteiKinyuhyoReportSource edit(IinYobihanteiKinyuhyoReportSource source) {
        source.printTimeStamp = item.get作成年月日();
        source.title = item.get帳票名();
        source.shinsakaiKaisaiNo = item.get審査会開催番号();
        source.listTaishoshaIchiran_1 = item.getNo();
        source.listTaishoshaIchiran_2 = item.get申請区分();
        source.listTaishoshaIchiran_3 = item.get被保険者区分();
        source.listTaishoshaIchiran_4 = item.get氏名();
        source.listTaishoshaIchiran_5 = item.get性別();
        source.listTaishoshaIchiran_6 = item.get年齢();
        source.listTaishoshaIchiran_7 = item.get前回二次判定();
        source.listTaishoshaIchiran_8 = item.get前回認定有効期間();
        source.listTaishoshaIchiran_9 = item.get一次判定();
        source.listTaishoshaIchiran_10 = item.get二次判定();
        source.listTaishoshaIchiran_11 = item.get状態像();
        source.listTaishoshaIchiran_12 = item.get認定有効期間();
        source.listTokuteiShippeiCode_1 = item.get特定疾病コード();
        source.listTokuteiShippeiName_1 = item.get特定疾病名称();

        return source;
    }
}
