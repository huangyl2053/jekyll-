...
public class KaigoNinteiChosainCollection implements Iterable {
    ...
    /**
     * 引数から介護事業者番号と介護調査員番号を渡し、それに対応した介護認定調査員を返します。<br />
     ...
     */
    public KaigoNinteiChosain get介護認定調査員(KaigoJigyoshaNo 介護事業者番号, KaigoChosainNo 介護調査員番号)
    ...
    private KaigoNinteiChosainCollection create介護認定調査員Collection(List<KaigoNinteiChosain> kaigoNinteiChosainList) {
        if (kaigoNinteiChosainList.isEmpty()) {
            return new KaigoNinteiChosainCollection(Collections.EMPTY_LIST);
        }
        return new KaigoNinteiChosainCollection(kaigoNinteiChosainList);
    }
