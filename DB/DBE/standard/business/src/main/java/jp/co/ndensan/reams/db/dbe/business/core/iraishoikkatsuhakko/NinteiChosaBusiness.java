 ...
 * @author n3509
 */
public class NinteiChosaBusiness {
    ...
    private static final RString NINTEICHOSAKATAMENGAIKYO = new RString("【認定調査票（片面・概況調査）出力区分】");
    private static final RString NINTEICHOSAKATAMENKIHON = new RString("【認定調査票（片面・基本調査）出力区分】");
    private static final RString NINTEICHOSARYOMEN = new RString("【認定調査票（両面）出力区分】");
    private static final RString TOKKIJIKO = new RString("【特記事項出力区分】");
    ...
    private static final RString TOKKIJIKO_CHOSAGUN = new RString("【特記事項（調査群記載あり）】");
    ...
    private final NinteiChosaProcessParamter processParamter;
     ...
     */
    public NinteiChosaBusiness(NinteiChosaProcessParamter processParamter) {
    ...
    private boolean is調査票等出力年月日() {
        return processParamter.is認定調査依頼書()
                || processParamter.is認定調査票_概況調査()
                || processParamter.is認定調査票_基本調査()
                || processParamter.is認定調査票_概況基本()
                || processParamter.is認定調査票_特記事項()
                || processParamter.is認定調査票_概況特記()
                || processParamter.is認定調査差異チェック票()
                || processParamter.is特記事項_調査群()
    ...
    public ChosaIraiIchiranhyoBodyItem setBodyItem(HomonChosaIraishoRelateEntity entity, int 連番, NinshoshaSource ninshoshaSource,
        ...
        return new ChosaIraiIchiranhyoBodyItem(
                ...
                get認定調査依頼一覧表名称付与(),
                entity.get事業者番号(),
                通知文Map != null && !通知文Map.isEmpty() ? 通知文Map.get(0) : RString.EMPTY,
                通知文Map != null && !通知文Map.isEmpty() ? 通知文Map.get(1) : RString.EMPTY,
                通知文Map != null && !通知文Map.isEmpty() ? 通知文Map.get(2) : RString.EMPTY,
    ...
    /**
     * 帳票「要介護認定調査票（基本調査）の帳票DBE221012」のItem設定メッソドです。 getCode(
     *
     * @param entity entity
    ...
    public ChosahyoGaikyochosaItem setDBE221011Item(HomonChosaIraishoRelateEntity entity) {
        ...
        return new ChosahyoGaikyochosaItem(
                ...
                getCode(記入者コードリスト, 1),
                getCode(記入者コードリスト, 2),
                getCode(記入者コードリスト, INT3),
                getCode(記入者コードリスト, INT4),
                getCode(記入者コードリスト, INT5),
                getCode(記入者コードリスト, INT6),
                getCode(記入者コードリスト, INT7),
                entity.get調査員氏名(),
                getCode(定調査委託先コードリスト, 0),
                getCode(定調査委託先コードリスト, 1),
                getCode(定調査委託先コードリスト, 2),
                getCode(定調査委託先コードリスト, INT3),
                getCode(定調査委託先コードリスト, INT4),
                getCode(定調査委託先コードリスト, INT5),
                getCode(定調査委託先コードリスト, INT6),
                getCode(定調査委託先コードリスト, INT7),
                getCode(定調査委託先コードリスト, INT8),
                getCode(定調査委託先コードリスト, INT9),
                getCode(定調査委託先コードリスト, INT11),
                getCode(定調査委託先コードリスト, INT12),
                entity.get事業者名称(),
                entity.get被保険者氏名カナ(),
                entity.get被保険者氏名(),
    ...
    public List<RString> set出力条件() {
        ...
        出力条件.add(builder.toRString());
        builder = new RStringBuilder();
        builder.append(NINTEICHOSAKATAMENGAIKYO);
        builder.append(get出力可否(processParamter.is認定調査票_概況調査()));
        出力条件.add(builder.toRString());
        builder = new RStringBuilder();
        builder.append(NINTEICHOSAKATAMENKIHON);
        builder.append(get出力可否(processParamter.is認定調査票_基本調査()));
        出力条件.add(builder.toRString());
        builder = new RStringBuilder();
        builder.append(NINTEICHOSARYOMEN);
        builder.append(get出力可否(processParamter.is認定調査票_概況基本()));
        出力条件.add(builder.toRString());
        builder = new RStringBuilder();
        builder.append(TOKKIJIKO);
        builder.append(get出力可否(processParamter.is認定調査票_特記事項()));
        出力条件.add(builder.toRString());
        builder = new RStringBuilder();
        builder.append(GAIKYOTOKKI);
        builder.append(get出力可否(processParamter.is認定調査票_概況特記()));
        出力条件.add(builder.toRString());
        builder = new RStringBuilder();
        ...
        出力条件.add(builder.toRString());
        builder = new RStringBuilder();
        builder.append(TOKKIJIKO_CHOSAGUN);
        builder.append(get出力可否(processParamter.is特記事項_調査群()));
        出力条件.add(builder.toRString());
        builder = new RStringBuilder();
        ...
        builder.append(get出力可否(processParamter.is認定調査依頼一覧表()));
        出力条件.add(builder.toRString());
        builder = new RStringBuilder();
    ...
    public GaikyotokkiA4Business setReportEntity(HomonChosaIraishoRelateEntity entity) {
        ...
        reportEntity.setHokenshaNo1(getCode(保険者番号リスト, 0));
        reportEntity.setHokenshaNo2(getCode(保険者番号リスト, 1));
        reportEntity.setHokenshaNo3(getCode(保険者番号リスト, 2));
        reportEntity.setHokenshaNo4(getCode(保険者番号リスト, INT3));
        reportEntity.setHokenshaNo5(getCode(保険者番号リスト, INT4));
        reportEntity.setHokenshaNo6(getCode(保険者番号リスト, INT5));
        ...
        reportEntity.setHihokenshaNo1(getCode(被保険者番号リスト, 0));
        reportEntity.setHihokenshaNo2(getCode(被保険者番号リスト, 1));
        reportEntity.setHihokenshaNo3(getCode(被保険者番号リスト, 2));
        reportEntity.setHihokenshaNo4(getCode(被保険者番号リスト, INT3));
        reportEntity.setHihokenshaNo5(getCode(被保険者番号リスト, INT4));
        reportEntity.setHihokenshaNo6(getCode(被保険者番号リスト, INT5));
        reportEntity.setHihokenshaNo7(getCode(被保険者番号リスト, INT6));
        reportEntity.setHihokenshaNo8(getCode(被保険者番号リスト, INT7));
        reportEntity.setHihokenshaNo9(getCode(被保険者番号リスト, INT8));
        reportEntity.setHihokenshaNo10(getCode(被保険者番号リスト, INT9));
    ...
    public GaikyotokkiA4Business setDBE221051Item(HomonChosaIraishoRelateEntity entity) {
        ...
        reportEntity.setHokenshaNo4(getCode(証記載保険者番号リスト, INT3));
        reportEntity.setHokenshaNo5(getCode(証記載保険者番号リスト, INT4));
        reportEntity.setHokenshaNo6(getCode(証記載保険者番号リスト, INT5));
        reportEntity.setHihokenshaNo1(getCode(被保険者番号リスト, 0));
        reportEntity.setHihokenshaNo2(getCode(被保険者番号リスト, 1));
        reportEntity.setHihokenshaNo3(getCode(被保険者番号リスト, 2));
        reportEntity.setHihokenshaNo4(getCode(被保険者番号リスト, INT3));
    ...
    private List<RString> getValueList(RString value) {
        ...
        for (int i = 0; i < value.length(); i++) {
            if (i == value.length() - 1) {
                valueList.add(value.substring(i));
            } else {
                valueList.add(value.substring(i, i + 1));
            }
        }
        return valueList;
    }
