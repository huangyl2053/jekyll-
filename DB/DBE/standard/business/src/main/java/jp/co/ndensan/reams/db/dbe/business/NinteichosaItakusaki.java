...
//TODO 市町村コードは介護で作成したShichosonCodeを使用するようにする　2014/02/14期限
    ...
    public NinteichosaItakusaki(RString 市町村コード, KaigoJigyoshaNo 介護事業者番号, JigyoshaNo 事業者番号,
        ...
        requireNonNull(市町村コード, Messages.E00003.replace("市町村コード", "認定調査委託先").getMessage());
        requireNonNull(事業者番号, Messages.E00003.replace("事業者番号", "認定調査委託先").getMessage());
        requireNonNull(介護事業者番号, Messages.E00003.replace("介護事業者番号", "認定調査委託先").getMessage());
        requireNonNull(有効区分, Messages.E00003.replace("介護保険事業者の状況", "認定調査委託先").getMessage());
