...
public final class ShinsakaiWariateIinMapper {
     ...
     * @param 審査会情報 審査会情報
     * @return 審査会割当委員
     * @throws IllegalArgumentException 審査会割当委員Entityがnullでは無いときに、審査会情報か審査会委員のいずれかにnullが渡された場合
     */
    ...
    /**
     * 引数から審査会割当委員EntityList・審査会委員List・審査会情報を受け取り、 それらの情報をマッピングして審査会割当委員Listを生成し、返却します。<br/>
     ...
     * @param 審査会情報 審査会情報
     * @return 審査会List
     * @throws IllegalArgumentException 審査会割当委員Entityが持つ委員コードと、委員情報の委員コードで、対応しないものが存在したとき
     */
...
//        throw new IllegalArgumentException(UrErrorMessages.存在しない.getMessage().replace("審査会割当情報に対応した委員").evaluate());
    ...
    public static DbT5106ShinsakaiWariateIinJohoEntity to審査員割当委員Entity(ShinsakaiWariateIin 審査会割当委員) {
        ...
        entity.setShinsainKubunCode(審査会割当委員.get認定審査員区分().getShinsainKubunCode().asCode());
        entity.setGogitaichoKubunCode(審査会割当委員.get合議体長区分().getGogitaichoKubunCode().asCode());
