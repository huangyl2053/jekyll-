...
public class NinchishoKoreishaJiritsudoGaizenseiHyokaKomokuTest {
    ...
    public static class get認知症高齢者自立度蓋然性評価コードのテスト extends DbeTestBase {
        ...
        @Before
        public void setUp() {
            sut = new NinchishoKoreishaJiritsudoGaizenseiHyokaKomoku(new NinchishoKoreishaJiritsudoGaizenseiHyokaCode(new RString("001")),
                    new RString("名称"));
        }

        @Test
        public void get認知症高齢者自立度蓋然性評価コードと_getCodeで取得した内容が_同一になる() {
            assertThat(sut.get認知症高齢者自立度蓋然性評価コード().value(), is(sut.getCode().value()));
        }
    }
}
