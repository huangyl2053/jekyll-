...
public class ShogaiKoreishaJiritsudoKomokuTest {
    ...
    public static class get認知症高齢者自立度蓋然性評価コードのテスト extends DbeTestBase {
        ...
        @Before
        public void setUp() {
            sut = new ShogaiKoreishaJiritsudoKomoku(new Code(new RString("001")), new RString("名称"), new RString("略称"));
        }

        @Test
        public void get障害高齢者自立度コードと_getCodeで取得した内容が_同一になる() {
            assertThat(sut.get障害高齢者自立度コード().value(), is(sut.get障害高齢者自立度コード().asCode().value()));
        }
    }
}
