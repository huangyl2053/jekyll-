...
public class NinteichosaItemForResultTest {
    ...
    public static class get回答結果 {
        ...
        public void 調査項目が選択形式の時_get回答結果は_該当の選択値を返す() {
            assertThat(createNinteichosaItemForResult(AS_選択項目).get回答結果().getCode(), is(new RString("1")));
        }

        @Test
        public void 調査項目が入力形式の時_get回答結果は_入力値を返す() {
            assertThat(createNinteichosaItemForResult(AS_入力項目).get回答結果().getCode(), is(new RString("自由入力")));
        }
    }
