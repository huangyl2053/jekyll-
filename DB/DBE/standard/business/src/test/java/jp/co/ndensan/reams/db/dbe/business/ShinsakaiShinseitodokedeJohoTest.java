...
import jp.co.ndensan.reams.db.dbe.business.core.Shinsakai.ShinsakaiShinseitodokedeJoho;
...
/**
 * {@link ShinsakaiShinseitodokedeJoho}のテストクラスです。
 *
 * @author n8223 朴義一
 */
@RunWith(Enclosed.class)
public class ShinsakaiShinseitodokedeJohoTest extends DbdTestBase {

    public static class getterSetterTest extends DbdTestBase {

        private static ShinsakaiShinseitodokedeJoho sut;

        @BeforeClass
        public static void test() {
            sut = new ShinsakaiShinseitodokedeJoho(DbT5120ShinseitodokedeJohoEntityGenerator.createDbT5120ShinseitodokedeJohoEntity());
        }

        @Test
        public void 戻り値の申請書管理番号は_設定した値と同じ申請書管理番号を返す() {
            assertThat(sut.get申請書管理番号(), is(DbT5120ShinseitodokedeJohoEntityGenerator.DEFAULT_申請書管理番号));
        }

        @Test
        public void 戻り値の申請届出代行区分コードは_設定した値と同じ申請届出代行区分コードを返す() {
            assertThat(sut.get申請届出代行区分コード(), is(DbT5120ShinseitodokedeJohoEntityGenerator.DEFAULT_申請届出代行区分コード));
        }

        @Test
        public void 戻り値の申請届出者氏名は_設定した値と同じ申請届出者氏名を返す() {
            assertThat(sut.get申請届出者氏名(), is(DbT5120ShinseitodokedeJohoEntityGenerator.DEFAULT_申請届出者氏名));
        }

        @Test
        public void 戻り値の申請届出者氏名カナは_設定した値と同じ申請届出者氏名カナを返す() {
            assertThat(sut.get申請届出者氏名カナ(), is(DbT5120ShinseitodokedeJohoEntityGenerator.DEFAULT_申請届出者氏名カナ));
        }

        @Test
        public void 戻り値の申請届出者続柄コードは_設定した値と同じ申請届出者続柄コードを返す() {
            assertThat(sut.get申請届出者続柄コード(), is(DbT5120ShinseitodokedeJohoEntityGenerator.DEFAULT_申請届出者続柄コード));
        }

        @Test
        public void 戻り値の申請届出代行事業者番号は_設定した値と同じ申請届出代行事業者番号を返す() {
            assertThat(sut.get申請届出代行事業者番号(), is(DbT5120ShinseitodokedeJohoEntityGenerator.DEFAULT_申請届出代行事業者番号));
        }

        @Test
        public void 戻り値の事業者区分は_設定した値と同じ事業者区分を返す() {
            assertThat(sut.get事業者区分(), is(DbT5120ShinseitodokedeJohoEntityGenerator.DEFAULT_事業者区分));
        }

        @Test
        public void 戻り値の申請届出者郵便番号は_設定した値と同じ申請届出者郵便番号を返す() {
            assertThat(sut.get申請届出者郵便番号(), is(DbT5120ShinseitodokedeJohoEntityGenerator.DEFAULT_申請届出者郵便番号));
        }

        @Test
        public void 戻り値の申請届出者住所は_設定した値と同じ申請届出者住所を返す() {
            assertThat(sut.get申請届出者住所(), is(DbT5120ShinseitodokedeJohoEntityGenerator.DEFAULT_申請届出者住所));
        }

        @Test
        public void 戻り値の申請届出者電話番号は_設定した値と同じ申請届出者電話番号を返す() {
            assertThat(sut.get申請届出者電話番号(), is(DbT5120ShinseitodokedeJohoEntityGenerator.DEFAULT_申請届出者電話番号));
        }
    }
    ...
    public static class SerializationProxyTest {
        ...
        private ShinsakaiShinseitodokedeJoho sut;

        @Before
        public void setUp() {
            sut = ShinsakaiShinseitodokedeJoho.newBuilder().build(); //builderを用いて、インスタンスを生成してください。
        }
