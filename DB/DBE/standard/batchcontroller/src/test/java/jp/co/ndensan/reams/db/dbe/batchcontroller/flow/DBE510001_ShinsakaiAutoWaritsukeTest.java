    ...
    public static class バッチフローテスト extends DbeTestDacBase {
        ...
        public void パラメーターが正しく設定されている場合_バッチフロー実施結果は_Successで返すこと() throws Exception {
            ...
            BatchExitStatus result = flowHelper.executeFlow(
                    new RString("DBE510001_ShinsakaiAutoWaritsuke"),
                    new RString("対象者自動割付"),
                    DBE510001_ShinsakaiAutoWaritsuke.class,
                    createParameter());
        ...
        private static DBE510001_ShinsakaiAutoWaritsukeParameter createParameter() {
            DBE510001_ShinsakaiAutoWaritsukeParameter parameter = new DBE510001_ShinsakaiAutoWaritsukeParameter();
            List<RString> shinsakaiKaisaiNo = new ArrayList<>();
            List<FlexibleDate> shinsakaiKaisaiYMD = new ArrayList<>();
            List<Integer> shinsakaiWaritsukeNinsu = new ArrayList<>();
            List<Integer> shinsakaiJidoWariateTeiin = new ArrayList<>();

            shinsakaiKaisaiNo.add(new RString("20150059"));
            shinsakaiKaisaiYMD.add(new FlexibleDate("20161201"));
            shinsakaiWaritsukeNinsu.add(10);
            shinsakaiJidoWariateTeiin.add(30);

            parameter.setShinsakaiKaisaiNo(shinsakaiKaisaiNo);
            parameter.setShinsakaiKaisaiYMD(shinsakaiKaisaiYMD);
            parameter.setShinsakaiWaritsukeNinsu(shinsakaiWaritsukeNinsu);
            parameter.setShinsakaiJidoWariateTeiin(shinsakaiJidoWariateTeiin);
            return parameter;
        }
