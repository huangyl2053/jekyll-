 ...
 */
public class ChkNinteiChosahyo12Process extends BatchProcessBase<YokaigoninteiEntity> {
    ...
    protected void process(YokaigoninteiEntity entity) {
        ...
        NinteiChosaJohohyo12Report report = new NinteiChosaJohohyo12Report(bodyItem);
    ...
    private void 共有ファイルを引き出す(RDateTime sharedFileId, RString sharedFileName) {
        if (sharedFileId != null) {
            ...
            try {
                SharedFile.copyToLocal(descriptor, new FilesystemPath(batchWrite12.getImageFolderPath()));
            } catch (Exception e) {
            }
        }
    }
    ...
    private RString getイメージファイル名(RString fileName) {
        ...
        if (!RString.isNullOrEmpty(getFilePath(batchWrite12.getImageFolderPath(), file))) {
            return file;
        }
    ...
    private void set出力条件表() {
        ...
        ReportOutputJokenhyoItem 帳票出力条件表パラメータ
                = new ReportOutputJokenhyoItem(
                        ReportIdDBE.DBE091012.getReportId().value(),
                        association.getLasdecCode_().getColumnValue(),
                        association.get市町村名(),
                        new RString(JobContextHolder.getJobId()),
                        ReportInfo.getReportName(SubGyomuCode.DBE認定支援, ReportIdDBE.DBE091012.getReportId().value()),
                        new RString(String.valueOf(reportSourceWriter12.pageCount().value())),
                        CSV出力有無,
                        CSVファイル名,
                        出力条件);
