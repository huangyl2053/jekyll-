...
public class ChkNinteiChosahyo32Process extends BatchProcessBase<YokaigoninteiEntity> {
    ...
    private void 共有ファイルを引き出す(RDateTime sharedFileId, RString sharedFileName) {
        if (sharedFileId != null) {
            ...
            try {
                SharedFile.copyToLocal(descriptor, new FilesystemPath(batchWrite32.getImageFolderPath()));
            } catch (Exception e) {
            }
        }
    }
    ...
    private RString getイメージファイル名(RString fileName) {
        ...
        if (!RString.isNullOrEmpty(getFilePath(batchWrite32.getImageFolderPath(), file))) {
            return file;
        }
