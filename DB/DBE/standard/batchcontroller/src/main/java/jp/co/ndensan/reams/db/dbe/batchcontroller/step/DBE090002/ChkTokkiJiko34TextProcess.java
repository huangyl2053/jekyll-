 ...
 * @author n3331
 */
public class ChkTokkiJiko34TextProcess extends BatchProcessBase<YokaigoninteiEntity> {
    ...
    protected void createWriter() {
        if (すべて.equals(DbBusinessConfig.get(ConfigNameDBE.情報提供資料の特記事項編集パターン, RDate.getNowDate(),
                SubGyomuCode.DBE認定支援))) {
            batchWrite = BatchReportFactory.createBatchReportWriter(ReportIdDBE.DBE517135.getReportId().value()).create();
        } else {
            batchWrite = BatchReportFactory.createBatchReportWriter(ReportIdDBE.DBE517136.getReportId().value()).create();
        }
