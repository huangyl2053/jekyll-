 ...
 */
public class ChkNinteiChosahyo22Process extends BatchProcessBase<YokaigoninteiEntity> {
    ...
    protected void process(YokaigoninteiEntity entity) {
        ...
        bodyItem.setサービス区分リスト_22(setサービス状況02(dbt5207Entity, dbt5208Entity));
        ...
        NinteiChosaJohohyo22Report report = new NinteiChosaJohohyo22Report(bodyItem);
    ...
    private void setマスキング情報(NinteiChosaJohohyoEntity ninteiEntity, YokaigoninteiEntity entity) throws IllegalArgumentException, NullPointerException {
    ...
    private void 共有ファイルを引き出す(RDateTime sharedFileId, RString sharedFileName) {
        if (sharedFileId != null) {
            ...
            try {
                SharedFile.copyToLocal(descriptor, new FilesystemPath(batchWrite22.getImageFolderPath()));
            } catch (Exception e) {
            }
        }
    }
    ...
    private RString getイメージファイル名(RString fileName) {
        ...
        if (!RString.isNullOrEmpty(getFilePath(batchWrite22.getImageFolderPath(), file))) {
            return file;
        }
    ...
    private void set出力条件表() {
        ...
        ReportOutputJokenhyoItem 帳票出力条件表パラメータ
                = new ReportOutputJokenhyoItem(
                        ReportIdDBE.DBE091022.getReportId().value(),
                        association.getLasdecCode_().getColumnValue(),
                        association.get市町村名(),
                        new RString(JobContextHolder.getJobId()),
                        ReportInfo.getReportName(SubGyomuCode.DBE認定支援, ReportIdDBE.DBE091022.getReportId().value()),
                        new RString(String.valueOf(reportSourceWriter22.pageCount().value())),
                        CSV出力有無,
                        CSVファイル名,
                        出力条件);
