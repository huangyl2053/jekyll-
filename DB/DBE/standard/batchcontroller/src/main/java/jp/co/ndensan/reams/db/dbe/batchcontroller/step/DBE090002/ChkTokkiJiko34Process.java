...
public class ChkTokkiJiko34Process extends BatchProcessBase<YokaigoninteiEntity> {
    ...
    protected void createWriter() {
        if (全イメージ.equals(DbBusinessConfig.get(ConfigNameDBE.情報提供資料の特記事項イメージパターン, RDate.getNowDate(),
            ...
            batchWrite = BatchReportFactory.createBatchReportWriter(ReportIdDBE.DBE517137.getReportId().value()).create();
        } else {
            batchWrite = BatchReportFactory.createBatchReportWriter(ReportIdDBE.DBE517138.getReportId().value()).create();
        }
