...
import jp.co.ndensan.reams.db.dbe.divcontroller.entity.parentdiv.DBE5810001.NinteiEnkiTsuchishoHakkoDivSpec;
...
public class NinteiEnkiTsuchishoHakkoValidationHandler {
     ...
     * @return バリデーション結果
     */
    public ValidationMessageControlPairs 変更内容保存チェック(ValidationMessageControlPairs pairs, NinteiEnkiTsuchishoHakkoDiv div) {
        IValidationMessages messages = ValidationMessagesFactory.createInstance();
        messages.add(ValidateChain.validateStart(div).ifNot(NinteiEnkiTsuchishoHakkoDivSpec.変更内容保存チェック)
                .thenAdd(NoInputMessages.変更内容保存チェック).messages());
        pairs.add(new ValidationMessageControlDictionaryBuilder().add(
                NoInputMessages.変更内容保存チェック, div.getDgHakkotaishosha()).build().check(messages));
        return pairs;
    }
     ...
     * @return バリデーション結果
     */
    public ValidationMessageControlPairs 選択行発行日チェック(ValidationMessageControlPairs pairs, NinteiEnkiTsuchishoHakkoDiv div) {
        IValidationMessages messages = ValidationMessagesFactory.createInstance();
        messages.add(ValidateChain.validateStart(div).ifNot(NinteiEnkiTsuchishoHakkoDivSpec.選択行発行日チェック)
                .thenAdd(NoInputMessages.選択行発行日チェック).messages());
        pairs.add(new ValidationMessageControlDictionaryBuilder().add(
                NoInputMessages.選択行発行日チェック, div.getDgHakkotaishosha()).build().check(messages));
        return pairs;
    }
    ...
    private static enum NoInputMessages implements IValidationMessage {
        ...
        変更内容保存チェック(UrErrorMessages.実行不可.getMessage(), "変更内容が保存されていないため、通知書発行"),
        選択行発行日チェック(UrErrorMessages.実行不可.getMessage(), "通知書発行日が設定されていない行があるため、通知書発行");
