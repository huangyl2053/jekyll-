    ...
    public IDownLoadServletResponse onClick_btnChosadataOutput(NinteichosaIraiDiv requestDiv, IDownLoadServletResponse response) {
        ...
        for (dgNinteiTaskList_Row row : requestDiv.getDgNinteiTaskList().getSelectedItems()) {
            ...
            if (!調査入力用データ.records().isEmpty()) {
                RString 調査員コード = 調査入力用データ.records().get(0).get認定調査員コード() == null ? RString.EMPTY
...
//        }
    ...
    private ChosaInputCsvEntity getChosaCsvData_99A(NinteichosaIraiBusiness 調査入力用データ,
        ...
        return new ChosaInputCsvEntity(
                ...
                調査入力用データ.get認定調査委託先コード() != null ? 調査入力用データ.get認定調査委託先コード().value() : RString.EMPTY,
                調査入力用データ.get調査委託先(),
    ...
    private ChosainInfoMobileCsvEntity getChosainInfoCsvData(NinteichosaIraiChosainBusiness 調査結果入力用調査員データ) {
        RString 調査委託区分;
        try {
            調査委託区分 = RString.isNullOrEmpty(調査結果入力用調査員データ.get調査委託区分())
                    ? RString.EMPTY : ChosaItakuKubunCode.toValue(調査結果入力用調査員データ.get調査委託区分()).get名称();
        } catch (IllegalArgumentException ex) {
            調査委託区分 = RString.EMPTY;
        }
        RString 地区略称 = 調査結果入力用調査員データ.get地区コード() != null
        ...
        RString 調査員;
        try {
            調査員 = RString.isNullOrEmpty(調査結果入力用調査員データ.get調査員資格()) ? RString.EMPTY
