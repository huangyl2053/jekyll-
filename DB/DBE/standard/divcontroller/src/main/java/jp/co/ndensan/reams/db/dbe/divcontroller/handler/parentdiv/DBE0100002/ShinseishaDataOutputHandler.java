 ...
 * @author N3212 竹内 和紀
 */
public class ShinseishaDataOutputHandler {
     ...
     */
    public ShinseishaDataOutputHandler(ShinseishaDataOutputDiv div) {
        this.div = div;
    }
    ...
    public ShinseiKensakuMapperParameter createSearchParameter(RString hihokenshaNo) {
        ...
        editNowPhaseForParameter(parameter);
     ...
     */
    public void set申請一覧データグリッド(SearchResult<ShinseiKensakuBusiness> searchResult) {
        List<dgShinseiJoho_Row> dataSource = new ArrayList<>();
        for (ShinseiKensakuBusiness business : searchResult.records()) {
            dgShinseiJoho_Row row = new dgShinseiJoho_Row();
            row.setHokensha((business.get市町村名称() != null) ? business.get市町村名称() : RString.EMPTY);
            row.setHihokenshaNo((business.get被保険者番号() != null) ? business.get被保険者番号() : RString.EMPTY);
            row.setShimei((business.get被保険者氏名() != null && business.get被保険者氏名().value() != null)
                    ? business.get被保険者氏名().value() : RString.EMPTY);
            row.getHihokenshaBirthDay().setValue((business.get生年月日() != null) ? business.get生年月日() : FlexibleDate.EMPTY);
            row.getHihokenshaNenrei().setValue(new Decimal(business.get年齢()));
            row.setHihokenshaSeibetsu((business.get性別() != null) ? Seibetsu.toValue(business.get性別().value()).get名称() : RString.EMPTY);
            ...
            row.getNijiHanteiNinteiYukoKaishiYMD().setValue((business.get二次判定認定有効開始年月日() != null)
                    ? business.get二次判定認定有効開始年月日() : FlexibleDate.EMPTY);
            row.getNijiHanteiNinteiYukoShuryoYMD().setValue((business.get二次判定認定有効終了年月日() != null)
                    ? business.get二次判定認定有効終了年月日() : FlexibleDate.EMPTY);
            row.setYubinno((business.get郵便番号() != null) ? business.get郵便番号().getEditedYubinNo() : RString.EMPTY);
            row.setTelno((business.get電話番号() != null) ? business.get電話番号().value() : RString.EMPTY);
            row.setJyusho((business.get住所() != null) ? business.get住所().getColumnValue() : RString.EMPTY);
            row.setShinseishoKanriNo(business.get申請書管理番号().value());
            row.setShoKisaiHokenshaNo((business.get証記載保険者番号() != null) ? business.get証記載保険者番号() : RString.EMPTY);
            row.setIkenshoIraiRirekiNo(new RString(business.get主治医意見書作成依頼履歴番号()));
            row.setNinteichosaIraiRirekiNo(new RString(business.get認定調査依頼履歴番号()));
            dataSource.add(row);
        }
        div.getDgShinseiJoho().setDataSource(dataSource);
        div.getDgShinseiJoho().getGridSetting().setLimitRowCount(div.getTxtMaxDisp().getValue().intValue());
    ...
    private void editNowPhaseForParameter(ShinseiKensakuMapperParameter parameter) {
    ...
    private RString get二次判定結果(ShinseiKensakuBusiness business) {
        if (KoroshoIfShikibetsuCode.認定ｿﾌﾄ2009_SP3.asCode().equals(business.get厚労省IF識別コード())
                || KoroshoIfShikibetsuCode.認定ｿﾌﾄ2009.asCode().equals(business.get厚労省IF識別コード())) {
            return YokaigoJotaiKubun09.toValue(business.get二次判定要介護状態区分コード().value()).get名称();
        } else if (KoroshoIfShikibetsuCode.認定ｿﾌﾄ2006_新要介護認定適用区分が未適用.asCode().equals(business.get厚労省IF識別コード())) {
            return YokaigoJotaiKubun06.toValue(business.get二次判定要介護状態区分コード().value()).get名称();
        } else if (KoroshoIfShikibetsuCode.認定ｿﾌﾄ2002.asCode().equals(business.get厚労省IF識別コード())) {
            return YokaigoJotaiKubun02.toValue(business.get二次判定要介護状態区分コード().value()).get名称();
        } else if (KoroshoIfShikibetsuCode.認定ｿﾌﾄ99.asCode().equals(business.get厚労省IF識別コード())) {
            return YokaigoJotaiKubun99.toValue(business.get二次判定要介護状態区分コード().value()).get名称();
        }
        return RString.EMPTY;
    }
}
