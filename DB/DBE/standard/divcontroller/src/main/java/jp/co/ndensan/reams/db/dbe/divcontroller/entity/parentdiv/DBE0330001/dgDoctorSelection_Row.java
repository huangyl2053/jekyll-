package jp.co.ndensan.reams.db.dbe.divcontroller.entity.parentdiv.DBE0330001;
/*
 * このコードはツールによって生成されました。
 * このファイルへの変更は、以下の状況下で不正な動作の原因になったり、
 * コードが再生成されるときに損失したりします。
 * Wed Dec 28 09:44:08 JST 2016 
 */



import jp.co.ndensan.reams.uz.uza.ui.binding.DataRow;
import jp.co.ndensan.reams.uz.uza.lang.RString;


/**
 * dgDoctorSelection_Row のクラスファイル 
 * 
 * @author 自動生成
 */

public class dgDoctorSelection_Row extends DataRow {
    // <editor-fold defaultstate="collapsed" desc="Created By UIDesigner ver：UZ-deploy-2016-12-16_10-54-17">

    private RString shujiiIryokikanCode;
    private RString shujiiIryokikanName;
    private RString yubinNo;
    private RString tel;
    private RString jusho;
    private RString doctorCode;
    private RString doctorName;
    private RString count;
    private RString shinseishoKanriNo;
    private RString shichosonCode;

    public dgDoctorSelection_Row() {
        super();
        this.shujiiIryokikanCode = RString.EMPTY;
        this.shujiiIryokikanName = RString.EMPTY;
        this.yubinNo = RString.EMPTY;
        this.tel = RString.EMPTY;
        this.jusho = RString.EMPTY;
        this.doctorCode = RString.EMPTY;
        this.doctorName = RString.EMPTY;
        this.count = RString.EMPTY;
        this.shinseishoKanriNo = RString.EMPTY;
        this.shichosonCode = RString.EMPTY;
        this.setOriginalData("shujiiIryokikanCode", shujiiIryokikanCode);
        this.setOriginalData("shujiiIryokikanName", shujiiIryokikanName);
        this.setOriginalData("yubinNo", yubinNo);
        this.setOriginalData("tel", tel);
        this.setOriginalData("jusho", jusho);
        this.setOriginalData("doctorCode", doctorCode);
        this.setOriginalData("doctorName", doctorName);
        this.setOriginalData("count", count);
        this.setOriginalData("shinseishoKanriNo", shinseishoKanriNo);
        this.setOriginalData("shichosonCode", shichosonCode);
    }

    public dgDoctorSelection_Row(RString shujiiIryokikanCode, RString shujiiIryokikanName, RString yubinNo, RString tel, RString jusho, RString doctorCode, RString doctorName, RString count, RString shinseishoKanriNo, RString shichosonCode) {
        super();
        this.setOriginalData("shujiiIryokikanCode", shujiiIryokikanCode);
        this.setOriginalData("shujiiIryokikanName", shujiiIryokikanName);
        this.setOriginalData("yubinNo", yubinNo);
        this.setOriginalData("tel", tel);
        this.setOriginalData("jusho", jusho);
        this.setOriginalData("doctorCode", doctorCode);
        this.setOriginalData("doctorName", doctorName);
        this.setOriginalData("count", count);
        this.setOriginalData("shinseishoKanriNo", shinseishoKanriNo);
        this.setOriginalData("shichosonCode", shichosonCode);
        this.shujiiIryokikanCode = shujiiIryokikanCode;
        this.shujiiIryokikanName = shujiiIryokikanName;
        this.yubinNo = yubinNo;
        this.tel = tel;
        this.jusho = jusho;
        this.doctorCode = doctorCode;
        this.doctorName = doctorName;
        this.count = count;
        this.shinseishoKanriNo = shinseishoKanriNo;
        this.shichosonCode = shichosonCode;
    }

    public RString getShujiiIryokikanCode() {
        return shujiiIryokikanCode;
    }

    public RString getShujiiIryokikanName() {
        return shujiiIryokikanName;
    }

    public RString getYubinNo() {
        return yubinNo;
    }

    public RString getTel() {
        return tel;
    }

    public RString getJusho() {
        return jusho;
    }

    public RString getDoctorCode() {
        return doctorCode;
    }

    public RString getDoctorName() {
        return doctorName;
    }

    public RString getCount() {
        return count;
    }

    public RString getShinseishoKanriNo() {
        return shinseishoKanriNo;
    }

    public RString getShichosonCode() {
        return shichosonCode;
    }

    public void setShujiiIryokikanCode(RString shujiiIryokikanCode) {
        this.setOriginalData("shujiiIryokikanCode", shujiiIryokikanCode);
        this.shujiiIryokikanCode = shujiiIryokikanCode;
    }

    public void setShujiiIryokikanName(RString shujiiIryokikanName) {
        this.setOriginalData("shujiiIryokikanName", shujiiIryokikanName);
        this.shujiiIryokikanName = shujiiIryokikanName;
    }

    public void setYubinNo(RString yubinNo) {
        this.setOriginalData("yubinNo", yubinNo);
        this.yubinNo = yubinNo;
    }

    public void setTel(RString tel) {
        this.setOriginalData("tel", tel);
        this.tel = tel;
    }

    public void setJusho(RString jusho) {
        this.setOriginalData("jusho", jusho);
        this.jusho = jusho;
    }

    public void setDoctorCode(RString doctorCode) {
        this.setOriginalData("doctorCode", doctorCode);
        this.doctorCode = doctorCode;
    }

    public void setDoctorName(RString doctorName) {
        this.setOriginalData("doctorName", doctorName);
        this.doctorName = doctorName;
    }

    public void setCount(RString count) {
        this.setOriginalData("count", count);
        this.count = count;
    }

    public void setShinseishoKanriNo(RString shinseishoKanriNo) {
        this.setOriginalData("shinseishoKanriNo", shinseishoKanriNo);
        this.shinseishoKanriNo = shinseishoKanriNo;
    }

    public void setShichosonCode(RString shichosonCode) {
        this.setOriginalData("shichosonCode", shichosonCode);
        this.shichosonCode = shichosonCode;
    }

    // </editor-fold>
}
