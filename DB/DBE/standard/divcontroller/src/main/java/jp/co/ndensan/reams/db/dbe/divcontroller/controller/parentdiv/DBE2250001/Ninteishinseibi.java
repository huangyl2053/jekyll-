...
import jp.co.ndensan.reams.db.dbe.divcontroller.entity.parentdiv.DBE2250001.NinteichosaIchiranErrorListCsvEntity;
...
public class Ninteishinseibi {
    ...
    private static final RString エラーリスト_EUCENTITYID = new RString("DBE250002");
    ...
    public IDownLoadServletResponse onClick_btnErrorListOutput(NinteishinseibiDiv div, IDownLoadServletResponse response) {
        ...
        try (CsvWriter<NinteichosaIchiranErrorListCsvEntity> csvWriter
                ...
                .hasHeader(true)
                .build()) {
            for (dgNinteiChosaData_Row row : getHandler(div).getNG行()) {
                csvWriter.writeLine(editErrorListCsvEntity(row));
                personalDataList.add(toPersonalData(row));
            }
            csvWriter.close();
        }
        SharedFileDescriptor descriptor = new SharedFileDescriptor(GyomuCode.DB介護保険, FilesystemName.fromString(出力名));
        descriptor = SharedFile.defineSharedFile(descriptor);
        CopyToSharedFileOpts opts = new CopyToSharedFileOpts().isCompressedArchive(false);
        SharedFile.copyToSharedFile(descriptor, new FilesystemPath(filePath), opts);
    ...
    private PersonalData toPersonalData(dgNinteiChosaData_Row row) {
        ShikibetsuCode shikibetsuCode = new ShikibetsuCode(row.getShokisaiHokenshaNo().substring(0, 5).concat(row.getHihokenshaNo().getValue()));
        ExpandedInformation expandedInformation = new ExpandedInformation(new Code("0001"), new RString("申請書管理番号"), row.getShinseishoKanriNo());
            ...
            for (dgNinteiChosaData_Row row : dataSource) {
                if (状態_変更.equals(row.getJyotai()) || 状態_警告.equals(row.getJyotai())) {
                    ...
                    NinteiChosaDataTorikomiManager.createInstance().save認定調査データ(
                            ...
                            get基本調査(row),
                            get基本調査調査項目(row),
                            get概況調査サービスの状況の登録処理(row),
                            get概況調査サービスの状況フラグの登録処理(row),
                            get概況調査記入項目の登録処理(row),
                            get概況調査施設利用の登録処理(row));
                }
            }
    ...
    private NinteichosahyoGaikyoChosa get概況調査(dgNinteiChosaData_Row row) {
        NinteichosahyoGaikyoChosaBuilder builder
                = new NinteichosahyoGaikyoChosa(
    ...
    private NinteichosahyoKihonChosa get基本調査(dgNinteiChosaData_Row row) {
        NinteichosahyoKihonChosaBuilder builder
                = new NinteichosahyoKihonChosa(
                        new ShinseishoKanriNo(row.getShinseishoKanriNo()),
                        row.getNinteichosaIraiRirekiNo().getValue().intValue())
                .createBuilderForEdit();
        builder.set申請書管理番号(new ShinseishoKanriNo(row.getShinseishoKanriNo()));
    ...
    private List<NinteichosahyoChosaItem> get基本調査調査項目(dgNinteiChosaData_Row row) {
    ...
    private List<NinteichosahyoServiceJokyo> get概況調査サービスの状況の登録処理(dgNinteiChosaData_Row row) {
    ...
    private List<NinteichosahyoServiceJokyoFlag> get概況調査サービスの状況フラグの登録処理(dgNinteiChosaData_Row row) {
    ...
    private List<NinteichosahyoKinyuItem> get概況調査記入項目の登録処理(dgNinteiChosaData_Row row) {
    ...
    private List<NinteichosahyoShisetsuRiyo> get概況調査施設利用の登録処理(dgNinteiChosaData_Row row) {
    ...
    private NinteichosaIchiranErrorListCsvEntity editErrorListCsvEntity(dgNinteiChosaData_Row row) {
        return new NinteichosaIchiranErrorListCsvEntity(
                row.getHokenja(),
                format日付(row.getNinteiShinseiYMD().getValue()),
                row.getHihokenshaNo().getValue(),
                row.getHihokenshaName(),
                row.getNinteiShinseiShinseijiKubunCode(),
                format日付(row.getNinteichosaIraiKanryoYMD().getValue()),
                row.getNinteiChosaItakusakiCode(),
                row.getNinteiChosaItakusakimei(),
                row.getNinteiChosainCode(),
                row.getNinteiChosainName(),
                format日付(row.getNinteichosaJisshiYMD().getValue()),
                row.getShinseishoKanriNo(),
                new RString(row.getNinteichosaIraiRirekiNo().getValue().toString()),
                row.getErrorJiyu());
    }
