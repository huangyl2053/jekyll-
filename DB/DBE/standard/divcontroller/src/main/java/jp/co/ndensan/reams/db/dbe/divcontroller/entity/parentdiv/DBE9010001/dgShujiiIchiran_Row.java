package jp.co.ndensan.reams.db.dbe.divcontroller.entity.parentdiv.DBE9010001;
/*
 * このコードはツールによって生成されました。
 * このファイルへの変更は、以下の状況下で不正な動作の原因になったり、
 * コードが再生成されるときに損失したりします。
 * Fri Jan 08 17:22:53 CST 2016 
 */



import jp.co.ndensan.reams.uz.uza.ui.binding.DataRow;
import jp.co.ndensan.reams.uz.uza.lang.RString;
import jp.co.ndensan.reams.uz.uza.ui.binding.TextBoxCode;


/**
 * dgShujiiIchiran_Row のクラスファイル 
 * 
 * @author 自動生成
 */

public class dgShujiiIchiran_Row extends DataRow {
    // <editor-fold defaultstate="collapsed" desc="Created By UIDesigner ver：バージョン情報無し">

    private RString jotai;
    private RString shichoson;
    private TextBoxCode shujiiIryoKikanCode;
    private RString iryoKikanCode;
    private RString shujiiIryoKikan;
    private RString shujiiIryoKikankana;
    private RString yubinNo;
    private RString jusho;
    private RString telNo;
    private RString faxNo;
    private RString daihyosha;
    private RString daihyoshakana;
    private RString jokyoFlag;
    private RString shichosonCode;

    public dgShujiiIchiran_Row() {
        super();
        this.jotai = RString.EMPTY;
        this.shichoson = RString.EMPTY;
        this.shujiiIryoKikanCode = new TextBoxCode();
        this.iryoKikanCode = RString.EMPTY;
        this.shujiiIryoKikan = RString.EMPTY;
        this.shujiiIryoKikankana = RString.EMPTY;
        this.yubinNo = RString.EMPTY;
        this.jusho = RString.EMPTY;
        this.telNo = RString.EMPTY;
        this.faxNo = RString.EMPTY;
        this.daihyosha = RString.EMPTY;
        this.daihyoshakana = RString.EMPTY;
        this.jokyoFlag = RString.EMPTY;
        this.shichosonCode = RString.EMPTY;
        this.setOriginalData("jotai", jotai);
        this.setOriginalData("shichoson", shichoson);
        this.setOriginalData("shujiiIryoKikanCode", shujiiIryoKikanCode);
        this.setOriginalData("iryoKikanCode", iryoKikanCode);
        this.setOriginalData("shujiiIryoKikan", shujiiIryoKikan);
        this.setOriginalData("shujiiIryoKikankana", shujiiIryoKikankana);
        this.setOriginalData("yubinNo", yubinNo);
        this.setOriginalData("jusho", jusho);
        this.setOriginalData("telNo", telNo);
        this.setOriginalData("faxNo", faxNo);
        this.setOriginalData("daihyosha", daihyosha);
        this.setOriginalData("daihyoshakana", daihyoshakana);
        this.setOriginalData("jokyoFlag", jokyoFlag);
        this.setOriginalData("shichosonCode", shichosonCode);
    }

    public dgShujiiIchiran_Row(RString jotai, RString shichoson, TextBoxCode shujiiIryoKikanCode, RString iryoKikanCode, RString shujiiIryoKikan, RString shujiiIryoKikankana, RString yubinNo, RString jusho, RString telNo, RString faxNo, RString daihyosha, RString daihyoshakana, RString jokyoFlag, RString shichosonCode) {
        super();
        this.setOriginalData("jotai", jotai);
        this.setOriginalData("shichoson", shichoson);
        this.setOriginalData("shujiiIryoKikanCode", shujiiIryoKikanCode);
        this.setOriginalData("iryoKikanCode", iryoKikanCode);
        this.setOriginalData("shujiiIryoKikan", shujiiIryoKikan);
        this.setOriginalData("shujiiIryoKikankana", shujiiIryoKikankana);
        this.setOriginalData("yubinNo", yubinNo);
        this.setOriginalData("jusho", jusho);
        this.setOriginalData("telNo", telNo);
        this.setOriginalData("faxNo", faxNo);
        this.setOriginalData("daihyosha", daihyosha);
        this.setOriginalData("daihyoshakana", daihyoshakana);
        this.setOriginalData("jokyoFlag", jokyoFlag);
        this.setOriginalData("shichosonCode", shichosonCode);
        this.jotai = jotai;
        this.shichoson = shichoson;
        this.shujiiIryoKikanCode = shujiiIryoKikanCode;
        this.iryoKikanCode = iryoKikanCode;
        this.shujiiIryoKikan = shujiiIryoKikan;
        this.shujiiIryoKikankana = shujiiIryoKikankana;
        this.yubinNo = yubinNo;
        this.jusho = jusho;
        this.telNo = telNo;
        this.faxNo = faxNo;
        this.daihyosha = daihyosha;
        this.daihyoshakana = daihyoshakana;
        this.jokyoFlag = jokyoFlag;
        this.shichosonCode = shichosonCode;
    }

    public RString getJotai() {
        return jotai;
    }

    public RString getShichoson() {
        return shichoson;
    }

    public TextBoxCode getShujiiIryoKikanCode() {
        return shujiiIryoKikanCode;
    }

    public RString getIryoKikanCode() {
        return iryoKikanCode;
    }

    public RString getShujiiIryoKikan() {
        return shujiiIryoKikan;
    }

    public RString getShujiiIryoKikankana() {
        return shujiiIryoKikankana;
    }

    public RString getYubinNo() {
        return yubinNo;
    }

    public RString getJusho() {
        return jusho;
    }

    public RString getTelNo() {
        return telNo;
    }

    public RString getFaxNo() {
        return faxNo;
    }

    public RString getDaihyosha() {
        return daihyosha;
    }

    public RString getDaihyoshakana() {
        return daihyoshakana;
    }

    public RString getJokyoFlag() {
        return jokyoFlag;
    }

    public RString getShichosonCode() {
        return shichosonCode;
    }

    public void setJotai(RString jotai) {
        this.setOriginalData("jotai", jotai);
        this.jotai = jotai;
    }

    public void setShichoson(RString shichoson) {
        this.setOriginalData("shichoson", shichoson);
        this.shichoson = shichoson;
    }

    public void setShujiiIryoKikanCode(TextBoxCode shujiiIryoKikanCode) {
        this.setOriginalData("shujiiIryoKikanCode", shujiiIryoKikanCode);
        this.shujiiIryoKikanCode = shujiiIryoKikanCode;
    }

    public void setIryoKikanCode(RString iryoKikanCode) {
        this.setOriginalData("iryoKikanCode", iryoKikanCode);
        this.iryoKikanCode = iryoKikanCode;
    }

    public void setShujiiIryoKikan(RString shujiiIryoKikan) {
        this.setOriginalData("shujiiIryoKikan", shujiiIryoKikan);
        this.shujiiIryoKikan = shujiiIryoKikan;
    }

    public void setShujiiIryoKikankana(RString shujiiIryoKikankana) {
        this.setOriginalData("shujiiIryoKikankana", shujiiIryoKikankana);
        this.shujiiIryoKikankana = shujiiIryoKikankana;
    }

    public void setYubinNo(RString yubinNo) {
        this.setOriginalData("yubinNo", yubinNo);
        this.yubinNo = yubinNo;
    }

    public void setJusho(RString jusho) {
        this.setOriginalData("jusho", jusho);
        this.jusho = jusho;
    }

    public void setTelNo(RString telNo) {
        this.setOriginalData("telNo", telNo);
        this.telNo = telNo;
    }

    public void setFaxNo(RString faxNo) {
        this.setOriginalData("faxNo", faxNo);
        this.faxNo = faxNo;
    }

    public void setDaihyosha(RString daihyosha) {
        this.setOriginalData("daihyosha", daihyosha);
        this.daihyosha = daihyosha;
    }

    public void setDaihyoshakana(RString daihyoshakana) {
        this.setOriginalData("daihyoshakana", daihyoshakana);
        this.daihyoshakana = daihyoshakana;
    }

    public void setJokyoFlag(RString jokyoFlag) {
        this.setOriginalData("jokyoFlag", jokyoFlag);
        this.jokyoFlag = jokyoFlag;
    }

    public void setShichosonCode(RString shichosonCode) {
        this.setOriginalData("shichosonCode", shichosonCode);
        this.shichosonCode = shichosonCode;
    }

    // </editor-fold>
}
