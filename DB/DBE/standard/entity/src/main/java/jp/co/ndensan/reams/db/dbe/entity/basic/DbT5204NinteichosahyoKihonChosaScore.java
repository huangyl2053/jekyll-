package jp.co.ndensan.reams.db.dbe.entity.basic;

import jp.co.ndensan.reams.uz.uza.util.db.IColumnDefinition;

/**
 * 認定調査票（基本調査素点）テーブルの項目定義クラスです。
 */
public enum DbT5204NinteichosahyoKihonChosaScore implements IColumnDefinition {
// <editor-fold defaultstate="collapsed" desc="Created By POJO Tool ver 1.3.9">
    /**
     * insertDantaiCd
     */
    insertDantaiCd(2147483647, 0),
    /**
     * insertTimestamp
     */
    insertTimestamp(29, 0),
    /**
     * insertReamsLoginId
     */
    insertReamsLoginId(2147483647, 0),
    /**
     * insertContextId
     */
    insertContextId(2147483647, 0),
    /**
     * isDeleted
     */
    isDeleted(1, 0),
    /**
     * updateCount
     */
    updateCount(10, 0),
    /**
     * lastUpdateTimestamp
     */
    lastUpdateTimestamp(29, 0),
    /**
     * lastUpdateReamsLoginId
     */
    lastUpdateReamsLoginId(2147483647, 0),
    /**
     * 申請書管理番号
     * <br/>要介護認定申請情報
     */
    shinseishoKanriNo(2147483647, 0),
    /**
     * 要介護認定調査履歴番号
     * <br/>認定調査依頼情報
     */
    ninteichosaRirekiNo(5, 0),
    /**
     * 厚労省IF識別コード
     * <br/>Enum（DBE：厚労省認定ソフトのバージョン）
     */
    koroshoIfShikibetsuCode(2147483647, 0),
    /**
     * 素点項目01
     */
    reserchItem01(5, 0),
    /**
     * 素点項目02
     */
    reserchItem02(5, 0),
    /**
     * 素点項目03
     */
    reserchItem03(5, 0),
    /**
     * 素点項目04
     */
    reserchItem04(5, 0),
    /**
     * 素点項目05
     */
    reserchItem05(5, 0),
    /**
     * 素点項目06
     */
    reserchItem06(5, 0),
    /**
     * 素点項目07
     */
    reserchItem07(5, 0),
    /**
     * 素点項目08
     */
    reserchItem08(5, 0),
    /**
     * 素点項目09
     */
    reserchItem09(5, 0),
    /**
     * 素点項目10
     */
    reserchItem10(5, 0),
    /**
     * 素点項目11
     */
    reserchItem11(5, 0),
    /**
     * 素点項目12
     */
    reserchItem12(5, 0),
    /**
     * 素点項目13
     */
    reserchItem13(5, 0),
    /**
     * 素点項目14
     */
    reserchItem14(5, 0),
    /**
     * 素点項目15
     */
    reserchItem15(5, 0),
    /**
     * 素点項目16
     */
    reserchItem16(5, 0),
    /**
     * 素点項目17
     */
    reserchItem17(5, 0),
    /**
     * 素点項目18
     */
    reserchItem18(5, 0),
    /**
     * 素点項目19
     */
    reserchItem19(5, 0),
    /**
     * 素点項目20
     */
    reserchItem20(5, 0),
    /**
     * 素点項目21
     */
    reserchItem21(5, 0),
    /**
     * 素点項目22
     */
    reserchItem22(5, 0),
    /**
     * 素点項目23
     */
    reserchItem23(5, 0),
    /**
     * 素点項目24
     */
    reserchItem24(5, 0),
    /**
     * 素点項目25
     */
    reserchItem25(5, 0),
    /**
     * 素点項目26
     */
    reserchItem26(5, 0),
    /**
     * 素点項目27
     */
    reserchItem27(5, 0),
    /**
     * 素点項目28
     */
    reserchItem28(5, 0),
    /**
     * 素点項目29
     */
    reserchItem29(5, 0),
    /**
     * 素点項目30
     */
    reserchItem30(5, 0),
    /**
     * 素点項目31
     */
    reserchItem31(5, 0),
    /**
     * 素点項目32
     */
    reserchItem32(5, 0),
    /**
     * 素点項目33
     */
    reserchItem33(5, 0),
    /**
     * 素点項目34
     */
    reserchItem34(5, 0),
    /**
     * 素点項目35
     */
    reserchItem35(5, 0),
    /**
     * 素点項目36
     */
    reserchItem36(5, 0),
    /**
     * 素点項目37
     */
    reserchItem37(5, 0),
    /**
     * 素点項目38
     */
    reserchItem38(5, 0),
    /**
     * 素点項目39
     */
    reserchItem39(5, 0),
    /**
     * 素点項目40
     */
    reserchItem40(5, 0),
    /**
     * 素点項目41
     */
    reserchItem41(5, 0),
    /**
     * 素点項目42
     */
    reserchItem42(5, 0),
    /**
     * 素点項目43
     */
    reserchItem43(5, 0),
    /**
     * 素点項目44
     */
    reserchItem44(5, 0),
    /**
     * 素点項目45
     */
    reserchItem45(5, 0),
    /**
     * 素点項目46
     */
    reserchItem46(5, 0),
    /**
     * 素点項目47
     */
    reserchItem47(5, 0),
    /**
     * 素点項目48
     */
    reserchItem48(5, 0),
    /**
     * 素点項目49
     */
    reserchItem49(5, 0),
    /**
     * 素点項目50
     */
    reserchItem50(5, 0),
    /**
     * 素点項目51
     */
    reserchItem51(5, 0),
    /**
     * 素点項目52
     */
    reserchItem52(5, 0),
    /**
     * 素点項目53
     */
    reserchItem53(5, 0),
    /**
     * 素点項目54
     */
    reserchItem54(5, 0),
    /**
     * 素点項目55
     */
    reserchItem55(5, 0),
    /**
     * 素点項目56
     */
    reserchItem56(5, 0),
    /**
     * 素点項目57
     */
    reserchItem57(5, 0),
    /**
     * 素点項目58
     */
    reserchItem58(5, 0),
    /**
     * 素点項目59
     */
    reserchItem59(5, 0),
    /**
     * 素点項目60
     */
    reserchItem60(5, 0),
    /**
     * 素点項目61
     */
    reserchItem61(5, 0),
    /**
     * 素点項目62
     */
    reserchItem62(5, 0),
    /**
     * 素点項目63
     */
    reserchItem63(5, 0),
    /**
     * 素点項目64
     */
    reserchItem64(5, 0),
    /**
     * 素点項目65
     */
    reserchItem65(5, 0),
    /**
     * 素点項目66
     */
    reserchItem66(5, 0),
    /**
     * 素点項目67
     */
    reserchItem67(5, 0),
    /**
     * 素点項目68
     */
    reserchItem68(5, 0),
    /**
     * 素点項目69
     */
    reserchItem69(5, 0),
    /**
     * 素点項目70
     */
    reserchItem70(5, 0),
    /**
     * 素点項目71
     */
    reserchItem71(5, 0),
    /**
     * 素点項目72
     */
    reserchItem72(5, 0),
    /**
     * 素点項目73
     */
    reserchItem73(5, 0),
    /**
     * 素点項目74
     */
    reserchItem74(5, 0),
    /**
     * 素点項目75
     */
    reserchItem75(5, 0),
    /**
     * 素点項目76
     */
    reserchItem76(5, 0),
    /**
     * 素点項目77
     */
    reserchItem77(5, 0),
    /**
     * 素点項目78
     */
    reserchItem78(5, 0),
    /**
     * 素点項目79
     */
    reserchItem79(5, 0),
    /**
     * 素点項目80
     */
    reserchItem80(5, 0),
    /**
     * 素点項目81
     */
    reserchItem81(5, 0),
    /**
     * 素点項目82
     */
    reserchItem82(5, 0),
    /**
     * 素点項目83
     */
    reserchItem83(5, 0),
    /**
     * 素点項目84
     */
    reserchItem84(5, 0),
    /**
     * 素点項目85
     */
    reserchItem85(5, 0),
    /**
     * 素点項目86
     */
    reserchItem86(5, 0),
    /**
     * 素点項目87
     */
    reserchItem87(5, 0),
    /**
     * 素点項目88
     */
    reserchItem88(5, 0),
    /**
     * 素点項目89
     */
    reserchItem89(5, 0),
    /**
     * 素点項目90
     */
    reserchItem90(5, 0),
    /**
     * 素点項目91
     */
    reserchItem91(5, 0),
    /**
     * 素点項目92
     */
    reserchItem92(5, 0),
    /**
     * 素点項目93
     */
    reserchItem93(5, 0),
    /**
     * 素点項目94
     */
    reserchItem94(5, 0),
    /**
     * 素点項目95
     */
    reserchItem95(5, 0),
    /**
     * 素点項目96
     */
    reserchItem96(5, 0),
    /**
     * 素点項目97
     */
    reserchItem97(5, 0),
    /**
     * 素点項目98
     */
    reserchItem98(5, 0),
    /**
     * 素点合計　第１郡
     */
    tokutenTotal1gun(5, 0),
    /**
     * 素点合計　第２郡
     */
    tokutenTotal2gun(5, 0),
    /**
     * 素点合計　第３郡
     */
    tokutenTotal3gun(5, 0),
    /**
     * 素点合計　第４郡
     */
    tokutenTotal4gun(5, 0),
    /**
     * 素点合計　第５郡
     */
    tokutenTotal5gun(5, 0),
    /**
     * 素点合計　第６郡
     */
    tokutenTotal6gun(5, 0),
    /**
     * 素点合計　第７郡
     */
    tokutenTotal7gun(5, 0);

    private final int maxLength;
    private final int scale;

    private DbT5204NinteichosahyoKihonChosaScore(int maxLength, int scale) {
        this.maxLength = maxLength;
        this.scale = scale;
    }

    /**
     * 項目の最大長のgetメソッドです。
     * 
     * @return 項目の最大長
     */
    public int getMaxLength() {
        return maxLength;
    }

    /**
     * 小数点以下の桁数のgetメソッドです。
     * 
     * @return 小数点以下の桁数
     */
    public int getScale() {
        return scale;
    }

// </editor-fold>
}
