...
public class KaigoNinteiChosainMapperTest extends TestBase {
    ...
    public static class toKaigoNinteiChosain extends TestBase {
        ...
        @Override
        public void setUp() {
            sut = KaigoNinteiChosainMapper.toKaigoNinteiChosain(create認定調査員Entity());
        }

        @Test(expected = NullPointerException.class)
        public void 引数にnullがわたったとき_NullpointerExceptionが発生する() {
            sut = KaigoNinteiChosainMapper.toKaigoNinteiChosain(null);
        }

        @Test
        public void entityから渡された市町村コードと_Mapping後の認定調査員が返す市町村コードが_同一になる() {
            assertThat(sut.get市町村コード(), is(市町村コード));
        }

        @Test
        public void entityから渡された介護事業者番号と_Mapping後の認定調査員が返す介護事業者番号が_同一になる() {
            assertThat(sut.get介護事業者番号(), is(介護事業者番号));
        }

        @Test
        public void entityから渡された介護調査員番号と_Mapping後の認定調査員が返す介護調査員番号が_同一になる() {
            assertThat(sut.get介護調査員番号(), is(介護調査員番号));
        }

        @Test
        public void entityから渡された事業者番号と_Mapping後の認定調査員が返す事業者番号が_同一になる() {
            assertThat(sut.get事業者番号(), is(事業者番号));
        }

        @Test
        public void entityから渡された調査員状況と_Mapping後の認定調査員が返す調査員状況が_同一になる() {
            assertThat(sut.is調査員状況(), is(調査員状況));
        }

        @Test
        public void entityから渡された調査員氏名と_Mapping後の認定調査員が返す調査員氏名が_同一になる() {
            assertThat(sut.get調査員氏名(), is(調査員氏名));
        }

        @Test
        public void entityから渡された調査員氏名カナと_Mapping後の認定調査員が返す調査員氏名カナが_同一になる() {
            assertThat(sut.get調査員氏名カナ(), is(調査員氏名カナ));
        }

        @Test
        public void entityから渡された性別と_Mapping後の認定調査員が返す性別が_同一になる() {
            assertThat(sut.get性別(), is(性別));
        }

        @Test
        public void entityから渡された調査員資格コードと_Mapping後の認定調査員が返す調査員資格コードが_同一になる() {
            assertThat(sut.get調査員資格コード(), is(調査員資格コード));
        }

        @Test
        public void entityから渡された地区コードと_Mapping後の認定調査員が返す地区コードが_同一になる() {
            assertThat(sut.get地区コード(), is(地区コード));
        }

        @Test
        public void entityから渡された郵便番号と_Mapping後の認定調査員が返す郵便番号が_同一になる() {
            assertThat(sut.get郵便番号(), is(郵便番号));
        }

        @Test
        public void entityから渡された住所と_Mapping後の認定調査員が返す住所が_同一になる() {
            assertThat(sut.get住所(), is(住所));
        }

        @Test
        public void entityから渡された電話番号と_Mapping後の認定調査員が返す電話番号が_同一になる() {
