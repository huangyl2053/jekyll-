...
//        throw new IllegalArgumentException(UrErrorMessages.存在しない.getMessage().replace("対応する要介護認定結果異動事由区分").evaluate());
        throw new IllegalArgumentException(UrErrorMessages.存在しない.getMessage().replace("対応する要介護認定結果異動事由区分").getMessage());
    }
}
