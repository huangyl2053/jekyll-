...
public enum DbeQuestionMessages implements IMessageGettable {
    ...
    認定調査完了申請者ですが割付解除(1, "認定調査完了申請者ですが、割付解除を行いますか？"),
    をクリアします(2, "?をクリアします。よろしいですか。？"),
    ...
    すでに予約済みです上書保存(8, "すでに予約済みです、上書保存して、よろしいですか。？"),
    二次判定結果が決定していませんが印刷(9, "二次判定結果が決定していませんが、印刷しますか？"),
    予約を上書保存(10, "予約を上書保存して、よろしいですか。？"),
    処理日より過去のスケジュールを変更(12, "処理日より過去のスケジュールを変更します。よろしいですか？"),
    対象データが多いので処理時間が掛ります(13, "対象データが多いので、処理時間が掛りますがよろしいですか？");
