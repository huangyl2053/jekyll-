...
import jp.co.ndensan.reams.db.dbe.definition.core.valueobject.JotaiAnteiseiCode;
...
public class JotaiAnteiseiCodeTest {

    private static JotaiAnteiseiCode sut;
    private static JotaiAnteiseiCode sut2;

    public static class コンストラクタのテスト extends DbeTestBase {

        @Test(expected = NullPointerException.class)
        public void 引数にnullが渡されたとき_NullPointerExceptionが発生する() {
            sut = new JotaiAnteiseiCode(null);
        }
    }
    ...
    public static class compareToのテスト extends DbeTestBase {
        ...
        @Before
        public void setUp() {
            sut = new JotaiAnteiseiCode(new Code("20"));
        }

        @Test
        public void 比較対象のほうが大きい場合_0より小さい値が返る() {
            sut2 = new JotaiAnteiseiCode(new Code("21"));
            assertThat(sut.compareTo(sut2) < 0, is(true));
        }

        @Test
        public void 比較対象と同値の場合_0が返る() {
            sut2 = new JotaiAnteiseiCode(new Code("20"));
            assertThat(sut.compareTo(sut2) == 0, is(true));
        }

        @Test
        public void 比較対象のほうが小さい場合_0より大きい値が返る() {
            sut2 = new JotaiAnteiseiCode(new Code("19"));
