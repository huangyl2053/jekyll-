 ...
 * @author LDNS
 */
public class IsExistIkenshoMemoTest extends DbeTestBase {

    @Test
    public void 引数にTrueを指定した場合_toValueは_であるを返す() {
        assertThat(IsExistIkenshoMemo.toValue(true), is(IsExistIkenshoMemo.である));
    }

    @Test
    public void 引数にFalseを指定した場合_toValueは_でないを返す() {
        assertThat(IsExistIkenshoMemo.toValue(false), is(IsExistIkenshoMemo.でない));
    }
    ...
    public void であるを指定した場合_get名称は_であるを返す() {
        assertThat(IsExistIkenshoMemo.である.get名称(), is(new RString("である")));
    }

    @Test
    public void でないを指定した場合_get名称は_でないを返す() {
        assertThat(IsExistIkenshoMemo.でない.get名称(), is(new RString("でない")));
    }
