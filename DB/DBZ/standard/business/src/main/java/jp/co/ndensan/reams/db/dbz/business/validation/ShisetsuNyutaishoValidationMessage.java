...
public enum ShisetsuNyutaishoValidationMessage implements IValidationMessage {
    ...
    private enum _Message implements IMessageGettable {
         ...
         * 終了日（退所日）なし
         */
        終了日設定なし(UrErrorMessages.入力値が不正_追加メッセージあり.getMessage().replace("前履歴の退所日が未設定")),
        /**
         * 入所日
         */
        入所日が未入力(UrErrorMessages.必須項目_追加メッセージあり.getMessage().replace("入所日")),
        /**
         * 入所施設
         */
        入所施設コードが未入力(UrErrorMessages.必須項目_追加メッセージあり.getMessage().replace("入所施設情報コード")),
        /**
         * 入所施設
         */
        入所施設名称が未入力(UrErrorMessages.必須項目_追加メッセージあり.getMessage().replace("入所施設情報名称")),
        /**
         * 日付の前後関係が逆転
         */
        入所年月日より前(UrErrorMessages.入力値が不正_追加メッセージあり.getMessage().replace("入所年月日よりも前")),
        /**
         * 退所日と次の履歴データの入所日の期間が重複
         */
        退所日と次の履歴データの入所日の期間が重複(UrErrorMessages.入力値が不正_追加メッセージあり.getMessage().replace("入退所期間が重複")), //NOPMD Enumでチェックする必要性がないためチェックしない
        /**
         * 入所日と前の履歴データの退所日の期間が重複
         */
        入所日と前の履歴データの退所日の期間が重複(UrErrorMessages.入力値が不正_追加メッセージあり.getMessage().replace("入退所期間が重複")), //NOPMD Enumでチェックする必要性がないためチェックしない
        /**
         * 入所日と期間が重複する履歴がある
         */
        入所日と期間が重複する履歴がある(UrErrorMessages.入力値が不正_追加メッセージあり.getMessage().replace("入退所期間が重複")), //NOPMD Enumでチェックする必要性がないためチェックしない
        /**
         * 退所日と期間が重複する履歴がある
         */
