...
public class RirekiShuseiDataPass implements Serializable, Cloneable {
    ...
    public NinteiChosaJokyoDataPass get調査状況用情報() {
        ...
        dataPass.set認定調査依頼年月日(ObjectUtil.defaultIfNull(認定調査依頼年月日, FlexibleDate.EMPTY.toRDate()));
        dataPass.set認定調査予定年月日(ObjectUtil.defaultIfNull(認定調査予定年月日, FlexibleDate.EMPTY.toRDate()));
        dataPass.set認定調査実施年月日(ObjectUtil.defaultIfNull(認定調査実施年月日, FlexibleDate.EMPTY.toRDate()));
        ...
        dataPass.set主治医意見書作成依頼年月日(ObjectUtil.defaultIfNull(主治医意見書作成依頼年月日, FlexibleDate.EMPTY.toRDate()));
        dataPass.set主治医意見書登録予定年月日(ObjectUtil.defaultIfNull(主治医意見書登録予定年月日, FlexibleDate.EMPTY.toRDate()));
        dataPass.set主治医意見書記入年月日(ObjectUtil.defaultIfNull(主治医意見書記入年月日, FlexibleDate.EMPTY.toRDate()));
        dataPass.set要介護認定一次判定結果コード(ObjectUtil.defaultIfNull(要介護認定一次判定結果コード, Code.EMPTY));
        dataPass.set要介護認定一次判定年月日(ObjectUtil.defaultIfNull(要介護認定一次判定年月日, FlexibleDate.EMPTY.toRDate()));
