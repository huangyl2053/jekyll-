...
public class RirekiShuseiDataPass implements Serializable {
    ...
    public NinteiChosaJokyoDataPass get調査状況用情報() {
        ...
        dataPass.set介護認定審査会資料作成年月日(ObjectUtil.defaultIfNull(介護認定審査会資料作成年月日, FlexibleDate.EMPTY));
        dataPass.set認定審査会予定年月日(ObjectUtil.defaultIfNull(認定審査会予定年月日, FlexibleDate.EMPTY));
        dataPass.set二次判定年月日(ObjectUtil.defaultIfNull(二次判定年月日, FlexibleDate.EMPTY));
        ...
        dataPass.set延期通知発行年月日(ObjectUtil.defaultIfNull(延期通知発行年月日, FlexibleDate.EMPTY));
