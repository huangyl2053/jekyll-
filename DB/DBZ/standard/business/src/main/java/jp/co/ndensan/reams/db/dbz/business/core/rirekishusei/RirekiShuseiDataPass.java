...
public class RirekiShuseiDataPass implements Serializable {
    ...
    public NinteiChosaJokyoDataPass get調査状況用情報() {
        ...
        dataPass.set認定調査員コード(認定調査員コード);
        dataPass.set調査員氏名(調査員氏名);
        dataPass.set認定調査委託先コード(認定調査委託先コード);
        dataPass.set事業者名称(事業者名称);
        dataPass.set認定郵便番号(認定郵便番号);
        dataPass.set認定住所(認定住所);
        dataPass.set認定電話番号(認定電話番号);
        dataPass.set認定調査依頼年月日(認定調査依頼年月日);
        dataPass.set認定調査予定年月日(認定調査予定年月日);
        dataPass.set認定調査実施年月日(認定調査実施年月日);
        dataPass.set調査委託区分(調査委託区分);
        dataPass.set訪問調査先名称(訪問調査先名称);
        dataPass.set訪問調査先郵便番号(訪問調査先郵便番号);
        dataPass.set訪問調査先住所(訪問調査先住所);
        dataPass.set訪問調査先電話番号(訪問調査先電話番号);
        dataPass.set主治医コード(主治医コード);
        dataPass.set主治医氏名(主治医氏名);
        dataPass.set主治医医療機関コード(主治医医療機関コード);
        dataPass.set医療機関名称(医療機関名称);
        dataPass.set主治医郵便番号(主治医郵便番号);
        dataPass.set主治医住所(主治医住所);
        dataPass.set主治医電話番号(主治医電話番号);
        dataPass.set主治医意見書作成依頼年月日(主治医意見書作成依頼年月日);
        dataPass.set主治医意見書登録予定年月日(主治医意見書登録予定年月日);
        dataPass.set主治医意見書記入年月日(主治医意見書記入年月日);
        dataPass.set指定医フラグ(指定医フラグ);
        dataPass.set要介護認定一次判定結果コード(要介護認定一次判定結果コード);
        dataPass.set要介護認定一次判定年月日(要介護認定一次判定年月日);
        dataPass.set要介護認定一次判定結果コード_認知症加算(要介護認定一次判定結果コード_認知症加算);
        dataPass.set介護認定審査会資料作成年月日(介護認定審査会資料作成年月日);
        dataPass.set認定審査会予定年月日(認定審査会予定年月日);
        dataPass.set二次判定年月日(二次判定年月日);
        dataPass.set二次判定要介護状態区分コード(二次判定要介護状態区分コード);
        dataPass.set二次判定認定有効期間(二次判定認定有効期間);
        dataPass.set二次判定認定有効開始年月日(二次判定認定有効開始年月日);
        dataPass.set二次判定認定有効終了年月日(二次判定認定有効終了年月日);
        dataPass.set延期通知発行年月日(延期通知発行年月日);
        dataPass.set延期通知発行回数(延期通知発行回数);
        dataPass.set申請書管理番号(申請書管理番号);
        dataPass.set厚労省IF識別コード(厚労省IF識別コード);
        dataPass.set市町村コード(市町村コード);
        return dataPass;
    }
