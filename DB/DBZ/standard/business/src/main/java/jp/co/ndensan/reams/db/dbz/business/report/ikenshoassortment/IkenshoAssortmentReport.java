 ...
 * @author n8438
 */
public final class IkenshoAssortmentReport extends Report<IkenshoAssortmentReportSource> {
     ...
     * @return IkenshoAssortmentReport
     */
    ...
    public void writeBy(ReportSourceWriter<IkenshoAssortmentReportSource> writer) {
        for (IkenshoAssortmentItem item: itemlist) {
            if (isRyomen && writer.pageCount().isOdd()
                    //両面にするレイアウト
                    && (item.getLayout() != IkenshoAssortmentLayout.主治医意見書記入用紙OCR_両面_カラー裏
                    && item.getLayout() != IkenshoAssortmentLayout.主治医意見書記入用紙OCR_片面_カラー裏
                    && item.getLayout() != IkenshoAssortmentLayout.主治医意見書記入用紙_両面_白黒_裏
                    && item.getLayout() != IkenshoAssortmentLayout.主治医意見書記入用紙_片面_白黒_裏
