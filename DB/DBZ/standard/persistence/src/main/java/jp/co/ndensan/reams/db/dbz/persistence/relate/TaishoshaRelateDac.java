...
import static jp.co.ndensan.reams.ua.uax.entity.db.basic.UaFt200FindShikibetsuTaisho.kanaMeisho;
...
public class TaishoshaRelateDac {
    ...
    public IItemList<TaishoshaRelateEntity> select資格対象者(ITrueFalseCriteria 条件, IPsmCriteria psm, boolean is内部結合, int 最大件数) {
        ...
        return to資格EntityList(((条件 != null)
                ...
                .order(by(kanaMeisho))
                .limit(最大件数))
                .toList(TaishoshaRelateEntity.class));
    }
    ...
    public IItemList<FukaTaishoshaRelateEntity> select賦課対象者(ITrueFalseCriteria 条件, IPsmCriteria psm, boolean is内部結合, int 最大件数) {
        ITableClause table = create賦課対象者TableClause(psm, is内部結合);
        return to賦課EntityList(((条件 != null)
                ? table.where(条件)
                .order(by(kanaMeisho), by(hihokenshaNo), by(fukaNendo, Order.DESC), by(tsuchishoNo), by(choteiNendo, Order.DESC))
                .limit(最大件数)
                : table
                .order(by(kanaMeisho), by(hihokenshaNo), by(fukaNendo, Order.DESC), by(tsuchishoNo), by(choteiNendo, Order.DESC))
                .limit(最大件数))
                .toList(FukaTaishoshaRelateEntity.class)
        );
    }
    ...
    private IItemList<TaishoshaRelateEntity> to資格EntityList(List<TaishoshaRelateEntity> entityList) {
    ...
    private IItemList<FukaTaishoshaRelateEntity> to賦課EntityList(List<FukaTaishoshaRelateEntity> entityList) {
