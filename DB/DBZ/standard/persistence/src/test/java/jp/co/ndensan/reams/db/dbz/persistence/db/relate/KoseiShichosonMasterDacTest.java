...
import jp.co.ndensan.reams.db.dbz.persistence.db.relate.KoseiShichosonMasterDac;
...
public class KoseiShichosonMasterDacTest {
    ...
    public static class selectByContainsKyuShichosonTest extends DbzTestDacBase {
        ...
        public void 旧市町村を含まない条件を指定した場合_GappeiKyuShichosonKubunが1であるものを除いたリストを返す() {
            IItemList<DbT7051KoseiShichosonMasterEntity> modelList = sut.selectBy(ContainsKyuShichoson.旧市町村を含まない);
            assertThat(modelList.size(), is(3));
        }

        @Test
        public void 旧市町村を含む条件を指定した場合_全件リストを返す() {
            IItemList<DbT7051KoseiShichosonMasterEntity> modelList = sut.selectBy(ContainsKyuShichoson.旧市町村を含む);
    ...
    public static class selectByContainsKyuShichoson_ShichosonCodeTest extends DbzTestDacBase {
        ...
        public void 市町村コードに合致し旧市町村を含まない条件を指定した場合_該当データを1件返す() {
            IItemList<DbT7051KoseiShichosonMasterEntity> modelList = sut.selectBy(new LasdecCode("223456"), ContainsKyuShichoson.旧市町村を含まない);
            assertThat(modelList.size(), is(1));
            assertThat(modelList.findFirst().get().getShichosonMeisho(), is(new RString("構成市町村1")));
        }

        @Test
        public void 市町村コードに合致し旧市町村を含む条件を指定した場合_該当データを1件返す() {
            IItemList<DbT7051KoseiShichosonMasterEntity> modelList = sut.selectBy(new LasdecCode("123458"), ContainsKyuShichoson.旧市町村を含む);
            assertThat(modelList.size(), is(1));
        }

        @Test
        public void 条件に合致するデータが存在しない場合_サイズ0のリストを返す() {
            IItemList<DbT7051KoseiShichosonMasterEntity> modelList = sut.selectBy(new LasdecCode("333333"), ContainsKyuShichoson.旧市町村を含む);
