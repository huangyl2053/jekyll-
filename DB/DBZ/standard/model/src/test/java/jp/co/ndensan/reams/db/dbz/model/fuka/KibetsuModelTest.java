...
public class KibetsuModelTest extends DbbTestBase {
    ...
    public static class getterSetterTest extends DbbTestBase {
        ...
        public void 戻り値の調定年度は_設定した値と同じ調定年度を返す() {
            sut.set調定年度(new ChoteiNendo(DbT2003KibetsuEntityGenerator.DEFAULT_調定年度));
            assertThat(sut.get調定年度().value(), is(DbT2003KibetsuEntityGenerator.DEFAULT_調定年度));
        }

        @Test
        public void 戻り値の賦課年度は_設定した値と同じ賦課年度を返す() {
            sut.set賦課年度(new FukaNendo(DbT2003KibetsuEntityGenerator.DEFAULT_賦課年度));
            assertThat(sut.get賦課年度().value(), is(DbT2003KibetsuEntityGenerator.DEFAULT_賦課年度));
        }
