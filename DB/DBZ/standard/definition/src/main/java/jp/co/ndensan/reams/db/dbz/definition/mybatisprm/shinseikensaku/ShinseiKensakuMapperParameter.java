 ...
 */
package jp.co.ndensan.reams.db.dbz.definition.mybatisprm.shinseikensaku;
...
public class ShinseiKensakuMapperParameter {
    ...
    public void set被保険者名(RString hihokenshaName) {
        ...
        RString converted = to半角カナOnlyOrRawTryToConvertかなto半角カナ(
                removedSpaces(RStringUtil.removeSqlSpecialChars(hihokenshaName))
        );
    ...
    /**
     * 指定の文字列のひらがなを半角カタカナに変換した結果、 すべて半角カタカナの文字列となる場合は、その文字列を返却します。 以外の場合は、そのままの文字列を返却します。
     *
     * @param aRawValue 変換対象文字列
     * @return {@link ConvertionResult}
     */
    static RString to半角カナOnlyOrRawTryToConvertかなto半角カナ(RString aRawValue) {
     ...
     */
    static RString removedSpaces(RString aRawValue) {
        if (RString.isNullOrEmpty(aRawValue)) {
            return aRawValue;
        }
