...
/**
 *
 * 意見書記入項目マッピング09Bを表す列挙型です。
 */
public enum IkenshoKinyuMapping09B {
    ...
    private IkenshoKinyuMapping09B(String code, String fullname) {
        this.code = new RString(code);
        this.fullName = new RString(fullname);
    }

    /**
     * 意見書記入項目マッピング09Bのコードを返します。
     *
     * @return 意見書記入項目マッピング09Bのコード
     */
    ...
    /**
     * 意見書記入項目マッピング09Bの名称を返します。
     *
     * @return 意見書記入項目マッピング09Bの名称
     */
    ...
    /**
     * 意見書記入項目マッピング09Bのコードと一致する内容を探します。
     *
     * @param code 意見書記入項目マッピング09Bのコード
     * @return {@code code} に対応する意見書記入項目マッピング09B
     */
    public static IkenshoKinyuMapping09B toValue(RString code) {
        for (IkenshoKinyuMapping09B ikenshoKinyuMapping99A : IkenshoKinyuMapping09B.values()) {
            if (ikenshoKinyuMapping99A.code.equals(code)) {
                return ikenshoKinyuMapping99A;
            }
        }
        throw new IllegalArgumentException(UrSystemErrorMessages.変換不可.getReplacedMessage("意見書記入項目マッピング09B"));
    }
}
