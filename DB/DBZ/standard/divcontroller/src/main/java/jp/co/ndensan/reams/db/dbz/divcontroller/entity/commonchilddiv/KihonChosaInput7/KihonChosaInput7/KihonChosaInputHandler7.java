...
public class KihonChosaInputHandler7 {
    ...
    private ArrayList<RString> setTokkiJikouNoList() {
        ...
        tokkiJikouNoList.add(NinteiChosaTokkiJikou.障害高齢者自立度.get認定調査票_特記情報_認定調査特記事項番号());
        tokkiJikouNoList.add(NinteiChosaTokkiJikou.認知症高齢者自立度.get認定調査票_特記情報_認定調査特記事項番号());
        return tokkiJikouNoList;
    }
    
    private void setDisabled(RString tokkiJikouNo) {
        if(NinteiChosaTokkiJikou.障害高齢者自立度.get認定調査票_特記情報_認定調査特記事項番号().equals(tokkiJikouNo)) {
            div.getBtnShogaiKoreisha().setDisabled(true);
        } else if (NinteiChosaTokkiJikou.認知症高齢者自立度.get認定調査票_特記情報_認定調査特記事項番号().equals(tokkiJikouNo)) {
