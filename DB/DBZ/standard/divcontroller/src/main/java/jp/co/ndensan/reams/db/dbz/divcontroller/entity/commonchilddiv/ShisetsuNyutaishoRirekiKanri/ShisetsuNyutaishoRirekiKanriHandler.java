...
public class ShisetsuNyutaishoRirekiKanriHandler {
    ...
    public void onSelectBySelectButton_dgShisetsuNyutaishoRireki(dgShisetsuNyutaishoRireki_Row row) {
        ...
        if (台帳種別表示機能.equals(new RString(div.getMode_RiyoType().toString()))) {
            div.getShisetsuNyutaishoInput().getCcdShisetsuJoho().set台帳種別(row.getDaichoShubetsuKey());
        }
        if (!適用除外者対象機能.equals(new RString(div.getMode_RiyoType().toString()))) {
    ...
    private ShisetsuNyutaisho 追加の処理(
        ...
        } else if (被保険者対象機能.equals(new RString(div.getMode_RiyoType().toString()))) {
            台帳種別 = row.getDaichoShubetsuKey();
            施設種類 = row.getShisetsuShuruiKey();
        } else if (他市町村住所地特例者対象機能.equals(new RString(div.getMode_RiyoType().toString()))) {
            台帳種別 = row.getDaichoShubetsuKey();
            施設種類 = row.getShisetsuShuruiKey();
        } else if (適用除外者対象機能.equals(new RString(div.getMode_RiyoType().toString()))) {
            台帳種別 = row.getDaichoShubetsuKey();
            施設種類 = row.getShisetsuShuruiKey();
        } else if (全施設対象機能.equals(new RString(div.getMode_RiyoType().toString()))) {
            施設種類 = row.getShisetsuShuruiKey();
        }
