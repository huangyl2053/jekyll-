...
public class ShikakuFuseigoShuseiMain {
    ...
    public ResponseData<ShikakuFuseigoShuseiMainDiv> onChange_ddlDaichoShubetsu(ShikakuFuseigoShuseiMainDiv div) {
        ShikakuFuseigoShusei business = ViewStateHolder.get(
                ViewStateKeys.資格不整合_整合性チェックの情報_宛名, ShikakuFuseigoShusei.class);
    ...
    public ResponseData<ShikakuFuseigoShuseiMainDiv> onSelectByModifyButton_dgShikakuFuseigoIchiran(ShikakuFuseigoShuseiMainDiv div) {
        if (!ResponseHolder.isReRequest()) {
            ...
            RString 不整合理由コード = ViewStateHolder.get(ViewStateKeys.資格不整合_不整合修正中,
            ...
            if (!不整合理由コード.equals(
                ...
                return ResponseData.of(div).addMessage(
                        DbzInformationMessages.不整合内容相違.getMessage().replace(
                                ...
                                ViewStateHolder.get(ViewStateKeys.資格不整合_不整合理由,
    ...
    private boolean 整合性チェックの処理(ShikakuFuseigoShusei business, RString 被保番号, RString 識別コード) {
        for (ShikakuFuseigoBusiness shikakuFusei : business.getBusinessList()) {
            if (識別コード.equals(shikakuFusei.get識別コード().value())) {
                ViewStateHolder.put(ViewStateKeys.資格不整合_不整合修正中, shikakuFusei);
                switch (DaichoType.toValue(shikakuFusei.get台帳種別())) {
                    case 被保険者:
                        ...
                        ViewStateHolder.put(ViewStateKeys.資格不整合_現在の資格の情報, result.get現在の資格の情報());
                        ViewStateHolder.put(ViewStateKeys.資格不整合_修正後の資格の情報, result.get修正後の資格の情報());
                        ViewStateHolder.put(ViewStateKeys.資格不整合_不整合理由, result.get不整合理由());
                        ViewStateHolder.put(ViewStateKeys.資格不整合_個人情報, result.get個人());
                        ViewStateHolder.put(ViewStateKeys.資格不整合_台帳状態, result.get被保険者台帳状態());
                        break;
                    case 適用除外者:
                        ...
                        ViewStateHolder.put(ViewStateKeys.資格不整合_現在の除外の情報, resultTekiyoJogai.get現在の除外の情報());
                        ViewStateHolder.put(ViewStateKeys.資格不整合_修正後の除外の情報, resultTekiyoJogai.get修正後の除外の情報());
                        ViewStateHolder.put(ViewStateKeys.資格不整合_不整合理由, resultTekiyoJogai.get不整合理由());
                        ViewStateHolder.put(ViewStateKeys.資格不整合_個人情報, resultTekiyoJogai.get個人());
                        ViewStateHolder.put(ViewStateKeys.資格不整合_台帳状態, resultTekiyoJogai.get適用除外者台帳状態());
                        break;
                    case 他市町村住所地特例者:
                        FuseigoCheckResultOfTatokurei resultTatokurei = serivce.checkAndCorrect他特例不整合(shikakuFusei.get個人情報(),
                                new ShikibetsuCode(識別コード));
                        ViewStateHolder.put(ViewStateKeys.資格不整合_現在の他特の情報, resultTatokurei.get現在の他特の情報());
                        ViewStateHolder.put(ViewStateKeys.資格不整合_修正後の他特の情報, resultTatokurei.get修正後の他特の情報());
                        ViewStateHolder.put(ViewStateKeys.資格不整合_不整合理由, resultTatokurei.get不整合理由());
                        ViewStateHolder.put(ViewStateKeys.資格不整合_個人情報, resultTatokurei.get個人());
                        ViewStateHolder.put(ViewStateKeys.資格不整合_台帳状態, resultTatokurei.get他市町村住所地特例台帳状態());
                        break;
                    default:
                        break;
                }
