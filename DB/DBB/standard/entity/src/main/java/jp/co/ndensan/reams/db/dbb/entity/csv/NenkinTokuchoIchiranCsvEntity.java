/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package jp.co.ndensan.reams.db.dbb.entity.csv;

import jp.co.ndensan.reams.uz.uza.io.csv.CsvField;
import jp.co.ndensan.reams.uz.uza.lang.RString;

/**
 * バッチ設計_DBB241001_特徴配信データ取り込みの一覧CSVのエンティティクラスです。
 *
 * @reamsid_L DBB-4270-020 chenyadong
 */
@lombok.Getter
@lombok.Setter
@SuppressWarnings("PMD.UnusedPrivateField")
public class NenkinTokuchoIchiranCsvEntity {

    @CsvField(order = 1, name = "市町村コード")
    private RString 市町村コード;
    @CsvField(order = 2, name = "義務者コード")
    private RString 義務者コード;
    @CsvField(order = 3, name = "義務者名称")
    private RString 義務者名称;
    @CsvField(order = 4, name = "通知内容コード")
    private RString 通知内容コード;
    @CsvField(order = 5, name = "通知内容")
    private RString 通知内容;
    @CsvField(order = 6, name = "作成年月日")
    private RString 作成年月日;
    @CsvField(order = 7, name = "基礎年金番号")
    private RString 基礎年金番号;
    @CsvField(order = 8, name = "年金コード")
    private RString 年金コード;
    @CsvField(order = 9, name = "年金名称")
    private RString 年金名称;
    @CsvField(order = 10, name = "生年月日")
    private RString 生年月日;
    @CsvField(order = 11, name = "性別")
    private RString 性別;
    @CsvField(order = 12, name = "カナ氏名")
    private RString カナ氏名;
    @CsvField(order = 13, name = "漢字氏名")
    private RString 漢字氏名;
    @CsvField(order = 14, name = "郵便番号")
    private RString 郵便番号;
    @CsvField(order = 15, name = "カナ住所")
    private RString カナ住所;
    @CsvField(order = 16, name = "漢字住所")
    private RString 漢字住所;
    @CsvField(order = 17, name = "各種区分")
    private RString 各種区分;
    @CsvField(order = 18, name = "各種区分名称")
    private RString 各種区分名称;
    @CsvField(order = 19, name = "処理結果コード")
    private RString 処理結果コード;
    @CsvField(order = 20, name = "処理結果内容")
    private RString 処理結果内容;
    @CsvField(order = 21, name = "各種年月日")
    private RString 各種年月日;
    @CsvField(order = 22, name = "金額1")
    private RString 金額1;
    @CsvField(order = 23, name = "金額2")
    private RString 金額2;
    @CsvField(order = 24, name = "金額3")
    private RString 金額3;
    @CsvField(order = 25, name = "共済年金証書記号番号")
    private RString 共済年金証書記号番号;
}
