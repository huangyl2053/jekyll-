...
public class TokuchoKariSanteiFukaManager {
    ...
    private static final char 文字列_0 = '0';
    ...
    private static final Code コード_08 = new Code("08");
    ...
    private static final RString 保険料段階を使用しない = new RString("0");
     ...
     */
    TokuchoKariSanteiFukaManager(MapperProvider mProvider, DbT2010FukaErrorListDac 賦課エラー一覧Dac,
    ...
    /**
     * {@link InstanceProvider#create}にて生成した{@link TokuchoKariSanteiFukaManager}のインスタンスを返します。
     *
     * @return {@link InstanceProvider#create}にて生成した{@link TokuchoKariSanteiFukaManager}のインスタンス
     */
    ...
    public void reflectShikakuToSaishinApril(FlexibleYear 調定年度, YMDHMS 調定日時) {
        ...
        for (SikakuSaisinnsikiRelateEntity 資格等最新化情報 : 資格等最新化情報List_４月開始) {
            ...
            SikakuSaisinnsiki sikakuSaisinnsiki = new SikakuSaisinnsiki(賦課の情報_４月開始, 徴収方法の情報_４月開始, 資格の情報_４月開始,
    ...
    private FukaJohoTempEntity set賦課の情報_業務概念(SikakuSaisinnsiki 賦課の情報_業務概念, YMDHMS 調定日時, FlexibleYear 調定年度) {
        FukaJoho 賦課の情報_４月開始 = 賦課の情報_業務概念.get賦課の情報_４月開始();
        ...
        賦課の情報一時Entity = modifyFukaJohoCommon_４月開始(調定年度, 資格の情報_４月開始, 生保の情報List_４月開始,
                老齢の情報List_４月開始, 賦課の情報_４月開始.get生保開始日(), 賦課の情報_４月開始.get生保廃止日(),
                賦課の情報_４月開始.get老年開始日(), 賦課の情報_４月開始.get老年廃止日(), 賦課の情報_４月開始.get世帯課税区分(),
                賦課の情報_４月開始.get課税区分(), 賦課の情報_４月開始.get合計所得金額(), 賦課の情報_４月開始.get公的年金収入額(),
    ...
    private void set特別徴収停止事由コード(RString 徴収方法の情報_徴収方法4月, FukaJohoTempEntity 賦課の情報一時Entity, ChoshuHoho 徴収方法の情報_４月開始, FukaJoho 賦課の情報_更正前) {
    ...
    private FukaJohoTempEntity modifyFukaJohoCommon_４月開始(FlexibleYear 調定年度,
        ...
        賦課の情報一時Entity.setShikakuSoshitsuYMD(資格の情報.get資格取得年月日());
        ...
        } else if (前年度生保廃止日.isEmpty() || 前年度生保開始日.isBeforeOrEquals(調定年度開始日)) {
        ...
        } else if (前年度老年廃止日.isEmpty() || 前年度老年廃止日.isBeforeOrEquals(調定年度開始日)) {
        ...
        if (保険料段階を使用する.equals(DbBusinessConfig.get(ConfigNameDBB.賦課基準_課税層保険料段階インデックス,
                RDate.getNowDate(), SubGyomuCode.DBB介護賦課))) {
    ...
    private void set老齢の情報_modifyFukaJohoCommon_４月開始(List<RoreiFukushiNenkinJukyusha> 老齢の情報List, FlexibleDate 調定年度開始日,
        ...
        // FukaKeisan 这个类的findOut賦課基準日 方法，崔林做的，还没写好
        // builder.set賦課期日()
    }
...
//            }
    ...
    public void createFukaJohoKeizoku(FlexibleYear 調定年度, YMDHMS 調定日時) {
        ...
        if (!業務概念_賦課情報_継続List.isEmpty()) {
            for (FukaJohokeizoku 業務概念_賦課情報_継続 : 業務概念_賦課情報_継続List) {
                ...
                // 賦課の情報の特徴期別金額01~03（仮算定分）を編集する。（「特徴継続者」）
                ...
                FukaJohoTempEntity 賦課の情報一時Entity = create賦課の情報一時Entity(賦課情報);
    ...
    public void createFukaJohoJune(FlexibleYear 調定年度, YMDHMS 調定日時) {
        ...
        for (FukaJohoLokukatuRelateEntity 賦課情報_6月開始 : 賦課情報_6月開始List) {
            ...
            FukaJohoLokukatu fukaJohoLokukatu = new FukaJohoLokukatu(賦課情報_6月開始.getChoteigaku(), 賦課情報_6月開始.getSeihoKaishiYMD(),
        ...
        if (!業務概念_賦課情報_6月開始List.isEmpty()) {
            for (FukaJohoLokukatu 業務概念_賦課情報_6月開始 : 業務概念_賦課情報_6月開始List) {
                ...
                // 賦課の情報の特徴期別金額01~03（仮算定分）を編集する。（「特徴6月開始」）
                if (被保険者番号Map.containsKey(賦課情報.get被保険者番号())) {
                    被保険者番号Map.remove(賦課情報.get被保険者番号());
                    int 枝番号 = Integer.valueOf(賦課情報.get通知書番号().value().substring(整数_14, 整数_16).toString());
                    被保険者番号Map.put(賦課情報.get被保険者番号(), 枝番号);
    ...
    private FukaJohoTempEntity create賦課の情報一時Entity(FukaJoho 賦課情報) {
        FukaJohoTempEntity 賦課の情報一時Entity = new FukaJohoTempEntity();
        ...
        賦課の情報一時Entity.setTkKibetsuGaku01(賦課情報.get特徴期別金額01());
        賦課の情報一時Entity.setTkKibetsuGaku02(賦課情報.get特徴期別金額02());
        賦課の情報一時Entity.setTkKibetsuGaku03(賦課情報.get特徴期別金額03());
        賦課の情報一時Entity.setTkKibetsuGaku04(Decimal.ZERO);
        賦課の情報一時Entity.setTkKibetsuGaku05(Decimal.ZERO);
        賦課の情報一時Entity.setTkKibetsuGaku06(Decimal.ZERO);
        賦課の情報一時Entity.setFuKibetsuGaku01(Decimal.ZERO);
    ...
    public FukaJoho modifyFukaJohoCommon(FlexibleYear 調定年度,
        ...
        賦課情報Builder.set資格喪失日(資格の情報.get資格取得年月日());
...
//        賦課情報Builder.set保険料段階_仮算定時(月別保険料段階.get保険料段階04月().edit表示用保険料段階());
        ...
        // 特徴期別金額01 特徴期別金額01 c特徴期別金額03
        // 特徴期別金額04~06、普徴期別金額01~14 业务概念没有set方法，在后面登陆DB的时候赋值的
...
//            賦課情報Builder.set生活保護扶助種類(min扶助種類コード.value().value());
    ...
    public void spoolTokuchoKarisanteiKekkaIchiran(FlexibleYear 調定年度, FlexibleYear 賦課年度, YMDHMS 調定日時, final RString 出力順ID) {
        調定日時 = new YMDHMS("20160708173939");
        ITokuchoKariSanteiFukaMapper mapper = mProvider.create(ITokuchoKariSanteiFukaMapper.class);
        ...
        publish特別徴収開始通知書発行一覧表(特徴仮算定計算後賦課情報EntityList, 調定年度, 調定日時);
    ...
    private void set特徴仮算定計算後賦課情報_特徴停止(TokuchoKariKeisangoFukaEntity entity, TokuchoKarisanteiKekkaIchiranTeisiEntity 特徴仮算定計算後賦課情報_特徴停止) {
...
//                    }
    ...
    private void publish特別徴収開始通知書発行一覧表(List<TokuchoKariKeisangoFukaEntity> 特徴仮算定計算後賦課情報EntityList,
            FlexibleYear 調定年度,
            YMDHMS 調定日時) {
...
//        headerList.add(タイトル_開始月);
//        headerList.add(タイトル_前年度保険料段階);
//        headerList.add(タイトル_前年度保険料);
...
//        headerList.add(タイトル_特徴停止);
//        headerList.add(タイトル_特徴停止理由);
        ...
        try (CsvListWriter csvListWriter = new CsvListWriter.InstanceBuilder(eucFilePath).setNewLine(NewLine.CRLF)
            ...
            for (TokuchoKariKeisangoFukaEntity entity : 特徴仮算定計算後賦課情報EntityList) {
                ...
                Code 特別徴収停止事由コード = entity.get特別徴収義務者コード();
                ...
                if (前年度保険料段階.isNullOrEmpty()) {
                    bodyList.add(RString.EMPTY);
                } else {
                    bodyList.add(前年度保険料段階.substring(整数_0, 整数_2).trimStart(文字列_0));
                }
                ...
                if (!entity.get仮徴収_年金コード().isNullOrEmpty()) {
                ...
                if (entity.get特別徴収停止事由コード().isNullOrEmpty()) {
    ...
    private RString get開始月(Code 特別徴収停止事由コード,
            Decimal 特徴期期別金額01,
            Decimal 特徴期期別金額02,
            Decimal 特徴期期別金額03) {
