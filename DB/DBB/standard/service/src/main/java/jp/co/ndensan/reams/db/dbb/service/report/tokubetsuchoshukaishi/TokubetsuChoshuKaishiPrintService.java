...
public class TokubetsuChoshuKaishiPrintService {
    ...
    /**
     * 特別徴収開始通知書（本算定）発行一覧表の printメソッド
     *
     ...
     */
    public void print(List<EditedHonSanteiTsuchiShoKyotsu> 編集後本算定通知書共通情報,
            NonyuTsuchIchiranBatchParameter バッチパラメータ, RString 帳票作成日時) {
        TokubetsuChoshuKaishiProperty property
                = new TokubetsuChoshuKaishiProperty();
        try (ReportManager reportManager = new ReportManager()) {
            try (ReportAssembler<TokubetsuChoshuKaishiSource> assembler = createAssembler(property, reportManager)) {
                ...
                if (バッチパラメータ != null && バッチパラメータ.get出力帳票entity() != null && !バッチパラメータ.get出力帳票entity().isEmpty()) {
                    for (NonyuTsuchIchiranentity entity : バッチパラメータ.get出力帳票entity()) {
                        if (ReportIdDBB.DBB200011.getReportId().equals(entity.get帳票ID())) {
                            出力順ID = entity.get出力順ID();
                            break;
                        }
                    }
                }
                if (RString.isNullOrEmpty(出力順ID)) {
                    executereport(編集後本算定通知書共通情報, バッチパラメータ, 帳票作成日時, 市町村コード, 市町村名, new ArrayList(),
                ...
                executereport(編集後本算定通知書共通情報, バッチパラメータ, 帳票作成日時, 市町村コード, 市町村名, 出力項目リスト,
    ...
    private void executereport(List<EditedHonSanteiTsuchiShoKyotsu> 編集後本算定通知書共通情報,
            NonyuTsuchIchiranBatchParameter バッチパラメータ, RString 帳票作成日時, RString 市町村コード, RString 市町村名,
        ...
        for (EditedHonSanteiTsuchiShoKyotsu editedhonsanteitsuchishokyotsu : 編集後本算定通知書共通情報) {
            TokubetsuChoshuKaishiReport report = new TokubetsuChoshuKaishiReport(
                    editedhonsanteitsuchishokyotsu, バッチパラメータ, 帳票作成日時, 市町村コード, 市町村名, 出力項目リスト, 改頁項目リスト, i);
