...
/**
 * 帳票設計_DBBPR51003_介護保険所得情報一覧表のPrintServiceです。
 *
...
public class KaigoHokenShotokuJohoIchiranPrintService {
     ...
     */
    public SourceDataCollection print介護保険所得情報(KaigoHokenShotokuTempEntity 所得情報一覧Entity,
            RString 導入形態コード,
            LasdecCode 市町村コード,
            RString 市町村名称,
            Long 出力順ID,
        ...
        List<RString> 出力順項目リスト = get出力順(並び順);
        List<RString> 改頁項目リスト = get改頁項目(並び順);
        ...
        try (ReportManager reportManager = new ReportManager()) {
            try (ReportAssembler<KaigoHokenShotokuJohoIchiranSource> assembler
                ...
                new KaigoHokenShotokuJohoIchiranReport(所得情報一覧Entity, 導入形態コード, 市町村コード, 市町村名称,
                        出力順項目リスト, 改頁項目リスト, association, 連番).writeBy(reportSourceWriter);
            }
