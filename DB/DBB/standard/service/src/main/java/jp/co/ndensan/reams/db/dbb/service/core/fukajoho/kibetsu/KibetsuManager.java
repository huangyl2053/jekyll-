 ...
 */
package jp.co.ndensan.reams.db.dbb.service.core.fukajoho.kibetsu;
...
import jp.co.ndensan.reams.db.dbb.persistence.db.mapper.relate.fukajoho.kibestu.IKibetsuMapper;
import jp.co.ndensan.reams.db.dbb.service.core.MapperProvider;
import jp.co.ndensan.reams.db.dbb.service.core.fukajoho.choteikyotsu.ChoteiKyotsuManager;
...
public class KibetsuManager {
    ...
    /**
     * 主キー1に合致する介護期別情報のリストを返します。
     *
     * @param 介護期別検索条件 介護期別検索条件
     * @return Kibetsuの{@code list}
     */
    @Transaction
    public List<Kibetsu> get介護期別情報リストBy主キー1(KibetsuMapperParameter 介護期別検索条件) {
        requireNonNull(介護期別検索条件, UrSystemErrorMessages.値がnull.getReplacedMessage("賦課の情報検索条件"));
        IKibetsuMapper mapper = mapperProvider.create(IKibetsuMapper.class);

        List<KibetsuEntity> relateEntityList = mapper.select介護期別リストBy主キー1(介護期別検索条件);

        ArrayList<Kibetsu> 介護期別List = new ArrayList<>();
        for (KibetsuEntity relateEntity : relateEntityList) {
            relateEntity.initializeMd5ToEntities();
            介護期別List.add(new Kibetsu(relateEntity));
        }
        return 介護期別List;
