 ...
 */
public class HonsanteiIdoGennendoTsuchisyoIkatsuHakoFath {
     ...
     * コンストラクタです。
     */
    public HonsanteiIdoGennendoTsuchisyoIkatsuHakoFath() {
        this.通知書発行後異動者Dac = InstanceProvider.create(DbT2017TsuchishoHakkogoIdoshaDac.class);
    }

    /**
     * {@link InstanceProvider#create}にて生成した{@link HonsanteiIdoGennendoTsuchisyoIkatsuHakoFath}のインスタンスを返します。
     *
     * @return HonsanteiIdoGennendoTsuchisyoIkatsuHakoFath
     */
    public static HonsanteiIdoGennendoTsuchisyoIkatsuHakoFath createInstance() {
        return InstanceProvider.create(HonsanteiIdoGennendoTsuchisyoIkatsuHakoFath.class);
    }
...
//            //  return ReportIdDBB.DBB100062.getReportName();
    ...
    public void publish決定変更通知書発行一覧表(RDateTime 帳票作成日時, RString 決定変更区分,
        ...
        try (CsvListWriter csvListWriter = new CsvListWriter.InstanceBuilder(eucFilePath).setNewLine(NewLine.CRLF)
            ...
            for (EditedHonSanteiTsuchiShoKyotsu 編集後本算定通知書共通情報 : 編集後本算定通知書共通情報List) {
                ...
                publish決定変更発行一覧表(編集後本算定通知書共通情報, bodyList);
                if (isNull(編集後本算定通知書共通情報.get更正後())) {
                    bodyList.add(RString.EMPTY);
                    bodyList.add(RString.EMPTY);
                    bodyList.add(RString.EMPTY);
    ...
    private void publish決定変更発行一覧表(EditedHonSanteiTsuchiShoKyotsu 編集後本算定通知書共通情報, List<RString> bodyList) {
        if (isNull(編集後本算定通知書共通情報.get編集後宛先())) {
            bodyList.add(RString.EMPTY);
            bodyList.add(RString.EMPTY);
            bodyList.add(RString.EMPTY);
    ...
    /**
     * 特徴開始通知書(仮算定）の発行後異動者登録するメソッドです。
     *
     * @param 帳票作成日時 YMDHMS
     * @param 帳票ID ReportId
     * @param entityList List<TmpIdoGennendoKarisanteiEntity>
     */
    public void insert通知書発行後異動者_特徴仮算定(YMDHMS 帳票作成日時, ReportId 帳票ID, List<TmpIdoGennendoKarisanteiEntity> entityList) {

        if (entityList == null || entityList.isEmpty()) {
            return;
        }
        int 連番 = INT_1;
        for (TmpIdoGennendoKarisanteiEntity entity : entityList) {
            FukaJoho fukaJoho = new FukaJoho(entity.get賦課の情報_更正後());
            DbT2017TsuchishoHakkogoIdoshaEntity dbt2017entity = new DbT2017TsuchishoHakkogoIdoshaEntity();
            dbt2017entity.setReportID(帳票ID);
            dbt2017entity.setChohyosakuseiTimestamp(帳票作成日時);
            dbt2017entity.setTsuchishoNo(fukaJoho.get通知書番号());
            dbt2017entity.setFukaNendo(fukaJoho.get賦課年度());
            dbt2017entity.setShikibetsuCode(fukaJoho.get識別コード());
            dbt2017entity.setHihokenshaNo(fukaJoho.get被保険者番号());
