...
public class TokubetuChosyutaisyosyaTorokuManager {
    ...
    public RString getShorizumiRenban(FlexibleYear 賦課年度) {
        ...
        if (処理日付管理マスタデータResultLst.isEmpty()) {
            return 空白;
        } else if (!is年度内連番処理済み(連番_0001, 処理日付管理マスタデータResultLst)) {
            return 空白;
     ...
     * @param 捕捉月 捕捉月
     */
    public void insChoshuHoho(FlexibleYear 賦課年度, HihokenshaNo 被保険者番号,
            RString 基礎年金番号, RString 年金コード, RString 特徴義務者コード, RString 捕捉月) {
