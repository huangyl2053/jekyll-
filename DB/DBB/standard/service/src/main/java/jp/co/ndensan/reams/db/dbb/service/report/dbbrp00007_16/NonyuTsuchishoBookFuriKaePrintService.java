...
import jp.co.ndensan.reams.db.dbb.business.report.dbbrp00007_16.NonyuTsuchishoBookFuriKaeAriCoverReport;
...
public class NonyuTsuchishoBookFuriKaePrintService {
        ...
        try (ReportManager reportManager = new ReportManager()) {
            try (ReportAssembler<HokenryoNonyuTsuchishoBookFuriKaeNashiCoverSource> assembler = createAssembler(property, reportManager)) {
                ...
                HokenryoNonyuTsuchishoBookFuriKaeNashiReport report
                        = HokenryoNonyuTsuchishoBookFuriKaeNashiReport.createFrom(本算定納入通知書情報, ninshoshaSource);
    ...
    private SourceDataCollection print全てページDBB100071printDevidedByPage(HonSanteiNonyuTsuchiShoJoho 本算定納入通知書情報) {
        NonyuTsuchishoBookFuriKaeAriCoverProperty property = new NonyuTsuchishoBookFuriKaeAriCoverProperty();
        try (ReportManager reportManager = new ReportManager()) {
            try (ReportAssembler<NonyuTsuchishoBookFuriKaeAriCoverSource> assembler = createAssembler(property, reportManager)) {
                ReportSourceWriter<NonyuTsuchishoBookFuriKaeAriCoverSource> reportSourceWriter
                        = new ReportSourceWriter(assembler);
    ...
    private void print全てページDBB100071(HonSanteiNonyuTsuchiShoJoho 本算定納入通知書情報, ReportManager reportManager) {
        ...
        try (ReportAssembler<NonyuTsuchishoBookFuriKaeAriCoverSource> assembler = createAssembler(property, reportManager)) {
            ...
            NinshoshaSource ninshoshaSource = ReportUtil.get認証者情報(SubGyomuCode.DBB介護賦課, 帳票分類ID,
                    new FlexibleDate(本算定納入通知書情報.get発行日().toDateString()),
                    NinshoshaDenshikoinshubetsuCode.保険者印.getコード(), KenmeiFuyoKubunType.付与なし, reportSourceWriter);
            NonyuTsuchishoBookFuriKaeAriCoverReport report
                    = NonyuTsuchishoBookFuriKaeAriCoverReport.createFrom(本算定納入通知書情報, ninshoshaSource);
            report.writeBy(reportSourceWriter);
        }
    }
