...
public class TokuchoTaishoshaIchiranSakusei {
    ...
    private static final RString STR06 = new RString("08");
    ...
    /**
     * get特徴未同定情報Listのメソドです。
     *
     * @param id TokuchoMidoteiJohoIdentifier
     * @return List<TokuchoMidoteiJoho>
     */
    public List<TokuchoMidoteiJoho> get特徴未同定情報List(TokuchoMidoteiJohoIdentifier id) {
        ...
        List<TokuchoMidoteiJoho> resultList = new ArrayList<>();
        for (DbT2019TokuchoMidoteiJohoEntity 特徴未同定情報 : 特徴未同定情報List) {
            resultList.add(new TokuchoMidoteiJoho(特徴未同定情報));
        }
        return resultList;
    }
    ...
    private void get仮徴収(TokuchoTaishoshaIchiranSakuseiEntity entity,
        ...
        if (entity.getDbt2001entity() != null) {
            ...
            if (分配集約_処理年度 != null && !分配集約_処理年度.isEmpty()
                    ...
                    && 分配集約_捕捉月 != null && !分配集約_捕捉月.isEmpty()) {
    ...
    private void get本徴収(TokuchoTaishoshaIchiranSakuseiEntity entity,
            NenkinTokuchoKaifuJohoManager manager,
            TokuchoDouteiKouhoshaShousaiJoho result,
            List<TokuchoDouteiKouhoshaShousaiJoho> 同定候補者詳細情報List) {
        FlexibleYear 分配集約_処理年度;
    ...
    private void get翌年度仮徴収(TokuchoTaishoshaIchiranSakuseiEntity entity,
            NenkinTokuchoKaifuJohoManager manager,
            TokuchoDouteiKouhoshaShousaiJoho result,
            List<TokuchoDouteiKouhoshaShousaiJoho> 同定候補者詳細情報List) {
        FlexibleYear 分配集約_処理年度;
    ...
    public void kakuninJotaiUpdate(FlexibleYear 処理年度, RString 基礎年金番号, RString 被保険者番号,
            ...
            RString 特別徴収義務者コード, TokuchoMidoteiJoho model,
        ...
        if (確認状況区分_同定済み.compareTo(確認状況区分) == NUM0) {
            ...
            for (TokuchoMidoteiJoho taishogaiModel : taishogaiModels) {
