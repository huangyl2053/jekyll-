...
public final class ShotokuJohoTorokuHandler {
    ...
    private void set初期状態_データ準備() {
        ...
        load世帯所得情報一覧(識別コード, 所得年度, 所得基準日時, false);
    ...
    public void load世帯所得情報一覧(ShikibetsuCode 識別コード, FlexibleYear 所得年度, YMDHMS 所得基準日時, boolean flag) {
        ...
        if (flag) {
            ...
            世帯員所得情報List = 世帯員所得情報Finder.get世帯員所得情報(識別コード,
                    FlexibleDate.getNowDate(), 所得年度, 所得基準日時, false);
        }
        ...
        if (null != 世帯員所得情報List && !世帯員所得情報List.isEmpty()) {
            ...
            for (SetaiinShotoku 世帯員所得情報 : 世帯員所得情報List) {
                ...
                row.setTxtKetsugo02(世帯員所得情報.get生年月日().wareki().toDateString().concat(RString.FULL_SPACE).concat(getNotNull(世帯員所得情報.get性別()))
                        .concat(改行タグ).concat(世帯員所得情報.get続柄()));
                ...
                row.setTxtJukiIdoJiyu(世帯員所得情報.get住民情報_異動事由());
