...
public class TokuchoTaishoshaIchiranHandler {
    ...
    public TokuchoTaishoshaIchiranSakuseiResult 特別徴収対象者一覧作成initialize() {
        ...
        if (result != null) {
            ...
            if (result.get特別徴収開始月() != null) {
                特別徴収開始月 = new FlexibleYearMonth(result.get特別徴収開始月()).wareki().firstYear(FirstYear.ICHI_NEN).toDateString();
            } else {
                特別徴収開始月 = RString.EMPTY;
            }
            if (result.get捕捉月表示内容() != null) {
                特別徴収開始月 = 特別徴収開始月.concat(RString.FULL_SPACE).concat(LEFT).concat(result.get捕捉月表示内容()).concat(RIGHT);
            }
        ...
        div.getChkShutsuryokuTaisho().setSelectedItemsByKey(new ArrayList());
    ...
    public void 特別徴収同定候補者一覧initialize(RString 年度, RString 捕捉月, RString 基礎年金番号, RString 年金コード,
            RString 特徴開始月) {
        if (!RString.isNullOrEmpty(特徴開始月) && !特徴開始月.startsWith(RString.FULL_SPACE) && !特徴開始月.startsWith(LEFT)) {
    ...
    private void set特別徴収同定候補者詳細情報(TokuchoDouteiKouhoshaShousaiJoho result) {
        ...
        if (result.get未同定年金情報_性別() != null) {
            div.getTxtSebetsu().setValue(SeibetsuCodeNenkinTokuchoType.toValue(result.get未同定年金情報_性別()).get性別名称());
        }
        ...
        if (result.get住基情報_性別() != null) {
            div.getTxtAtenaSebetsu().setValue(SeibetsuCodeNenkinTokuchoType.toValue(result.get住基情報_性別()).get性別名称());
        }
        if (result.get住基情報_住民種別() != null) {
            div.getTxtJuminshu().setValue(JuminShubetsu.toValue(result.get住基情報_住民種別()).toRString());
        }
        if (result.get住基情報_住登日() != null) {
            div.getTxtJutoYMD().setValue(new FlexibleDate(result.get住基情報_住登日().toString()));
        }
        ...
        if (result.get被保険者台帳_資格喪失年月日() != null) {
