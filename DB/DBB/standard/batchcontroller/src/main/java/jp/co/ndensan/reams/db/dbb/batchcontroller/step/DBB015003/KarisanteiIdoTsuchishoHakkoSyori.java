 ...
 */
public class KarisanteiIdoTsuchishoHakkoSyori {
     ...
     * コンストラクタです。
     */
    public KarisanteiIdoTsuchishoHakkoSyori() {
    ...
    /**
     * 特別徴収開始通知書（仮算定）発行一覧表タイトルList取得するメソッドです。
     *
     * @return List<RString> 特別徴収開始通知書（仮算定）発行一覧表タイトルList
     */
    ...
    /**
     * 特別徴収開始通知書（仮算定）発行一覧表bodyList取得するメソッドです。
     *
     * @param 調定年度 FlexibleYear
     * @param 帳票作成日時 RDateTime
     * @param 編集後仮算定通知書共通情報 EditedKariSanteiTsuchiShoKyotsu
     * @return List<RString> 特別徴収開始通知書（仮算定）発行一覧表bodyList
     */
    ...
    /**
     * 仮算定額変更通知書発行一覧表タイトルList取得するメソッドです。
     *
     * @return List<RString> 仮算定額変更通知書発行一覧表タイトルList
     */
    public List<RString> get仮算定額変更通知書発行一覧表タイトルList() {
        List<RString> headerList = new ArrayList<>();
    ...
    /**
     * 仮算定額変更通知書発行一覧表bodyList取得するメソッドです。
     *
     * @param 調定年度 FlexibleYear
     * @param 帳票作成日時 RDateTime
     * @param 編集後仮算定通知書共通情報 EditedKariSanteiTsuchiShoKyotsu
     * @return List<RString> 仮算定額変更通知書発行一覧表bodyList
     */
    public List<RString> get仮算定額変更通知書発行一覧表bodyList(FlexibleYear 調定年度, RDateTime 帳票作成日時,
            EditedKariSanteiTsuchiShoKyotsu 編集後仮算定通知書共通情報) {
        List<RString> bodyList = new ArrayList<>();
        bodyList.add(帳票作成日時.getDate().seireki().separator(Separator.SLASH).fillType(FillType.BLANK).toDateString());
        bodyList.add(帳票作成日時.getTime().toFormattedTimeString(DisplayTimeFormat.HH_mm_ss));
    ...
    /**
     * 納入通知書発行一覧表タイトList取得するメソッドです。
     *
     * @return List<RString> 納入通知書発行一覧表タイトList
     */
     ...
     */
    public List<RString> get納入通知書発行一覧表bodyList(RDateTime 帳票作成日時, RString 出力期,
            EditedKariSanteiTsuchiShoKyotsu 編集後仮算定通知書共通情報) {
        List<RString> bodyList = new ArrayList<>();
        bodyList.add(帳票作成日時.getDate().seireki().separator(Separator.SLASH).fillType(FillType.BLANK).toDateString());
        bodyList.add(帳票作成日時.getTime().toFormattedTimeString(DisplayTimeFormat.HH_mm_ss));
    ...
    private void set次期以降期別金額(EditedKariSanteiTsuchiShoKyotsu 編集後仮算定通知書共通情報, RString 出力期, List<RString> bodyList) {
        ...
        if (Integer.parseInt(出力期.toString()) == 1) {
            bodyList.add(定値区分_0);
            return;
        }
        if (isNull(編集後仮算定通知書共通情報.get更正後()) || isNull(編集後仮算定通知書共通情報.get更正後().get更正後普徴期別金額リスト())
    ...
    private void set対象者追加含む情報_更正後(UeT0511NenkinTokuchoKaifuJohoEntity 対象者_追加含む_情報,
        ...
        if (対象者_追加含む_情報.getTsuchiNaiyoCode() != null) {
            tmpEntity.setUeT0511after_tsuchiNaiyoCode(new TsuchiNaiyoCode(対象者_追加含む_情報.getTsuchiNaiyoCode()));
        }
        ...
        if (対象者_追加含む_情報.getRenkeiShubetsu() != null) {
            tmpEntity.setUeT0511after_renkeiShubetsu(new RenkeiShubetsu(対象者_追加含む_情報.getRenkeiShubetsu()));
        }
        ...
        tmpEntity.setUeT0511after_dtTokubetsuChoshuGimushaCode(対象者_追加含む_情報.getDtTokubetsuChoshuGimushaCode());
        if (対象者_追加含む_情報.getDtTsuchiNaiyoCode() != null) {
            tmpEntity.setUeT0511after_dtTsuchiNaiyoCode(new TsuchiNaiyoCode(対象者_追加含む_情報.getDtTsuchiNaiyoCode()));
        }
        tmpEntity.setUeT0511after_dtBaitaiCode(対象者_追加含む_情報.getDtBaitaiCode());
        if (対象者_追加含む_情報.getDtTokubetsuChoshuSeidoCode() != null) {
            tmpEntity.setUeT0511after_dtTokubetsuChoshuSeidoCode(
                    new TokubetsuChoshuSeidoCode(対象者_追加含む_情報.getDtTokubetsuChoshuSeidoCode()));
        }
        ...
        if (対象者_追加含む_情報.getDtSeibetsu() != null) {
            tmpEntity.setUeT0511after_dtSeibetsu(new SeibetsuCodeNenkinTokucho(対象者_追加含む_情報.getDtSeibetsu()));
        }
