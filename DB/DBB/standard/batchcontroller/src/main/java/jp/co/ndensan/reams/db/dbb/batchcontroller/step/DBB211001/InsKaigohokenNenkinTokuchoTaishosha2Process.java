...
/**
 * （特徴異動）年金特徴対象者情報の登録します。
 *
 * @reamsid_L DBB-1840-040 xuzhao
 */
public class InsKaigohokenNenkinTokuchoTaishosha2Process extends BatchProcessBase<TokuchoIraiInsertDataEntity> {

    private static final RString MYBATIS_SELECT_ID = new RString("jp.co.ndensan.reams.db.dbb.persistence"
            + ".db.mapper.relate.tokuchosoufujohosakuseibatch.ITokuChoSoufuJohoSakuseiBatchMapper.get対象者情報追加用データ_特徴異動");
    private static final RString DT各種区分_03 = new RString("03");
    private static final RString 個人コード区分_0 = new RString("0");
    private static final RString 捕捉月_06月 = new RString("06");
    private static final RString 捕捉月_08月 = new RString("08");
    ...
    private static final RString 待機フラグ_待機あり = new RString("1");
    ...
    private InsKaigohokenNenkinTokuchoTaishosha2ProcessParameter parameter;
    ...
    private UeT0515KaigohokenNenkinTokuchoTaishoshaJoho550Entity 介護保険年金特徴対象者情報の設定(TokuchoIraiInsertDataEntity t) {
        ...
        if (!DT各種区分_03.equals(entity.getDtKakushuKubun())) {
            ...
            entity.setTaikiFlag(get待機フラグ(対象者情報.getHosokuTsuki()));
        }
        return entity;
    }
    ...
    private RString get待機フラグ(RString 捕捉月) {
        RString 特別徴収_特徴開始月_6月捕捉 = DbBusinessConfig.get(ConfigNameDBB.特別徴収_特徴開始月_6月捕捉,
                parameter.getシステム日時().getDate(), SubGyomuCode.DBB介護賦課);
        RString 特別徴収_特徴開始月_8月捕捉 = DbBusinessConfig.get(ConfigNameDBB.特別徴収_特徴開始月_8月捕捉,
                parameter.getシステム日時().getDate(), SubGyomuCode.DBB介護賦課);

        if (捕捉月_06月.equals(捕捉月)
