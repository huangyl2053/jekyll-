 ...
 */
public class PrtNonyuTsuchishoProcess extends SimpleBatchProcessBase {
    ...
    protected void process() {
        ...
        try {
            manager.prtNonyuTsuchisho(processParameter.get調定年度(), processParameter.get賦課年度(),
                    ...
                    processParameter.get納入_発行日(), processParameter.get納入_出力方法(), processParameter.get納入_出力期(),
                    processParameter.get納入_対象者(), processParameter.get納入_生活保護対象者をまとめて先頭に出力(),
                    processParameter.get納入_ページごとに山分け(), 出力帳票一覧.get出力順ID(),
                    出力帳票一覧.get帳票ID(), processParameter.is一括発行起動フラグ());
