 ...
 */
public class PrtKetteiTsuchishoProcess extends BatchProcessBase<HonsanteiTsuchishoTempEntity> {
    ...
    private NinshoshaSource dbb100039NinshoshaSource;
    private NinshoshaSource dbb100040NinshoshaSource;
    ...
    private BatchReportWriter<KaigoHokenHokenryogakuKetteiTsuchishoA4TateReportSource> dbb100040reportWriter;
    private ReportSourceWriter<KaigoHokenHokenryogakuKetteiTsuchishoA4TateReportSource> dbb100040ReportSourceWriter;
    ...
    protected void initialize() {
        ...
        帳票名 = manager.get帳票名_決定(出力帳票一覧.get帳票ID().getColumnValue());
        ...
        通知書定型文 = ReportUtil.get通知文(SubGyomuCode.DBB介護賦課,
                決定変更通知書_帳票分類ID, KamokuCode.EMPTY, 定型文文字サイズ, INT_1, FlexibleDate.getNowDate());
        ...
        myBatisParameter = PrtKetteiTsuchishoParameter.createSelectByKeyParam(定値_決定区分, 出力順);
    }
    ...
    protected void process(HonsanteiTsuchishoTempEntity tempEntity) {
        ...
        介護保険料額決定通知書.set文書番号(processParameter.get決定変更_文書番号());
    ...
    private int publish決定通知書(KaigoHokenHokenryogakuKetteiTsuchishoJoho 介護保険料額決定通知書) {
        ...
        List<KaigoHokenHokenryogakuKetteiTsuchishoBusiness> items = new ArrayList<>();
        KaigoHokenHokenryogakuKetteiTsuchishoBusiness business = new KaigoHokenHokenryogakuKetteiTsuchishoBusiness();
        business.set文書番号(介護保険料額決定通知書.get文書番号());
        business.set本算定決定通知書情報(介護保険料額決定通知書.get本算定決定通知書情報());
        business.set調定事由リスト(介護保険料額決定通知書.get調定事由リスト());
        business.set通知書定型文(介護保険料額決定通知書.get通知書定型文());
        business.setCompKaigoToiawasesakiSource(compKaigoToiawasesakiSource);

        if (ReportIdDBB.DBB100039.getReportId().equals(出力帳票一覧.get帳票ID())) {
            business.setNinshoshaSource(dbb100039NinshoshaSource);
            items.add(business);
            KaigoHokenHokenryogakuKetteiTsuchishoB5YokoReport report = new KaigoHokenHokenryogakuKetteiTsuchishoB5YokoReport(items);
            report.writeBy(dbb100039ReportSourceWriter);
            return dbb100039reportWriter.getPageCount();
        } else if (ReportIdDBB.DBB100040.getReportId().equals(出力帳票一覧.get帳票ID())) {
            business.setNinshoshaSource(dbb100040NinshoshaSource);
            items.add(business);
            KaigoHokenHokenryogakuKetteiTsuchishoA4TateReport report = new KaigoHokenHokenryogakuKetteiTsuchishoA4TateReport(items);
            report.writeBy(dbb100040ReportSourceWriter);
            return dbb100040reportWriter.getPageCount();
        }
        return 0;
    }
    ...
    private void initialize決定通知書() {
        if (ReportIdDBB.DBB100039.getReportId().equals(出力帳票一覧.get帳票ID())) {
            ...
            dbb100040NinshoshaSource = ReportUtil.get認証者情報(SubGyomuCode.DBB介護賦課,
                    ...
                    NinshoshaDenshikoinshubetsuCode.保険者印.getコード(),
                    KenmeiFuyoKubunType.付与なし,
                    dbb100040ReportSourceWriter);
        }
    }

    private void close決定通知書() {
        if (ReportIdDBB.DBB100039.getReportId().equals(出力帳票一覧.get帳票ID())) {
            dbb100039reportWriter.close();
        } else if (ReportIdDBB.DBB100040.getReportId().equals(出力帳票一覧.get帳票ID())) {
            dbb100040reportWriter.close();
        }
    }
}
