...
public class PrtHenkoTsuchishoProcess extends SimpleBatchProcessBase {
    ...
    protected void process() {
        //TODO 「変更通知書出力対象区分」 バッチパラメータ．「変更通知書」　≪出力内容≫　対象者 :引数が存在しない
        ...
        try {
            manager.prtHenkoTsuchisho(processParameter.get調定年度(), processParameter.get決定変更_発行日(), processParameter.get決定変更_文書番号(),
                    processParameter.get帳票作成日時().getRDateTime(), 出力帳票一覧.get出力順ID(), 出力帳票一覧.get帳票ID(),
                    processParameter.is一括発行起動フラグ(), processParameter.get打分け条件情報(), null);
