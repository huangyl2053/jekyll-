...
public class PntTokuchouKaishiTutishoKarisanteiProcess extends SimpleBatchProcessBase {
    ...
    protected void process() {
        ...
        KarisanteiIdoTsuchishoIkkatsuHakkoResult result = null;
        try {
            result = manager.pntTokuchouKaishiTutishoKarisantei(processParameter.get調定年度(), processParameter.get特徴_発行日(),
                    processParameter.get帳票作成日時(), 出力帳票一覧.get出力順ID(), 出力帳票一覧.get帳票ID());
        } catch (InvocationTargetException ex) {
            Logger.getLogger(PntTokuchouKaishiTutishoKarisanteiProcess.class.getName()).log(Level.SEVERE, null, ex);
        }
        if (result == null) {
            return;
        }
        ...
        manager.publishTokuchoKaishiTsuchishoHonsantei(result, 編集後仮算定通知書共通情報List,
                地方公共団体, 仮算定特徴開始通知書情報List, 総ページ数);
    }

    private int publish特徴開始通知書(TyouhyouEntity 出力帳票一覧, KariTokuchoKaishiTsuchisyoJoho 仮算定特徴開始通知書情報,
