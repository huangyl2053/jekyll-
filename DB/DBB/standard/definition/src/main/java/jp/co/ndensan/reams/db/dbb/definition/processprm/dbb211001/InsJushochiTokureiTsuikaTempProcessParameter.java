...
/**
 * バッチ設計_DBBBT82001_特徴送付情報作成の（住所地特例該当）追加用データ作成のProcessParameterです。
 *
...
public class InsJushochiTokureiTsuikaTempProcessParameter implements IBatchProcessParameter {
     ...
     * @param システム日時 YMDHMS
     */
    public InsJushochiTokureiTsuikaTempProcessParameter(RYear 賦課年度, RString 処理対象月, YMDHMS システム日時) {
        this.賦課年度 = 賦課年度;
        if (処理対象月.length() == 1) {
    ...
    /**
     * SelectJushochiDataMyBatisParameterの取得です。
     *
     * @param is年金特徴回付情報が取得できない boolean
     * @param 通知内容コードList List<RString>
     * @return SelectJushochiDataMyBatisParameter
     */
    public SelectJushochiDataMyBatisParameter toSelectJushochiDataMyBatisParameter(
            boolean is年金特徴回付情報が取得できない, List<RString> 通知内容コードList) {
        return new SelectJushochiDataMyBatisParameter(賦課年度, is処理対象月が８月以降(),
