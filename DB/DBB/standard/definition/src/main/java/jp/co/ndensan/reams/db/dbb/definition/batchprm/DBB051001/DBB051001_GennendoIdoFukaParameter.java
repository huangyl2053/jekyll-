 ...
 */
package jp.co.ndensan.reams.db.dbb.definition.batchprm.DBB051001;
...
public class DBB051001_GennendoIdoFukaParameter extends BatchParameterBase {
     ...
     *
     * @return DBB051003_GennendoIdoTsuchishoHakkoParameter
     */
    public DBB051003_GennendoIdoTsuchishoHakkoParameter toDBB051003_GennendoIdoTsuchishoHakkoParameter() {
        DBB051003_GennendoIdoTsuchishoHakkoParameter dbb051003 = new DBB051003_GennendoIdoTsuchishoHakkoParameter();
        dbb051003.setChoteiNendo(choteiNendo);
        dbb051003.set賦課年度(賦課年度);
        dbb051003.set処理対象(処理対象);
        dbb051003.set抽出開始日時(抽出開始日時);
        dbb051003.set抽出終了日時(抽出終了日時);
        dbb051003.set特徴捕捉分(特徴捕捉分);
        dbb051003.set依頼金額計算(依頼金額計算);
        dbb051003.set出力帳票List(出力帳票List);
        dbb051003.set特徴_出力対象(特徴_出力対象);
        dbb051003.set特徴_発行日(特徴_発行日);
        dbb051003.set決定_チェックボックス(決定_チェックボックス);
        dbb051003.set決定_文書番号(決定_文書番号);
        dbb051003.set決定_発行日(決定_発行日);
        dbb051003.set変更_チェックボックス(変更_チェックボックス);
        dbb051003.set変更_対象者(変更_対象者);
        dbb051003.set変更_文書番号(変更_文書番号);
        dbb051003.set変更_発行日(変更_発行日);
        dbb051003.set納入_対象者(納入_対象者);
        dbb051003.set納入_口座振替者(納入_口座振替者);
        dbb051003.set納入_発行日(納入_発行日);
        dbb051003.set納入_出力方法(納入_出力方法);
        dbb051003.set納入_出力期(納入_出力期);
        dbb051003.set納入_生活保護対象者(納入_生活保護対象者);
        dbb051003.set納入_ページごとに山分け(納入_ページごとに山分け);
        dbb051003.set一括発行起動フラグ(一括発行起動フラグ);
        dbb051003.set随時フラグ(随時フラグ);
        dbb051003.set算定期(算定期);
        dbb051003.set画面移動フラグ(画面移動フラグ);
        return dbb051003;
    }
}
