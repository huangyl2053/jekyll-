...
public class TokuchoIraikingakuKeisanHoho8GatsuTest extends DbbTestBase {
    ...
    public void 引数に1を指定した場合_toValueは_年額除12乗仮徴収月数除年金支給回数を返す() {
        assertThat(TokuchoIraikingakuKeisanHoho8Gatsu.toValue(new RString("1")), is(TokuchoIraikingakuKeisanHoho8Gatsu.法定計算方法_6で割る
        ));
    }
    ...
    public void 年額除12乗仮徴収月数除年金支給回数を指定した場合_getコードは_1を返す() {
        assertThat(TokuchoIraikingakuKeisanHoho8Gatsu.法定計算方法_6で割る.getコード(), is(new RString("1"))
        );
    }
    ...
    public void 年額除12乗仮徴収月数除年金支給回数を指定した場合_get名称は_法定計算方法_6で割るを返す() {
        assertThat(TokuchoIraikingakuKeisanHoho8Gatsu.法定計算方法_6で割る.get名称(), is(new RString("年額÷12Ｘ仮徴収月数÷年金支給回数"))
        );
    }

    @Test
    public void 年額を納期数_4期_で割るを指定した場合_get名称は_年額を納期数_4期_で割るを返す() {
        assertThat(TokuchoIraikingakuKeisanHoho8Gatsu.年額を納期数_4期_で割る.get名称(), is(new RString("年額を納期数（4期）で割る")));
    }
