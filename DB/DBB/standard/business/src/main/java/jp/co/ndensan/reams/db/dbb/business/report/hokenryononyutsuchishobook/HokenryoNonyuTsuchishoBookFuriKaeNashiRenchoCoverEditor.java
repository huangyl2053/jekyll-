...
public class HokenryoNonyuTsuchishoBookFuriKaeNashiRenchoCoverEditor implements IHokenryoNonyuTsuchishoBookFuriKaeNashiRenchoCoverEditor {
    ...
    private void editレイヤ１(HokenryoNonyuTsuchishoBookFuriKaeNashiRenchoCoverSource source) {
        ...
        if (更正後 != null) {
            ...
            source.cover_kaisanMeisaiHokenryoRitsu = decimalFormatter_toコンマ区切りRString(更正後.get保険料率(), 0);
        ...
        if (納入通知書期情報リスト.size() >= LIST_SIZE_2
            ...
            source.cover_keisanMeisaishoKiNofuGaku3 = 納入通知書期情報リスト.get(LIST_SIZE_1).get調定額表記();
        }
        ...
        source.cover_noutsuRenban = isバッチ ? new RString(本算定納入通知書情報.get連番()).padLeft("0", INT6) : RString.EMPTY;
    }

    private void edit編集後個人And編集後口座(
            HokenryoNonyuTsuchishoBookFuriKaeNashiRenchoCoverSource source, EditedKojin 編集後個人, EditedKoza 編集後口座) {
        if (編集後個人 != null) {
    ...
    private void edit期月情報(HokenryoNonyuTsuchishoBookFuriKaeNashiRenchoCoverSource source) {
        if (納入通知書期情報リスト == null) {
            return;
        }
    ...
    private void edit特徴額(HokenryoNonyuTsuchishoBookFuriKaeNashiRenchoCoverSource source) {
    ...
    private void edit納期別明細書の表記(HokenryoNonyuTsuchishoBookFuriKaeNashiRenchoCoverSource source) {
