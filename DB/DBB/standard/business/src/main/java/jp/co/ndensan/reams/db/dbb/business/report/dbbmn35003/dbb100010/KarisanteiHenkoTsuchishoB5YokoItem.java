/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package jp.co.ndensan.reams.db.dbb.business.report.dbbmn35003.dbb100010;

import jp.co.ndensan.reams.uz.uza.lang.RString;
import lombok.Getter;
import lombok.Setter;

/**
 * 仮算定額変更【B5横タイプ】の情報です。
 *
 * @reamsid_L DBB-0820-050 xuyue
 */
@Setter
@Getter
@SuppressWarnings("PMD.UnusedPrivateField")
public class KarisanteiHenkoTsuchishoB5YokoItem {

    private RString bunshoNo;
    private RString listKibetsu_1;
    private RString listKibetsu_2;
    private RString listKibetsu_3;
    private RString listKibetsu_4;
    private RString listKibetsu_5;
    private RString listKibetsu_6;
    private RString listKibetsu_7;
    private RString listKibetsu_8;
    private RString birthYMD;
    private RString seibetsu;
    private RString hyojicode1;
    private RString hyojicode2;
    private RString hyojicode3;
    private RString hyojicodeName1;
    private RString hyojicodeName2;
    private RString hyojicodeName3;
    private RString tsuchishoNo;
    private RString setaiCode;
    private RString kongoNofuSubekiGaku;
    private RString kongoNofuSubekiGakuChotei;
    private RString nofuSubekiGaku;
    private RString mitoraikiNofuzumiGaku;
    private RString hihokenshaNo;
    private RString shikibetsuCode;
    private RString shutokuYmdMae;
    private RString shutokuYmdAto;
    private RString soshitsuYmdMae;
    private RString soshitsuYmdAto;
    private RString choteiNendo;
    private RString bankCode;
    private RString bankName;
    private RString kozaMeigi;
    private RString kozaNo;
    private RString kozaShurui;
    private RString title1;
    private RString title2;
    private RString title3;
    private RString hokenryoGakuMae;
    private RString hokenryoGakuAto;
    private RString zogenGaku;
    private RString nofuzumiGaku;
    private RString koremadeChoshuho;
    private RString koremadeTokuchoGimusha;
    private RString koremadeTokuchoTaishoNenkin;
    private RString korekaraChoshuho;
    private RString korekaraTokuchoGimusha;
    private RString korekaraTokuchoTaishoNenkin;
    private RString zanteiHokenryoGaku;
    private RString choteiJiyu1;
    private RString choteiJiyu2;
    private RString choteiJiyu3;
    private RString choteiJiyu4;
    private RString customerBarCode;
    private RString yubinNo1;
    private RString gyoseiku1;
    private RString jusho1;
    private RString jusho2;
    private RString jusho3;
    private RString jushoText;
    private RString katagaki1;
    private RString katagaki2;
    private RString katagakiSmall1;
    private RString katagakiSmall2;
    private RString katagakiText;
    private RString shimei1;
    private RString shimei2;
    private RString shimeiSmall1;
    private RString shimeiSmall2;
    private RString shimeiText;
    private RString meishoFuyo1;
    private RString meishoFuyo2;
    private RString dainoKubunMei;
    private RString samaBun1;
    private RString samaBun2;
    private RString samabunShimei1;
    private RString samabunShimei2;
    private RString samabunShimeiSmall1;
    private RString samabunShimeiSmall2;
    private RString samabunShimeiText;
    private RString kakkoLeft1;
    private RString kakkoLeft2;
    private RString kakkoRight1;
    private RString kakkoRight2;
    private RString setainusimei;
    private RString samaKata;
    private RString denshiKoin;
    private RString hakkoYMD;
    private RString koinMojiretsu;
    private RString koinShoryaku;
    private RString ninshoshaShimeiKakenai;
    private RString ninshoshaShimeiKakeru;
    private RString ninshoshaYakushokuMei;
    private RString choshaBushoName;
    private RString naisenLabel;
    private RString naisenNo;
    private RString shozaichi;
    private RString tantoName;
    private RString telNo;
    private RString yubinBango;

}
