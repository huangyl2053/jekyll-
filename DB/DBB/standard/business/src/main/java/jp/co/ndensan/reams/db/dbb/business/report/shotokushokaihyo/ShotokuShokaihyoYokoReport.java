 ...
 */
public class ShotokuShokaihyoYokoReport extends Report<ShotokuShokaihyoYokoSource> {
     ...
     */
    public ShotokuShokaihyoYokoReport(NushiJuminJohoResult 所得照会票,
    ...
    @Override
    public void writeBy(ReportSourceWriter<ShotokuShokaihyoYokoSource> reportSourceWriter) {
        List<List<SetaiInn>> 世帯員リスト = createList(所得照会票.get世帯員リスト(), INT_4);
        for (int i = INT_0; i < 世帯員リスト.size(); i++) {
            所得照会票.set世帯員リスト(世帯員リスト.get(i));
            IShotokuShokaihyoYokoEditor editor = new ShotokuShokaihyoYokoEditor(所得照会票, sourceBuilder, 文書タイトル, 送付先担当課名称);
            IShotokuShokaihyoYokoBuilder builder = new ShotokuShokaihyoYokoBuilder(editor);
