...
public class TokubetsuChoshuKaishiTsuchishoKariOverlayB5YokoReport extends
    ...
    private final TokubetsuChoshuKaishiTsuchishoKariOverlayB5YokoItem target;
     ...
     */
    public TokubetsuChoshuKaishiTsuchishoKariOverlayB5YokoReport(KariTokuchoKaishiTsuchisyoJoho 仮算定特徴開始通知書情報,
            RString 通知書定型文１, RString 通知書定型文２) {
        this.仮算定特徴開始通知書情報 = 仮算定特徴開始通知書情報;
        this.通知書定型文１ = 通知書定型文１;
        this.通知書定型文２ = 通知書定型文２;
        target = new TokubetsuChoshuKaishiTsuchishoKariOverlayB5YokoItem();
    }
     ...
     */
    public static TokubetsuChoshuKaishiTsuchishoKariOverlayB5YokoReport createForm(
            @NonNull KariTokuchoKaishiTsuchisyoJoho 仮算定特徴開始通知書情報,
            RString 通知書定型文１, RString 通知書定型文２) {
        return new TokubetsuChoshuKaishiTsuchishoKariOverlayB5YokoReport(仮算定特徴開始通知書情報, 通知書定型文１, 通知書定型文２);
    }

    @Override
    public void writeBy(ReportSourceWriter<TokubetsuChoshuKaishiTsuchishoKariOverlayB5YokoSource> writer) {
        setItems();
        ITokubetsuChoshuKaishiTsuchishoKariOverlayB5YokoEditor editor = new TokubetsuChoshuKaishiTsuchishoKariOverlayB5YokoEditor(target);
    ...
    private void setItems() {
        target.set通知文1(通知書定型文１);
        target.set通知文2(通知書定型文２);
    }
    ...
    private void setItem() {
        target.set年度１(仮算定特徴開始通知書情報.get編集後仮算定通知書共通情報().get調定年度().wareki().eraType(EraType.KANJI)
        ...
        // TODO 被保険者番号２
        target.set被保険者番号２(仮算定特徴開始通知書情報.get編集後仮算定通知書共通情報().get被保険者番号().value());
        target.set識別コード２(仮算定特徴開始通知書情報.get編集後仮算定通知書共通情報().get識別コード().value());
        target.set被保険者名称２(仮算定特徴開始通知書情報.get編集後仮算定通知書共通情報().get編集後個人().get名称().getName().value());
        ...
        target.set特徴対象年金名称２(仮算定特徴開始通知書情報.get編集後仮算定通知書共通情報().get更正後().get更正後特別徴収対象年金());
        ...
        target.set年度５(仮算定特徴開始通知書情報.get編集後仮算定通知書共通情報().get調定年度().wareki().eraType(EraType.KANJI)
                .firstYear(FirstYear.GAN_NEN).fillType(FillType.BLANK).toDateString().concat(NENDO));
        target.set保険料段階(仮算定特徴開始通知書情報.get編集後仮算定通知書共通情報().get前年度情報().get前年度保険料段階());
        target.set通知文1(通知書定型文１);
        target.set保険料額4月2(DecimalFormatter.toコンマ区切りRString(仮算定特徴開始通知書情報.get編集後仮算定通知書共通情報()
                .get更正後().get更正後特徴期別金額02(), 0));
        target.set保険料率(DecimalFormatter.toコンマ区切りRString(仮算定特徴開始通知書情報.get編集後仮算定通知書共通情報()
        ...
        target.set通知書番号１(仮算定特徴開始通知書情報.get編集後仮算定通知書共通情報().get通知書番号().value());
        target.set世帯コード１(仮算定特徴開始通知書情報.get編集後仮算定通知書共通情報().get編集後個人().get世帯コード().value());
        // TODO 被保険者番号１
        target.set被保険者番号１(仮算定特徴開始通知書情報.get編集後仮算定通知書共通情報().get被保険者番号().value());
        target.set識別コード１(仮算定特徴開始通知書情報.get編集後仮算定通知書共通情報().get識別コード().value());
        target.set被保険者名称１(仮算定特徴開始通知書情報.get編集後仮算定通知書共通情報().get編集後個人().get名称().getName().value());
        ...
        target.set特徴対象年金名称１(仮算定特徴開始通知書情報.get編集後仮算定通知書共通情報().get更正後().get更正後特別徴収対象年金());
