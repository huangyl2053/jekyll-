...
/**
 * 介護保険料減免取消通知書 B5横タイプReportするクラスです。
 *
 * @reamsid_L DBB-0740-060 chenaoqi
 */
public class GemmenTorikesiTsuchiShoB5YokoReport extends Report<KaigoHokenHokenryoGenmenTorikeshiTsuchishoB5YokoSource> {
     ...
     */
    public GemmenTorikesiTsuchiShoB5YokoReport(
            RString 文書番号,
    ...
    @Override
    public void writeBy(ReportSourceWriter<KaigoHokenHokenryoGenmenTorikeshiTsuchishoB5YokoSource> writer) {
        for (int i = INDEX_ZERO; i < INDEX_FOURTEEN; i++) {
            IGemmenTorikesiTsuchiShoB5YokoEditor editor = new GemmenTorikesiTsuchiShoB5YokoEditor(
                    文書番号,
                    減免取消通知書情報,
                    通知書定型文,
                    介護問合せ先ソースビルダー,
                    ...
                    随時リスト,
                    編集後宛先, i);
            IGemmenTorikesiTsuchiShoB5YokoBuilder builder = new GemmenTorikesiTsuchiShoB5YokoBuilder(editor);
