...
public class KariSanteiTsuchiShoKyotsuKomokuHenshu {
    ...
    public EditedKariSanteiTsuchiShoKyotsu create仮算定通知書共通情報(KariSanteiTsuchiShoKyotsu 仮算定通知書情報) {
        ...
        Decimal 普徴既に納付すべき額 = get納付額By賦課情報(普徴メソッド_賦課, 仮算定通知書情報.get賦課の情報_更正前().get賦課情報(),
                1, new KoseiTsukiHantei().find更正月(RDate.getNowDate()).get期AsInt());
        Decimal 普徴今後納付すべき額 = get納付額By賦課情報(普徴メソッド_賦課, 仮算定通知書情報.get賦課の情報_更正後().get賦課情報(),
                1, new FuchoKiUtil().get期月リスト().get最終法定納期().get期AsInt());
        Decimal 特徴既に納付すべき額 = get納付額By賦課情報(特徴メソッド_賦課, 仮算定通知書情報.get賦課の情報_更正前().get賦課情報(),
                1, new TokuchoKiUtil().get期月リスト().get月の期(Tsuki.toValue(
                                new RString(RDate.getNowDate().getMonthValue()).padZeroToLeft(2))).get期AsInt());
        Decimal 更正後特徴期別金額合計 = get納付額By賦課情報(特徴メソッド_賦課, 仮算定通知書情報.get賦課の情報_更正後().get賦課情報(), 1, 期_3);
    ...
    private EditedKariSanteiTsuchiShoKyotsuBeforeCorrection get更正前(
        ...
        更正前.set更正前資格取得日(賦課情報_更正前.get資格取得日().wareki().eraType(EraType.KANJI)
                .firstYear(FirstYear.GAN_NEN).separator(Separator.JAPANESE).fillType(FillType.BLANK).toDateString());
        更正前.set更正前資格取得日_西暦(賦課情報_更正前.get資格取得日().seireki()
        ...
        更正前.set保険料率(賦課情報_更正前.get算定年額保険料2() != null ? new RString(賦課情報_更正前.get算定年額保険料2().toString())
                : new RString(賦課情報_更正前.get算定年額保険料1().toString()));
        更正前.set更正前特別徴収義務者(CodeMaster.getCodeMeisho(特別徴収義務者コード,
                仮算定通知書情報.get対象者_追加含む_情報_更正前().getDT特別徴収義務者コード().value()));
        更正前.set更正前特別徴収対象年金(CodeMaster.getCodeMeisho(年金コード,
                new Code(仮算定通知書情報.get徴収方法情報_更正前().get仮徴収_年金コード().substring(0, 2))));
        ...
        更正前.set生保開始日(賦課情報_更正前.get生保開始日().wareki().eraType(EraType.KANJI)
                .firstYear(FirstYear.GAN_NEN).separator(Separator.JAPANESE).fillType(FillType.BLANK).toDateString());
        更正前.set生保開始日_西暦(賦課情報_更正前.get生保開始日().seireki()
                .separator(Separator.SLASH).fillType(FillType.ZERO).toDateString());
    ...
    private EditedKariSanteiTsuchiShoKyotsuAfterCorrection get更正後(
        ...
        更正後.set更正後資格取得日(賦課情報_更正後.get資格取得日().wareki().eraType(EraType.KANJI)
                .firstYear(FirstYear.GAN_NEN).separator(Separator.JAPANESE).fillType(FillType.BLANK).toDateString());
        更正後.set更正後資格取得日_西暦(賦課情報_更正後.get資格取得日().seireki()
                .separator(Separator.SLASH).fillType(FillType.ZERO).toDateString());
        ...
        更正後.set期間_至(new FlexibleDate(賦課情報_更正後.get月割終了年月2().getYearValue(),
                ...
                .firstYear(FirstYear.GAN_NEN).separator(Separator.JAPANESE).fillType(FillType.BLANK).toDateString());
        更正後.set期間_至_西暦(new FlexibleDate(賦課情報_更正後.get月割終了年月2().getYearValue(),
        ...
        更正後.set保険料率(賦課情報_更正後.get算定年額保険料2() != null ? new RString(賦課情報_更正後.get算定年額保険料2().toString())
                : new RString(賦課情報_更正後.get算定年額保険料1().toString()));
        更正後.set更正後特別徴収義務者(CodeMaster.getCodeMeisho(特別徴収義務者コード,
                仮算定通知書情報.get対象者_追加含む_情報_更正後().getDT特別徴収義務者コード().value()));
        更正後.set更正後特別徴収対象年金(CodeMaster.getCodeMeisho(年金コード,
                new Code(仮算定通知書情報.get徴収方法情報_更正後().get仮徴収_年金コード().substring(0, 2))));
        ...
        更正後.set生保開始日(賦課情報_更正後.get生保開始日().wareki().eraType(EraType.KANJI)
                .firstYear(FirstYear.GAN_NEN).separator(Separator.JAPANESE).fillType(FillType.BLANK).toDateString());
        更正後.set生保開始日_西暦(賦課情報_更正後.get生保開始日().seireki()
                .separator(Separator.SLASH).fillType(FillType.ZERO).toDateString());
