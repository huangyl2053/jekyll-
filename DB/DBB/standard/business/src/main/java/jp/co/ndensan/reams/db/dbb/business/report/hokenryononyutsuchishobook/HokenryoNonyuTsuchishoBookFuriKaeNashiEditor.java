...
/**
 * 保険料納入通知書（本算定）【ブックタイプ】（口振依頼なし）通知書 HokenryoNonyuTsuchishoBookFuriKaeNashiEditor
 *
 * @reamsid_L DBB-9110-010 huangh
 */
public class HokenryoNonyuTsuchishoBookFuriKaeNashiEditor implements IHokenryoNonyuTsuchishoBookFuriKaeNashiEditor {
    ...
    private static final RString HOSHI2 = new RString("**");
    private static final RString HOSHI6 = new RString("******");
    private static final RString HOSHI9 = new RString("*********");
    private static final RString HOSHI10 = new RString("**********");
    private static final RString HOSHI11 = new RString("***********");
    private static final RString HOSHI12 = new RString("************");
    private static final RString HOSHI19 = new RString("*******************");
    private static final RString HOSHI20 = new RString("********************");
     ...
     */
    protected HokenryoNonyuTsuchishoBookFuriKaeNashiEditor(
     ...
     */
    @Override
    public HokenryoNonyuTsuchishoBookFuriKaeNashiCoverSource edit(HokenryoNonyuTsuchishoBookFuriKaeNashiCoverSource source) {
    ...
    private void editレイヤ１(HokenryoNonyuTsuchishoBookFuriKaeNashiCoverSource source) {
        ...
        if (納入通知書期情報リスト != null && 納入通知書期情報リスト.get(0) != null) {
            source.cover_keisanMeisaishoKiTitle1
                    = new RString("第").concat(new RString(納入通知書期情報リスト.get(0).get期())).concat(new RString("期"));
            source.cover_keisanMeisaishoKibetsuNofuGaku1 = decimalFormatter_toコンマ区切りRString(
                    rstringToDecimal(納入通知書期情報リスト.get(0).get調定額表記()), 0);
        }
        if (納入通知書期情報リスト != null && 納入通知書期情報リスト.size() >= LIST_SIZE_2
                && 納入通知書期情報リスト.get(LIST_SIZE_1).get期() > 0) {
            source.cover_keisanMeisaishoKiTitle3 = TSUGIKI_IKOU;
            source.cover_keisanMeisaishoKibetsuNofuGaku3 = decimalFormatter_toコンマ区切りRString(
                    rstringToDecimal(納入通知書期情報リスト.get(LIST_SIZE_1).get調定額表記()), 0);
        }
...
//        source.cover_yen4 = EN;
    ...
    private void edit編集後個人And編集後口座(
            HokenryoNonyuTsuchishoBookFuriKaeNashiCoverSource source, EditedKojin 編集後個人, EditedKoza 編集後口座) {
        if (編集後個人 != null) {
    ...
    private void editCompNofushoBookItem(HokenryoNonyuTsuchishoBookFuriKaeNashiCoverSource source) {
        ...
        if (納入通知書期情報 != null && 1 == 納入通知書期情報.getブック開始位置()) {
            ...
            source.cover_nofuzumishoTitleTsuki1 = HOSHI2;
            source.cover_nofozumishoTitleNendo1 = HOSHI6;
            source.cover_ryoshushoTitleNendo1 = HOSHI6;
            source.cover_ryoshushoTitleKi1 = HOSHI2;
            source.cover_ryoshushoTitleTsuki1 = HOSHI2;
            source.cover_nofushoTitleNendo1 = HOSHI6;
            source.cover_nofushoTitleKi1 = HOSHI2;
            source.cover_nofushoTitleTsuki1 = HOSHI2;
            source.cover_nofuzumishoOCR11 = HOSHI19;
            source.cover_nofuzumishoOCR21 = HOSHI20;
            source.cover_nofuzumishoOCR31 = HOSHI12;
            source.cover_nofuzumishoNokigen1 = HOSHI11;
            source.cover_ryoshushoHokenryoGaku1 = HOSHI10;
            source.cover_nofushoHokenryoGaku1 = HOSHI10;
            source.cover_nofuzumishoNofuGaku1 = HOSHI10;
            source.cover_ryoshushoNokigen1 = HOSHI11;
            source.cover_nofushoNokigen1 = HOSHI11;
            source.cover_ryoshushoNofuIn1 = HOSHI2;
    ...
    private void editCompNinshosha(HokenryoNonyuTsuchishoBookFuriKaeNashiCoverSource source) {
    ...
    private void editCompSofubutsuAtesaki(HokenryoNonyuTsuchishoBookFuriKaeNashiCoverSource source) {
    ...
    private void edit期月情報(HokenryoNonyuTsuchishoBookFuriKaeNashiCoverSource source) {
        if (納入通知書期情報リスト == null) {
            return;
        }
    ...
    private void edit特徴額(HokenryoNonyuTsuchishoBookFuriKaeNashiCoverSource source) {
    ...
    private void edit納期別明細書の表記(HokenryoNonyuTsuchishoBookFuriKaeNashiCoverSource source) {
        if (納入通知書期情報リスト != null) {
            if (納入通知書期情報リスト.size() >= LIST_SIZE_1) {
                source.cover_nokibetsuMeisaishoKi1 = new RString("第").concat(納入通知書期情報リスト.get(0).get期表記()).concat(new RString("期"));
                source.cover_nokibetsuMeisaishoFuchoNofuGaku1
                        = decimalFormatter_toコンマ区切りRString(rstringToDecimal(納入通知書期情報リスト.get(0).get調定額表記()), 0);
                source.cover_nokibetsuMeisaishoFuchoNofuZumiGaku1
                        = decimalFormatter_toコンマ区切りRString(rstringToDecimal(納入通知書期情報リスト.get(0).get収入額表記()), 0);
                source.cover_nokibetsuMeisaishoFuchoSaGaku1
                        = decimalFormatter_toコンマ区切りRString(rstringToDecimal(納入通知書期情報リスト.get(0).get差額表記()), 0);
            ...
            if (納入通知書期情報リスト.size() >= LIST_SIZE_2) {
                ...
                source.cover_nokibetsuMeisaishoFuchoNofuGaku2
                        = decimalFormatter_toコンマ区切りRString(rstringToDecimal(納入通知書期情報リスト.get(1).get調定額表記()), 0);
                source.cover_nokibetsuMeisaishoFuchoNofuZumiGaku2
                        = decimalFormatter_toコンマ区切りRString(rstringToDecimal(納入通知書期情報リスト.get(1).get収入額表記()), 0);
                source.cover_nokibetsuMeisaishoFuchoSaGaku2
                        = decimalFormatter_toコンマ区切りRString(rstringToDecimal(納入通知書期情報リスト.get(1).get差額表記()), 0);
            ...
            if (納入通知書期情報リスト.size() >= LIST_SIZE_3) {
                ...
                source.cover_nokibetsuMeisaishoFuchoNofuGaku3
                        = decimalFormatter_toコンマ区切りRString(rstringToDecimal(納入通知書期情報リスト.get(2).get調定額表記()), 0);
                source.cover_nokibetsuMeisaishoFuchoNofuZumiGaku3
                        = decimalFormatter_toコンマ区切りRString(rstringToDecimal(納入通知書期情報リスト.get(2).get収入額表記()), 0);
                source.cover_nokibetsuMeisaishoFuchoSaGaku3
                        = decimalFormatter_toコンマ区切りRString(rstringToDecimal(納入通知書期情報リスト.get(2).get差額表記()), 0);
        ...
        if (納入通知書期情報リスト != null && 納入通知書期情報リスト.size() >= LIST_SIZE_8) {
        ...
        Decimal 収入額Count = Decimal.ZERO;
        Decimal 差額Count = Decimal.ZERO;
        if (納入通知書期情報リスト != null) {
            for (NonyuTsuchiShoKiJoho 納入通知書期情報 : 納入通知書期情報リスト) {
                調定額Count = 調定額Count.add(rstringToDecimal(納入通知書期情報.get調定額表記()));
                収入額Count = 収入額Count.add(rstringToDecimal(納入通知書期情報.get収入額表記()));
                差額Count = 差額Count.add(rstringToDecimal(納入通知書期情報.get差額表記()));
            }
        }
        source.cover_nokibetsuMeisaishoFuchoNofuGaku11
                = decimalFormatter_toコンマ区切りRString(調定額Count, 0);
        source.cover_nokibetsuMeisaishoFuchoNofuZumiGaku11
                = decimalFormatter_toコンマ区切りRString(収入額Count, 0);
        source.cover_nokibetsuMeisaishoFuchoSaGaku11
                = decimalFormatter_toコンマ区切りRString(差額Count, 0);
    }
}
