/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package jp.co.ndensan.reams.db.dbb.business.report.nonyutsuchishohonsanteihakkoichiran;

import jp.co.ndensan.reams.uz.uza.lang.RString;

/**
 * 帳票設計_DBBRP43002_2_保険料納入通知書（本算定）NonyuTsuchIchiranItem
 *
 * @reamsid_L DBB-0780-120 yangchenbing
 */
@SuppressWarnings("PMD.UnusedPrivateField")
@lombok.Getter
@lombok.Setter
public class NonyuTsuchIchiranItem {

    private RString printTimeStamp;
    private RString nendo;
    private RString nendoBunKakko;
    private RString nendoBun1;
    private RString nendoBun2;
    private RString hokenshaNo;
    private RString hokenshaName;
    private RString shutsuryokujun1;
    private RString shutsuryokujun2;
    private RString shutsuryokujun3;
    private RString shutsuryokujun4;
    private RString shutsuryokujun5;
    private RString hyojicodeName1;
    private RString hyojicodeName2;
    private RString hyojicodeName3;
    private RString shotokuDankaiTitle;
    private RString listUpper_1;
    private RString listUpper_2;
    private RString listUpper_3;
    private RString listUpper_4;
    private RString listUpper_5;
    private RString listUpper_6;
    private RString listUpper_7;
    private RString listUpper_8;
    private RString listUpper_9;
    private RString listUpper_10;
    private RString listUpper_11;
    private RString listUpper_12;
    private RString listUpper_13;
    private RString listUpper_14;
    private RString listToki_1;
    private RString listLower_1;
    private RString listLower_2;
    private RString listLower_3;
    private RString listLower_4;
    private RString listLower_5;
    private RString listLower_6;
    private RString listLower_7;
    private RString listLower_8;
    private RString listJiki_1;
}
