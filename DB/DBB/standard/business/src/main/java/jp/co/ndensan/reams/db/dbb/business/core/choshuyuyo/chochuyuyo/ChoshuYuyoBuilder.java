...
public class ChoshuYuyoBuilder {
     ...
     * 護期別徴収猶予情報に関連できる調定共通情報である場合、下記の処理に遷移します。<br>
     * キーが一致する場合は調定共通情報リストに護期別徴収猶予情報{@link Kibetsu}をセットします。<br>
     * キーが一致しない場合、新たに追加します。<br>
     *
     * @param 介護期別徴収猶予情報 {@link ChoshuYuyo}
     * @return {@link ChoshuYuyoBuilder}
     * @throws IllegalStateException キーが一致しない場合
     */
    public ChoshuYuyoBuilder setKibetsuChoshuYuyo(KibetsuChoshuYuyo 介護期別徴収猶予情報) {
        if (hasSameIdentifier(介護期別徴収猶予情報.identifier())) {
            kibetsuChoshuYuyo.add(介護期別徴収猶予情報);
            return this;
        }
        throw new IllegalArgumentException(UrErrorMessages.不正.toString());
    }

    private boolean hasSameIdentifier(KibetsuChoshuYuyoIdentifier 介護期別徴収猶予情報識別子) {
        return (id.get調定年度().equals(介護期別徴収猶予情報識別子.get調定年度())
                && id.get賦課年度().equals(介護期別徴収猶予情報識別子.get賦課年度())
                && id.get通知書番号().equals(介護期別徴収猶予情報識別子.get通知書番号())
                && id.get履歴番号() == 介護期別徴収猶予情報識別子.get履歴番号());
    }
