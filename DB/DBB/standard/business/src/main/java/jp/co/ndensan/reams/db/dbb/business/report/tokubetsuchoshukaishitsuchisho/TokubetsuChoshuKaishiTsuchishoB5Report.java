...
public class TokubetsuChoshuKaishiTsuchishoB5Report extends Report<TokubetsuChoshuKaishiTsuchishoB5Source> {
     ...
     */
    public TokubetsuChoshuKaishiTsuchishoB5Report(EditedHonSanteiTsuchiShoKyotsu 編集後本算定通知書共通情報, RString 宛名連番) {
     ...
     * @return TokubetsuChoshuKaishiTsuchishoB5Reportクラス
     */
    public static TokubetsuChoshuKaishiTsuchishoB5Report createForm(
            @NonNull EditedHonSanteiTsuchiShoKyotsu 編集後本算定通知書共通情報,
            RString 宛名連番) {
        return new TokubetsuChoshuKaishiTsuchishoB5Report(編集後本算定通知書共通情報, 宛名連番);
    }

    @Override
    public void writeBy(ReportSourceWriter<TokubetsuChoshuKaishiTsuchishoB5Source> writer) {
        setItem();
        ITokubetsuChoshuKaishiTsuchishoB5Editor editor = new TokubetsuChoshuKaishiTsuchishoB5Editor(target);
    ...
    private void setItem() {
        ...
        target.set世帯コード2(編集後本算定通知書共通情報.get編集後個人().get世帯コード().value());
        target.set通知書番号2(編集後本算定通知書共通情報.get通知書番号().value());
        target.set被保険者氏名2(編集後本算定通知書共通情報.get編集後個人().get名称().getName().value());
        target.set生年月日2(編集後本算定通知書共通情報.get編集後個人().get生年月日());
        target.set性別2(編集後本算定通知書共通情報.get編集後個人().get性別());
        target.set特別徴収業務者名2(編集後本算定通知書共通情報.get更正後().get特別徴収義務者());
        target.set特別徴収対象年金名2(編集後本算定通知書共通情報.get更正後().get特別徴収対象年金());
        target.set宛名連番2(宛名連番);
        ...
        target.set保険料額2月本徴収額(set特徴期別金額(特徴期別金額6期));
        target.set世帯主名1(編集後本算定通知書共通情報.get編集後個人().get世帯主名().value());
        ...
        target.set郵便番号(sofubutsuAtesakiSource.yubinNo);
