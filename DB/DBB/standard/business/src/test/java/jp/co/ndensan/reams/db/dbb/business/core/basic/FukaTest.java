...
//TODO 主キー名を置換してください
        ...
        public void 主キー名1がnullである場合に_NullPointerExceptionが発生する() {
            sut = new Fuka(null, 主キー名2);
        }

        @Test(expected = NullPointerException.class)
        public void 主キー名2がnullである場合に_NullPointerExceptionが発生する() {
            sut = new Fuka(主キー名1, null);
        }

        @Test
        public void 指定したキーが保持するDbT2002FukaEntityにセットされている() {
            sut = new Fuka(主キー名1, 主キー名2);
    ...
    public static class getterテスト extends DbbTestBase {
        ...
        @Test
        public void get減免前介護保険料（年額）は_entityが持つ減免前介護保険料（年額）を返す() {
            assertThat(sut.get減免前介護保険料（年額）(), is(FukaEntity.getGemmenMaeHokenryo()));
        }
        ...
        @Test
        public void get確定介護保険料（年額）は_entityが持つ確定介護保険料（年額）を返す() {
            assertThat(sut.get確定介護保険料（年額）(), is(FukaEntity.getKakuteiHokenryo()));
        }

        @Test
        public void get保険料段階（仮算定時）は_entityが持つ保険料段階（仮算定時）を返す() {
            assertThat(sut.get保険料段階（仮算定時）(), is(FukaEntity.getHokenryoDankaiKarisanntei()));
        }
